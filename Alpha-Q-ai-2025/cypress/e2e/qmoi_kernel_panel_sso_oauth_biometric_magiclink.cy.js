...
describe('QMOI Kernel Panel SSO/OAuth E2E - Biometric & Magic Link', () => {
  it('allows login via biometric SSO', () => {
    cy.visit('/auth/callback?provider=biometric&token=biometric-mock-token&biometric=true');
  ...
  it('allows login via magic link', () => {
    cy.visit('/auth/callback?provider=magiclink&token=magiclink-mock-token&email=user@example.com');
