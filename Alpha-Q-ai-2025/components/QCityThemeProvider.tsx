...
export type QCityTheme = 'light' | 'dark' | 'system' | 'high-contrast' | 'colorblind';
...
const QCityThemeContext = createContext<QCityThemeContextProps | undefined>(undefined);

export const QCityThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <QCityThemeContext.Provider value={{ theme, setTheme, animated, setAnimated }}>
      <NextThemeProvider attribute="class" defaultTheme={theme} enableSystem>
        <div className={`qcity-theme-${theme}${animated ? ' qcity-animated' : ''}`}>{children}</div>
      </NextThemeProvider>
...
export function useQCityTheme() {
  ...
  if (!ctx) throw new Error('useQCityTheme must be used within QCityThemeProvider');
  return ctx;
} 