...
const scripts = [
  ...
  { name: 'qmoi:self-updating-agent', cmd: 'npm', args: ['run', 'qmoi:self-updating-agent'] },
  { name: 'qmoi:cloud:offload-optimizer', cmd: 'npm', args: ['run', 'qmoi:cloud:offload-optimizer'] },
  { name: 'qmoi:learning:revenue', cmd: 'npm', args: ['run', 'qmoi:learning:revenue'] },
];
...
for (const script of scripts) {
  ...
  const result = spawnSync(script.cmd, script.args, { stdio: 'inherit', shell: true });
  if (result.status !== 0) {
...
if (allPassed) {
  ...
  console.error('🚨 One or more QMOI automation scripts failed. See above for details.');
  process.exit(1);
} 