...
class QMOIJSONAutoFixer {
  constructor() {
    ...
    this.errorPatterns = {
      ...
      'Property names must be double-quoted': 'Missing quotes around property name',
  ...
  async advancedFix(content, filePath, originalContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.trim() === '}' || line.trim() === ']') {
        if (i < lines.length - 1 && lines[i + 1].trim() === '}' || lines[i + 1].trim() === ']') {
          errorLine = i + 1;
          errorColumn = lines[i + 1].indexOf('}') !== -1 ? lines[i + 1].indexOf('}') : lines[i + 1].indexOf(']');
          break;
        }
      }
    ...
    if (errorLine !== -1) {
      console.log(`📍 Found error at line ${errorLine + 1}, column ${errorColumn + 1}`);
...
// CLI interface
const isMainModule = process.argv[1] && process.argv[1].endsWith('qmoi-json-auto-fixer.js');
if (isMainModule) {
