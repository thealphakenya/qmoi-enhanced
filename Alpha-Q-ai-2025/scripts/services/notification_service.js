...
Sent by QMOI AI Automation System
  ...
  async sendSlackNotification(title, message) {
    ...
    try {
      ...
      await this.log(`Failed to send Slack notification: ${error.message}`, 'ERROR');
      return false;
    }
  }
  ...
  async sendDiscordNotification(title, message) {
    if (!this.config.discord.enabled || !this.config.discord.webhook_url) {
      await this.log('Discord notifications disabled or webhook not configured');
      return false;
    }
    ...
    try {
      ...
      await this.log(`Failed to send Discord notification: ${error.message}`, 'ERROR');
      return false;
    }
  }
  ...
  async sendNotification(title, message, channels = ['console']) {
    ...
    try {
      ...
      if (channels.includes('discord')) {
        const discordResult = await this.sendDiscordNotification(title, message);
      ...
      await this.log(`Notification sent: ${title} (${results.filter(r => r.success).length}/${results.length} channels)`);
      return notification;
  ...
  getNotificationStats() {
    ...
    const partial = this.notifications.filter(n => n.status === 'partial').length;
  ...
  async testNotifications() {
    ...
    testResults.push(await this.sendNotification('Test Console', 'This is a test console notification', ['console']));

    // Test email notifications
    if (this.config.email.enabled) {
      testResults.push(await this.sendNotification('Test Email', 'This is a test email notification', ['email']));
    }

    // Test Slack notifications
    if (this.config.slack.enabled) {
      testResults.push(await this.sendNotification('Test Slack', 'This is a test Slack notification', ['slack']));
    }

    // Test Discord notifications
    if (this.config.discord.enabled) {
      testResults.push(await this.sendNotification('Test Discord', 'This is a test Discord notification', ['discord']));
    }
