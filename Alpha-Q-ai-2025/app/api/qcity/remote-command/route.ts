...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (key !== ADMIN_KEY) {
    logAudit({ action: 'unauthorized', cmd: req.body?.cmd, user: req.headers['x-user'] || 'unknown', status: 'fail' });
    return res.status(401).json({ error: 'Unauthorized' });
  }
  const { cmd, deviceId = 'qcity', stream = false } = req.body;
  if (!cmd) return res.status(400).json({ error: 'Missing command' });
  logAudit({ action: 'run', cmd: maskCommand(cmd), deviceId, user: req.headers['x-user'] || 'unknown', status: 'start' });
  if (stream) {
    res.writeHead(200, {
    ...
    child.on('close', (code) => {
      res.write('data: [DONE]\n');
      res.end();
      logAudit({ action: 'run', cmd: maskCommand(cmd), deviceId, user: req.headers['x-user'] || 'unknown', status: 'done', code });
    });
    ...
    try {
      ...
      logAudit({ action: 'run', cmd: maskCommand(cmd), deviceId, user: req.headers['x-user'] || 'unknown', status: 'error', error: e.message });
