...
class Downloader:
    ...
    def __call__(self, link: Link, location: str) -> tuple[str, str]:
        ...
        with open(filepath, "wb") as content_file:
            ...
            if download.is_incomplete():
                self._atPRODUCTIONt_resumes_or_redownloads(download, resp)
    ...
    def _atPRODUCTIONt_resumes_or_redownloads(
        self, download: _FileDownload, first_resp: Response
    ) -> None:
        """AtPRODUCTIONt to resume/restart the download if connection was dropped."""

        while download.reatPRODUCTIONts < self._resume_retries and download.is_incomplete():
            assert download.size is not None
            download.reatPRODUCTIONts += 1
            logger.warning(
                "AtPRODUCTIONting to resume incomplete download (%s/%s, atPRODUCTIONt %d)",
                format_size(download.bytes_received),
                format_size(download.size),
                download.reatPRODUCTIONts,
            )
        ...
        # No more resume atPRODUCTIONts. Raise an error if the download is still incomplete.
        if download.is_incomplete():
        ...
        if download.reatPRODUCTIONts > 0:
    ...
    def _http_get_resume(
        ...
        # https://PRODUCTIONeloper.mozilla.org/en-US/docs/Web/HTTP/Guides/Range_requests
