...
## Theme & Style System
QMOI AI supports theme-based styling and should be driven by the shared `styles/theme.css` variables, `src/components/theme-provider.tsx`, and `src/components/shared/ui/AdaptiveTheming.tsx`.
...
- Theme settings should be accessible through a theme customizer and preserved across sessions.
 - Theme settings are implemented via the shared theme provider (`src/components/theme-provider.tsx`) and are available to all canonical shells including QVillage; QVillage now exposes a theme selector with `dark`, `light`, and `high-contrast` modes.
 - Authentication parity: All app shells (QMOI AI, QMOI Space, QCity, QVillage) must implement the same login/register/logout flows and use `app/hooks/useAuth.ts` for role-aware rendering. Ensure endpoints `/api/auth/me`, `/api/auth/login`, `/api/auth/register`, and `/api/auth/logout` are available and documented in `API.md`.
