...
- Reads `ALLMDFILESREFS.md` (falls back to scanning for .md files if missing).
- For target md files (API.md, API_a1.md, ENDPOINTS.md) it runs `generate_api_docs.py` to produce candidate content.
- If generated content differs from the on-disk file, write a draft patch under `tools/patches/` (not applied) and record a report JSON.
