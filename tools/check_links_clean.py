#!/usr/bin/env python3
"""Check links found in tools/link_report.json and report status.

Usage: python tools/check_links_clean.py tools/link_report.json > tools/link_report_status.json
"""
import json
import sys
import requests
...
def check_url(url):
    ...
    if parsed.scheme in ("http", "https") and parsed.hostname in ("localhost", "127.0.0.1"):
        return {"url": url, "status": "local", "code": None}
    try:
        ...
        if code >= 400 or code == 405:
            ...
            code = r.status_code
        return {"url": url, "status": "ok" if code < 400 else "error", "code": code}
...
def main(report_path):
    ...
    urls = {}
    for e in entries:
        urls.setdefault(e["url"], []).append({"file": e["file"], "line": e["line"]})
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in as_completed(futures):
            res = fut.result()
            res["locations"] = urls.get(res["url"], [])
            results.append(res)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: check_links_clean.py tools/link_report.json", file=sys.stderr)
        sys.exit(2)
    main(sys.argv[1])
#!/usr/bin/env python3
"""
