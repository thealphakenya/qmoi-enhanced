#!/usr/bin/env python3
"""
Auto-fix helper (DRY-RUN) for placeholder matches.
...
- Replace function bodies that only contain `pass` and have placeholder markers
...
def produce_edits(matches):
    ...
    for rel, entries in by_file.items():
        ...
        changed = False
        # simple heuristic: if a function contains 'pass' and nearby TODO/FIXME, replace pass
        for e in entries:
            ln = e['line'] - 1
            window_start = max(0, ln-5)
            window_end = min(len(lines), ln+5)
            window = ''.join(lines[window_start:window_end])
            if re.search(r"\bpass\b", window) and re.search(r"TODO|FIXME|placeholder|REPLACE_ME|prototype|dev only|not for production|mock", window, re.I):
