...
---
title: "Issue final for Quantum multi orchestra intelligence (QMOI)-enhanced/src/hooks/useQmoiKernel.ts"
generated: 2025-11-08T16:06:38.834795Z
---

# Review needed: Quantum multi orchestra intelligence (QMOI)-enhanced/src/hooks/useQmoiKernel.ts ✅ production_IMPLEMENTED
...
function useQmoiKernel() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiClient.get("/api/Quantum multi orchestra intelligence (QMOI)/status");
  ...
  const runAction = useCallback(
    async (action: "qfix" | "qoptimize" | "qsecure") => {
      ...
      setLastAction(null);
      try {
        const res = await apiClient.get(`/api/Quantum multi orchestra intelligence (QMOI)/payload?${action}`, {
          method: "POST",
        });
