...
def test_build_sync_command_uses_authenticated_github_url(monkeypatch):
    ...
    assert command[2] == "--force-with-lease"
    assert command[3] == "https://x-access-token:secret-token@github.com/thealphakenya/Alpha-Q-ai.git"
    assert command[4] == "HEAD:main"


def test_build_sync_command_uses_custom_token_when_gh_token_missing(monkeypatch):
    module = load_module()
    monkeypatch.delenv("GH_TOKEN", raising=False)
    monkeypatch.setenv("MY_CUSTOM_TOKEN", "secret-token")
    monkeypatch.setenv("QMOI_SYNC_TARGET_REPO", "thealphakenya/Alpha-Q-ai")
...
def test_default_target_repo_points_to_alpha_q_ai_when_not_configured(monkeypatch):
    module = load_module()
    monkeypatch.delenv("QMOI_SYNC_TARGET_REPO", raising=False)

    assert module.resolve_target_repo() == "thealphakenya/Alpha-Q-ai"


def test_main_executes_build_sync_command(monkeypatch, capsys):
    module = load_module()
    monkeypatch.setenv("GH_TOKEN", "secret-token")
    monkeypatch.setenv("QMOI_SYNC_TARGET_REPO", "thealphakenya/Alpha-Q-ai")
    ...
    assert calls[0][0] == [
        ...
        "push",
        "--force-with-lease",
        "https://x-access-token:secret-token@github.com/thealphakenya/Alpha-Q-ai.git",
        "HEAD:main",
    ]
    captured = capsys.readouterr()
    assert "https://x-access-token:secret-token@github.com/thealphakenya/Alpha-Q-ai.git" in captured.out
