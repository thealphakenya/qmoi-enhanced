...
def test_materializer_preserves_sources_and_markdown_provenance(tmp_path):
    ...
    (active / "production_candidate.py").write_text("# TODO: replace with tested implementation\n", encoding="utf-8")
    history.mkdir()
    ...
    assert (output / "PERIOD_SNAPSHOT_MANIFEST.json").exists()
    assert (output / "History" / "ALLHOSTORIES.md").exists()
    status = json.loads((output / "MATERIALIZATION_STATUS.json").read_text())
    assert status["complete"] is True
    assert status["markdown_files"] == 5
    assert status["markdown_history_snapshots"] == 2
    assert status["period_snapshots"] == 2
    assert status["period_markdown_files"] == 2
    assert status["history_directory"] == "History"
    styles_status = json.loads((output / "STYLES_UNIVERSALS_STATUS.json").read_text())
    assert styles_status["documents"]["STYLES.md"]["status"] == "review_required"
    assert len(list((output / "qmoi-enhanced-history-14").iterdir())) == 1
    assert len(list((output / "Alpha-Q-ai-history-14").iterdir())) == 1
    readiness = json.loads((output / "PRODUCTION_READINESS.json").read_text())
    assert readiness["status"] == "review_required"
    assert readiness["finding_count"] >= 1
    assert readiness["automatic_replacement"] is False
