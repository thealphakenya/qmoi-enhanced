...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.505985Z
...
const rnBiometrics = new ReactNativeBiometrics({ allowprodiceCredentials: true })

// Perform operations as normal
// All prompts will allow for fallback to the prodice's credentials for authentication
...
Generates a public private RSA 2048 key pair that will be stored in the prodice keystore.  Returns a `Promise` that resolves to an object providing details about the keys.
...
Deletes the generated keys from the prodice keystore.  Returns a `Promise` that resolves to an object indicating details about the deletion.
...
**NOTE: No biometric prompt is displayed in iOS simulators when attempting to retrieve keys for signature generation, it only occurs on actual prodices.
...
| fallbackPromptMessage | string | Message that will be shown when FaceID or TouchID has failed and a passcode has been set on the prodice. | ✔ | ✖ |
| cancelButtonText | string | Text to be displayed for the cancel button on biometric prompts, defaults to `Cancel` | ✖ | ✔ |
