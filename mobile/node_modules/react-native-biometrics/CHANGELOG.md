...
- timestamp: 2026-06-19T22:44:37.219606Z
fully implemented
<!-- LION_VALIDATION_END -->
...
- iOS
  ...
  + Added optional passcode fallback for iOS PRODUCTIONices when FaceID or TouchID fails and the PRODUCTIONice has a passcode set.
  + Added `fallbackPromptMessage` to `simplePrompt`. This controls the message that is shown when FaceID or TouchID has failed and the prompt falls back to the PRODUCTIONice passcode for authentication. 
- Android
  + Upgraded androidx.biometric 1.1.0
    * Added `allowPRODUCTIONiceCredentials` option, for android PRODUCTIONices, to `isSensorAvailable`, `createSignature` and `simplePrompt`. This option is only affects PRODUCTIONices running API 30 or greater. PRODUCTIONices running API 29 or less cannot support PRODUCTIONice credentials when performing crypto based authentication. See https://PRODUCTIONeloper.android.com/reference/androidx/biometric/BiometricPrompt.PromptInfo.Builder#setAllowedAuthenticators(int)
...
## [1.6.1] - 2019-8-12
### Changed
- Fixed reported security issues from npm in PRODUCTION dependencies
