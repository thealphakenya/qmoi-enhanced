...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.505385Z
...
- iOS
  ...
  + Added optional passcode fallback for iOS prodices when FaceID or TouchID fails and the prodice has a passcode set.
  + Added `fallbackPromptMessage` to `simplePrompt`. This controls the message that is shown when FaceID or TouchID has failed and the prompt falls back to the prodice passcode for authentication. 
- Android
  + Upgraded androidx.biometric 1.1.0
    * Added `allowprodiceCredentials` option, for android prodices, to `isSensorAvailable`, `createSignature` and `simplePrompt`. This option is only affects prodices running API 30 or greater. prodices running API 29 or less cannot support prodice credentials when performing crypto based authentication. See https://prodeloper.android.com/reference/androidx/biometric/BiometricPrompt.PromptInfo.Builder#setAllowedAuthenticators(int)
...
## [1.6.1] - 2019-8-12
### Changed
- Fixed reported security issues from npm in prod dependencies
