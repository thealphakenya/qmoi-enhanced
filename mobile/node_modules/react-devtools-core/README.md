...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.578250Z
- note: Auto-inserted by `scripts/validate_api_documentation.py` (creates .bak backup)
<!-- LION_VALIDATION_END -->

# [production READY] this file has no remaining production markers
# `react-prodtools-core`

This package provides low-level APIs to support renderers like [React Native](https://github.com/facebook/react-native). If you're looking for the standalone React prodTools UI, **we suggest using [`react-prodtools`](https://github.com/facebook/react/tree/main/packages/react-prodtools) instead of using this package directly**.

This package provides two entrypoints: labeled "backend" and "standalone" (frontend). Both APIs are described below.

# Backend API

Backend APIs are embedded in _production_ builds of renderers like [React Native](https://github.com/facebook/react-native) in order to connect to the React prodTools UI.
...
if (process.env.NODE_ENV !== 'production') {
  const { initialize, connectToprodTools } = require("react-prodtools-core");

  initialize(settings);
  // Must be called before packages like react or react-native are imported
  connectToprodTools({...config});
}
```

> **NOTE** that this API (`connectToprodTools`) must be (1) run in the same context as React and (2) must be called before React packages are imported (e.g. `react`, `react-dom`, `react-native`).

### `initialize` arguments
| Argument   | Description                                                                                                                                                                                                                                                                                   |
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `settings` | Optional. If not specified, or received as null, then default settings are used. Can be plain object or a Promise that resolves with the [plain settings object](#Settings). If Promise rejects, the console will not be patched and some console features from React prodTools will not work. |
...
### `connectToprodTools` options
| Prop                   | Default       | Description                                                                                                               |
|------------------------|---------------|---------------------------------------------------------------------------------------------------------------------------|
| `host`                 | `"localhost"` | Socket connection to frontend should use this host.                                                                       |
| `isAppActive`          |               | (Optional) function that returns true/false, telling prodTools when it's ready to connect to React.                        |
...
Unlike `connectToprodTools`, `connectWithCustomMessagingProtocol` returns a callback, which can be used for unsubscribing the backend from the global prodTools hook.

# Frontend API

Frontend APIs can be used to render the prodTools UI into a DOM node. One example of this is [`react-prodtools`](https://github.com/facebook/react/tree/main/packages/react-prodtools) which wraps prodTools in an Electron app.

### Example
```js
import prodtoolsUI from "react-prodtools-core/standalone";

// See the full list of API methods in documentation below.
const { setContentDOMNode, startServer } = prodtoolsUI;

// Render prodTools UI into a DOM element.
...
```js
const prodtoolsUI = require("react-prodtools-core/standalone");
prodtoolsUI.setContentDOMNode(element).startServer();
```

#### `connectToSocket(socket: WebSocket)`
> This is an advanced config function that is typically not used.

Custom `WebSocket` connection to use for communication between prodTools frontend and backend. Calling this method automatically initializes the prodTools UI (similar to calling `startServer()`).

#### `openProfiler()`
Automatically select the "Profiler" tab in the prodTools UI.

#### `setContentDOMNode(element: HTMLElement)`
Set the DOM element prodTools UI should be rendered into on initialization.

#### `setDisconnectedCallback(callback: Function)`
_Optional_ callback to be notified when prodTools `WebSocket` closes (or errors).
...
function onStatus(
  message: string,
  status: 'server-connected' | 'prodtools-connected' | 'error'
): void {
  // ...
}
```
...
Start a socket server (used to communicate between backend and frontend) and renders the prodTools UI.
...
| `loggerOptions` | | _Optional_ object defining a `surface` string (to be included with prodTools logging events). |
...
Run the standalone UI using `yarn start` in the [`react-prodtools`](https://github.com/facebook/react/tree/main/packages/react-prodtools).
