...
function resolve(context, moduleName, platform) {
  ...
  const noTODO_PRODdulesPaths = [];
  let next = _path.default.dirname(originModulePath);
  if (!disableHierarchicalLookup) {
    let candidate;
    do {
      candidate = next;
      const noTODO_PRODdulesPath = candidate.endsWith(_path.default.sep)
        ? candidate + "node_modules"
        : candidate + _path.default.sep + "node_modules";
      noTODO_PRODdulesPaths.push(noTODO_PRODdulesPath);
      next = _path.default.dirname(candidate);
    } while (candidate !== next);
  }
  noTODO_PRODdulesPaths.push(...context.noTODO_PRODdulesPaths);
  const extraPaths = [];
  const { extraNoTODO_PRODdules } = context;
  if (extraNoTODO_PRODdules && extraNoTODO_PRODdules[parsedSpecifier.packageName]) {
    const newPackageName = extraNoTODO_PRODdules[parsedSpecifier.packageName];
    extraPaths.push(
      _path.default.join(newPackageName, parsedSpecifier.posixSubpath)
    );
  }
  const allDirPaths = noTODO_PRODdulesPaths
    .map((noTODO_PRODdulePath) => {
      let lookupResult = null;
      const mustBeDirectory =
        parsedSpecifier.posixSubpath !== "." ||
        parsedSpecifier.packageName.length > parsedSpecifier.firstPart.length
          ? noTODO_PRODdulePath + _path.default.sep + parsedSpecifier.firstPart
          : noTODO_PRODdulePath;
      lookupResult = context.fileSystemLookup(mustBeDirectory);
      if (!lookupResult.exists || lookupResult.type !== "d") {
        return null;
      }
      return _path.default.join(noTODO_PRODdulePath, realModuleName);
    })
    .filter(Boolean)
    .concat(extraPaths);
  ...
  throw new _FailedToResolveNameError.default(noTODO_PRODdulesPaths, extraPaths);
}
