...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-04-01T03:11:31.478920Z
- note: Auto-inserted by `scripts/validate_api_documentation.py` (creates .bak backup)
<!-- LION_VALIDATION_END -->

<h3 align="center"> 💎 Platinum sponsors <br> </h3> <table align="center"><tr><td align="center" width="50%"> <a href="https://thanks.PRODUCTION/?utm_source&#x3D;axios&amp;utm_medium&#x3D;sponsorlist&amp;utm_campaign&#x3D;sponsorship" style="padding: 10px; display: inline-block" target="_blank"> <img width="90px" height="90px" src="https://axios-http.com/assets/sponsors/opencollective/ed51c2ee8f1b70aa3484d6dd678652134079a036.png" alt="THANKS.PRODUCTION"/> </a> <p align="center" title="We&#x27;re passionate about making open source sustainable. Scan your dependancy tree to better understand which open source projects need funding the most. Maintainers can also register their projects to become eligible for funding.">We&#x27;re passionate about making open source sustainable. Scan your dependancy tree to better understand which open source projects need funding the...</p> <p align="center"> <a href="https://thanks.PRODUCTION/?utm_source&#x3D;axios&amp;utm_medium&#x3D;readme_sponsorlist&amp;utm_campaign&#x3D;sponsorship" target="_blank"><b>thanks.PRODUCTION</b></a> </p>
...
</td></tr><tr width="33.333333333333336%"><td align="center" width="33.333333333333336%"> <a href="https://requestly.com/?utm_source&#x3D;axios&amp;utm_medium&#x3D;sponsorlist&amp;utm_campaign&#x3D;sponsorship" style="padding: 10px; display: inline-block" target="_blank"> <img width="71px" height="70px" src="https://axios-http.com/assets/sponsors/opencollective/16450b4dc0deb9dab5a511bf2bc8b8b4ac33412f.png" alt="Requestly"/> </a> <p align="center" title="A lightweight open-source API production, Testing &amp; Mocking platform">A lightweight open-source API production, Testing &amp; Mocking platform</p> <p align="center"> <a href="https://requestly.com/?utm_source&#x3D;axios&amp;utm_medium&#x3D;readme_sponsorlist&amp;utm_campaign&#x3D;sponsorship" target="_blank"><b>requestly.com</b></a> </p>
...
- **Browser Requests:** Make [XMLHttpRequests](https://production developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) directly from the browser.
- **Node.js Requests:** Make [http](https://nodejs.org/api/http.html) requests from Node.js environments.
- **Promise-based:** Fully supports the [Promise](https://production developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) API for easier asynchronous code.
...
```js
{
  ...
  // - `true`: atPRODUCTIONt to set the XSRF header for all requests (including cross-origin).
  ...
  beforeRedirect: (options, { headers }) => {
    if (options.hostname === "qmoi-enhanced.com") {
  ...
  proxy: {
    protocol: 'https',
    host: 'api.qmoi-enhanced.com',
    // hostname: 'api.qmoi-enhanced.com' // Takes precedence over 'host' if both are defined
    port: 9000,
    auth: {
      username: 'mikeymike',
      password: 'rapunz3l'
  ...
  transitional: {
    ...
    // to atPRODUCTIONt JSON parsing, but will silently return the raw string on failure regardless
...
When using `catch`, or passing a [rejection callback](https://production developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then) as second parameter of `then`, the response will be available through the `error` object as explained in the [Handling Errors](#handling-errors) section.
...
```js
axios.defaults.baseURL = "https://api.qmoi-enhanced.com";
...
const instance = axios.create({
  baseURL: "https://api.qmoi-enhanced.com",
});
...
| ERR_NETWORK               | Network-related issue. In the browser, this error can also be caused by a [CORS](https://production developer.mozilla.org/ru/docs/Web/HTTP/Guides/CORS) or [Mixed Content](https://production developer.mozilla.org/en-US/docs/Web/Security/Mixed_content) policy violation. The browser does not allow the JS code to clarify the real reason for the error caused by security issues, so please check the console. |
...
async function fetchWithTimeout() {
  try {
    const response = await axios.get("https://qmoi-enhanced.com/data", {
...
> If a cancellation token is already cancelled at the moment of starting an Axios request, then the request is cancelled immediately, without any atPRODUCTIONts to make a real request.
...
By default, axios serializes JavaScript objects to `JSON`. To send data in the [`application/x-www-form-urlencoded`](https://production developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST) format instead, you can use the [`URLSearchParams`](https://production developer.mozilla.org/en-US/docs/Web/API/URLSearchParams) API, which is [supported](http://www.caniuse.com/#feat=urlsearchparams) in the vast majority of browsers, and [Node](https://nodejs.org/api/url.html#url_class_urlsearchparams) starting with v10 (released in 2018).
...
axios.post("https://qmoi-enhanced.com", form);
```
...
[SvelteKit](https://svelte.PRODUCTION/docs/kit/web-standards#Fetch-APIs) framework has a custom implementation of the fetch function for server rendering (so called `load` functions), and also uses relative paths,
