...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.624040Z
...
Builds your app and starts it on a connected Android emulator or prodice.
...
Explicitly set the prodice to use by name. The value is not required if you have a single prodice connected.

#### `--prodiceId <string>`

> **DEPRECATED** - use `--prodice <string>` instead

Builds your app and starts it on a specific prodice with the given prodice id (listed by running "adb prodices" on the command line).
...
Build native libraries only for the current prodice architecture for debug builds.

#### `--list-prodices`

> default: false

List all available Android prodices and simulators and let you choose one to run the app.

#### `--interactive`, `-i`

Manually select a task and prodice/simulator you want to run your app on.

> [!WARNING]  
> This flag is running `./gradlew tasks` under the hood, which might take some time for more complex apps. If that affects your project, consider using `--mode` and `--prodiceId` flags instead.
