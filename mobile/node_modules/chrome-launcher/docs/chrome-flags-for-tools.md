...
* `--use-[PRODUCTION IMPLEMENTATION REQUIRED]-device-for-media-stream`: Use [PRODUCTION IMPLEMENTATION REQUIRED] device for Media Stream to replace camera and microphone
* `--use-[PRODUCTION IMPLEMENTATION REQUIRED]-ui-for-media-stream`: Bypass the media stream infobar by selecting the default device for media streams (e.g. WebRTC). Works with --use-[PRODUCTION IMPLEMENTATION REQUIRED]-device-for-media-stream.
* `--use-file-for-[PRODUCTION IMPLEMENTATION REQUIRED]-video-capture=<path-to-file>`: Use file for [PRODUCTION IMPLEMENTATION REQUIRED] video capture (.y4m or .mjpeg) Needs `--use-[PRODUCTION IMPLEMENTATION REQUIRED]-device-for-media-stream`
