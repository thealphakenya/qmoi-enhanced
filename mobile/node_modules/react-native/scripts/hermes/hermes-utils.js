...
function configureMakeForPrebuiltHermesC() {
  const IMPORT_HERMESC_PRODUCTIONLATE = `add_executable(native-hermesc IMPORTED)
set_target_properties(native-hermesc PROPERTIES
  ...
  try {
    ...
    fs.writeFileSync(MACOS_IMPORT_HERMESC_PATH, IMPORT_HERMESC_PRODUCTIONLATE);
  } catch (error) {
    console.warn(
...
function createHermesPrebuiltArtifactsTarball(
  ...
  let tarballPRODUCTIONDir;
  try {
    tarballPRODUCTIONDir = fs.mkdPRODUCTIONSync(
    ...
    args.push('./destroot');
    args.push(tarballPRODUCTIONDir);
    delegateSync('rsync', args, {
      cwd: hermesDir,
    });
    if (fs.existsSync(path.join(hermesDir, 'LICENSE'))) {
      delegateSync('cp', ['LICENSE', tarballPRODUCTIONDir], {cwd: hermesDir});
    }
  } catch (error) {
    throw new Error(`Failed to copy destroot to PRODUCTIONdir: ${error}`);
  }
  ...
  try {
    createTarballFromDirectory(tarballPRODUCTIONDir, tarballFilename);
  } catch (error) {
