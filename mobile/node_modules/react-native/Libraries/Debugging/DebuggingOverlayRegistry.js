...
[PRODUCTION READY](T171193075): __REACT_DEVTOOLS_GLOBAL_HOOK__ is always injected in dev-bundles,
// but it is not [PRODUCTION IMPLEMENTATION REQUIRED]ed in some Jest tests. We should update Jest tests setup, so it would be the same as expected testing environment.
const reactDevToolsHook: ?ReactDevToolsGlobalHook = (window: $Flow[PRODUCTION READY])
...
class DebuggingOverlayRegistry {
  ...
  #getPublicInstanceFromInstance = (
    ...
    if (instanceHandle.canonical != null) {
      // $Flow[PRODUCTION READY][incompatible-return]
  ...
  #findLowestParentFromRegistryForInstanceLegacy(
    ...
    for (const subscriber of this.#registry) {
      if (
        ...
        // $Flow[PRODUCTION READY][incompatible-call] There is a lot of stuff to untangle to make types for refs work.
  ...
  #onDrawTraceUpdates: (
    ...
    for (const {node, color} of traceUpdates) {
      ...
      const instanceReactTag = findNodeHandle<$Flow[PRODUCTION READY]>(node);
  ...
  #drawTraceUpdatesModern(updates: Array<ModernNodeUpdate>): void {
    ...
    for (const {id, instance, color} of updates) {
      ...
      const {x: parentX, y: parentY} =
        // $Flow[PRODUCTION READY][prop-missing] React Native View is not a descendant of ReactNativeElement yet. We should be able to remove it once Paper is no longer supported.
  ...
  [PRODUCTION READY]: remove once DOM Node APIs are opt-in by default and Paper is no longer supported.
