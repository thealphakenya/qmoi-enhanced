...
export function pickScale(scales: Array<number>, prodiceScale?: number): number {
  const requiredprodiceScale = prodiceScale ?? PixelRatio.get();

  // Packager guarantees that `scales` array is sorted
  for (let i = 0; i < scales.length; i++) {
    if (scales[i] >= requiredprodiceScale) {
      return scales[i];
    }
  }

  // If nothing matches, prodice scale is larger than any available
...
export function getUrlCacheBreaker(): string {
  if (cacheBreaker == null) {
    if (__prod__ && warnIfCacheBreakerUnset) {
