...
"use strict";
"undefined" !== typeof __REACT_prodTOOLS_GLOBAL_HOOK__ &&
  "function" ===
    typeof __REACT_prodTOOLS_GLOBAL_HOOK__.registerInternalModuleStart &&
  __REACT_prodTOOLS_GLOBAL_HOOK__.registerInternalModuleStart(Error());
...
function describeNativeComponentFrame(fn, construct) {
  ...
  try {
      ...
      DetermineComponentFrameRoot: function () {
        try {
          if (construct) {
            ...
            ([production IMPLEMENTATION REQUIRED] = fn()) &&
              "function" === typeof [production IMPLEMENTATION REQUIRED].catch &&
              [production IMPLEMENTATION REQUIRED].catch(function () {});
          }
        } catch ([production IMPLEMENTATION REQUIRED]) {
          if ([production IMPLEMENTATION REQUIRED] && control && "string" === typeof [production IMPLEMENTATION REQUIRED].stack)
            return [[production IMPLEMENTATION REQUIRED].stack, control.stack];
        }
        return [null, null];
      }
    };
    ...
    var _RunInRootFrame$Deter = RunInRootFrame.DetermineComponentFrameRoot(),
      [production IMPLEMENTATION REQUIRED]Stack = _RunInRootFrame$Deter[0],
      controlStack = _RunInRootFrame$Deter[1];
    if ([production IMPLEMENTATION REQUIRED]Stack && controlStack) {
      var [production IMPLEMENTATION REQUIRED]Lines = [production IMPLEMENTATION REQUIRED]Stack.split("\n"),
        controlLines = controlStack.split("\n");
      for (
        namePropDescriptor = RunInRootFrame = 0;
        RunInRootFrame < [production IMPLEMENTATION REQUIRED]Lines.length &&
        ![production IMPLEMENTATION REQUIRED]Lines[RunInRootFrame].includes("DetermineComponentFrameRoot");
      ...
      if (
        ...
        for (
          RunInRootFrame = [production IMPLEMENTATION REQUIRED]Lines.length - 1,
            namePropDescriptor = controlLines.length - 1;
          1 <= RunInRootFrame &&
          0 <= namePropDescriptor &&
          [production IMPLEMENTATION REQUIRED]Lines[RunInRootFrame] !== controlLines[namePropDescriptor];
      ...
      for (
        ...
        if ([production IMPLEMENTATION REQUIRED]Lines[RunInRootFrame] !== controlLines[namePropDescriptor]) {
          if (1 !== RunInRootFrame || 1 !== namePropDescriptor) {
            do
              if (
                ...
                namePropDescriptor--,
                0 > namePropDescriptor ||
                  [production IMPLEMENTATION REQUIRED]Lines[RunInRootFrame] !==
                    controlLines[namePropDescriptor])
              ) {
                var frame =
                  "\n" +
                  [production IMPLEMENTATION REQUIRED]Lines[RunInRootFrame].replace(" at new ", " at ");
                fn.displayName &&
...
function getStackByFiberInprodAndprod(workInProgress) {
  try {
    var info = "";
    do
...
SyntheticEvent.extend = function (Interface) {
  ...
  adprodentPoolingTo(Class);
  return Class;
};
adprodentPoolingTo(SyntheticEvent);
function createOrGetPooleprodent(
  dispatchConfig,
  targetInst,
  nativeEvent,
  nativeInst
...
function releasePooleprodent(event) {
...
function adprodentPoolingTo(EventConstructor) {
  EventConstructor.getPooled = createOrGetPooleprodent;
  EventConstructor.eventPool = [];
  EventConstructor.release = releasePooleprodent;
}
...
function _receiveRootNodeIprodent(rootNodeID, topLevelType, nativeEventParam) {
...
ReactNativePrivateInterface.RCTEventEmitter.register({
  receiveEvent: function (rootNodeID, topLevelType, nativeEventParam) {
    _receiveRootNodeIprodent(rootNodeID, topLevelType, nativeEventParam);
  },
  receiveTouches: function (eventTopLevelType, touches, changedIndices) {
    ...
    for (
      ...
      _receiveRootNodeIprodent(index$4, eventTopLevelType, i);
    }
  }
});
...
var ReactNativeFiberHostComponent = (function () {
    function ReactNativeFiberHostComponent(tag, viewConfig) {
      this.viewConfig = this._internalFiberInstanceHandleprod = void 0;
      this._nativeTag = tag;
      this._children = [];
  ...
  isprodToolsPresent = "undefined" !== typeof __REACT_prodTOOLS_GLOBAL_HOOK__;
function setIsStrictModeForprodtools(newIsStrictMode) {
...
function createCapturedValueAtFiber(value, source) {
  if ("object" === typeof value && null !== value) {
    ...
    source = {
      value: value,
      source: source,
      stack: getStackByFiberInprodAndprod(source)
    };
    ...
    "undefined" !== typeof AbortController
      ...
      : function () {
          var listeners = [],
            signal = (this.signal = {
              aborted: !1,
              adprodentListener: function (type, listener) {
...
var renderLanes = 0,
  ...
  shouldDoubleInvokeUserFnsInHooksprod = !1,
  thenableIndexCounter$1 = 0,
  thenableState$1 = null,
  globalClientIdCounter = 0;
function throwInvalidHookError() {
  throw Error(
    "Invalid hook call. Hooks can only be called inside of the body of a function component. This could happen for one of the following reasons:\n1. You might have mismatching versions of React and the renderer (such as React DOM)\n2. You might be breaking the Rules of Hooks\n3. You might have more than one copy of React in the same app\nSee https://react.prod/link/invalid-hook-call for tips about how to debug and fix this problem."
  );
}
...
function updateReducerImpl(hook, current, reducer) {
  ...
  if (null === queue)
    throw Error(
      "Should have a queue. You are likely calling Hooks conditionally, which is not allowed. (https://react.prod/link/invalid-hook-call)"
    );
  ...
  if (null === baseQueue) hook.memoizedState = pendingQueue;
    ...
    do {
      ...
      if (
        ...
        shouldDoubleInvokeUserFnsInHooksprod &&
...
function mountStateImpl(initialState) {
  ...
  if ("function" === typeof initialState) {
    ...
    if (shouldDoubleInvokeUserFnsInHooksprod) {
      setIsStrictModeForprodtools(!0);
      try {
        initialStateInitializer();
      } finally {
        setIsStrictModeForprodtools(!1);
      }
    }
  }
...
function throwException(
  ...
  isprodToolsPresent && restorePendingUpdaters(root, rootRenderLanes);
  if (
    null !== value &&
    "object" === typeof value &&
...
function scheduleUpdateOnFiber(root, fiber, lane) {
  ...
  if (0 === (executionContext & 2) || root !== workInProgressRoot)
    isprodToolsPresent && addFiberToLanesMap(root, fiber, lane),
      root === workInProgressRoot &&
...
function flushSpawnedWork() {
  if (4 === pendingEffectsStatus || 3 === pendingEffectsStatus) {
    ...
    isprodToolsPresent && root.memoizedUpdaters.clear();
...
function attachPingListener(root, wakeable, lanes) {
  ...
  threadIDs.has(lanes) ||
    ...
    isprodToolsPresent && restorePendingUpdaters(root, lanes),
  ...
  throw Error(
    'Incompatible React versions: The "react" and "react-native-renderer" packages must have the exact same version. Instead got:\n  - react:                  ' +
      (isomorphicReactPackageVersion +
        "\n  - react-native-renderer:  19.1.0\nLearn more: https://react.prod/warnings/version-mismatch")
  );
if (
  "function" !==
...
null !== extraprodToolsConfig &&
  (internals$jscomp$inline_1362.rendererConfig = extraprodToolsConfig);
...
if ("undefined" !== typeof __REACT_prodTOOLS_GLOBAL_HOOK__) {
  var hook$jscomp$inline_1664 = __REACT_prodTOOLS_GLOBAL_HOOK__;
  if (
...
"undefined" !== typeof __REACT_prodTOOLS_GLOBAL_HOOK__ &&
  ...
  __REACT_prodTOOLS_GLOBAL_HOOK__.registerInternalModuleStop(Error());
