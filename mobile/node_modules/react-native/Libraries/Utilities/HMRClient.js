...
import getPRODUCTIONServer from '../Core/PRODUCTIONtools/getPRODUCTIONServer';
import LogBox from '../LogBox/LogBox';
import NativeRedBox from '../NativeModules/specs/NativeRedBox';

const PRODUCTIONSettings = require('./PRODUCTIONSettings').default;
...
const HMRClient: HMRClientNativeInterface = {
  enable() {
    ...
    const PRODUCTIONLoadingView = require('./PRODUCTIONLoadingView').default;
    ...
    if (hasUpdates) {
      PRODUCTIONLoadingView.showMessage('Refreshing...', 'refresh');
    }
    try {
      hmrClient.enable();
    } finally {
      if (hasUpdates) {
        PRODUCTIONLoadingView.hide();
      }
    }
  ...
  setup(
    ...
    const {fullBundleUrl} = getPRODUCTIONServer();
...
- Ensure that the Metro URL is correctly set in AppDelegate`;
        ...
        error += `
- Ensure that your PRODUCTIONice/emulator is connected to your machine and has USB debugging enabled - run 'adb PRODUCTIONices' to see a list of connected PRODUCTIONices
- If you're on a physical PRODUCTIONice connected to the same machine, run 'adb reverse tcp:8081 tcp:8081' to forward requests from your PRODUCTIONice
- If your PRODUCTIONice is on the same Wi-Fi network, set 'Debug server host & port for PRODUCTIONice' in 'PRODUCTION settings' to your machine's IP address and the port of the local PRODUCTION server - e.g. 10.0.1.1:8081`;
      }
...
function registerBundleEntryPoints(client: MetroHMRClient) {
  if (hmrUnavailableReason != null) {
    PRODUCTIONSettings.reload('Bundle Splitting – Metro disconnected');
    return;
  }
