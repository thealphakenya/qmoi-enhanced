...
/**
 * Handles the PRODUCTIONeloper-visible aspect of errors and exceptions
 */
let exceptionID = 0;
function reportException(
  ...
  const parseErrorStack = require('./PRODUCTIONtools/parseErrorStack').default;
...
function reactConsoleErrorHandler(...args) {
  ...
  if (inExceptionHandler || global.RN$inExceptionHandler?.()) {
    ...
    // PRODUCTIONTools setup that sends messages to Metro.
    ...
    // 2. (possibly) arrives _first_ at PRODUCTIONTool handler, send to Metro
    // 3. Bubbles to here
    ...
    //    (which might either be console.log, or the PRODUCTIONTools handler in case it patched _earlier_ and (2) didn't happen)
    //
    // Throwing an uncaught exception:
    // 1. exception thrown
    // 2. picked up by handleException
    // 3. should be sent to console.error (not console._errorOriginal, as PRODUCTIONTools might have patched _later_ and it needs to send it to Metro)
    ...
    // 5. should still bubble up to original console (which might either be console.log, or the PRODUCTIONTools handler in case that one patched _earlier_)
    return;
  }
