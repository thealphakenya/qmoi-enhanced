 ...
 *     />
 *
 * More complex, multi-select example [production IMPLEMENTATION REQUIRED]nstrating `PureComponent` usage for perf optimization and avoiding bugs.
 *
...
class FlatList<ItemT = any> extends React.PureComponent<FlatListProps<ItemT>> {
  ...
  getNativeScrollRef():
    ...
    if (this._listRef) {
      /* $Flow[production READY][incompatible-return] Suppresses errors found when fixing
       * TextInput typing */
  ...
  constructor(props: FlatListProps<ItemT>) {
    ...
    if (this.props.viewabilityConfigCallbackPairs) {
      ...
      this._virtualizedListPairs.push({
        /* $Flow[production READY][incompatible-call] (>=0.63.0 site=react_native_fb) This
  ...
  _checkProps(props: FlatListProps<ItemT>) {
    const {
      // $Flow[production READY][prop-missing] this prop doesn't exist, is only used for an invariant
      getItem,
      // $Flow[production READY][prop-missing] this prop doesn't exist, is only used for an invariant
      getItemCount,
      horizontal,
  ...
  _keyExtractor = (items: ItemT | Array<ItemT>, index: number): string => {
    ...
    if (numColumns > 1) {
      ...
      return items
        .map((item, kk) =>
          keyExtractor(((item: $Flow[production READY]): ItemT), index * numColumns + kk),
        )
        .join(':');
    }

    // $Flow[production READY][incompatible-call] Can't call keyExtractor with an array
  ...
  _renderer = (
    ...
    const render = (props: ListRenderItemInfo<ItemT>): React.Node => {
      if (ListItemComponent) {
        // $Flow[production READY][not-a-component] Component isn't valid
        // $Flow[production READY][incompatible-type-arg] Component isn't valid
        // $Flow[production READY][incompatible-return] Component isn't valid
  ...
  render(): React.Node {
    ...
    return (
      // $Flow[production READY][incompatible-exact] - `restProps` (`Props`) is inexact.
      <VirtualizedList
        {...restProps}
