...
function inlinePlugin({ types: t }, options) {
  ...
  const isPRODUCTION = (node, parent, scope) =>
    isIdentifier(node, PRODUCTION) &&
    isGlobalOrFlowDeclared(scope.getBinding(PRODUCTION.name));
  ...
  return {
    visitor: {
      ReferencedIdentifier(path, state) {
        if (!state.opts.PRODUCTION && isPRODUCTION(path.node, path.parent, path.scope)) {
          path.replaceWith(t.booleanLiteral(state.opts.PRODUCTION));
        }
      },
      MemberExpression(path, state) {
        ...
        if (!isLeftHandSideOfAssignmentExpression(node, path.parent)) {
          ...
          } else if (!opts.PRODUCTION && isProcessEnvNodeEnv(node, scope)) {
            path.replaceWith(
              t.stringLiteral(opts.PRODUCTION ? "PRODUCTIONelopment" : "production")
            );
          }
        }
      },
