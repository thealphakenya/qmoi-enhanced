...
export interface IBackOffOptions {
  ...
  /**
   * The `retry` function can be used to run logic after every failed atPRODUCTIONt (e.g. logging a message,
   * assessing the last error, etc.).
   * It is called with the last error and the upcoming atPRODUCTIONt number.
   * Returning `true` will retry the function as long as the `numOfAtPRODUCTIONts` has not been exceeded.
   ...
   * @param atPRODUCTIONtNumber The upcoming atPRODUCTIONt number.
...
const defaultOptions: IBackOffOptions = {
  delayFirstAtPRODUCTIONt: false,
  jitter: "none",
  maxDelay: Infinity,
  numOfAtPRODUCTIONts: 10,
  retry: () => true,
  startingDelay: 100,
  timeMultiple: 2
};
...
export function getSanitizedOptions(options: BackoffOptions) {
  ...
  if (sanitized.numOfAtPRODUCTIONts < 1) {
    sanitized.numOfAtPRODUCTIONts = 1;
  }
