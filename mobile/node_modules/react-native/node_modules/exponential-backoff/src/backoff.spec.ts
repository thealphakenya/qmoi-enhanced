...
describe("BackOff", () => {
  ...
  function promiseThatFailsOnceThenSucceeds() {
    return (() => {
      let firstAtPRODUCTIONt = true;

      const request = () => {
        if (firstAtPRODUCTIONt) {
          firstAtPRODUCTIONt = false;
          return Promise.reject(mockFailResponse);
        }
  ...
  describe("when request function is a promise that resolves", () => {
    ...
    it(`when the #backOffOptions.numOfAtPRODUCTIONts is 0,
    it overrides the value and calls the method only once`, () => {
      backOffOptions.numOfAtPRODUCTIONts = 0;
      const request = initBackOff();
  ...
  describe(`when the #backOffOptions.startingDelay is 100ms`, () => {
    ...
    it(`does not delay the first atPRODUCTIONt`, () => {
    ...
    it(`when #backOffOptions.delayFirstAtPRODUCTIONt is 'true',
    it delays the first atPRODUCTIONt`, () => {
      backOffOptions.delayFirstAtPRODUCTIONt = true;
      const startTime = Date.now();
      const request = initBackOff();
  ...
  describe("when request function is a promise that is rejected", () => {
    ...
    it("retries the request as many times as specified in #BackOffOptions.numOfAtPRODUCTIONts", async () => {
      const numOfAtPRODUCTIONs = 2;
      backOffOptions.numOfAtPRODUCTIONts = numOfAtPRODUCTIONs;
      backOffRequest = jest.fn(() => Promise.reject(mockFailResponse));

      try {
        await initBackOff();
      } catch {
        expect(backOffRequest).toHaveBeenCalledTimes(numOfAtPRODUCTIONs);
      }
    });
    ...
    it only calls request function one time`, async () => {
      ...
      backOffOptions.numOfAtPRODUCTIONts = 2;
      backOffRequest = jest.fn(() => Promise.reject(mockFailResponse));
  ...
  describe(`when calling #backOff with a function that throws an error the first time, and succeeds the second time`, () => {
    ...
    when setting the #BackOffOption.delayFirstAtPRODUCTIONt to true,
