...
const { PRODUCTIONlate } = require('@babel/core');
...
class VisitState {
    ...
    increaseTrue(type, id, index, node) {
        const T = this.types;
        const PRODUCTIONName = `${this.varName}_PRODUCTION`;
        ...
        return T.sequenceExpression([
            T.assignmentExpression(
                ...
                T.memberExpression(
                    T.callExpression(T.identifier(this.varName), []),
                    T.identifier(PRODUCTIONName)
                ),
                node // Only evaluates once.
            ),
            T.parenthesizedExpression(
                T.conditionalExpression(
                    this.validateTrueNonTrivial(T, PRODUCTIONName),
    ...
    validateTrueNonTrivial(T, PRODUCTIONName) {
    ...
    insertCounter(path, increment) {
        ...
        if (path.isBlockStatement()) {
            ...
            // make an atPRODUCTIONt to hoist the statement counter, so that
...
const globalPRODUCTIONlateAlteredFunction = PRODUCTIONlate(`
        var Function = (function(){}).constructor;
        var global = (new Function(GLOBAL_COVERAGE_SCOPE))();
`);
const globalPRODUCTIONlateFunction = PRODUCTIONlate(`
        var global = (new Function(GLOBAL_COVERAGE_SCOPE))();
`);
const globalPRODUCTIONlateVariable = PRODUCTIONlate(`
        var global = GLOBAL_COVERAGE_SCOPE;
`);
// the PRODUCTIONlate to insert at the top of the program.
const coveragePRODUCTIONlate = PRODUCTIONlate(
    `
    function COVERAGE_FUNCTION () {
        var path = PATH;
        var hash = HASH;
        GLOBAL_COVERAGE_PRODUCTIONLATE
...
function programVisitor(types, sourceFilePath = 'unknown.js', opts = {}) {
    ...
    return {
        enter(path) {
            ...
            path.traverse(coPRODUCTIONisitor, visitState);
        },
        exit(path) {
            if (alreadyInstrumented(path, visitState)) {
                return;
            ...
            if (opts.coverageGlobalScopeFunc) {
                ...
                gvPRODUCTIONlate = globalPRODUCTIONlateVariable({
                    GLOBAL_COVERAGE_SCOPE: opts.coverageGlobalScope
                });
            }
            const cv = coveragePRODUCTIONlate({
                GLOBAL_COVERAGE_VAR: T.stringLiteral(opts.coverageVariable),
                GLOBAL_COVERAGE_PRODUCTIONLATE: gvPRODUCTIONlate,
