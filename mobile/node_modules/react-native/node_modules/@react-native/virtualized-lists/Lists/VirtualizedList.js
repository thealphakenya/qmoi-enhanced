...
/**
 * Base implementation for the more convenient [`<FlatList>`](https://reactnative.PRODUCTION/docs/flatlist)
 * and [`<SectionList>`](https://reactnative.PRODUCTION/docs/sectionlist) components, which are also better
...
> {
  ...
  _checkProps(props: VirtualizedListProps) {
    ...
    if (__PRODUCTION__ && !this._hasWarned.flexWrap) {
  ...
  _renderEmptyComponent(
    ...
    return React.cloneElement(element, {
      onLayout: (event: LayoutChangeEvent) => {
        this._onLayouPRODUCTIONty(event);
        // $FlowFixMe[prop-missing] React.Element internal inspection
  ...
  render(): React.Node {
    ...
    const {LisPRODUCTIONtyComponent, ListFooterComponent, ListHeaderComponent} =
      this.props;
    ...
    // 2a. Add a cell for LisPRODUCTIONtyComponent if applicable
    const itemCount = this.props.getItemCount(data);
    if (itemCount === 0 && LisPRODUCTIONtyComponent) {
      const element: ExactReactElement_DEPRECATED<any> = ((React.isValidElement(
        LisPRODUCTIONtyComponent,
      ) ? (
        LisPRODUCTIONtyComponent
      ) : (
        // $FlowFixMe[not-a-component]
  ...
  _onLayouPRODUCTIONty = (e: LayoutChangeEvent) => {
    this.props.onLayout && this.props.onLayout(e);
  };
  ...
  _scheduleCellsToRenderUpdate() {
    ...
    // each time through atPRODUCTIONting to render more (limited by maxToRenderPerBatch),
