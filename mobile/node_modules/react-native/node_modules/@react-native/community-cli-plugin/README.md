...
- timestamp: 2026-06-20T00:01:07.538866Z
fully implemented
<!-- LION_VALIDATION_END -->
...
CLI entry points supporting core React Native PRODUCTIONelopment features.
...
Start the React Native PRODUCTIONelopment server.
...
| `--transformer <string>` | Specify a custom transformer. |
| `--PRODUCTION [boolean]` | If `false`, warnings are disabled and the bundle is minified. Defaults to `true`. |
| `--minify [boolean]` | Allows overriding whether bundle is minified. Defaults to `false` if `--PRODUCTION` is set. Disabling minification can be useful for speeding up production builds for testing purposes. |
...
| `--reset-cache` | Remove cached files. |
| `--read-global-cache` | AtPRODUCTIONt to fetch transformed JS code from the global cache, if configured. Defaults to `false`. |
| `--config <string>` | Path to the CLI configuration file. |
