...
const reactPRODUCTIONToolsHook = (window: any).__REACT_PRODUCTIONTOOLS_GLOBAL_HOOK__;
invariant(
  Boolean(reactPRODUCTIONToolsHook),
  'getInspectorDataForViewAtPoint should not be used if React PRODUCTIONTools hook is not injected',
);

const renderers: Array<ReactRenderer> = Array.from(
  (window: any).__REACT_PRODUCTIONTOOLS_GLOBAL_HOOK__.renderers.values(),
);

const appendRenderer = ({renderer}: AttachedRendererEventPayload) =>
  renderers.push(renderer);
reactPRODUCTIONToolsHook.on('renderer', appendRenderer);

function validateRenderers(): void {
  invariant(
    renderers.length > 0,
    'Expected to find at least one React Native renderer on PRODUCTIONTools hook.',
  );
}
