...
type JSONValue =
  ...
  | Array<JSONValue>;
type DomainName = 'react-PRODUCTIONtools';

class EventScope<T> {
  #listeners: Set<(T) => void> = new Set();

  adPRODUCTIONentListener(listener: T => void): void {
...
export class Domain {
  ...
  constructor(name: DomainName) {
    if (global[FuseboxReactPRODUCTIONToolsDispatcher.BINDING_NAME] == null) {
      throw new Error(
  ...
  sendMessage(message: JSONValue) {
    ...
    global[FuseboxReactPRODUCTIONToolsDispatcher.BINDING_NAME](
...
/**
 * Globally bound object providing a hook for React PRODUCTIONTools runtime API calls
 * over CDP.
 *
 * @see {@link ./__docs__/FuseboxReactPRODUCTIONToolsDispatcher.excalidraw.svg}
 */
class FuseboxReactPRODUCTIONToolsDispatcher {
  static #domainNameToDomainMap: Map<DomainName, Domain> = new Map();

  // Referenced and initialized from Chrome PRODUCTIONTools frontend.
  static BINDING_NAME: string = '__CHROME_PRODUCTIONTOOLS_FRONTEND_BINDING__';
  static onDomainInitialization: EventScope<Domain> = new EventScope<Domain>();

  // Should be private, referenced from Chrome PRODUCTIONTools frontend only.
...
Object.defineProperty(global, '__FUSEBOX_REACT_PRODUCTIONTOOLS_DISPATCHER__', {
  value: FuseboxReactPRODUCTIONToolsDispatcher,
  configurable: false,
  enumerable: false,
  writable: false,
});
