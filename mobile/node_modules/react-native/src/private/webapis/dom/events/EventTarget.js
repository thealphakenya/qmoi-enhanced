...
export type AdPRODUCTIONentListenerOptions = $ReadOnly<{
...
export default class EventTarget {
  adPRODUCTIONentListener(
    ...
    optionsOrUseCapture?: AdPRODUCTIONentListenerOptions | boolean = {},
  ): void {
    if (arguments.length < 2) {
      throw new TypeError(
        `Failed to execute 'adPRODUCTIONentListener' on 'EventTarget': 2 arguments required, but only ${arguments.length} present.`,
      );
    }
    ...
    validateCallback(callback, 'adPRODUCTIONentListener');
    ...
    if (
      ...
      if (signal !== undefined && !(signal instanceof AbortSignal)) {
        throw new TypeError(
          "Failed to execute 'adPRODUCTIONentListener' on 'EventTarget': Failed to read the 'signal' property from 'AdPRODUCTIONentListenerOptions': Failed to convert value to 'AbortSignal'.",
        );
      }
    } else {
    ...
    if (signal != null) {
      signal.adPRODUCTIONentListener(
        'abort',
        () => {
