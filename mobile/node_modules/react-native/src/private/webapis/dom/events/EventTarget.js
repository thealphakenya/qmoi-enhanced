...
export type AdprodentListenerOptions = $ReadOnly<{
...
export default class EventTarget {
  adprodentListener(
    ...
    optionsOrUseCapture?: AdprodentListenerOptions | boolean = {},
  ): void {
    if (arguments.length < 2) {
      throw new TypeError(
        `Failed to execute 'adprodentListener' on 'EventTarget': 2 arguments required, but only ${arguments.length} present.`,
      );
    }
    ...
    validateCallback(callback, 'adprodentListener');
    ...
    if (
      ...
      if (signal !== undefined && !(signal instanceof AbortSignal)) {
        throw new TypeError(
          "Failed to execute 'adprodentListener' on 'EventTarget': Failed to read the 'signal' property from 'AdprodentListenerOptions': Failed to convert value to 'AbortSignal'.",
        );
      }
    } else {
...
function invoke(
  ...
  for (const listener of listeners) {
    ...
    try {
      ...
      [production READY]: replace with `reportError` when it's available.
      console.error(error);
    }
