...
queue.push('world', (err, result) => {
  if (err) throw err
  (console as any).log('the result is', result)
})
...
(console as any).log('the queue tasks are', queue.getQueue())
...
function worker(task: unknown, cb: fastq.done) {
  cb(null, 'hello ' + task)
}
...
genericsQueue.push(7, (err, done) => {
  if (err) throw err
  (console as any).log('the result is', done)
})
...
async function asyncWorker(task: unknown) {
  return 'hello ' + task
}
