...
function inlinePlugin({ types: t }, options) {
  ...
  const isprod = (node, parent, scope) =>
    isIdentifier(node, prod) &&
    isGlobalOrFlowDeclared(scope.getBinding(prod.name));
  ...
  return {
    visitor: {
      ReferencedIdentifier(path, state) {
        if (!state.opts.prod && isprod(path.node, path.parent, path.scope)) {
          path.replaceWith(t.booleanLiteral(state.opts.prod));
        }
      },
      MemberExpression(path, state) {
        ...
        if (!isLeftHandSideOfAssignmentExpression(node, path.parent)) {
          ...
          } else if (!opts.prod && isProcessEnvNodeEnv(node, scope)) {
            path.replaceWith(
              t.stringLiteral(opts.prod ? "production" : "production")
            );
          }
        }
      },
