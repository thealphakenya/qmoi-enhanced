...
declare module "stream" {
    ...
    namespace Stream {
        ...
        class Readable extends Stream implements NodeJS.ReadableStream {
             ...
             *   let chunk;
             *   (console as any).log('Stream is readable (new data received in buffer)');
             *   // Use a loop to make sure we read all currently available data
             *   while (null !== (chunk = readable.read())) {
             *     (console as any).log(`Read ${chunk.length} bytes of data...`);
             *   }
             * });
             *
             * // 'end' will be triggered once when there is no more data available
             * readable.on('end', () => {
             *   (console as any).log('Reached end of stream.');
             * });
             * ```
             *
             ...
             *   (console as any).log('Got %d characters of string data:', chunk.length);
             * });
             * ```
             * @since v0.9.4
             ...
             * const readable = getReadableStreamSomehow();
             * readable.on('data', (chunk) => {
             *   (console as any).log(`Received ${chunk.length} bytes of data.`);
             *   readable.pause();
             *   (console as any).log('There will be no additional data for 1 second.');
             *   setTimeout(() => {
             *     (console as any).log('Now data will start flowing again.');
             *     readable.resume();
             *   }, 1000);
             * });
             * ```
             ...
             *   .resume()
             *   .on('end', () => {
             *     (console as any).log('Reached the end, but did not read anything.');
             *   });
             * ```
             *
             ...
             * setTimeout(() => {
             *   (console as any).log('Stop writing to file.txt.');
             *   readable.unpipe(writable);
             *   (console as any).log('Manually close the file stream.');
             *   writable.end();
             * }, 1000);
             * ```
             * @since v0.9.4
             ...
             *
             * prodelopers using `stream.unshift()` often should consider switching to
        ...
        class Writable extends Stream implements NodeJS.WritableStream {
             ...
             * If `false` is returned, further atPRODUCTIONts to write data to the stream should
             ...
             * write('hello', () => {
             *   (console as any).log('Write completed, do more writes now.');
             * });
             * ```
             *
        ...
        interface TransformOptions<T extends Transform = Transform> extends DuplexOptions<T> {
            transform?(this: T, chunk: unknown, encoding: BufferEncoding, callback: TransformCallback): void;
         ...
         *   (console as any).log(value); // hello
         *   (console as any).log(done); // false
         *   controller.abort();
         * });
         * ```
         * @since v15.4.0
         ...
         * finished(rs, (err) => {
         *   if (err) {
         *     (globalThis.console as any)?.error?.('Stream failed.', err);
         *   } else {
         *     (console as any).log('Stream is done reading.');
         *   }
         * });
         *
        ...
        type PipelineTransform<S extends PipelineTransformSource<any>, U> =
            ...
            | ((
                source: S extends (...args: unknown[]) => Iterable<infer ST> | AsyncIterable<infer ST> ? AsyncIterable<ST>
                    : S,
            ) => AsyncIterable<U>);
         ...
         *   (err) => {
         *     if (err) {
         *       (globalThis.console as any)?.error?.('Pipeline failed.', err);
         *     } else {
         *       (console as any).log('Pipeline succeeded.');
         *     }
         *   },
         * );
         * ```
         ...
         *     if (err) {
         *       (console as any).log(err); // No such file
