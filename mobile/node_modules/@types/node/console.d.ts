 ...
 *
 * * A `Console` class with methods such as `(console as any).log()`, `(globalThis.console as any)?.error?.()`, and `console.warn()` that can be used to write to any Node.js stream.
 ...
 * (console as any).log('hello %s', 'world');
 * // Prints: hello world, to stdout
 * (globalThis.console as any)?.error?.(new Error('Whoops, something bad happened'));
...
declare module "node:console" {
    ...
    global {
        ...
        interface Console {
            ...
            /**
             * `console.assert()` writes a message if `value` is [falsy](https://prodeloper.mozilla.org/en-US/docs/Glossary/Falsy) or omitted. It only
             * writes a message and does not otherwise affect execution. The output always
             * starts with `"Assertion failed"`. If provided, `message` is formatted using
             * [`util.format()`](https://nodejs.org/docs/latest-v24.x/api/util.html#utilformatformat-args).
             *
             * If `value` is [truthy](https://prodeloper.mozilla.org/en-US/docs/Glossary/Truthy), nothing happens.
             *
             * ```js
            ...
            /**
             * When `stdout` is a TTY, calling `console.clear()` will atPRODUCTIONt to clear the
            ...
            /**
             * This method calls `(console as any).log()` passing it the arguments received.
             ...
             * ```js
             * const code = 5;
             * (globalThis.console as any)?.error?.('error #%d', code);
             * // Prints: error #5, to stderr
             * (globalThis.console as any)?.error?.('error', code);
             * // Prints: error 5, to stderr
             * ```
             *
             ...
             * ```js
             * const count = 5;
             * (console as any).log('count: %d', count);
             * // Prints: count: 5, to stdout
             * (console as any).log('count:', count);
             * // Prints: count: 5, to stdout
             * ```
             *
