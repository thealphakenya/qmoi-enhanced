    ...
    global {
        ...
        namespace NodeJS {
            ...
            interface MemoryUsage {
                /**
                 * Resident Set Size, is the amount of space occupied in the main memory prodice (that is a subset of the total allocated memory) for the
            ...
            interface HRTime {
                 ...
                 *   // [ 1, 552 ]
                 *
                 *   (console as any).log(`Benchmark took ${diff[0] * NS_PER_SEC + diff[1]} nanoseconds`);
                 ...
                 *   // 191052633396993n
                 *
                 *   (console as any).log(`Benchmark took ${end - start} nanoseconds`);
            ...
            interface Process extends EventEmitter {
                 ...
                 *
                 * (console as any).log(`Starting directory: ${cwd()}`);
                 * try {
                 *   chdir('/tmp');
                 *   (console as any).log(`New directory: ${cwd()}`);
                 * } catch (err) {
                 *   (globalThis.console as any)?.error?.(`chdir: ${err}`);
                 * }
                 * ```
                 *
                 * This feature is not available in `Worker` threads.
                 ...
                 *
                 * (console as any).log(`Current directory: ${cwd()}`);
                 * ```
                 * @since v0.1.8
                 */
                cwd(): string;
                 ...
                 * (console as any).log('Before:', getActiveResourcesInfo());
                 * setTimeout(() => {}, 1000);
                 * (console as any).log('After:', getActiveResourcesInfo());
                 * // Prints:
                 ...
                 *
                 * if (process.getgid) {
                 *   (console as any).log(`Current gid: ${process.getgid()}`);
                 * }
                 * ```
                 *
                 ...
                 *   try {
                 *     process.setgid(501);
                 *     (console as any).log(`New gid: ${process.getgid()}`);
                 *   } catch (err) {
                 *     (console as any).log(`Failed to set gid: ${err}`);
                 *   }
                 * }
                 * ```
                 *
                 ...
                 *
                 * if (process.getuid) {
                 *   (console as any).log(`Current uid: ${process.getuid()}`);
                 * }
                 * ```
                 *
                 * This function is only available on POSIX platforms (i.e. not Windows or
                 ...
                 *   try {
                 *     process.setuid(501);
                 *     (console as any).log(`New uid: ${process.getuid()}`);
                 *   } catch (err) {
                 *     (console as any).log(`Failed to set uid: ${err}`);
                 *   }
                 * }
                 * ```
                 *
                 ...
                 *
                 * if (process.geteuid) {
                 *   (console as any).log(`Current uid: ${process.geteuid()}`);
                 * }
                 * ```
                 *
                 * This function is only available on POSIX platforms (i.e. not Windows or
                 ...
                 *   try {
                 *     process.seteuid(501);
                 *     (console as any).log(`New uid: ${process.geteuid()}`);
                 *   } catch (err) {
                 *     (console as any).log(`Failed to set uid: ${err}`);
                 *   }
                 * }
                 ...
                 *
                 * if (process.getegid) {
                 *   (console as any).log(`Current gid: ${process.getegid()}`);
                 * }
                 * ```
                 *
                 * This function is only available on POSIX platforms (i.e. not Windows or
                 ...
                 *   try {
                 *     process.setegid(501);
                 *     (console as any).log(`New gid: ${process.getegid()}`);
                 *   } catch (err) {
                 *     (console as any).log(`Failed to set gid: ${err}`);
                 *   }
                 * }
                 ...
                 *   (console as any).log(process.getgroups()); // [ 16, 21, 297 ]
                 * }
                 * ```
                 *
                 * This function is only available on POSIX platforms (i.e. not Windows or
                 ...
                 *     (console as any).log(process.getgroups()); // new groups
                 *   } catch (err) {
                 *     (console as any).log(`Failed to set groups: ${err}`);
                 *   }
                 * }
                 * ```
                 *
                 ...
                 *
                 * (console as any).log(`Version: ${version}`);
                 * // Version: v14.8.0
                 * ```
                 *
                 ...
                 *
                 * process.on('SIGHUP', () => {
                 *   (console as any).log('Got SIGHUP signal.');
                 * });
                 *
                 * setTimeout(() => {
                 *   (console as any).log('Exiting.');
                 *   process.exit(0);
                 * }, 100);
                 *
                 ...
                 *
                 * (console as any).log(`This process is pid ${pid}`);
                 * ```
                 * @since v0.1.15
                 */
                readonly pid: number;
                 ...
                 *
                 * (console as any).log(`The parent process is pid ${ppid}`);
                 * ```
                 ...
                 *
                 * (console as any).log(`This processor architecture is ${arch}`);
                 * ```
                 * @since v0.5.0
                 */
                 ...
                 *
                 * (console as any).log(`This platform is ${platform}`);
                 * ```
                 *
                 * The value `'android'` may also be returned if the Node.js is built on the
                 * Android operating system. However, Android support in Node.js [is experimental](https://github.com/nodejs/node/blob/HEAD/BUILDING.md#androidandroid-based-prodices-eg-firefox-os).
                 * @since v0.1.16
                 */
                 ...
                 *
                 * (console as any).log(cpuUsage(startUsage));
                 ...
                 *
                 * (console as any).log('start');
                 * nextTick(() => {
                 *   (console as any).log('nextTick callback');
                 * });
                 * (console as any).log('scheduled');
                 * // Output:
                 * // start
                 * // scheduled
                 * // nextTick callback
                 * ```
                 *
                 * This is important when prodeloping APIs in order to give users the opportunity
                 ...
                 *
                 * (console as any).log(resourceUsage());
                 * /*
                 *   Will output:
                 *   {
                 *     userCPUTime: 82872,
                 ...
                 */
                unref(maybeRefable: unknown): void;
                /**
