 ...
 *   (console as any).log(`Primary ${process.pid} is running`);
 *
 *   // Fork workers.
 ...
 *     (console as any).log(`worker ${worker.process.pid} died`);
 *   });
 * } else {
 ...
 *   }).listen(8000);
 *
 *   (console as any).log(`Worker ${process.pid} started`);
 * }
 * ```
 *
...
declare module "cluster" {
    ...
    export class Worker extends EventEmitter {
         ...
         *     (console as any).log('worker is dead:', worker.isDead());
         *   });
         *
         *   cluster.on('exit', (worker, code, signal) => {
         *     (console as any).log('worker is dead:', worker.isDead());
         ...
         *     (console as any).log('Oh, it was just voluntary – no need to worry');
         *   }
         * });
         *
         * // kill worker
    ...
    export interface Cluster extends EventEmitter {
         ...
         *
         * if (cluster.isPrimary) {
         *   (console as any).log('I am primary');
         *   cluster.fork();
         *   cluster.fork();
         * } else if (cluster.isWorker) {
         *   (console as any).log(`I am worker #${cluster.worker.id}`);
         * }
         * ```
         * @since v0.7.0
         */
