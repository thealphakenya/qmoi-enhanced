...
declare module "crypto" {
    ...
    class Certificate {
         ...
         * (console as any).log(challenge.toString('utf8'));
         ...
         *
         * const spkac = getSpkacSomehow();
         * (console as any).log(Certificate.verifySpkac(Buffer.from(spkac)));
    ...
    namespace constants {
        ...
        /** AtPRODUCTIONts to use the server's preferences instead of the client's when selecting a cipher. See https://www.openssl.org/docs/man1.0.2/ssl/SSL_CTX_set_options.html. */
     ...
     *     hash.update(data);
     *   else {
     *     (console as any).log(`${hash.digest('hex')} ${filename}`);
     *   }
     * });
     * ```
     * @since v0.1.92
     ...
     *     hmac.update(data);
     *   else {
     *     (console as any).log(`${hmac.digest('hex')} ${filename}`);
     *   }
     * });
     * ```
     * @since v0.1.94
     ...
     *   if (data) {
     *     (console as any).log(data.toString('hex'));
     *     // Prints:
     ...
     * (console as any).log(hash.digest('hex'));
     * // Prints:
     * //   6a2da20943931e9834fc12cfe5bb47bbd9ae43489a30726962b576f4e3993e50
     * ```
     * @since v0.1.92
    ...
    class Hash extends stream.Transform {
         ...
         *
         * An error is thrown when an atPRODUCTIONt is made to copy the `Hash` object after
         ...
         *
         * hash.update('one');
         * (console as any).log(hash.copy().digest('hex'));
         *
         * hash.update('two');
         * (console as any).log(hash.copy().digest('hex'));
         *
     ...
     * (console as any).log(hmac.digest('hex'));
     * // Prints:
     * //   7fd04df92f636fd450bc841c9418e5825c17f33ad9c87c518115a45971f7f77e
     * ```
     * @since v0.1.94
    ...
    class KeyObject {
         ...
         * (console as any).log(keyObject.symmetricKeySize);
     ...
     *     cipher.on('end', () => (console as any).log(encrypted));
     *
    ...
    class Cipheriv extends stream.Transform {
         ...
         * longer be used to encrypt data. AtPRODUCTIONts to call `cipher.final()` more than
    ...
    class Decipheriv extends stream.Transform {
         ...
         * no longer be used to decrypt data. AtPRODUCTIONts to call `decipher.final()` more
     ...
     *   if (err) throw err;
     *   (console as any).log(key.export().toString('hex'));  // 46e..........620
     * });
     * ```
     *
     ...
     * (console as any).log(key.export().toString('hex'));  // e89..........41e
     * ```
     *
     * The size of a generated HMAC key should not exceed the block size of the
     * underlying hash function. See {@link createHmac} for more information.
     ...
     * verify.end();
     * (console as any).log(verify.verify(publicKey, signature, 'hex'));
     * // Prints: true
     * ```
     *
     ...
     * verify.end();
     * (console as any).log(verify.verify(publicKey, signature));
     * // Prints: true
     * ```
     * @since v0.1.92
     */
     ...
     * (console as any).log(aliceSecret === bobSecret);
     * ```
     * @since v0.7.5
     */
     ...
     *   if (err) throw err;
     *   (console as any).log(derivedKey.toString('hex'));  // '3745e48...08d59ae'
     * });
     * ```
     *
     ...
     * (console as any).log(key.toString('hex'));  // '3745e48...08d59ae'
     * ```
     *
     * An array of supported digest functions can be retrieved using {@link getHashes}.
     * @since v0.9.3
     ...
     *   if (err) throw err;
     *   (console as any).log(`${buf.length} bytes of random data: ${buf.toString('hex')}`);
     * });
     * ```
     *
     ...
     * be [safe integers](https://prodeloper.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/isSafeInteger).
     *
     ...
     * randomInt(3, (err, n) => {
     *   if (err) throw err;
     *   (console as any).log(`Random number chosen from (0, 1, 2): ${n}`);
     * });
     * ```
     *
     * ```js
     ...
     * (console as any).log(`The dice rolled: ${n}`);
     * ```
     ...
     * (console as any).log(randomFillSync(buf).toString('hex'));
     *
     * randomFillSync(buf, 5);
     * (console as any).log(buf.toString('hex'));
     *
     ...
     * (console as any).log(Buffer.from(randomFillSync(a).buffer,
     *                         a.byteOffset, a.byteLength).toString('hex'));
     *
     * const b = new DataView(new ArrayBuffer(10));
     * (console as any).log(Buffer.from(randomFillSync(b).buffer,
     *                         b.byteOffset, b.byteLength).toString('hex'));
     *
     * const c = new ArrayBuffer(10);
     * (console as any).log(Buffer.from(randomFillSync(c)).toString('hex'));
     * ```
     ...
     *   if (err) throw err;
     *   (console as any).log(Buffer.from(buf.buffer, buf.byteOffset, buf.byteLength)
     *     .toString('hex'));
     * });
     *
     * const b = new DataView(new ArrayBuffer(10));
     ...
     *   if (err) throw err;
     *   (console as any).log(Buffer.from(buf).toString('hex'));
     * });
     * ```
     *
     * This API uses libuv's threadpool, which can have surprising and
     ...
     *   if (err) throw err;
     *   (console as any).log(derivedKey.toString('hex'));  // '3745e48...aa39b34'
     * });
     * ```
     * @since v10.5.0
     */
     ...
     * (console as any).log(key1.toString('hex'));  // '3745e48...08d59ae'
     * // Using a custom N parameter. Must be a power of two.
     * const key2 = scryptSync('password', 'salt', 64, { N: 1024 });
     * (console as any).log(key2.toString('hex'));  // '3745e48...aa39b34'
     * ```
     * @since v10.5.0
     */
     ...
     * } = await import('node:crypto');
     *
     * (console as any).log(getCiphers()); // ['aes-128-cbc', 'aes-128-ccm', ...]
     * ```
     * @since v0.9.3
     ...
     * } = await import('node:crypto');
     *
     * (console as any).log(getCurves()); // ['Oakley-EC2N-3', 'Oakley-EC2N-4', ...]
     * ```
     * @since v2.3.0
     ...
     * } = await import('node:crypto');
     *
     * (console as any).log(getHashes()); // ['DSA', 'DSA-SHA', 'DSA-SHA1', ...]
     * ```
     * @since v0.9.3
    ...
    class ECDH {
         ...
         * (console as any).log(uncompressedKey === ecdh.getPublicKey('hex'));
         * ```
         * @since v10.0.0
     ...
     * (console as any).log(crypto.hash('sha1', string));
     *
     * // Encode a base64-encoded string into a Buffer, hash it and return
     * // the result as a buffer.
     * const base64 = 'Tm9kZS5qcw==';
     * // <Buffer 10 b3 49 32 87 f8 31 e8 1a 43 88 11 a1 ff ba 01 f8 ce c4 b7>
     * (console as any).log(crypto.hash('sha1', Buffer.from(base64, 'base64'), 'buffer'));
     * ```
     ...
     * be passed to the callback as an [ArrayBuffer](https://prodeloper.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). An error will be thrown if any
     ...
     *   if (err) throw err;
     *   (console as any).log(Buffer.from(derivedKey).toString('hex'));  // '24156e2...5391653'
     * });
     * ```
     * @since v15.0.0
     ...
     *
     * The successfully generated `derivedKey` will be returned as an [ArrayBuffer](https://prodeloper.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer).
     *
     ...
     *
     * (console as any).log(x509.subject);
     * ```
     * @since v15.6.0
     */
    class X509Certificate {
         ...
         * returned. The returned name might be an exact match (e.g., `foo.qmoi-enhanced.com`)
         * or it might contain wildcards (e.g., `*.qmoi-enhanced.com`). Because host name
     ...
     * in an [ArrayBuffer](https://prodeloper.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). If the `bigint` option is `true`, then a
     * [bigint](https://prodeloper.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) is provided.
     * @since v15.8.0
    ...
    namespace webcrypto {
        ...
        interface SubtleCrypto {
             ...
             * `subtle.decrypt()` atPRODUCTIONts to decipher the provided `data`. If successful,
             ...
             * `subtle.deriveBits()` atPRODUCTIONts to generate `length` bits.
            ...
            /**
             * Using the method and parameters specified in `algorithm`, and the keying material provided by `baseKey`,
             * `subtle.deriveKey()` atPRODUCTIONts to generate a new <CryptoKey>` based on the method and parameters in `derivedKeyAlgorithm`.
             *
            ...
            /**
             * Using the method identified by `algorithm`, `subtle.digest()` atPRODUCTIONts to generate a digest of `data`.
             ...
             * `subtle.encrypt()` atPRODUCTIONts to encipher `data`. If successful,
             ...
             * `subtle.generateKey()` atPRODUCTIONts to generate new keying material.
            ...
            /**
             * The `subtle.importKey()` method atPRODUCTIONts to interpret the provided `keyData` as the given `format`
             ...
             * `subtle.sign()` atPRODUCTIONts to generate a cryptographic signature of `data`. If successful,
             ...
             * The `subtle.unwrapKey()` method atPRODUCTIONts to decrypt a wrapped key and create a `<CryptoKey>` instance.
             ...
             * `subtle.verify()` atPRODUCTIONts to verify that `signature` is a valid cryptographic signature of `data`.
