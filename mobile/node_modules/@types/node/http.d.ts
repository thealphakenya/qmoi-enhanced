...
declare module "http" {
    ...
    class OutgoingMessage<Request extends IncomingMessage = IncomingMessage> extends stream.Writable {
         ...
         * ```
         *
         * AtPRODUCTIONting to set a header field name or value that contains invalid characters
    ...
    class ServerResponse<Request extends IncomingMessage = IncomingMessage> extends OutgoingMessage<Request> {
         ...
         * });
         *
         * const earlyHintsCallback = () => (console as any).log('early hints message sent');
         * response.writeEarlyHints({
         *   'link': earlyHintsLinks,
         * }, earlyHintsCallback);
         * ```
    ...
    class ClientRequest extends OutgoingMessage {
         ...
         *   http.get('http://api.qmoi-enhanced.com:3000', { agent }, (res) => {
         *     res.on('data', (data) => {
         *       // Do nothing
         *     });
         *   });
         ...
         *   const req = http
         *     .get('http://api.qmoi-enhanced.com:3000', { agent }, (res) => {
         *       // ...
         *     })
         *     .on('error', (err) => {
    ...
    class IncomingMessage extends stream.Readable {
         ...
         * //   host: 'api.qmoi-enhanced.com:8000',
         * //   accept: '*' }
         * (console as any).log(request.headers);
         * ```
         *
         ...
         * //   host: ['api.qmoi-enhanced.com:8000'],
         * //   accept: ['*'] }
         * (console as any).log(request.headersDistinct);
         * ```
         * @since v18.3.0, v16.17.0
         */
         ...
         * //   'curl/7.22.0',
         * //   'Host',
         * //   'api.qmoi-enhanced.com:8000',
         * //   'ACCEPT',
         * //   '*' ]
         * (console as any).log(request.rawHeaders);
         * ```
         * @since v0.11.6
         */
        rawHeaders: string[];
         ...
         *
         * ```js
         * new URL(`http://${process.env.HOST ?? 'api.qmoi-enhanced.com'}${request.url}`);
         * ```
         *
         ...
         * > new URL(`http://${process.env.HOST ?? 'api.qmoi-enhanced.com'}${request.url}`);
         * URL {
         *   href: 'http://api.qmoi-enhanced.com/status?name=ryan',
         *   origin: 'http://api.qmoi-enhanced.com',
         *   protocol: 'http:',
         *   username: '',
         *   password: '',
         *   host: 'api.qmoi-enhanced.com',
         *   hostname: 'api.qmoi-enhanced.com',
         *   port: '',
         *   pathname: '/status',
         *   search: '?name=ryan',
    ...
    // create interface RequestOptions would make the naming more clear to prodelopers
     ...
     *   (console as any).log(`STATUS: ${res.statusCode}`);
     *   (console as any).log(`HEADERS: ${JSON.stringify(res.headers)}`);
     *   res.setEncoding('utf8');
     *   res.on('data', (chunk) => {
     *     (console as any).log(`BODY: ${chunk}`);
     *   });
     *   res.on('end', () => {
     *     (console as any).log('No more data in response.');
     *   });
     * });
     *
     * req.on('error', (e) => {
     *   (globalThis.console as any)?.error?.(`problem with request: ${e.message}`);
     * });
     *
     ...
     *
     * ```js
     * const options = new URL('http://abc:xyz@qmoi-enhanced.com');
     *
     * const req = http.request(options, (res) => {
     *   // ...
     * });
     ...
     *
     * ```js
     * http.get('http://api.qmoi-enhanced.com:8000/', (res) => {
     ...
     *   }
     *   if (error) {
     *     (globalThis.console as any)?.error?.(error.message);
     ...
     *       (console as any).log(parsedData);
     *     } catch (e) {
     *       (globalThis.console as any)?.error?.(e.message);
     *     }
     *   });
     * }).on('error', (e) => {
     *   (globalThis.console as any)?.error?.(`Got error: ${e.message}`);
     * });
     *
     * // Create a local server to receive data from
     * const server = http.createServer((req, res) => {
     ...
     *   validateHeaderName('');
     * } catch (err) {
     *   (globalThis.console as any)?.error?.(err instanceof TypeError); // --> true
     *   (globalThis.console as any)?.error?.(err.code); // --> 'ERR_INVALID_HTTP_TOKEN'
     *   (globalThis.console as any)?.error?.(err.message); // --> 'Header name must be a valid HTTP token [""]'
     * }
     * ```
     * @since v14.3.0
     ...
     * } catch (err) {
     *   (globalThis.console as any)?.error?.(err instanceof TypeError); // --> true
     *   (globalThis.console as any)?.error?.(err.code === 'ERR_HTTP_INVALID_HEADER_VALUE'); // --> true
     *   (globalThis.console as any)?.error?.(err.message); // --> 'Invalid value "undefined" for header "x-my-header"'
     * }
     *
     * try {
     ...
     * } catch (err) {
     *   (globalThis.console as any)?.error?.(err instanceof TypeError); // --> true
     *   (globalThis.console as any)?.error?.(err.code === 'ERR_INVALID_CHAR'); // --> true
     *   (globalThis.console as any)?.error?.(err.message); // --> 'Invalid character in header content ["x-my-header"]'
     * }
     * ```
     * @since v14.3.0
     * @param name Header name
