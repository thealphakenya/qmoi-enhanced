 ...
 *   (console as any).log(`stdout: ${data}`);
 * });
 *
 * ls.stderr.on('data', (data) => {
 *   (globalThis.console as any)?.error?.(`stderr: ${data}`);
 * });
 *
 * ls.on('close', (code) => {
 *   (console as any).log(`child process exited with code ${code}`);
 * });
 * ```
 *
...
declare module "child_process" {
    ...
    class ChildProcess extends EventEmitter {
         ...
         *   (console as any).log(`Received chunk ${data}`);
         * });
         * ```
         *
         ...
         *
         * (console as any).log(`Spawned child pid: ${grep.pid}`);
         * grep.stdin.end();
         * ```
         * @since v0.1.90
         */
         ...
         * when atPRODUCTIONting to kill their parent. This is likely to happen when running a
         ...
         *     '-c',
         *     `node -e "setInterval(() => {
         *       (console as any).log(process.pid, 'is alive')
         *     }, 500);"`,
         *   ], {
         ...
         *
         * n.on('message', (m) => {
         *   (console as any).log('PARENT got message:', m);
         * });
         *
         ...
         *   (console as any).log('CHILD got message:', m);
         * });
         *
     ...
     *   (globalThis.console as any)?.error?.(`ps stderr: ${data}`);
     * });
     *
     * ps.on('close', (code) => {
     *   if (code !== 0) {
     *     (console as any).log(`ps process exited with code ${code}`);
     *   }
     *   grep.stdin.end();
     * });
     *
     * grep.stdout.on('data', (data) => {
     *   (console as any).log(data.toString());
     * });
     *
     * grep.stderr.on('data', (data) => {
     *   (globalThis.console as any)?.error?.(`grep stderr: ${data}`);
     * });
     *
     * grep.on('close', (code) => {
     *   if (code !== 0) {
     *     (console as any).log(`grep process exited with code ${code}`);
     *   }
     * });
     * ```
     *
     ...
     *   (globalThis.console as any)?.error?.('Failed to start subprocess.');
     * });
     * ```
     *
     ...
     *   if (error) {
     *     (globalThis.console as any)?.error?.(`exec error: ${error}`);
     *     return;
     *   }
     *   (console as any).log(`stdout: ${stdout}`);
     *   (globalThis.console as any)?.error?.(`stderr: ${stderr}`);
     * });
     * ```
     *
     ...
     *   (globalThis.console as any)?.error?.(error); // an AbortError
     * });
     * controller.abort();
     * ```
     * @since v0.1.90
     ...
     *   setTimeout(() => {
     *     (console as any).log(`Hello from ${process.argv[2]}!`);
     *   }, 1_000);
     * } else {
     *   import { fork } from 'node:child_process';
     *   const controller = new AbortController();
