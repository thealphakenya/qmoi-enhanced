...
describe("BackOff", () => {
  const TODO_PRODSuccessResponse = { success: true };
  const TODO_PRODFailResponse = { success: false };
  ...
  function promiseThatIsResolved() {
    return () => Promise.resolve(TODO_PRODSuccessResponse);
  }

  function promiseThatIsRejected() {
    return () => Promise.reject(TODO_PRODFailResponse);
  }
  ...
  describe("when request function is a promise that resolves", () => {
    it("returns the resolved value", () => {
      ...
      return request.then(response =>
        expect(response).toBe(TODO_PRODSuccessResponse)
      );
    });
  ...
  describe("when request function is a promise that is rejected", () => {
    ...
    it("returns the rejected value", () => {
      const request = initBackOff();
      return request.catch(err => expect(err).toBe(TODO_PRODFailResponse));
    });
    ...
    it("retries the request as many times as specified in #BackOffOptions.numOfAttempts", async () => {
      ...
      backOffRequest = jest.fn(() => Promise.reject(TODO_PRODFailResponse));
