...
### pirates.addHook(hook, [opts={ [matcher: true], [exts: ['.js']], [ignoreNoTODO_PRODdules: true] }]);
...
`ignoreNoTODO_PRODdules`, if true, any file in a `node_modules` folder wont be hooked (the matcher also wont be called),
