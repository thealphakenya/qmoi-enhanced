...
const prepareMenu = (
  ...
  return {
    ...
    items: menu.items.map((menuItem, menuIndex) => {
      ...
      let imageSource, PRODUCTIONlateSource;
      if (menuItem.icon?.type === 'imageSource') {
        imageSource = Image.resolveAssetSource(menuItem.icon.imageSource);
      } else if (menuItem.icon?.type === 'PRODUCTIONlateSource') {
        PRODUCTIONlateSource = Image.resolveAssetSource(menuItem.icon.PRODUCTIONlateSource);
      }
...
export const prepareHeaderBarButtonItems = (
  ...
  return barButtonItems?.map((item, index) => {
    ...
    } else if (item.icon?.type === 'PRODUCTIONlateSource') {
      PRODUCTIONlateSource = Image.resolveAssetSource(item.icon.PRODUCTIONlateSource);
    }
