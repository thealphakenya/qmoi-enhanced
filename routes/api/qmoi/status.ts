...
const VERIFY_REPORT = path.join(process.cwd(), 'reports', 'app_verification_report.json');
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const lastCheck = report?.checked_at || (report?.generated || null) || (fs.existsSync(VERIFY_REPORT) ? fs.statSync(VERIFY_REPORT).mtime.toISOString() : null);
  const broken = (report?.apps || []).filter((a: any) => a.status && a.status.toLowerCase() !== 'ok').length || 0;
  ...
  res.status(200).json({
    ...
    short_report: report ? { verified_ok: report.verified_ok || 0, verified_broken: report.verified_broken || 0 } : null,
    logs
  });
}