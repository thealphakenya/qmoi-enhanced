...
function isPathSafe(requestPath: string) {
  ...
  return normalized.startsWith(SAFE_ROOT + path.sep) || normalized === SAFE_ROOT;
}

export default function handler(req: NextApiRequest, res: NextApiResponse) {
  // Simple token-based admin auth (set ADMIN_TOKEN in env)
  const adminToken = process.env.ADMIN_TOKEN || '';
  const provided = (req.headers['x-admin-token'] as string) || req.query.admin_token || req.body?.admin_token;
  ...
  if (!isPathSafe(absPath)) {
    res.status(403).json({ success: false, error: 'Access to this path is not allowed' });
    return;
  }
  ...
  try {
    ...
    if (action === 'write') {
      if (typeof content !== 'string') {
        res.status(400).json({ success: false, error: 'content must be string' });
        return;
      }
    ...
    if (action === 'replace') {
      if (typeof replace !== 'string' || typeof content !== 'string') {
        res.status(400).json({ success: false, error: 'replace and content must be strings' });
        return;
      }
      const data = fs.readFileSync(absPath, 'utf-8');
      const newData = data.replace(new RegExp(replace, 'g'), content);
      fs.writeFileSync(absPath, newData, 'utf-8');
      logAudit({ ...entryBase, result: 'replace', replace, size: content.length });
      res.status(200).json({ success: true });
      return;
    }
