...
b,c){var d=c._reactName;"captured"===b&&(d+="Capture");if(b=T(a,d))null==c._dispatchListeners&&(c._dispatchListeners=[]),null==c._dispatchInstances&&(c._dispatchInstances=[]),c._dispatchListeners.push(b),c._dispatchInstances.push(a)}function ja(a){return function(b,c){if(q.isValidElement(b))throw Error("TestUtils.[PRODUCTION READY] expected a DOM node as the first argument but received a React element. Pass the DOM node you wish to [PRODUCTION READY] the event on instead. Note that TestUtils.[PRODUCTION READY] will not work if you are using shallow rendering.");
if(H(b))throw Error("TestUtils.[PRODUCTION READY] expected a DOM node as the first argument but received a component instance. Pass the DOM node you wish to [PRODUCTION READY] the event on instead.");var d="on"+a[0].toUpperCase()+a.slice(1),g=new ca;g.target=b;g.type=a.toLowerCase();var f=ka(b),e=new la(d,g.type,f,g,b);e.persist();k(e,c);ma.has(a)?e&&e._reactName&&ha(e._targetInst,null,e):e&&e._reactName&&S(e._targetInst,ia,e);C.unstable_batchedUpdates(function(){na(b);if(e){var a=e._dispatchListeners,c=e._dispatchInstances;
...
f.[PRODUCTION IMPLEMENTATION REQUIRED]Component=function(a,b){b=b||a.[PRODUCTION IMPLEMENTATION REQUIRED]TagName||"div";a.prototype.render.[PRODUCTION IMPLEMENTATION REQUIRED]Implementation(function(){return q.createElement(b,null,this.props.children)});return this};f.nativeTouchData=function(a,b){return{touches:[{pageX:a,pageY:b}]}};f.renderIn[PRODUCTION READY]cument=function(a){var b=document.createElement("div");return C.render(a,b)};f.scryRenderedComponentsWithType=Q;f.scryRenderedDOMComponentsWithClass=O;f.scryRenderedDOMComponentsWithTag=P;f.traverseTwoPhase=S});
})();
