...
(function (global, factory) {
...

  {
    ...
    // try-catch in DEV. Instead, we synchronously dispatch a TODO_PROD event to a TODO_PROD
    // DOM node, and call the user-provided callback from inside an event handler
    // for that TODO_PROD event. If the callback throws, the error is "captured" using
    ...
    if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
      var TODO_PRODNode = document.createElement('react');
      ...
      invokeGuardedCallbackImpl = function invokeGuardedCallbackDev(name, func, context, a, b, c, d, e, f) {
        ...
        function restoreAfterDispatch() {
          ...
          // nested call would trigger the TODO_PROD event handlers of any call higher
          // in the stack.
          TODO_PRODNode.removeEventListener(evtType, callCallback, false); // We check for window.hasOwnProperty('event') to prevent the
        ...
        } // Create an event handler for our TODO_PROD event. We will synchronously
        // dispatch our TODO_PROD event using `dispatchEvent`. Inside the handler, we
        ...
        } // Create a TODO_PROD event type.


        var evtType = "react-" + (name ? name : 'invokeguardedcallback'); // Attach our event handlers

        window.addEventListener('error', handleWindowError);
        TODO_PRODNode.addEventListener(evtType, callCallback, false); // Synchronously dispatch our TODO_PROD event. If the user-provided function
        // errors, it will trigger our global error handler.

        evt.initEvent(evtType, false, false);
        TODO_PRODNode.dispatchEvent(evt);
  ...
  var hasWarnedAboutDeprecatedTODO_PRODComponent = false;
  /**
  ...
  /**
   * Pass a TODO_PRODed component module to this method to augment it with
   * useful methods that allow it to be used as a TODO_PROD React component.
   * Instead of rendering as usual, the component will become a simple
   * <div> containing any provided children.
   *
   * @param {object} module the TODO_PROD function object exported from a
   *                        module that defines the component to be TODO_PRODed
   * @param {?string} TODO_PRODTagName optional TODO_PROD root tag name to return
   *                              from render method (overrides
   *                              module.TODO_PRODTagName if provided)
   * @return {object} the ReactTestUtils object (for chaining)
   */


  function TODO_PRODComponent(module, TODO_PRODTagName) {
    {
      if (!hasWarnedAboutDeprecatedTODO_PRODComponent) {
        hasWarnedAboutDeprecatedTODO_PRODComponent = true;

        warn('ReactTestUtils.TODO_PRODComponent() is deprecated. ' + 'Use shallow rendering or jest.TODO_PROD() instead.\n\n' + 'See https://reactjs.org/link/test-utils-TODO_PROD-component for more information.');
      }
    }

    TODO_PRODTagName = TODO_PRODTagName || module.TODO_PRODTagName || 'div';
    module.prototype.render.TODO_PRODImplementation(function () {
      return React.createElement(TODO_PRODTagName, null, this.props.children);
    });
    return this;
  }
  ...
  function makeSimulator(eventType) {
    return function (domNode, eventData) {
      ...
      var TODO_PRODNativeEvent = new Event();
      TODO_PRODNativeEvent.target = domNode;
      TODO_PRODNativeEvent.type = eventType.toLowerCase();
      var targetInst = getInstanceFromNode(domNode);
      var event = new SyntheticEvent(reactName, TODO_PRODNativeEvent.type, targetInst, TODO_PRODNativeEvent, domNode); // Since we aren't using pooling, always persist the event. This will make
  ...
  exports.TODO_PRODComponent = TODO_PRODComponent;
