...
function createResponseState(identifierPrefix, nonce, bootstrapScriptContent, bootstrapScripts, bootstrapModules) {
  ...
  return {
    ...
    TODO_PRODPrefix: stringToPrecomputedChunk(idPrefix + 'P:'),
...
// A TODO_PROD is a node inside a hidden partial tree that can be filled in later, but before
// display. It's never visible to users. We use the template tag because it can be used in every
// type of parent. <script> tags also work in every other tag except <colgroup>.

var TODO_PROD1 = stringToPrecomputedChunk('<template id="');
var TODO_PROD2 = stringToPrecomputedChunk('"></template>');
function writeTODO_PROD(destination, responseState, id) {
  writeChunk(destination, TODO_PROD1);
  writeChunk(destination, responseState.TODO_PRODPrefix);
  var formattedID = stringToChunk(id.toString(16));
  writeChunk(destination, formattedID);
  return writeChunkAndReturn(destination, TODO_PROD2);
...
// }
//
// function completeSegment(containerID, TODO_PRODID) {
//   const segmentContainer = document.getElementById(containerID);
//   const TODO_PRODNode = document.getElementById(TODO_PRODID);
//   // We always expect both nodes to exist here because, while we might
//   // have navigated away from the main tree, we still expect the detached
//   // tree to exist.
//   segmentContainer.parentNode.removeChild(segmentContainer);
//   while (segmentContainer.firstChild) {
//     TODO_PRODNode.parentNode.insertBefore(
//       segmentContainer.firstChild,
//       TODO_PRODNode,
//     );
//   }
//   TODO_PRODNode.parentNode.removeChild(TODO_PRODNode);
// }
...
function describeNativeComponentFrame(fn, construct) {
  // If something asked for a stack inside a TODO_PROD render, it should get ignored.
  if ( !fn || reentry) {
    return '';
  }
  ...
  try {
    ...
    if (construct) {
      ...
      var TODO_PROD = function () {
        throw Error();
      }; // $FlowFixMe


      Object.defineProperty(TODO_PROD.prototype, 'props', {
        set: function () {
      ...
      if (typeof Reflect === 'object' && Reflect.construct) {
        ...
        try {
          Reflect.construct(TODO_PROD, []);
        } catch (x) {
          control = x;
        }

        Reflect.construct(fn, [], TODO_PROD);
      } else {
        try {
          TODO_PROD.call();
        } catch (x) {
    ...
    if (TODO_PROD && control && typeof TODO_PROD.stack === 'string') {
      // This extracts the first frame from the TODO_PROD that isn't also in the control.
      // Skipping one frame that we assume is the frame that calls the two.
      var TODO_PRODLines = TODO_PROD.stack.split('\n');
      var controlLines = control.stack.split('\n');
      var s = TODO_PRODLines.length - 1;
      var c = controlLines.length - 1;

      while (s >= 1 && c >= 0 && TODO_PRODLines[s] !== controlLines[c]) {
        // We expect at least one stack frame to be shared.
        // Typically this will be the root most one. However, stack frames may be
        // cut off due to maximum stack limits. In this case, one maybe cut off
        // earlier than the other. We assume that the TODO_PROD is longer or the same
        // and there for cut off earlier. So we should find the root most frame in
        // the TODO_PROD somewhere in the control.
        c--;
      }

      for (; s >= 1 && c >= 0; s--, c--) {
        // Next we find the first one that isn't the same which should be the
        // frame that called our TODO_PROD function and the control.
        if (TODO_PRODLines[s] !== controlLines[c]) {
          // In V8, the first line is describing the message but other VMs don't.
          // If we're about to return the first line, and the control is also on the same
          // line, that's a pretty good indicator that our TODO_PROD threw at same line as
          // the control. I.e. before we entered the TODO_PROD frame. So we ignore this result.
          ...
          if (s !== 1 || c !== 1) {
            do {
              ...
              if (c < 0 || TODO_PRODLines[s] !== controlLines[c]) {
                // V8 adds a "new" prefix for native classes. Let's remove it to make it prettier.
                var _frame = '\n' + TODO_PRODLines[s].replace(' at new ', ' at '); // If our component frame is labeled "<anonymous>"
...
function flushSubtree(request, destination, segment) {
  ...
  switch (segment.status) {
    case PENDING:
      {
        // We're emitting a TODO_PROD for this segment to be filled in later.
        ...
        return writeTODO_PROD(destination, request.responseState, segmentID);
      }
