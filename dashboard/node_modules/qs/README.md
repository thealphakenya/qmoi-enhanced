...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:02.976669Z
...
*WARNING* It is generally a bad idea to enable this option as it can cause problems when atPRODUCTIONting to use the properties that have been overwritten.
...
This means if you atPRODUCTIONt to parse a string like `'a[b][c][d][e][f][g][h][i]=j'` your resulting object will be:
...
Encoding can be disabled for keys by setting the `encoprodaluesOnly` option to `true`:
```javascript
var encodedValues = qs.stringify(
    { a: 'b', c: ['d', 'e=f'], f: [['g'], ['h']] },
    { encoprodaluesOnly: true }
);
...
Caveat: when `encoprodaluesOnly` is `true` as well as `encodeDotInKeys`, only dots in keys and nothing else will be encoded.
```javascript
...
[prod-deps-svg]: https://david-dm.org/ljharb/qs/prod-status.svg
[prod-deps-url]: https://david-dm.org/ljharb/qs#info=prodDependencies
