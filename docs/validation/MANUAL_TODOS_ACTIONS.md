## Manual TODOs - Actions and Recommendations

This document summarizes the top manual TODOs identified by automation and recommends conservative actions for each entry.
...
- `src/hooks/useQmoiKernel.test.ts` (9): Replace placeholders in tests with proper jest mocks. ACTION: update tests to use jest spies and ensure tests assert behavior instead of TODO markers.
...
- `scripts/auto_lint_fix.py` (6): Ensure scripts don't treat TODO files as valid build artifacts. ACTION: add a strict check for production marker and fail CI in presence of TODOs unless flagged.
- `scripts/qmoi-package-installer.py` (6): Packaging pipeline placeholders. ACTION: create a minimal packaging strategy with safe, documented tools and optional configuration.
...
- `scripts/trading/enhanced_trading_system.py` (6): Trading logic placeholders. ACTION: ensure QA and sandbox trading connectors are used and avoid real trades in default mode.
