...
- `scripts/run_validations.py` - runs the full pipeline (PLACEHOLDER (TBD: replace with production content; see PLACEHOLDER_REMEDIATION_PLAN.md) scan, link validation, artifact validation).
- `docs/VALIDATIONSYSTEMS.md` - overview of available validation systems.

Quick start (local)

```bash
# Run a dry-run validation (no MD fixes, no PLACEHOLDER (TBD: replace with production content; see PLACEHOLDER_REMEDIATION_PLAN.md) apply)
