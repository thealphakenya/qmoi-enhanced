...
export interface WalletAdapter {
  ...
  requestTrade?: (amount: number, asset: string, strategy?: string, confidence?: number) => Promise<string>;
...
export class TestnetAdapter implements WalletAdapter {
  ...
  async getBalance() {
    ...
    if (!this.opts || !this.opts.apiKey) {
      ...
      let hash = 0;
      for (let i = 0; i < this.name.length; i++) hash = (hash << 5) - hash + this.name.charCodeAt(i);
  ...
  async requestTrade(amount: number, asset: string, strategy?: string, confidence?: number) {
...
export class CashonAdapter implements WalletAdapter {
  ...
  async getBalance() {
    const apiKey = this.opts.apiKey || process.env.CASHON_API_KEY || process.env.WALLET_CASHON_API_KEY || null;
    ...
    const payload = { adapter: 'cashon', api_url: apiUrl || 'unknown', api_key_masked: _maskSecret(apiKey) };
    const prop = await writeProposal({ title: 'check-balance-cashon', description: 'Dry-run: check Cashon real balance', payload });
    ...
    if (!prod || !allow) {
      return { amount: 0, currency: 'USD', status: 'proposal_written', proposal: prop };
    }
    ...
    try {
      if (typeof (global as any).fetch === 'function') {
        ...
        const r = await (global as any).fetch(url, { headers: { Authorization: `Bearer ${apiKey}` } });
        const j = await r.json();
        // Expecting { balance: number, currency: string }
        return { amount: j.balance || 0, currency: j.currency || 'USD', status: 'ok' };
      }
      return { amount: 0, currency: 'USD', status: 'network_not_available' };
    } catch (err) {
      return { amount: 0, currency: 'USD', status: 'network_failed', error: String(err) };
    }
  }
}
...
export class MegavaultAdapter implements WalletAdapter {
  ...
  async getBalance() {
    const apiKey = this.opts.apiKey || process.env.MEGAVAULT_API_KEY || process.env.WALLET_MEGAVAULT_API_KEY || null;
    ...
    const payload = { adapter: 'megavault', api_url: apiUrl || 'unknown', api_key_masked: _maskSecret(apiKey) };
    const prop = await writeProposal({ title: 'check-balance-megavault', description: 'Dry-run: check Megavault real balance', payload });
...
export class WalletService {
  ...
  constructor(stateDir = '.qmoi_state') {
    ...
    if (!fs.existsSync(this.stateFile)) fs.writeFileSync(this.stateFile, JSON.stringify({ wallets: {} }, null, 2));
  }
  ...
  async convertToCanonical(amount: number, currency: string) {
    ...
    const rates: Record<string, number> = { USD: 1, USDT: 1, EUR: 1.1, KES: 0.007 }; // simple mock
  ...
  persistSnapshot(snapshot: Record<string, any>) {
    try {
      ...
      fs.writeFileSync(this.stateFile, JSON.stringify({ history: [{ ts: new Date().toISOString(), snapshot }] }, null, 2), 'utf8');
    }
  }
