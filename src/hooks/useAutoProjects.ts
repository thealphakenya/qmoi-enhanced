...
export const useAutoProjects = (): UseAutoProjectsReturn => {
  ...
  const createProject = useCallback(
    async (projectData: Omit<AutoProject, "id" | "status" | "startedAt">) => {
      ...
      try {
        ...
        (console as any).error("Error creating project:", error);
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const updateProjectStatus = useCallback(
    async (id: string, status: AutoProject["status"]) => {
      ...
      try {
        ...
        (console as any).error("Error updating project status:", error);
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const generateDailyPlan = useCallback(async () => {
    ...
    try {
      ...
      (console as any).error("Error generating daily plan:", error);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const createWhatsAppGroup = useCallback(async () => {
    try {
      ...
      (console as any).error("Error creating WhatsApp group:", error);
    }
  }, []);
  ...
  const postToWhatsAppGroup = useCallback(async (message: string) => {
    try {
      ...
      (console as any).error("Error posting to WhatsApp group:", error);
    }
  }, []);
