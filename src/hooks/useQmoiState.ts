...
interface QmoiState {
  ...
  activity: 'processing' | 'learning' | 'creating' | 'analyzing' | 'optimizing' | 'planning';
  pulse: {
    bpm: number;
...
export const useQmoiState = (): UseQmoiStateReturn => {
    ...
    const updateInterval = setInterval(() => {
      setState(prev => ({
        ...
        pulse: {
          ...
          rhythm: ['steady', 'accelerating', 'decelerating'][Math.floor(Math.random() * 3)] as any
        },
        system: {
          ...prev.system,
  ...
  const updateProjects = useCallback((projects: Partial<QmoiState['projects']>) => {
    setState(prev => ({ 
      ...prev, 
