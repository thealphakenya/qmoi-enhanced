...
export type NetworkType = 'wifi' | 'cellular' | 'zero-rated' | 'ethernet' | 'unknown';
...
export class NetworkManager extends EventEmitter {
  ...
  async scanNetworks(): Promise<Network[]> {
    ...
    // Simulate scan
    this.networks = [
      { id: 'wifi-1', type: 'wifi', name: 'Home WiFi', signalStrength: 80, isConnected: false },
      { id: 'cell-1', type: 'cellular', name: '4G LTE', signalStrength: 60, isConnected: false },
      { id: 'zero-1', type: 'zero-rated', name: 'Zero Free', signalStrength: 50, isConnected: false, isZeroRated: true }
    ];
  ...
  async connectBestNetwork(): Promise<Network | null> {
    ...
    const sorted = this.networks.sort((a, b) => {
      const priority = (n: Network) => n.type === 'wifi' ? 4 : n.type === 'ethernet' ? 3 : n.type === 'cellular' ? 2 : n.isZeroRated ? 1 : 0;
