  ...
  for (let i = 0; i <= maxRetries; i++) {
    try {
      ...
      if (i < maxRetries) {
        ...
        console.warn(`[Retry ${i + 1}/${maxRetries}] ${endpoint} in ${delay}ms`);
...
export async function verifyProduct(query: string, forceRefresh = false): Promise<string> {
...
export async function emergencyAction(action: string, payload: any): Promise<any> {
...
if (typeof window !== 'undefined') {
  setInterval(() => {
    ...
    if (removed > 0) console.debug(`[Cache] Cleaned up ${removed} stale entries`);
  }, 10 * 60 * 1000);
}
