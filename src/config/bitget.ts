...
export interface BitgetConfig {
  ...
  security: {
    ...
    securityLevel: "comprehensive" | "advanced" | "enterprise";
    advanced: {
      mfa: {
        enabled: boolean;
        methods: string[];
...
export class BitgetManager extends EventEmitter {
      ...
      security: {
        ...
        advanced: {
          ...
          monitoring: {
            realtime: {
              ...
              alertThresholds: {
                ...
                loginAttempts: 5, // per hour
                withdrawalAttempts: 3, // per day
          ...
          accessControl: {
            rbac: {
              enabled: true,
              roles: ["admin", "trader", "viewer", "api_user"],
              permissions: {
                admin: ["all"],
  ...
  private generateEncryptionKey(): Buffer {
    ...
    return crypto.pbkdf2Sync(this.config.secretKey, salt, 100000, 32, "sha256");
  }
  ...
  private checkSessionTimeout(): void {
    const now = new Date();
    const sessionAge =
      (now.getTime() - this.securityStatus.lastAuthAt.getTime()) / 1000;
  ...
  private checkFailedAttempts(): void {
    if (
      ...
      this.emit("accountLocked", {
        timestamp: new Date(),
        expiry: this.securityStatus.lockExpiry,
      });
    }
  }
  ...
  private checkMultipleIpChanges(): boolean {
    const config = this.config.security.anomalyDetection;
    return (
      this.securityStatus.securityMetrics.ipChanges.length >
      config.ipChangeThreshold
    );
  }
  ...
  private async rotateEncryptionKeys(): Promise<void> {
    ...
    if (now >= this.securityStatus.encryptionKeys.nextRotation) {
      ...
      this.securityStatus.encryptionKeys.previous =
        this.securityStatus.encryptionKeys.current;
  ...
  public async validateRequest(request: unknown): Promise<boolean> {
    ...
    if (this.securityStatus.isLocked) {
      if (
        this.securityStatus.lockExpiry &&
  ...
  public async updateSecurityConfig(
    ...
    this.emit("securityConfigUpdated", {
      timestamp: new Date(),
