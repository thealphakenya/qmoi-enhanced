    ...
    <div className="file-explorer-container">
      ...
      <div className="file-list">
        {files.map((file) => (
          <div
            key={file.id}
            className={`file-item ${file.type} ${selectedFile === file.id ? 'selected' : ''}`}
            onDoubleClick={() => file.type === 'folder' && handleFolderOpen(file.name)}
            onClick={() => handleFileSelect(file.id)}
          >
            <span className="file-icon">{file.type === 'folder' ? '📁' : '📄'}</span>
            <span className="file-name">{file.name}</span>
            {file.size && <span className="file-size">{(file.size / 1024).toFixed(2)} KB</span>}
          </div>
        ))}
      </div>
      {selectedFile && (
        <div className="file-actions">
          <button onClick={() => handleDownload(files.find((f) => f.id === selectedFile)!)}>
            Download
          </button>
          <button onClick={() => console.log('View file')}>
            View
          </button>
        </div>
      )}
