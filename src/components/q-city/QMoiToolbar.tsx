...
const initialApps = [
  ...
  { name: 'QFileManager', icon: '🗂️', status: 'synced', tooltip: 'File Manager' },
...
export default function QMoiToolbar() {
    ...
    const interval = setInterval(() => {
      setApps(prev => prev.map(app => {
        ...
        if (app.name === 'QWifi') {
          ...
          const wifi = ['connected', 'disconnected', 'connecting'][getRandomInt(0, 2)];
        ...
        if (app.name === 'QDevice') {
          ...
          const health = ['optimized', 'needs attention', 'updating'][getRandomInt(0, 2)];
          return { ...app, status: health };
        }
        return app;
      }));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (n > 8) {
        ...
        const notif: Notification = { type, message, actionLabel, onAction, timestamp: Date.now() };
  ...
  return visible ? (
    <>
      {notification && (
        <div style={{
          position: 'fixed', bottom: 90, left: 32, zIndex: 1100, background: notification.type === 'error' ? '#f44336' : notification.type === 'warning' ? '#ff9800' : notification.type === 'success' ? '#4caf50' : '#333', color: '#fff', padding: '10px 20px', borderRadius: 10, boxShadow: '0 2px 8px #0008', fontSize: 16, display: 'flex', alignItems: 'center', gap: 12
        }}>
          <span>{notification.message}</span>
          {notification.actionLabel && (
            <button onClick={() => handleAction(notification)} style={{marginLeft: 8, background: '#fff', color: '#222', border: 'none', borderRadius: 6, padding: '2px 10px', cursor: 'pointer', fontWeight: 600}}>{notification.actionLabel}</button>
          )}
        </div>
      )}
      {showHistory && (
        <div style={{
          position: 'fixed', bottom: 120, left: 32, zIndex: 1100, background: '#222', color: '#e0ffe0', padding: 16, borderRadius: 12, boxShadow: '0 2px 8px #0008', maxHeight: 300, overflowY: 'auto', minWidth: 320
        }}>
          <h4 style={{margin: 0, marginBottom: 8}}>Notification Center</h4>
          <button onClick={() => setShowHistory(false)} style={{position: 'absolute', top: 8, right: 12, background: 'none', border: 'none', color: '#e0ffe0', fontSize: 18, cursor: 'pointer'}}>✖</button>
          <ul style={{listStyle: 'none', padding: 0, margin: 0}}>
            {notificationHistory.map((n, i) => (
              <li key={i} style={{marginBottom: 8, borderBottom: '1px solid #444', paddingBottom: 4}}>
                <span style={{fontWeight: 600, color: n.type === 'error' ? '#f44336' : n.type === 'warning' ? '#ff9800' : n.type === 'success' ? '#4caf50' : '#e0ffe0'}}>{n.message}</span>
                <span style={{marginLeft: 8, fontSize: 12, color: '#aaa'}}>{new Date(n.timestamp).toLocaleTimeString()}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div style={{
        ...
        borderRadius: 16, boxShadow: '0 4px 24px #0008',
        padding: 8, display: 'flex', gap: 16, alignItems: 'center',
        minHeight: 56, minWidth: 56, userSelect: 'none',
      }}>
        {apps.map(app => (
          <div key={app.name}
            title={app.tooltip}
            style={{ position: 'relative', cursor: 'pointer', fontSize: 28, padding: 4 }}
            onClick={() => openApp(app.name)}
            onContextMenu={e => showContextMenu(e, app.name)}
          >
            <span>{app.icon}</span>
            <span style={{
              position: 'absolute', top: 0, right: 0, fontSize: 10, background: app.status === 'error' ? '#f00' : app.status === 'warning' ? '#ff0' : '#0f0', color: '#111', borderRadius: 8, padding: '0 4px', minWidth: 16, textAlign: 'center',
              display: app.status ? 'inline-block' : 'none',
            }}>{app.status}</span>
          </div>
        ))}
        <button onClick={() => setVisible(false)} style={{marginLeft: 12, background: 'none', border: 'none', color: 'inherit', fontSize: 18, cursor: 'pointer'}}>✖</button>
        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} style={{marginLeft: 4, background: 'none', border: 'none', color: 'inherit', fontSize: 18, cursor: 'pointer'}}>🌓</button>
        <button onClick={() => setShowHistory(h => !h)} style={{marginLeft: 4, background: 'none', border: 'none', color: 'inherit', fontSize: 18, cursor: 'pointer'}}>🔔</button>
      </div>
    </>
  ) : (
    <button onClick={() => setVisible(true)} style={{position: 'fixed', bottom: 24, left: 24, zIndex: 1000, fontSize: 28, borderRadius: 16, background: '#222', color: '#e0ffe0', border: 'none', padding: 8, boxShadow: '0 4px 24px #0008'}}>☰</button>
  );
} 