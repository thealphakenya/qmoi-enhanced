...
export default function PluginPanel() {
  ...
  return (
    <div className="p-4 bg-gray-900 rounded-lg shadow-lg" aria-label="Plugin Management Panel">
      <h2 className="text-xl font-bold mb-4 text-cyan-400 flex items-center">Plugins<HelpLink href="/docs/PLUGINS.md" label="Plugin System Documentation" /></h2>
      ...
      <form className="mb-4 flex flex-col gap-2" onSubmit={handleUpload} aria-label="Upload Plugin Form">
        <label className="text-xs text-gray-300" htmlFor="plugin-upload">Upload Plugin (.js/.ts):</label>
        <input
          id="plugin-upload"
          type="file"
          accept=".js,.ts"
          ...
          {plugins.map((p, i) => (
            <li key={i} className="mb-2 flex flex-col gap-1 bg-gray-800 rounded p-2">
              <div className="flex items-center gap-2">
                ...
                >{removing === p ? 'Removing...' : 'Remove'}</button>
                <button
                ...
                >{configuring === p ? 'Close Config' : 'Configure'}</button>
                <span className="text-green-400">Loaded</span>
              </div>
              {configuring === p && (
                <form className="flex flex-col gap-1 mt-2" onSubmit={e => { e.preventDefault(); handleConfig(p); }} aria-label={`Config form for ${p}`}>
                  <label className="text-xs text-gray-300" htmlFor={`config-key-${i}`}>Config Key:</label>
                  <input
                    id={`config-key-${i}`}
                    type="text"
                    className="border rounded px-2 py-1 text-xs bg-gray-900 text-gray-100"
                    value={pluginConfig[p] || ''}
                    onChange={e => setPluginConfig(cfg => ({ ...cfg, [p]: e.target.value }))}
                  ...
                  >{configuring === p ? 'Save' : 'Save'}</button>
                </form>
              )}
            </li>
          ))}
