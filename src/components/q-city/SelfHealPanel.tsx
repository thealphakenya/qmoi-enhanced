...
const SelfHealPanel: React.FC = () => {
  ...
  const [options, setOptions] = useState({ forceClean: false, essentialsOnly: false, diagnosticsOnly: false });
  ...
  const handleSelfHeal = async () => {
    ...
    try {
      ...
      const es = new EventSource(API_URL + '?token=' + encodeURIComponent(token) + '&opts=' + encodeURIComponent(JSON.stringify(options)));
      ...
      es.onmessage = (event) => {
        if (event.data === '[DONE]') {
          ...
          const entry = { ts: new Date().toISOString(), log: logBuffer, options };
  ...
  return (
    <div style={{ border: '1px solid #ccc', borderRadius: 8, padding: 16, margin: 16 }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <label><input type="checkbox" checked={options.forceClean} onChange={() => handleOptionChange('forceClean')} /> Force Clean</label>
        <label style={{ marginLeft: 12 }}><input type="checkbox" checked={options.essentialsOnly} onChange={() => handleOptionChange('essentialsOnly')} /> Essentials Only</label>
        <label style={{ marginLeft: 12 }}><input type="checkbox" checked={options.diagnosticsOnly} onChange={() => handleOptionChange('diagnosticsOnly')} /> Diagnostics Only</label>
      </div>
      <button onClick={handleSelfHeal} disabled={running} style={{ padding: '8px 16px', fontWeight: 'bold' }}>
        {running ? 'Running...' : 'Self-Heal NPM'}
      </button>
      <button onClick={handleSchedule} style={{ marginLeft: 8 }}>Schedule</button>
      <button onClick={handleClearHistory} style={{ marginLeft: 8 }}>Clear History</button>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        {history.length === 0 ? <div>No history yet.</div> : (
          <ul style={{ maxHeight: 120, overflow: 'auto', background: '#f4f4f4', padding: 8 }}>
            {history.map((h, i) => (
              <li key={i} style={{ marginBottom: 6 }}>
                <b>{h.ts}</b> - <span>{JSON.stringify(h.options)}</span>
                <button style={{ marginLeft: 8 }} onClick={() => setLog(h.log)}>View Log</button>
              </li>
            ))}
          </ul>
        )}
