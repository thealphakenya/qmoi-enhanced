...
export function QMOIDashboard() {
  const { state, updateAvatar, updateVoice, updateMood, updateEnergy } = useQMOIState();
  ...
  const getMoodColor = (mood: string) => {
    switch (mood) {
      ...
      case 'playful': return 'text-pink-500';
      case 'wise': return 'text-indigo-500';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Monitor and control QMOI's state and settings</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={state.isOnline ? 'default' : 'secondary'}>
            {state.isOnline ? 'Online' : 'Offline'}
          </Badge>
          <Badge variant="outline" className={getHealthColor(state.systemHealth)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Current Avatar</CardTitle>
                <User className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {state.currentAvatar === 'default' ? 'Default QMOI' : 
                   state.currentAvatar.split('-').map(word => 
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Current Voice</CardTitle>
                <Volume2 className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">
                  {state.currentVoice.split('-').map(word => 
                    word.charAt(0).toUpperCase() + word.slice(1)
                  ).join(' ')}
                </div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${getMoodColor(state.mood)}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">{state.responseTime}ms</span>
                  </div>
                  <Progress value={Math.max(0, 100 - (state.responseTime / 2))} className="h-2" />
                </div>
                
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">Accuracy</span>
                    <span className="text-sm text-muted-foreground">{state.accuracy}%</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">{state.userSatisfaction}%</span>
                  </div>
