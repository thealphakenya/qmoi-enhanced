...
export function Chatbot({
  ...
  const [internalChatHistory, setInternalChatHistory] = useState<any[]>(propChatHistory ?? []);
  const [inputMessage, setInputMessage] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [internalSelectedModel, setInternalSelectedModel] = useState<string>(propSelectedModel ?? 'Auto');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  const chatHistory = propChatHistory ?? internalChatHistory;
  const setChatHistory = propSetChatHistory ?? ((h: any[]) => setInternalChatHistory(h));
  const selectedModel = propSelectedModel ?? internalSelectedModel;
  const setSelectedModel = propSetSelectedModel ?? ((m: string) => setInternalSelectedModel(m));
