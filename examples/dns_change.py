#!/usr/bin/env python3
"""Example script demonstrating the provider API.

Shows how to use the provider interface to make DNS changes safely.
All providers support dry-run first and require explicit apply.

Example usage:
    # Dry run
    python examples/dns_change.py example.com test.example.com 1.2.3.4
    
    # Apply with QMOI_PROVISION_DNS=1
    QMOI_PROVISION_DNS=1 python examples/dns_change.py example.com test.example.com 1.2.3.4
"""
import os
import sys
from typing import Dict, Any

from scripts.providers import (
    CloudflareProvider,
    Route53Provider,
    NetlifyProvider,
    ProviderError
)


def try_provider(provider_class, domain: str, records: Dict[str, Any]):
    """Try to use a provider, catching credential errors."""
    try:
        provider = provider_class()
        plan = provider.plan_dns_change(domain, records)
        print(f'\n{provider.name} plan:')
        print(f'Changes planned: {len(plan["changes"])}')
        for change in plan["changes"]:
            print(f'  - {change["action"]}: {change["record"]["name"]} '
                  f'({change["record"]["type"]} -> {change["record"]["content"]})')

        # Only try to apply if QMOI_PROVISION_DNS is set
        if os.getenv('QMOI_PROVISION_DNS'):
            plan['dry_run'] = False
            print('\nApplying changes...')
            result = provider.apply_dns_change(plan)
            print(f'Applied {len(result["applied"])} changes')
            if result['errors']:
                print(f'Errors: {len(result["errors"])}')
                for error in result['errors']:
                    print(f'  - {error["error"]}')

            # Verify the changes
            print('\nVerifying DNS...')
            verify = provider.verify_dns(domain)
            if verify['verified']:
                print('All records verified!')
            else:
                print(f'Verification errors: {len(verify["errors"])}')
                for error in verify['errors']:
                    print(f'  - {error["error"]}')

    except ProviderError as e:
        print(f'{provider_class.__name__} error: {e}')


def main():
    """Example script entry point."""
    if len(sys.argv) != 4:
        print('Usage: dns_change.py <domain> <record_name> <target>')
        sys.exit(1)

    domain = sys.argv[1]
    name = sys.argv[2]
    target = sys.argv[3]

    # Build the record set
    records = {
        name: {
            'type': 'A' if target.replace('.', '').isdigit() else 'CNAME',
            'content': target
        }
    }

    # Try each provider
    try_provider(CloudflareProvider, domain, records)
    try_provider(Route53Provider, domain, records)
    try_provider(NetlifyProvider, domain, records)


if __name__ == '__main__':
    main()