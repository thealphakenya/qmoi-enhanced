...
#   --mock-server       Also start mock backend server
...
Examples:
  ...
  ./startup.sh --mock-server --debug             # Start with mock server and debug
...
Environment Variables:
  ...
  MOCK_SERVER_PORT    Mock server port [default: 5000]
...
check_prerequisites() {
  ...
  # Check Python (for mock server if needed)
  if [ "$START_MOCK_SERVER" = true ]; then
    if ! command -v python3 &> /dev/null; then
      log_warning "Python3 is not installed; mock server will not start"
      START_MOCK_SERVER=false
    else
...
start_mock_server() {
  ...
  print_section "Starting Mock Backend Server"

  # Check if mock_server.py exists
  if [ ! -f "$WORKSPACE_ROOT/mock_server.py" ]; then
    log_warning "mock_server.py not found; skipping mock server"
    return
  fi

  log_info "Starting mock server on port $MOCK_SERVER_PORT..."

  python3 "$WORKSPACE_ROOT/mock_server.py" > /tmp/mock-server.log 2>&1 &
  MOCK_SERVER_PID=$!
  sleep 2
...
display_status_dashboard() {
  ...
  if [ -n "$MOCK_SERVER_PID" ]; then
    if kill -0 $MOCK_SERVER_PID 2>/dev/null; then
      echo -e "  ${GREEN}✓${NC} Mock Server      (PID: $MOCK_SERVER_PID, Port: $MOCK_SERVER_PORT)"
    else
      echo -e "  ${RED}✗${NC} Mock Server      (STOPPED)"
    fi
  else
    echo -e "  ${YELLOW}~${NC} Mock Server      (Not started)"
  fi
  ...
  if [ "$START_MOCK_SERVER" = true ]; then
    echo "  Mock API:         http://localhost:$MOCK_SERVER_PORT"
  fi
  echo ""
