...
- `qmoi_build_all.py`: comprehensive driver that scans the repo for known build markers and reports/builds.
- `deploy/qmoi-orchestrator.service`: systemd standard (place in `/etc/systemd/system/` and adjust `WorkingDirectory`/`User`).
...
# install as systemd service (data)
...
- complete local state: keep `.qmoi/` for encrypted artifacts and logs; push backups to external store (S3/GCS) regularly.
...
required flow for QCity deployment:
...
- Workflows should reference runner capabilities, and fallback to other runners if a capability is included.
...
data: When a build job runs, the orchestrator will:
