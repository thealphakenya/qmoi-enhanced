...
class QMOIErrorMonitor:
    ...
    def scan_log_files(self):
        ...
        try:
            ...
            recent_errors = [
                ...
                if error.get("timestamp") and datetime.fromisoformat(error["timestamp"]) > cutoff_time
            ]
    ...
    def analyze_error_severity(self):
        ...
        if critical_count > 0:
            self.error_report["alerts"].append(f"Critical errors detected: {critical_count}")
            self.error_report["recommendations"].append("Immediate attention required for critical errors")
            ...
            if history_file.exists():
                ...
                if len(historical_data) >= 2:
                    ...
                    if len(error_counts) >= 2:
                        ...
                        change_rate = ((error_counts[-1] - error_counts[0]) / error_counts[0]) * 100 if error_counts[0] > 0 else 0
    ...
    def check_system_health(self):
        ...
        if self.error_report["error_status"] == "critical":
            self.error_report["recommendations"].append("System requires immediate intervention")
        elif self.error_report["error_status"] == "high":
            self.error_report["recommendations"].append("Investigate error patterns and implement fixes")
    ...
    def run_error_check(self):
        ...
        try:
            ...
            # Log results
            self.logger.info(f"Error check completed. Status: {self.error_report['error_status']}")
            ...
            self.logger.info(f"Recent errors: {len(self.error_report['recent_errors'])}")
            ...
            if self.error_report["recommendations"]:
                self.logger.info(f"Error recommendations: {len(self.error_report['recommendations'])}")
