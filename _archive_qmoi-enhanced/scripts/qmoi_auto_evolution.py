...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('qmoi_evolution.log'),
...
class QmoiEvolutionEngine:
    ...
    async def _analyze_system(self) -> Dict[str, Any]:
        ...
        low_usage_features = [f for f, usage in analysis["feature_usage"].items() if usage < 0.1]
...
"""
    ...
    async def _apply_feature_improvement(self, feature: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if tests:
                await self._write_code_to_file(f"tests/features/{feature['feature']}", tests)
