...
class QMOISpaceApp {
    ...
    initializePWA() {
        ...
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.register('/sw.js')
                ...
                .catch(error => {
                    console.error('QMOI Space: Service Worker registration failed:', error);
                });
        }
    ...
    updateSystemStatusDisplay(data) {
        ...
        // Update memory usage
        const memoryElement = document.querySelector('.status-item:nth-child(2) span');
        if (memoryElement && data.memory) {
            memoryElement.textContent = `Memory: ${data.memory}%`;
        }
        
        // Update storage usage
        const storageElement = document.querySelector('.status-item:nth-child(3) span');
        if (storageElement && data.storage) {
            storageElement.textContent = `Storage: ${data.storage}%`;
        }
        
        // Update network latency
        const networkElement = document.querySelector('.status-item:nth-child(4) span');
    ...
    handleKeyboardShortcuts(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key >= '1' && e.key <= '6') {
            e.preventDefault();
            const tabs = ['dashboard', 'ai-chat', 'gaming', 'development', 'revenue', 'analytics'];
