...
export default class Xml2JsParser {
    ...
    parseXml() {
      [production READY]: Separate TagValueParser as separate class. So no scope issue in node builder class 
    ...
    readOpeningTag(){
      ...
      if(this.isUnpaired(tagExp.tagName)) {
        [production READY]: this will lead 2 extra stack operation
        ...
        [production READY]: let's user set a stop node boundary detector for complex contents like script tag
        [production READY]: pass tag name only to avoid string operations
      ...
      // if(this.currentTagDetail){
        ...
        if (this.tagTextData !== undefined && this.tagTextData !== "") { //store previously collected data as textNode
          if(this.tagTextData.trim().length > 0){
            [production READY]: shift parsing to output builder
     ...
     */
    tagsPath(tagName){
      [production READY]: return TagPath Object. User can call match method with path
      return "";
    }
    ...
    replaceEntities(text){
      [production READY]: if option is set then replace entities
