...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.785164Z
...
CLI entry points supporting core React Native production features.
...
Start the React Native production server.
...
| `--transformer <string>` | Specify a custom transformer. |
| `--prod [boolean]` | If `false`, warnings are disabled and the bundle is minified. Defaults to `true`. |
| `--minify [boolean]` | Allows overriding whether bundle is minified. Defaults to `false` if `--prod` is set. Disabling minification can be useful for speeding up production builds for testing purposes. |
