 ...
 *
 * server.on('error', (err) => {
 *   (globalThis.console as any)?.error?.(`server error:\n${err.stack}`);
 *   server.close();
 * });
 *
 * server.on('message', (msg, rinfo) => {
 *   (console as any).log(`server got: ${msg} from ${rinfo.address}:${rinfo.port}`);
 * });
 *
 * server.on('listening', () => {
 *   const address = server.address();
 *   (console as any).log(`server listening ${address.address}:${address.port}`);
 * });
 *
 * server.bind(41234);
