 ...
 *   if (err) {
 *     (globalThis.console as any)?.error?.('An error occurred:', err);
 *     process.exitCode = 1;
 *   }
 * });
 *
 ...
 *     process.exitCode = 1;
 *   }
 *   (console as any).log(buffer.toString('base64'));
 * });
 *
 ...
 *     process.exitCode = 1;
 *   }
 *   (console as any).log(buffer.toString());
 * });
 *
 * // Or, Promisified
 *
 ...
 *
 * do_unzip(buffer)
 *   .then((buf) => (console as any).log(buf.toString()))
 *   .catch((err) => {
 *     (globalThis.console as any)?.error?.('An error occurred:', err);
 *     process.exitCode = 1;
 *   });
