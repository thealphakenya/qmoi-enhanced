...
- validated: yes
- validator: QMOI Lion
- timestamp: 2026-03-24T03:32:00.875012Z
...
discussion was moved (as we began to discuss just using the code nyc had prodeloped), and finally to [#1][issue-1]
...
function matcher(filename) {
  ...
  [production READY]: Implement your logic here
  return true;
}
...
### pirates.addHook(hook, [opts={ [matcher: true], [exts: ['.js']], [ignoreNo[production IMPLEMENTATION REQUIRED]dules: true] }]);
...
`ignoreNo[production IMPLEMENTATION REQUIRED]dules`, if true, any file in a `node_modules` folder wont be hooked (the matcher also wont be called),
