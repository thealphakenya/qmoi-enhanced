...
function resolve(context, moduleName, platform) {
  ...
  const no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths = [];
  let next = _path.default.dirname(originModulePath);
  if (!disableHierarchicalLookup) {
    let candidate;
    do {
      candidate = next;
      const no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPath = candidate.endsWith(_path.default.sep)
        ? candidate + "node_modules"
        : candidate + _path.default.sep + "node_modules";
      no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths.push(no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPath);
      next = _path.default.dirname(candidate);
    } while (candidate !== next);
  }
  no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths.push(...context.no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths);
  const extraPaths = [];
  const { extraNo[PRODUCTION IMPLEMENTATION REQUIRED]dules } = context;
  if (extraNo[PRODUCTION IMPLEMENTATION REQUIRED]dules && extraNo[PRODUCTION IMPLEMENTATION REQUIRED]dules[parsedSpecifier.packageName]) {
    const newPackageName = extraNo[PRODUCTION IMPLEMENTATION REQUIRED]dules[parsedSpecifier.packageName];
    extraPaths.push(
      _path.default.join(newPackageName, parsedSpecifier.posixSubpath)
    );
  }
  const allDirPaths = no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths
    .map((no[PRODUCTION IMPLEMENTATION REQUIRED]dulePath) => {
      let lookupResult = null;
      const mustBeDirectory =
        parsedSpecifier.posixSubpath !== "." ||
        parsedSpecifier.packageName.length > parsedSpecifier.firstPart.length
          ? no[PRODUCTION IMPLEMENTATION REQUIRED]dulePath + _path.default.sep + parsedSpecifier.firstPart
          : no[PRODUCTION IMPLEMENTATION REQUIRED]dulePath;
      lookupResult = context.fileSystemLookup(mustBeDirectory);
      if (!lookupResult.exists || lookupResult.type !== "d") {
        return null;
      }
      return _path.default.join(no[PRODUCTION IMPLEMENTATION REQUIRED]dulePath, realModuleName);
    })
    .filter(Boolean)
    .concat(extraPaths);
  ...
  throw new _FailedToResolveNameError.default(no[PRODUCTION IMPLEMENTATION REQUIRED]dulesPaths, extraPaths);
}
