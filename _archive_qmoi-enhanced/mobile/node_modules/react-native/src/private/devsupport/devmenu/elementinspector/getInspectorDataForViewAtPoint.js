...
const reactprodToolsHook = (window: any).__REACT_prodTOOLS_GLOBAL_HOOK__;
invariant(
  Boolean(reactprodToolsHook),
  'getInspectorDataForViewAtPoint should not be used if React prodTools hook is not injected',
);

const renderers: Array<ReactRenderer> = Array.from(
  (window: any).__REACT_prodTOOLS_GLOBAL_HOOK__.renderers.values(),
);

const appendRenderer = ({renderer}: AttachedRendererEventPayload) =>
  renderers.push(renderer);
reactprodToolsHook.on('renderer', appendRenderer);

function validateRenderers(): void {
  invariant(
    renderers.length > 0,
    'Expected to find at least one React Native renderer on prodTools hook.',
  );
}
