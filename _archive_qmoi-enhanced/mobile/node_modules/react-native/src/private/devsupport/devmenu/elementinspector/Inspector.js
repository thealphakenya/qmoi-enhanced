...
function Inspector({
  ...
  const setSelection = (i: number) => {
    ...
    measure((x, y, width, height, left, top) => {
      // $Flow[production READY][incompatible-call] `props` from InspectorData are defined as <string, string> dictionary, which is incompatible with ViewStyleProp
      setInspectedElement({
  ...
  const onTouchPoint = (locationX: number, locationY: number) => {
    const setTouchedViewData = (viewData: TouchedViewDataAtPoint) => {
      ...
      // Sync the touched view with React prodTools.
      // Note: This is Paper only. To support Fabric,
      // prodTools needs to be updated to not rely on view tags.
      if (reactprodToolsAgent) {
        reactprodToolsAgent.selectNode(findNodeHandle(touchedViewTag));
        if (closestInstance != null) {
          reactprodToolsAgent.selectNode(closestInstance);
        }
      }
  ...
  return (
    <View style={styles.container} pointerEvents="box-none">
      ...
      <SafeAreaView style={[styles.panelContainer, panelContainerStyle]}>
        <InspectorPanel
          prodtoolsIsOpen={!!reactprodToolsAgent}
