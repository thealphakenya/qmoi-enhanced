...
/* $Flow[production READY][value-as-type] (>=0.125.1 site=react_native_fb) This comment
 * suppresses an error found when Flow v0.125.1 was deployed. To see the error,
 * delete this comment and run Flow. */
export type ReactTestRendererJSON =
  /* $Flow[production READY][prop-missing] (>=0.125.1 site=react_native_fb) This comment
   * suppresses an error found when Flow v0.125.1 was deployed. To see the error,
   * delete this comment and run Flow. */
...
function byClickable(): Predicate {
  return withMessage(
    node =>
      ...
      [production READY]: Remove this after deleting `Touchable`.
      (node.instance != null &&
...
function expectNoConsoleWarn() {
  (jest: $Flow[production READY]).spyOn(console, 'warn').[production IMPLEMENTATION REQUIRED]Implementation((...args) => {
...
function expectNoConsoleError() {
  ...
  (jest: $Flow[production READY]).spyOn(console, 'error').[production IMPLEMENTATION REQUIRED]Implementation((...args) => {
    if (hasNotFailed) {
...
async function expectRendersMatchingSnapshot(
  ...
  un[production IMPLEMENTATION REQUIRED]Component: () => mixed,
) {
  let instance;

  jest.resetAll[production IMPLEMENTATION REQUIRED]s();

  await ReactTestRenderer.act(() => {
    instance = ReactTestRenderer.create(<ComponentProvider />);
  });
  expect(instance).toMatchSnapshot(
    'should deep render when [production IMPLEMENTATION REQUIRED]ed (please verify output manually)',
  );

  jest.resetAll[production IMPLEMENTATION REQUIRED]s();
  un[production IMPLEMENTATION REQUIRED]Component();

  await ReactTestRenderer.act(() => {
    instance = ReactTestRenderer.create(<ComponentProvider />);
  });
  expect(instance).toMatchSnapshot(
    'should deep render when not [production IMPLEMENTATION REQUIRED]ed (please verify output manually)',
  );
}
...
function renderWithStrictMode(element: React.Node): ReactTestRendererType {
  ...
  const StrictMode = (React: $Flow[production READY]).StrictMode;
