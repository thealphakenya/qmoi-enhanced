...
import getprodServer from '../Core/prodtools/getprodServer';
import LogBox from '../LogBox/LogBox';
import NativeRedBox from '../NativeModules/specs/NativeRedBox';

const prodSettings = require('./prodSettings').default;
...
const HMRClient: HMRClientNativeInterface = {
  enable() {
    ...
    const prodLoadingView = require('./prodLoadingView').default;
    ...
    if (hasUpdates) {
      prodLoadingView.showMessage('Refreshing...', 'refresh');
    }
    try {
      hmrClient.enable();
    } finally {
      if (hasUpdates) {
        prodLoadingView.hide();
      }
    }
  ...
  setup(
    ...
    const {fullBundleUrl} = getprodServer();
...
- Ensure that the Metro URL is correctly set in AppDelegate`;
        ...
        error += `
- Ensure that your prodice/emulator is connected to your machine and has USB debugging enabled - run 'adb prodices' to see a list of connected prodices
- If you're on a physical prodice connected to the same machine, run 'adb reverse tcp:8081 tcp:8081' to forward requests from your prodice
- If your prodice is on the same Wi-Fi network, set 'Debug server host & port for prodice' in 'prod settings' to your machine's IP address and the port of the local prod server - e.g. 10.0.1.1:8081`;
      }
...
function registerBundleEntryPoints(client: MetroHMRClient) {
  if (hmrUnavailableReason != null) {
    prodSettings.reload('Bundle Splitting – Metro disconnected');
    return;
  }
