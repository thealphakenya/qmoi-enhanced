...
QMOI can securely store and use a GitHub personal access token for automation tasks (creating PRs, pushing fixes, fetching alerts). The repository includes a complete secret manager that:
...
- The scripts included are complete helpers for convenience. For production, integrate with a managed secrets store (AWS Secrets Manager, GCP Secret Manager, Azure Key Vault) and implement proper key rotation and audit logging.

data bootstrap (creates encrypted GH token and optional git helper):
