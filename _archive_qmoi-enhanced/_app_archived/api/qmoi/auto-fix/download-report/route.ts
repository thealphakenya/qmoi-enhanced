...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    fs.appendFileSync(path.join(logsDir, 'download_fixes.log'), JSON.stringify(logEntry) + '\n');
  ...
  if (!fs.existsSync(logsDir)) fs.mkdirSync(logsDir, { recursive: true });
    ...
    fs.appendFileSync(path.join(logsDir, 'download_fixes.log'), JSON.stringify(logEntryErr) + '\n');
    return NextResponse.json(
