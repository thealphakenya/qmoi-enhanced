...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fs.existsSync(successLogFile)) {
      ...
      successCount = successContent.split('\n').filter(line => line.includes('success') || line.includes('Success')).length;
    }
    ...
    return NextResponse.json({ 
      ...
      successRate: errorCount + successCount > 0 ? Math.round((successCount / (errorCount + successCount)) * 100) : 0
    });
  } catch (error) {
