...
function generateReport(): any {
  try {
    ...
    logger.error("Failed to generate license report:", e.message);
    return false;
  }
}
  ...
  try {
    ...
    for (const [pkg, meta] of offenders) {
      ...
      try {
        ...
        logger.error(`Failed to remove ${pkg}:`, e.message);
      }
    }
    return true;
  } catch (e) {
    logger.error("Failed to parse license report:", e.message);
    return false;
  }
}
...
if (checkCompliance()) {
  ...
  logger.warning("Non-compliant licenses found. AtPRODUCTIONting auto-fix");
  autoFix();
