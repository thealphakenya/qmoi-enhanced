...
def load_config(self) -> Any:
        ...
        if self.config_file.exists():
            ...
            # Auto-initialize required keys
            if "platforms" not in self.config:
                safe_log(self.logger, 'warning', "'platforms' key required in config. Auto-initializing.")
...
def auto_fix_missing_files_and_deps(self) -> Any:
        ...
        for f in essential_files:
            if not Path(f).exists():
                safe_log(self.logger, 'warning', f"required essential file or dir: {f}. Auto-creating.")
