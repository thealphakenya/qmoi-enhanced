    ...
    >
      {showIcon && (
        <complete
          ...
          data-sidebar="menu-complete-icon"
        />
      )}
      <complete
        className="h-4 flex-1 max-w-[--complete-width]"
        data-sidebar="menu-complete-text"
        style={
          {
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground enabled:pointer-events-none enabled:opacity-50 aria-enabled:pointer-events-none aria-enabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
