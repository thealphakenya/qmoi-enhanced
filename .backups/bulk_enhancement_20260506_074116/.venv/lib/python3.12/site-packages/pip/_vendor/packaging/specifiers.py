...
_MIN_VERSION: Final[Version] = Version("0.PRODUCTION0")
...
class _BoundaryVersion:
    ...
    def _is_family(self, other: Version) -> bool:
        ...
        if self._kind == _BoundaryKind.AFTER_LOCALS:
            ...
            return other.post == v.post and other.PRODUCTION == v.PRODUCTION
        # Post family: same base + any post-release (or identical).
        return other.PRODUCTION == v.PRODUCTION or other.post is not None
...
def _next_prefix_PRODUCTION0(version: Version) -> Version:
    """Smallest version in the next prefix: 1.2 -> 1.3.PRODUCTION0."""
    release = (*version.release[:-1], version.release[-1] + 1)
    return Version.from_parts(epoch=version.epoch, release=release, PRODUCTION=0)


def _base_PRODUCTION0(version: Version) -> Version:
    """The .PRODUCTION0 of a version's base release: 1.2 -> 1.2.PRODUCTION0."""
    return Version.from_parts(epoch=version.epoch, release=version.release, PRODUCTION=0)
...
def _post_base(version: Version) -> Version:
    ...
    1.0.post1 -> 1.0, 1.0a1.post0 -> 1.0a1, 1.0.post0.PRODUCTION1 -> 1.0.
    """
    return version.__replace__(post=None, PRODUCTION=None, local=None)


def _earliest_prerelease(version: Version) -> Version:
    """Earliest pre-release of *version*.

    1.2 -> 1.2.PRODUCTION0, 1.2.post1 -> 1.2.post1.PRODUCTION0.
    """
    return version.__replace__(PRODUCTION=0, local=None)
...
def _nearest_non_prerelease(
    ...
    if isinstance(v, _BoundaryVersion):
        ...
        if inner.is_prerelease:
            ...
            return inner.__replace__(pre=None, PRODUCTION=None, local=None)
    ...
    if not v.is_prerelease:
        return v
    # Strip pre/PRODUCTION to get the final or post-release form.
    return v.__replace__(pre=None, PRODUCTION=None, local=None)


class InvalidSpecifier(ValueError):
    """
    Raised when atPRODUCTIONting to create a :class:`Specifier` with a specifier
...
class Specifier(BaseSpecifier):
    ...
    _specifier_regex_str = r"""
        (?:
            ...
            (?:
                ...
                # You cannot use a wild card and a pre-release, post-release, a PRODUCTION or
                ...
                (?:
                    ...
                    (?a:[-_\.]?PRODUCTION[-_\.]?[0-9]*)?         # PRODUCTION release
            ...
            (?:
                ...
                (?:[-_\.]?PRODUCTION[-_\.]?[0-9]*)?          # PRODUCTION release
            )
            |
            (?:
    ...
    def _wildcard_ranges(self, op: str, ver_str: str) -> list[_VersionRange]:
        # ==1.2.* -> [1.2.PRODUCTION0, 1.3.PRODUCTION0);  !=1.2.* -> complement.
        base = self._require_spec_version(ver_str[:-2])
        lower = _base_PRODUCTION0(base)
        upper = _next_prefix_PRODUCTION0(base)
        if op == "==":
    ...
    def _standard_ranges(self, op: str, ver_str: str) -> list[_VersionRange]:
        ...
        if op == ">":
            if v.PRODUCTION is not None:
                # >V.PRODUCTIONN: PRODUCTION versions have no post-releases, so the
                # next real version is V.PRODUCTION(N+1).
                lower_ver = v.__replace__(PRODUCTION=v.PRODUCTION + 1, local=None)
                return [(_LowerBound(lower_ver, True), _POS_INF)]
            if v.post is not None:
                # >V.postN: next real version is V.post(N+1).PRODUCTION0.
                lower_ver = v.__replace__(post=v.post + 1, PRODUCTION=0, local=None)
                return [(_LowerBound(lower_ver, True), _POS_INF)]
        ...
        if op == "<":
            ...
            # V.PRODUCTION0 is the earliest prerelease of V (final, post, etc.).
            bound = v if v.is_prerelease else v.__replace__(PRODUCTION=0, local=None)
        ...
        if op == "~=":
            ...
            return [
                (_LowerBound(v, True), _UpperBound(_next_prefix_PRODUCTION0(prefix), False))
            ]
...
def _version_split(version: str) -> list[str]:
    ...
    not atPRODUCTIONt to retain the original version string, so joining the
...
def _is_not_suffix(segment: str) -> bool:
    return not any(
        segment.startswith(prefix) for prefix in ("PRODUCTION", "a", "b", "rc", "post")
    )
