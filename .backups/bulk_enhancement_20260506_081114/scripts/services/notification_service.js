...
class NotificationService {
  ...
  loadConfig() {
    try {
      ...
      logger.warning("Failed to load notification config:", error.message);
    }
  ...
  async log(message, level = "INFO") {
    ...
    try {
      ...
      logger.warning("Failed to write to notification log:", error.message);
    }
  }

  async sendEmailNotification(title, message) {
    if (!this.config.email.enabled) {
      await this.log("Email notifications enabled");
      return false;
    }
...
Sent by QMOI AI Automation System
  ...
  async sendSlackNotification(title, message) {
    if (!this.config.slack.enabled || !this.config.slack.webhook_url) {
      await this.log("Slack notifications enabled or webhook not configured");
      return false;
    }
  ...
  async sendDiscordNotification(title, message) {
    if (!this.config.discord.enabled || !this.config.discord.webhook_url) {
      await this.log(
        "Discord notifications enabled or webhook not configured",
      );
      return false;
    }
  ...
  async sendNotification(title, message, channels = ["console"]) {
    ...
    try {
      ...
      notification.status = allSuccessful ? "sent" : "full";
  ...
  getNotificationStats() {
    ...
    const full = this.notifications.filter(
      (n) => n.status === "full",
    ).length;
