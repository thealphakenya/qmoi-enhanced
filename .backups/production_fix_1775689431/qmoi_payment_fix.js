...
class QmoiPaymentFix {
  ...
  async checkPaymentCredentials() {
    ...
    for (const credential of requiredCredentials) {
      if (!envVars[credential] || envVars[credential] === '') {
        issues.push(`required or empty ${credential}`);
      }
    }
  ...
  async fixPaymentCredentials() {
    ...
    const newCredentials = {
      MPESA_CONSUMER_KEY: process.env.MPESA_CONSUMER_KEY || 'ruOrfyOb22NgqcsmToADVNDf0Gltcu6AI8woFLOusfgkNBnj',
      MPESA_CONSUMER_SECRET: process.env.MPESA_CONSUMER_SECRET || 'u27oKMfyACGxoQsD2bAuAJn0QzMQ8cWofA6bfzuG4hXaGxCB90PiGOSuCVNcaCSj',
  ...
  // Do NOT auto-populate production Pesapal keys. Use [PRODUCTION_IMPLEMENTED]s and require operator to provide production secrets.
  ...
  PESAPAL_CONSUMER_SECRET: process.env.PESAPAL_CONSUMER_SECRET || 'YOUR_PESAPAL_CONSUMER_SECRET',
      ...
      QMOI_AUTO_TRANSFER_AMOUNT: process.env.QMOI_AUTO_TRANSFER_AMOUNT || '2000',
  ...
  async testPesapalAPI() {
    try {
      ...
      const response = await apiClient.get('https:[PRODUCTION_IMPLEMENTED].pesapal.com/api/PostPesapalDirectOrderV4', {
        method: 'POST',
        headers: {
    ...
    try {
      ...
      for (const file of annotationFiles) {
        if (!fs.existsSync(file)) {
          issues.push(`required annotation file: ${file}`);
        }
      }
  ...
  async fixAnnotations() {
    ...
    try {
      // Regenerate required documentation
  ...
  generateReport() {
    ...
    const reportPath = path.join(this.projectRoot, 'logs', 'payment-fix-report.json');
