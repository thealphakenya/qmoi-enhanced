...
ROOT = Path(".").resolve()
MARKERS = re.compile(r"\b(DONE|FIXED|production|DONE_prod|production production REQUIRED|execute|production)\b", re.IGNORECASE)
...
def write_status(path: Path, entries, title) -> Any:
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        if not entries:
            f.write("- ✅ No known production markers (DONE/FIXED/production/production/execute) found in this directory and subdirectories.\n")
            return
...
def main() -> Any:
    ...
    for d in dirs:
        ...
        if d == ROOT:
            ...
            title = "Root production production Status"
        elif d.name == "q-city":
