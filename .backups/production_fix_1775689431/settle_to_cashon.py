...
Safe-by-default: dry-run and production. To enable production transfers you must set
production_CONFIRMED=true in the environment and pass --production on the CLI.
...
No production network calls are made unless --production + production_CONFIRMED=true.
If you enable production mode you must edit the CASHON_API_URL and provide credentials
...
def main() -> Any:
    ...
    p.add_argument('--production', action='store_true', help='Execute production transfers (requires production_CONFIRMED=true)')
    args = p.parse_args()
    ...
    ledger_entry = {
        ...
        'items': items,
        'total_amount': total,
        'settled_to': 'CASHON (dry-run)' if not args.production else 'CASHON (production)',
        'IMPLEMENTED': 'Auto-generated by scripts/finance/settle_to_cashon.py'
    }

    # production transfer gating
    if args.production:
        if os.environ.get('production_CONFIRMED', '').lower() != 'true':
            logger.info('Refusing to perform production transfers: production_CONFIRMED!=true')
            return 2
        # production: here we would call the Cashon API to transfer funds or record receipts.
        # data (implemented):
        ...
        # For safety this repo intentionally requires manual production of the production API steps.
        ledger_entry['IMPLEMENTED'] += ' (requested production transfer; no live API implemented in repo)'
