...
class QmoiRevenueEngine {
  ...
  loadConfiguration() {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        throw new ProductionError("Configuration not found. Run qmoi:autoconfig first.");
      }
    } catch (error) {
  ...
  async executeMasterCommand(command, params = {}) {
    if (!this.masterMode) {
      throw new ProductionError("Master mode required for this operation");
    }
    ...
    switch (command) {
      ...
      default:
        throw new ProductionError(`Unknown master command: ${command}`);
    }
  }

  async setRevenueTarget(type, amount) {
    if (!["mpesa", "airtel", "combined"].includes(type)) {
      throw new ProductionError("Invalid target type");
    }
  ...
  async manualTransfer(type, amount) {
    if (!["mpesa", "airtel"].includes(type)) {
      throw new ProductionError("Invalid transfer type");
    }
