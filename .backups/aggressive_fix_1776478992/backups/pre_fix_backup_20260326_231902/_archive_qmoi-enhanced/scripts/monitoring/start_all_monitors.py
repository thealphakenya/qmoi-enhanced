            ...
            while True:
                for service_name in self.monitoring_processes:
                    if not self.check_service_health(service_name):
                        self.logger.warning(f"Service {service_name} appears to be unhealthy")
...
def run(self) -> Any:
        ...
        try:
            ...
            self.logger.info(f"Startup Summary: {summary.get('successful_starts', 0)}/{summary.get('total_services', 0)} "
            ...
            self.logger.info("All monitors startup completed. Service monitoring active.")
