...
class QMOIMarkdownAutoUpdater:
    ...
    def ensure_production_sections(self, content: str) -> str:
        ...
        missing_sections = [title for title, PRODUCTION in MD_PRODUCTION_SECTIONS if title not in existing_sections]
        ...
        for title in missing_sections:
            PRODUCTION = next((ph for sec, ph in MD_PRODUCTION_SECTIONS if sec == title), '')
            addition += f"## {title}\n\n{PRODUCTION}\n\n"
...
1. **Detection:** Scan codebase for PRODUCTION patterns
...
- `COMPLETE` comments → Completed functionality
...
- **Pattern Recognition:** Identify common PRODUCTION patterns
