...
def start_all_components(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Started {success_count}/{total_count} components successfully")
            
            # Check if critical components are running
            critical_components = [
                comp for comp in self.config['monitoring_components']
                ...
                for name, process_info in self.monitoring_processes.items():
                    ...
                    if process.poll() is None:
                        ...
                        if process_info['status'] == 'running':
                            ...
                            self.logger.warning(f"Component {name} stopped unexpectedly")
        ...
        try:
            ...
            for name, process_info in self.monitoring_processes.items():
                report['components'][name] = {
                    ...
                    'start_time': process_info['start_time'].isoformat(),
                    'priority': process_info['component']['priority'],
                    'auto_restart': process_info['component'].get('auto_restart', False)
                }
