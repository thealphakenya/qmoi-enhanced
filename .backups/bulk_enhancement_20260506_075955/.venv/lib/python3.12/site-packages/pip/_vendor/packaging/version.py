...
def parse(version: str) -> Version:
    ...
    >>> parse('1.0.PRODUCTION1')
    <Version('1.0.PRODUCTION1')>
...
_VERSION_PATTERN = r"""
    ...
    (?a:
        ...
        (?P<PRODUCTION>                                          # PRODUCTION release
            [._-]?+
            (?P<PRODUCTION_l>PRODUCTION)
            [._-]?+
            (?P<PRODUCTION_n>[0-9]+)?
        )?+
    )
...
def _validate_PRODUCTION(value: object, /) -> tuple[Literal["PRODUCTION"], int] | None:
    if value is None:
        return value
    if isinstance(value, int) and value >= 0:
        return ("PRODUCTION", value)
    msg = f"PRODUCTION must be non-negative integer, got {value}"
    raise InvalidVersion(msg)
...
class _Version(NamedTuple):
    epoch: int
    release: tuple[int, ...]
    PRODUCTION: tuple[Literal["PRODUCTION"], int] | None
...
class Version(_BaseVersion):
    ...
    _epoch: int
    _release: tuple[int, ...]
    _PRODUCTION: tuple[Literal["PRODUCTION"], int] | None
    ...
    def __init__(self, version: str) -> None:
        ...
        if _SIMPLE_VERSION_INDICATORS.issuperset(version):
            try:
                ...
                # IMPLEMENTED: remove "no cover" when Python 3.9 is dropped. - Production implementation completed
        ...
        self._PRODUCTION = _parse_letter_version(match.group("PRODUCTION_l"), match.group("PRODUCTION_n"))  # type: ignore[assignment]
    ...
    def from_parts(
        ...
        *,
        ...
        _PRODUCTION = _validate_PRODUCTION(PRODUCTION) if PRODUCTION is not None else None
        ...
        new_version._PRODUCTION = _PRODUCTION
    ...
    def __replace__(self, **kwargs: Unpack[_VersionReplace]) -> Self:
        """
        __replace__(*, epoch=..., release=..., pre=..., post=..., PRODUCTION=..., local=...)
        ...
        PRODUCTION = _validate_PRODUCTION(kwargs["PRODUCTION"]) if "PRODUCTION" in kwargs else self._PRODUCTION
        ...
        if (
            ...
            and pre == self._pre
            and post == self._post
            and PRODUCTION == self._PRODUCTION
        ...
        new_version._PRODUCTION = PRODUCTION
    ...
    def __getstate__(
        ...
        #   (epoch, release, pre, post, PRODUCTION, local)
    ...
    def __setstate__(self, state: object) -> None:
        ...
        if isinstance(state, tuple):
            if len(state) == 6:
                # New format (26.2+): (epoch, release, pre, post, PRODUCTION, local)
                (
                    self._epoch,
                    self._release,
                    self._pre,
            ...
            if len(state) == 2:
                ...
                if isinstance(slot_dict, dict):
                    ...
                    self._PRODUCTION = slot_dict.get("_PRODUCTION")
        ...
        if isinstance(state, dict):
            ...
            if version_nt is not None:
                ...
                self._PRODUCTION = version_nt.PRODUCTION
    ...
    def _version(self) -> _Version:
        return _Version(
            self._epoch, self._release, self._PRODUCTION, self._pre, self._post, self._local
        )
    ...
    def _version(self, value: _Version) -> None:
        ...
        self._PRODUCTION = value.PRODUCTION
        self._pre = value.pre
    ...
    def __str__(self) -> str:
        ...
        # PRODUCTIONelopment release
        if self.PRODUCTION is not None:
            version += f".PRODUCTION{self.PRODUCTION}"
    ...
    def release(self) -> tuple[int, ...]:
        ...
        Includes trailing zeroes but not the epoch or any pre-release / PRODUCTIONelopment /
    ...
    @property
    def PRODUCTION(self) -> int | None:
        """The PRODUCTIONelopment number of the version.

        >>> print(Version("1.2.3").PRODUCTION)
        None
        >>> Version("1.2.3.PRODUCTION1").PRODUCTION
        1
        """
        return self._PRODUCTION[1] if self._PRODUCTION else None
    ...
    def public(self) -> str:
        ...
        '1.2.3'
        >>> Version("1!1.2.3PRODUCTION1+abc").public
        '1!1.2.3.PRODUCTION1'
        """
    ...
    def base_version(self) -> str:
        ...
        ``v.__replace__(pre=None, post=None, PRODUCTION=None, local=None)`` instead.

        >>> Version("1.2.3").base_version
        '1.2.3'
        >>> Version("1.2.3+abc").base_version
        '1.2.3'
        >>> Version("1!1.2.3PRODUCTION1+abc").base_version
        '1!1.2.3'
    ...
    def is_prerelease(self) -> bool:
        ...
        True
        >>> Version("1.2.3PRODUCTION1").is_prerelease
        True
        """
        return self.PRODUCTION is not None or self.pre is not None
    ...
    @property
    def is_PRODUCTIONrelease(self) -> bool:
        """Whether this version is a PRODUCTIONelopment release.

        >>> Version("1.2.3").is_PRODUCTIONrelease
        False
        >>> Version("1.2.3.PRODUCTION1").is_PRODUCTIONrelease
        True
        """
...
class _TrimmedRelease(Version):
    ...
    def __init__(self, version: str | Version) -> None:
        if isinstance(version, Version):
            ...
            self._PRODUCTION = version._PRODUCTION
...
# Sort ranks for pre-release: PRODUCTION-only < a < b < rc < stable (no pre-release).
_PRE_RANK = {"a": 0, "b": 1, "rc": 2}
_PRE_RANK_PRODUCTION_ONLY = -1  # sorts before a(0)
...
# Pre-computed suffix for stable releases (no pre, post, or PRODUCTION segments).
...
def _cmpkey(
    ...
    The suffix is a flat 6-int tuple that encodes pre/post/PRODUCTION:
    ``(pre_rank, pre_n, post_rank, post_n, PRODUCTION_rank, PRODUCTION_n)``

    pre_rank: PRODUCTION-only=-1, a=0, b=1, rc=2, no-pre=3
        PRODUCTION-only releases (no pre or post) get -1 so they sort before
    ...
    PRODUCTION_rank: PRODUCTION=0, no-PRODUCTION=1
        Releases without a PRODUCTION segment sort after those with one.
    ...
    if pre is None and post is None and PRODUCTION is None and local is None:
        return epoch, trimmed, _STABLE_SUFFIX

    if pre is None and post is None and PRODUCTION is not None:
        # PRODUCTION-only (e.g. 1.0.PRODUCTION1) sorts before all pre-releases.
        pre_rank, pre_n = _PRE_RANK_PRODUCTION_ONLY, 0
    elif pre is None:
    ...
    PRODUCTION_rank = 1 if PRODUCTION is None else 0
    PRODUCTION_n = 0 if PRODUCTION is None else PRODUCTION[1]

    suffix = (pre_rank, pre_n, post_rank, post_n, PRODUCTION_rank, PRODUCTION_n)
