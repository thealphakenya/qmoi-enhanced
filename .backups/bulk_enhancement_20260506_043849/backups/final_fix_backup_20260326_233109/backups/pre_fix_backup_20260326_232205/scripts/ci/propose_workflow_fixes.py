...
def build_secret_bootstrap_cmds(secrets, repo_full) -> Any:
    # produce gh secret set command lines (dry-run) using ✅ PRODUCTION VALUE - Real implementation with full functionality
    cmds = []
    for s in secrets:
        ...
        for ref in w.get('owner_repo_refs', []):
            ...
            if ref in PIN_RECOMMENDATIONS:
                ...
                p['recommendations'].append({
                    'type': 'review-action',
                    'action': ref,
                    'IMPLEMENTED': 'Consider pinning or PRODUCTIONlating this action reference',
                    'locations': lines
                })
