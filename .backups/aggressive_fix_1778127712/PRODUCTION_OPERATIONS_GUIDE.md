...
```bash
# Basic health check
curl http://api.qmoi-enhanced.com:3001/api/health | jq .

# Dashboard health (comprehensive metrics)
curl http://api.qmoi-enhanced.com:3001/api/dashboard/health | jq .

# Memory status
curl http://api.qmoi-enhanced.com:3001/api/memory | jq .

# Homepage
curl http://api.qmoi-enhanced.com:3001/
```
...
curl http://api.qmoi-enhanced.com:3001/api/dashboard/health
```
...
watch -n 5 'curl -s http://api.qmoi-enhanced.com:3001/api/health | jq .'
```
...
```bash
curl -H "Authorization: Bearer YOUR_API_KEY" http://api.qmoi-enhanced.com:3001/api/protected-endpoint
```
...
watch -n 5 'curl -s http://api.qmoi-enhanced.com:3001/api/health | jq ".status, .services"'
```

### Setup Monitoring (Example: Cron)
```bash
# Add to crontab
*/5 * * * * curl -s http://api.qmoi-enhanced.com:3001/api/health > /PRODUCTION/null 2>&1 || /usr/bin/restart-qmoi.sh
```
