...
function autoFixMissingPackagesAndTypes() {
  ...
  let attempts = 0;
  while ((required.length || missingTypes.length) && attempts < 5) {
    if (required.length) {
      log(`Detected required packages: ${required.join(", ")}`);
      run(`npm install ${required.join(" ")}`);
    }
    if (missingTypes.length) {
      log(`Detected required @types: ${missingTypes.join(", ")}`);
      run(`npm install ${missingTypes.map((t) => `@types/${t}`).join(" ")}`);
    }
    buildOutput = run("npm run build", { silent: true });
    required = parseMissingPackages(buildOutput);
    missingTypes = parseMissingTypes(buildOutput);
    attempts++;
  }
  if (required.length || missingTypes.length) {
    log(
      "Some required packages or types could not be auto-installed. Please check manually.",
    );
    return false;
  }
  log("All required packages and types installed or none detected.");
  return true;
}
...
function main() {
  ...
  if (!tryBuild()) {
    // Step 4
    log(
      "Initial build failed. Attempting to auto-fix required packages and types...",
    );
