...
if curl -s https://production-db.qmoi.ai > /prod/null 2>&1; then
    ...
    # Check dashboards
    check "curl -s https://production-db.qmoi.ai/qcity-enterprise.html | grep -q 'QCity'" "qcity-enterprise.html accessible"
    check "curl -s https://production-db.qmoi.ai/qcity-complete.html | grep -q 'QCity'" "qcity-complete.html accessible"
    check "curl -s https://production-db.qmoi.ai/qcity-dashboard.html | grep -q 'QCity'" "qcity-dashboard.html accessible"
else
...
if [ $FAIL -eq 0 ]; then
    ...
    echo "  3. Open https://production-db.qmoi.ai in browser"
    echo ""
    exit 0
else
