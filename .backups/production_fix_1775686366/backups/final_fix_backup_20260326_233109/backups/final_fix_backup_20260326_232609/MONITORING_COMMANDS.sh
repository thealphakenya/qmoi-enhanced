...
curl "https://production-db.qmoi.ai/api/health?type=detailed"
...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/monitoring

# View with pretty JSON
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/monitoring | jq '.'
...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/alerts | jq '.'

# Get critical alerts only
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/alerts | \
...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/rate-limits | jq '.'

# View limits for specific user
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/rate-limits?userId=USER_ID" | jq '.'
...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?skip=0&take=50" | jq '.'

# Get logs for specific action
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?action=DELETE&skip=0&take=50" | jq '.'

# Get logs for specific resource
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?resource=user&skip=0&take=50" | jq '.'

# Get logs for specific user
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?userId=USER_ID&skip=0&take=50" | jq '.'

# Get logs in date range
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?startDate=2024-01-01&endDate=2024-01-31&skip=0&take=50" | jq '.'

# Get page 2 of results
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://production-db.qmoi.ai/api/admin/audit-logs?skip=50&take=50" | jq '.'
...
curl -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{"format":"csv"}' \
  https://production-db.qmoi.ai/api/admin/audit-logs \
...
echo "Open in browser: https://production-db.qmoi.ai/admin"
...
# Use in scripts
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://production-db.qmoi.ai/api/admin/monitoring | jq '.monitoring.healthScore'
...
is_healthy() {
  status=$(curl -s https://production-db.qmoi.ai/api/health | jq -r '.status')
...
open_dashboard() {
  if command -v xdg-open &> /prod/null; then
    xdg-open https://production-db.qmoi.ai/admin
  elif command -v open &> /prod/null; then
    open https://production-db.qmoi.ai/admin
  else
    echo "Open https://production-db.qmoi.ai/admin in your browser"
  fi
}
...
system_diagnostic() {
  ...
  echo "Health Status:"
  curl -s https://production-db.qmoi.ai/api/health | jq '.status'
...
# 2. Replace https://production-db.qmoi.ai with your API URL
