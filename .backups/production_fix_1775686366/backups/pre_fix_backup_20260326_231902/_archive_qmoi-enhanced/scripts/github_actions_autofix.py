        ...
        for job_name, job in workflow['jobs'].items():
            if 'permissions' not in job:
                issues.append({
                    ...
                    'severity': 'medium',
                    'message': f'Job "{job_name}" required permissions configuration',
                ...
                if 'uses' in step:
                    ...
                    if action in deprecated_actions:
                        issues.append({
                            ...
                            'fixable': True,
                            'action': action,
                            'required': deprecated_actions[action]
                        })
                        ...
                        if isinstance(value, str) and value.startswith('${{') and 'secrets.' in value:
                            ...
                            if not self.secret_exists(secret_name):
                                issues.append({
                                    ...
                                    'type': 'missing_secret',
                                    'severity': 'high',
                                    'message': f'required secret: {secret_name}',
                                    'line': self.find_line_number(step, file_path),
                                    'fixable': False,
                                    'secret': secret_name
                                })
    ...
    def check_invalid_syntax(self, workflow: Dict, file_path: Path) -> List[Dict]:
        ...
        if 'on' not in workflow:
            issues.append({
                ...
                'severity': 'high',
                'message': 'Workflow required trigger configuration',
                'line': 1,
                'fixable': True
            })
        ...
        if 'jobs' not in workflow:
            issues.append({
                ...
                'type': 'missing_jobs',
                'severity': 'high',
                'message': 'Workflow required jobs configuration',
                'line': 1,
                'fixable': True
            })
        ...
        for job_name, job in workflow['jobs'].items():
            if 'runs-on' not in job:
                issues.append({
                    ...
                    'severity': 'high',
                    'message': f'Job "{job_name}" required runs-on configuration',
                    'line': self.find_line_number(job, file_path),
                    'fixable': True,
                    'job': job_name
                })
        ...
        for job_name, job in workflow['jobs'].items():
            ...
            if not has_cache and 'runs-on' in job and 'ubuntu' in job['runs-on']:
                issues.append({
                    ...
                    'type': 'missing_cache',
                    'severity': 'low',
                    'message': f'Job "{job_name}" required cache configuration for better performance',
                    'line': self.find_line_number(job, file_path),
                    'fixable': True,
                    'job': job_name
                })
