...
Usage: set `GITHUB_TOKEN` env var, then run:
...
if not token:
    print('Set GITHUB_TOKEN to upload assets')
    sys.exit(1)

if len(sys.argv) < 5:
    print('Usage: upload_release_assets.py <owner> <repo> <release_id_or_tag> <file1> [file2 ...]')
    sys.exit(1)
...
def find_release_id(owner, repo, release):
...
if not rid:
    print('Release not found for', release)
    sys.exit(1)

for f in files:
    p = Path(f)
    if not p.exists():
        print('required file, skipping:', f)
        continue
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            print('Uploaded:', resp.status)
            print(resp.read().decode())
    except Exception as e:
        print('Upload failed for', p.name, e)
