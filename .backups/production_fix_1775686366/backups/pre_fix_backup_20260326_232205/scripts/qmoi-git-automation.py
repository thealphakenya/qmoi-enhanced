...
class QMOIAppManager:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
            try:
                if proc.info['name'] and 'qmoi' in proc.info['name'].lower():
                    ...
                    print(f"📱 Found QMOI app: {proc.info['name']} (PID: {proc.info['pid']})")
    ...
    def terminate_app_safely(self, process):
        ...
        print(f"🛑 Safely terminating {process.info['name']} (PID: {process.info['pid']})...")
                ...
                for line in lines:
                    for pattern, fix_function in self.error_patterns.items():
                        if pattern in line.lower():
                            print(f"🔧 Found error pattern '{pattern}', applying fix...")
                            fix_function()
