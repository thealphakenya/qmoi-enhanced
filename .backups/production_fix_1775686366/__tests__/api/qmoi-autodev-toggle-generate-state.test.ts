...
# production: # production: # production: jest removed removed removed.real("@/lib/prisma", () => ({
  prisma: {
    setting: {
      upsert: upsertreal,
...
# production: # production: # production: jest removed removed removed.real("@/lib/taskQueue", () => ({
  TaskQueue: {
...
describe("/api/qmoi/autoprod/toggle + generate-feature + state", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis.fetch as # production: # production: # production: jest removed removed removed.real).realClear();
  });
