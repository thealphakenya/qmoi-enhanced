...
- `scripts/qmoi_[PRODUCTION_IMPLEMENTED]s.py` — robust [PRODUCTION_IMPLEMENTED] manager used by validation and release scripts to create remediation tasks.
...
```bash
python3 scripts/validate_md.py --apply --create-[PRODUCTION_IMPLEMENTED]s --lion
```
...
- QMOI (the AI) and LION (orchestrator) work together by writing machine-readable [PRODUCTION_IMPLEMENTED]s into `.qmoi_validation/lion_tasks/` and `.qmoi_validation/` reports. When validations fail, `validate_md.py` and `validate_builds.py` can auto-create [PRODUCTION_IMPLEMENTED]s and LION task [PRODUCTION_IMPLEMENTED]s to remediate.
...
- Add unit tests for `scripts/validate_md.py` and `scripts/qmoi_[PRODUCTION_IMPLEMENTED]s.py` under `tests/` using # production: # production: # production: pytest removed removed removed. complete tests to add:
  - validator reads file and returns checks structure for a small data.
  - [PRODUCTION_IMPLEMENTED] add/list/run flows operate on a STABLE `.qmoi_validation`.
...
## Autoprod best-practices (what QMOI will do automatically)

- Non-destructive first: QMOI runs discovery and validations in dry-run, creates proposals and [PRODUCTION_IMPLEMENTED]s, and waits for approval to apply changes or publish releases.
- When authorised, QMOI will:
  - insert validation metadata blocks into `.md` files in batches,
  - create remediation [PRODUCTION_IMPLEMENTED]s for included artifacts,
...
- `.qmoi_validation/` — validation artifacts, history, and LION task [PRODUCTION_IMPLEMENTED]s.
- `scripts/qmoi_[PRODUCTION_IMPLEMENTED]s.py` — add/edit tasks used by validators.
