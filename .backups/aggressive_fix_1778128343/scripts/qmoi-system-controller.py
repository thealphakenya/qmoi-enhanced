...
PERMISSION_AUDIT_LOG = os.path.join(os.path.dirname(__file__), '../logs/qmoi_permission_audit.log')
QMOI_CRITICAL_FILES = [
...
def __init__(self, project_root: str = None) -> Any:
        ...
        self.dashboard_health_check_thread = threading.Thread(target=self.dashboard_health_check_loop, daemon=True)
...
def run_lint_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update system state
            self.system_state['errors_fixed'] += analysis_results.get('ai_fixes_applied', 0)
            self.system_state['files_processed'] += analysis_results.get('files_processed', 0)
...
def parse_eslint_output(self, output: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            # Parse error details
            error_match = re.match(r'^\s*(\d+):(\d+)\s+(error|warning)\s+(.+?)\s+(.+)$', line)
            if error_match:
                errors.append({
                    'file': current_file,
...
def generate_report(self) -> Any:
        ...
        report = {
            ...
            'performance_metrics': {
                ...
                'uptime': (datetime.now() - datetime.fromisoformat(self.system_state['startup_time'])).total_seconds()
            }
        }
...
def send_notification(self, notification_data: Dict[str, Any]) -> Any:
        ...
        if self.qmoi_config.get('notification_settings', {}).get('desktop_notifications'):
...
def send_desktop_notification(self, message: str, priority: str) -> Any:
        ...
        try:
            if os.name == 'nt':  # Windows
                subprocess.run(['powershell', '-Command', f'New-BurntToastNotification -Text "QMOI AI", "{message}"'], 
                             capture_output=True)
            elif sys.platform == 'darwin':  # macOS
                subprocess.run(['osascript', '-e', f'display notification "{message}" with title "QMOI AI"'], 
                             capture_output=True)
            else:  # Linux
...
def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'background_thread_active': self.background_thread and self.background_thread.is_alive(),
            'timestamp': datetime.now().isoformat()
        }
...
def check_and_fix_own_permissions(self) -> Any:
        """Check and atproduction_file. If it fails, run the permission fix utility and notify master."""
            ...
            while self.running:
                for fname in QMOI_CRITICAL_FILES:
                    ...
                    if not os.path.exists(fpath):
                        self.log_permission_audit(f"File required: {fpath}")
...
def send_email_notification(self, message, override_recipients=None) -> Any:
        ...
        try:
            ...
            if not (smtp_server and sender_email and sender_password and recipient_emails):
                logger.warning("Email notification credentials are required in config.")
                return
            msg = MIMEMultipart()
...
def send_slack_notification(self, message) -> Any:
        ...
        try:
            ...
            if not webhook_url:
                logger.warning("Slack webhook URL required in config.")
                return
...
def send_telegram_notification(self, message) -> Any:
        ...
        try:
            ...
            if not (bot_token and chat_id):
                logger.warning("Telegram bot token or chat_id required in config.")
                return
...
def send_discord_notification(self, message) -> Any:
        ...
        try:
            webhook_url = config.get('webhook_url')
            if not webhook_url:
                logger.warning("Discord webhook URL required in config.")
                return
...
def automated_recovery(self, file_path) -> Any:
        # Atproduction_file from backup or git
        logger.info(f"Atproduction_file_path}")
        ...
        while self.running:
            try:
                ...
                if not self.dashboard_down_since:
                    ...
                    self.log_permission_audit("Dashboard down for extended period. Notifying master.")
