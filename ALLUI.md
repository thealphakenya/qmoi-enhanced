# ALLUI.md - Complete UI Inventory

## Current Live UI Apps

- `/qalpha` → `app/qalpha/page.tsx`
- `/qcity` → `app/qcity/page.tsx`
- `/qmoi-ai` → `app/qmoi-ai/page.tsx`
- `/qmoi-space` → `app/qmoi-space/page.tsx`
- `/qvillage` → `app/qvillage/page.tsx`

## Actual Page Routes

- `app/admin/page.tsx` → `/admin`
- `app/admin/master/page.tsx` → `/admin/master`
- `app/admin/master/activity/page.tsx` → `/admin/master/activity`
- `app/admin/master/login/page.tsx` → `/admin/master/login`
- `app/admin/master/security/page.tsx` → `/admin/master/security`
- `app/admin/master/settings/page.tsx` → `/admin/master/settings`
- `app/dev/page.tsx` → `/dev`
- `app/devices/page.tsx` → `/devices`
- `app/friendship/page.tsx` → `/friendship`
- `app/master/email/page.tsx` → `/master/email`
- `app/master/links/page.tsx` → `/master/links`
- `app/master/tracks/page.tsx` → `/master/tracks`
- `app/qalpha/page.tsx` → `/qalpha`
- `app/qcity/page.tsx` → `/qcity`
- `app/qmoi-ai/page.tsx` → `/qmoi-ai`
- `app/qmoi-space/page.tsx` → `/qmoi-space`
- `app/qvillage/page.tsx` → `/qvillage`

## Shared UI Components

- `app/components/AdminDashboard.tsx`
- `app/components/AnalyticsCenter.tsx`
- `app/components/AnalyticsDashboard.tsx`
- `app/components/ApiManagementConsole.tsx`
- `app/components/AudibleConversation.tsx`
- `app/components/AuditLogViewer.tsx`
- `app/components/BackupRestoreManager.tsx`
- `app/components/ChatMessaging.tsx`
- `app/components/ClientUISettings.tsx`
- `app/components/CollaborationHub.tsx`
- `app/components/CommunicationHub.tsx`
- `app/components/ComplianceManager.tsx`
- `app/components/ContentManagementSystem.tsx`
- `app/components/DataVisualizationPanel.tsx`
- `app/components/DeploymentManager.tsx`
- `app/components/FileUploadDownload.tsx`
- `app/components/FloatingPreviewWindow.tsx`
- `app/components/GlobalOperationsCenter.tsx`
- `app/components/HelpGuide.tsx`
- `app/components/InnovationLab.tsx`
- `app/components/IntegrationManager.tsx`
- `app/components/KnowledgeBase.tsx`
- `app/components/LcSpaces.tsx`
- `app/components/Marketplace.tsx`
- `app/components/MonitoringDashboard.tsx`
- `app/components/NotificationCenter.tsx`
- `app/components/PreviewWindow.tsx`
- `app/components/QCityErrorManager.tsx`
- `app/components/QI.tsx`
- `app/components/QIStateWindow.tsx`
- `app/components/QMOIAutoFixDashboard.tsx`
- `app/components/QMOIAutoSetup.tsx`
- `app/components/QMOIMasterDashboard.tsx`
- `app/components/QVillage.tsx`
- `app/components/QVillageDatasetsPanel.tsx`
- `app/components/QiSpaces.tsx`
- `app/components/ResourceManager.tsx`
- `app/components/SecurityMonitor.tsx`
- `app/components/SettingsPanel.tsx`
- `app/components/SupportTicketSystem.tsx`
- `app/components/TestingAutomationSuite.tsx`
- `app/components/TrainingCenter.tsx`
- `app/components/VisualEnhancement.tsx`
- `app/components/WalletPanel.tsx`
- `app/components/WorkflowAutomationEngine.tsx`
- `app/components/audio/AudioMonitor.tsx`
- `app/components/auth/BiometricEnrollment.tsx`
- `app/components/auth/PrivacyMaskSettings.tsx`
- `app/components/auth/UniversalRouteGuard.tsx`
- `app/components/language/LanguageSelector.tsx`
- `app/components/styles/index.ts`
- `app/components/wallet/WalletList.tsx`
- `src/components/@vercel/analytics/next.ts`
- `src/components/@vercel/analytics/next.tsx`
- `src/components/AssetOverview.tsx`
- `src/components/CollaborationLayer.tsx`
- `src/components/ComponentGallery.tsx`
- `src/components/DownloadQCity.tsx`
- `src/components/ErrorBoundary.tsx`
- `src/components/FeedbackLoop.tsx`
- `src/components/FileExplorer.tsx`
- `src/components/FloatingAQ.tsx`
- `src/components/GitStatus.tsx`
- `src/components/LcSpaces.tsx`
- `src/components/OfflineCacheService.tsx`
- `src/components/PluginRegistry.tsx`
- `src/components/PreviewWindow.tsx`
- `src/components/QI.tsx`
- `src/components/QIStateWindow.tsx`
- `src/components/QI_Enhanced.tsx`
- `src/components/QiSpaces.tsx`
- `src/components/SelfHealingWindows.tsx`
- `src/components/TradingHistory.tsx`
- `src/components/TradingStatus.tsx`
- `src/components/UISettings.tsx`
- `src/components/UniversalWindowManager.tsx`
- `src/components/UsageAnalytics.tsx`
- `src/components/VersionedStates.tsx`
- `src/components/WindowTelemetryPanel.tsx`
- `src/components/alpha-q-ai-system.tsx`
- `src/components/auth/RoleGate.tsx`
- `src/components/financial/ProtectedFinancialFeatures.tsx`
- `src/components/master/DomainHealthTable.tsx`
- `src/components/master/DomainStats.tsx`
- `src/components/qalpha/QAlphaShell.tsx`
- `src/components/qalpha/index.ts`
- `src/components/qmoi/AvatarDisplay.tsx`
- `src/components/qmoi/FriendshipManagement.tsx`
- `src/components/qmoi/FriendshipUI.tsx`
- `src/components/qmoi/ProjectManagement.tsx`
- `src/components/qmoi/QMOIAIShell.tsx`
- `src/components/qmoi/QMOIChat.tsx`
- `src/components/qmoi/QMOISpaceShell.tsx`
- `src/components/qmoi/VoiceSelector.tsx`
- `src/components/qmoi/index.ts`
- `src/components/qvillage/QVillageShell.tsx`
- `src/components/qvillage/index.ts`
- `src/components/release-notes.tsx`
- `src/components/shared/ai/AITradingRules.tsx`
- `src/components/shared/ai/AutomationEngine.tsx`
- `src/components/shared/ai/Chatbot.tsx`
- `src/components/shared/ai/ChatbotEnhanced.tsx`
- `src/components/shared/ai/FederatedLearningService.tsx`
- `src/components/shared/ai/PredictiveToolRecommender.tsx`
- `src/components/shared/ai/index.ts`
- `src/components/shared/dashboards/GlobalOperationsDashboard.tsx`
- `src/components/shared/dashboards/RevenueAnalyticsDashboard.tsx`
- `src/components/shared/dashboards/index.ts`
- `src/components/shared/index.ts`
- `src/components/shared/system/ConsciousnessMonitoring.tsx`
- `src/components/shared/system/GlobalHotkeyService.tsx`
- `src/components/shared/system/GlobalNotificationCenter.tsx`
- `src/components/shared/system/index.ts`
- `src/components/shared/tools/AssetOverview.tsx`
- `src/components/shared/tools/DownloadQCity.tsx`
- `src/components/shared/tools/FileExplorer.tsx`
- `src/components/shared/tools/GitStatus.tsx`
- `src/components/shared/tools/PluginRegistry.tsx`
- `src/components/shared/tools/index.ts`
- `src/components/shared/ui/AccessibilityAdjuster.tsx`
- `src/components/shared/ui/AdaptiveTheming.tsx`
- `src/components/shared/ui/PrivacyModeToggle.tsx`
- `src/components/shared/ui/index.ts`
- `src/components/shared/voice/VoiceSelector.tsx`
- `src/components/shared/voice/index.ts`
- `src/components/vercel-analytics-next.ts`

## Notes
This inventory is generated from the current UI component and page entrypoint files in the repository.

<!-- LION_VALIDATION_START -->
## 🦁 L — Validated by Quantum multi orchestra intelligence (QMOI) Lion

- validated: yes
- validator: Quantum multi orchestra intelligence (QMOI) Lion
- timestamp: 2026-06-28T19:36:35.408808Z
- production status: ❌ needs production implementation
- status tags: needs-production, nonproduction
- lines: 179
- words: 449
- characters: 6991
- headings: 6
- links: 0
- images: 0
- tables: 0
- lion validation block: present
<!-- LION_VALIDATION_END -->
