...
export const Chatbot: React.FC<ChatbotProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingMessageId !== null) {
      ...
      setChatHistory(chatHistory.map(msg =>
        msg.id === editingMessageId ? { ...msg, content: editMessageValue } : msg
      ));
    ...
    if (uploadedFile) {
      newHistory.push({
        ...
        content: uploadedFile.type.startsWith('image') ? (
          <img src={uploadedFile.url} alt={uploadedFile.name} className="max-h-32 rounded border mt-2" />
        ) : uploadedFile.type.startsWith('audio') ? (
          <audio controls src={uploadedFile.url} className="mt-2" />
        ) : (
          <a href={uploadedFile.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline mt-2 block">{uploadedFile.name}</a>
        ),
        sender: 'user',
        timestamp: new Date().toISOString(),
        file: uploadedFile,
  ...
  return (
    <div
      ...
      aria-label={screenReader ? 'QMOI Chatbot. Accessible chat interface.' : undefined}
    >
      {/* Settings Bar */}
          ...
          >
            <div className="flex items-center gap-2">
              <div className="text-sm flex-1">
                {editingMessageId === message.id ? (
                  <form onSubmit={handleSubmit} className="flex gap-2">
                    ...
                    />
                    <button type="submit" className="bg-green-600 px-2 py-1 rounded text-white">Save</button>
                    <button type="button" onClick={() => { setEditingMessageId(null); setEditMessageValue(''); setInputMessage(''); }} className="bg-gray-600 px-2 py-1 rounded text-white">Cancel</button>
                  </form>
                ) : (
                  message.content
                )}
              ...
              <div className="flex items-center gap-1">
                {message.reactions && message.reactions.map((r: string, i: number) => (
                ...
                {reactionPickerId === message.id && (
                  <div className="absolute z-10 bg-white text-black rounded shadow p-1 flex gap-1 top-8 right-0">
                    {["👍","❤️","😂","😮","😢","👏"].map(r => (
                      <button key={r} onClick={() => handleAddReaction(message.id, r)} className="hover:bg-gray-200 rounded px-1">{r}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              {message.sender === 'user' && editingMessageId !== message.id && (
                <>
                  <button
                    ...
                    onClick={() => handleEditMessage(message.id, typeof message.content === 'string' ? message.content : '')}
                    title="Edit"
                  >✏️</button>
                  <button
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 items-center">
        ...
        <button
          type="submit"
          disabled={isLoading || uploading || ((editingMessageId !== null ? !editMessageValue.trim() : (!inputMessage.trim() && !uploadedFile)))}
          className="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white px-4 py-2 rounded"
        >
          {editingMessageId !== null ? 'Save' : (uploading ? 'Uploading...' : 'Send')}
        </button>
        {editingMessageId !== null && (
          <button
            type="button"
            onClick={() => { setEditingMessageId(null); setEditMessageValue(''); setInputMessage(''); }}
      ...
      {uploadedFile && (
        <div className="mt-2">
          <span className="text-xs text-green-300">Ready to send: {uploadedFile.name}</span>
          ...
          {!(uploadedFile.type.startsWith('image') || uploadedFile.type.startsWith('audio')) && (
            <a href={uploadedFile.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline mt-2 block">{uploadedFile.name}</a>
          )}
        </div>
      )}
