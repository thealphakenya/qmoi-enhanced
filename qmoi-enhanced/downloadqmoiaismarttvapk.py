...
def get_latest_github_release_info():
    ...
    try:
        ...
        log_activity(
            "Failed to fetch latest GitHub Smart TV APK URL", {"error": str(e)}
        )
    return None, None
    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            if is_valid_apk(path):
                ...
                log_activity(
                    f"Smart TV APK too small after download",
...
if not url:
    ...
    download_dirs = [
        ensure_download_dir("smarttv", "latest"),
        ensure_download_dir("smarttv", version_folder),
    ]
    ...
    if download_apk(url, apk_paths[0]):
        if apk_paths[0] != apk_paths[1]:
            try:
                ...
                log_activity(
                    "Copied Smart TV APK to versioned folder",
                    {"from": apk_paths[0], "to": apk_paths[1]},
                )
                print(f"Also saved: {apk_paths[1]}")
            except Exception as e:
                log_activity(
                    "Failed to copy Smart TV APK to versioned folder", {"error": str(e)}
                )
    else:
