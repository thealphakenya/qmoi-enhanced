...
function installExtension() {
  try {
    ...
    fs.writeFileSync('.qmoi-installed', 'QMOI extension installed at ' + new Date().toISOString());
...
function ensurePermanent() {
  ...
  fs.writeFileSync('.qmoi-permanent', 'QMOI extension is permanent until uninstalled.');
}
...
function injectFloatingChat() {
  try {
    ...
    fs.writeFileSync('.qmoi-floating-chat', 'QMOI floating chat UI injected at ' + new Date().toISOString());
...
function main() {
  ...
  injectFloatingChat();
  console.log('QMOI extension is healthy, fully automated, and floating chat is active.');
}
