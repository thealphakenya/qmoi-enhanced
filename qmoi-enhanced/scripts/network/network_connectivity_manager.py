...
class NetworkConnectivityManager:
    ...
    def initialize_connection_methods(self):
        ...
        self.connection_methods = {
            ...
            "ethernet": ConnectionMethod(
                ...
                enabled=True,
                priority=2,
                config={"auto_connect": True, "dhcp": True},
            ),
            "mobile_hotspot": ConnectionMethod(
                name="Mobile Hotspot",
                enabled=True,
                priority=3,
                config={"auto_connect": True, "ssid": "", "password": ""},
            ),
            ...
            "zero_rated": ConnectionMethod(
                ...
                enabled=True,
                priority=6,
                config={"sites": self.zero_rated_sites, "auto_connect": True},
            ),
        }
    ...
    def get_current_network_status(self) -> NetworkStatus:
        ...
        return NetworkStatus(
            connected=connected,
            method=(
                self.current_connection.name if self.current_connection else "unknown"
            ),
            speed=speed,
            latency=latency,
    ...
    def scan_wifi_networks(self) -> List[Dict]:
        ...
        try:
            if self.os_type == "windows":
                ...
                result = subprocess.run(
                    ["iwlist", "scan"], capture_output=True, text=True
                )
                current_ssid = None
                current_security = None
                ...
                result = subprocess.run(
                    ["airport", "-s"], capture_output=True, text=True
                )
                for line in result.stdout.split("\n")[1:]:  # Skip header
                    if line.strip():
                        parts = line.split()
...
<WLANProfile xmlns="http://www.microsoft.com/networking/WLAN/profile/v1">
        ...
        <security>
            ...
            <sharedKey>
                ...
                <keyMaterial>{password}</keyMaterial>
                    ...
                    # Add profile
                    subprocess.run(
                        ["netsh", "wlan", "add", "profile", f"filename={ssid}.xml"],
                        check=True,
                    )

                    # Connect
                    subprocess.run(
                        ["netsh", "wlan", "connect", f"name={ssid}"], check=True
                    )
    ...
    psk="{password}"
                ...
                if password:
                    ...
                    subprocess.run(
                        ["networksetup", "-setairportnetwork", "en0", ssid], check=True
                    )
            ...
            for available_network in available_networks:
                if available_network["ssid"] == saved_network["ssid"]:
                    if self.connect_wifi(
                        saved_network["ssid"], saved_network.get("password")
                    ):
                        return True
                ...
                for provider_name, vpn_config in self.vpn_providers.items():
                    try:
                        ...
                        if self.check_internet_connectivity():
                            ...
                            self.current_connection.last_used = datetime.now()
                            logger.info(
                                f"Successfully connected to VPN: {provider_name}"
                            )
                            return True
    ...
    def connect_qcity(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                self.connection_methods["qcity"].config["endpoint"], timeout=10
            )
            ...
            for site in self.zero_rated_sites:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.current_connection.last_used = datetime.now()
                        logger.info(
                            f"Successfully connected via zero-rated site: {site}"
                        )
                        return True
                except Exception:
                    continue
    ...
    def ensure_connectivity(self) -> bool:
        ...
        sorted_methods = sorted(
            self.connection_methods.values(), key=lambda x: x.priority
        )
            ...
            while self.monitoring_active:
                try:
                    ...
                    if not self.check_internet_connectivity():
                        logger.warning(
                            "Internet connectivity lost, attempting to restore"
                        )
    ...
    def get_connection_statistics(self) -> Dict:
        ...
        return {
            "current_connection": (
                self.current_connection.name if self.current_connection else "none"
            ),
            "connection_methods": {
                name: {
    ...
    def optimize_dns(self):
        ...
        try:
            ...
            if self.os_type == "windows":
                ...
                subprocess.run(
                    ["networksetup", "-setdnsservers", "Wi-Fi"] + fast_dns_servers,
                    check=True,
                )
    ...
    def update_zero_rated_endpoints(self):
        ...
        try:
            ...
            response = requests.get(
                "https://qcity.ai/api/zero-rated-endpoints", timeout=10
            )
            if response.status_code == 200:
                self.zero_rated_sites = response.json().get(
                    "endpoints", self.zero_rated_sites
                )
