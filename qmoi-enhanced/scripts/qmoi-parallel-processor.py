...
class QMOIParallelProcessor:
    ...
    def execute_parallel(
        self, tasks: List[Task], max_concurrent: int = None
    ) -> Dict[str, TaskResult]:
        ...
        self.logger.info(
            f"Executing {len(tasks)} tasks with max {max_concurrent} concurrent"
        )
    ...
    def _sort_by_dependencies(self, tasks: List[Task]) -> List[Task]:
        ...
        while remaining_tasks:
            ...
            for task in remaining_tasks:
                if all(
                    dep in [t.id for t in sorted_tasks] for dep in task.dependencies
                ):
...
class PerformanceTracker:
    ...
    def get_average_execution_time(self) -> float:
        ...
        if not self.successful_tasks:
            return 0.0
        return sum(task.execution_time for task in self.successful_tasks) / len(
            self.successful_tasks
        )
    ...
    def get_total_memory_usage(self) -> float:
        ...
        return sum(
            task.memory_usage for task in self.successful_tasks + self.failed_tasks
        )
