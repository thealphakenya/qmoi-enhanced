...
class GitHubActionsAutoFix:
    ...
    def __init__(self):
        ...
        self.repo_name = os.environ.get("GITHUB_REPOSITORY", "Alpha-Q-ai").split("/")[
            -1
        ]
    ...
    def scan_workflow_files(self) -> List[Dict]:
        ...
        for workflow_file in self.workflows_dir.glob("*.yml"):
            try:
                ...
                file_issues = self.analyze_workflow_file(
                    workflow_file, workflow_yaml, workflow_content
                )
    ...
    def analyze_workflow_file(
        self, file_path: Path, workflow_yaml: Dict, content: str
    ) -> List[Dict]:
    ...
    def fix_missing_jobs(self, issue: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            return {"type": "missing_jobs", "file": str(file_path), "status": "fixed"}
    ...
    def create_github_issue(self, issue: Dict) -> bool:
        ...
        if (
            not self.master_config["create_issues_for_unfixable"]
...
""",
    ...
    def get_recommendation_for_issue(self, issue: Dict) -> str:
        ...
        return recommendations.get(
            issue["type"], "Review the issue and apply appropriate fixes."
        )
    ...
    def trigger_self_healing_if_needed(self, errors: List[Dict]) -> None:
        ...
        if errors:
            logger.info(
                "Dependency errors detected in workflow logs. Triggering self-healing script..."
            )
            try:
                result = subprocess.run(
                    [sys.executable, "scripts/qmoi_self_healing_enhanced.py"],
                    capture_output=True,
                    text=True,
                    timeout=900,
                )
                logger.info(
                    "Self-healing script output: " + result.stdout + result.stderr
                )
            except Exception as e:
    ...
    def generate_recommendations(
        self, issues: List[Dict], fixes: List[Dict]
    ) -> List[str]:
        ...
        if len(issues) == 0:
            ...
            recommendations.append(
                f"Found {len(issues)} issues, applied {len(fixes)} fixes"
            )
