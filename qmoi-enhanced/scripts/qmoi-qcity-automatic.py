...
class QMOIQCityAutomatic:
    ...
    def run_comprehensive_qcity(self):
        ...
        try:
            ...
            qcity_steps = [
                ...
                (
                    "python scripts/autotest/advanced_autotest_system.py",
                    "QMOI Autotest",
                ),
            ]
            for command, description in qcity_steps:
                ...
                while retries < max_retries:
                    try:
                        logger.info(
                            f"\U0001f504 Running: {description} (Attempt {retries+1})"
                        )
                        ...
                        if result.returncode == 0:
                            ...
                            self.send_notification(
                                f"{description} succeeded", result.stdout
                            )
                            break
                        else:
                            logger.error(
    ...
    def send_notification(self, subject, message):
        try:
            ...
            subprocess.run(
                f'python scripts/qmoi_notification_manager.py "{subject}" "{message}"',
                shell=True,
            )
        except Exception as e:
    ...
    def update_platform_status(self):
        ...
        try:
            ...
            vercel_result = subprocess.run(
                "vercel --version", shell=True, capture_output=True, text=True
            )
            ...
            # Check Gitpod status
            gitpod_result = subprocess.run(
                "gp --version", shell=True, capture_output=True, text=True
            )
