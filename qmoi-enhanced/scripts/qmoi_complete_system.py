...
class QMOICompleteSystem:
    ...
    def start_component(
        ...
        try:
            ...
            if not script_path or not os.path.exists(script_path):
                self.logger.error(
                    f"Script not found for {component_name}: {script_path}"
                )
                return False
            ...
            if process.poll() is None:
                ...
                self.logger.error(
                    f"✗ Failed to start {component_name}: {stderr.decode()}"
                )
    ...
    def restart_component(self, component_name: str) -> bool:
        ...
        try:
            ...
            if component_name in self.config["system_components"]["core_services"]:
                component_config = self.config["system_components"]["core_services"][
                    component_name
                ]
    ...
    def start_core_services(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Started {success_count}/{len(core_services)} core services"
            )
            ...
            for service_name in critical_services:
                if (
                    service_name in self.components
                    and self.components[service_name]["status"] == "running"
                ):
                    critical_running += 1
    ...
    def start_monitoring_services(self):
        ...
        try:
            ...
            monitoring_services = self.config["system_components"][
                "monitoring_services"
            ]
            ...
            for service_name, script_path in monitoring_services.items():
                try:
                    if os.path.exists(script_path):
                        ...
                        self.logger.warning(
                            f"Monitoring script not found: {script_path}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error starting monitoring service {service_name}: {e}"
                    )
        ...
        while self.system_active:
            try:
                ...
                for component_name, component_info in self.components.items():
                    if component_info["config"].get("health_check", False):
                        if not self.check_component_health(component_name):
                            self.logger.warning(
                                f"Component {component_name} appears unhealthy"
                            )

                            # Auto-restart if enabled
                            if component_info["config"].get("auto_restart", False):
                                max_attempts = self.config["auto_recovery"][
                                    "max_attempts"
                                ]
                                if component_info["restart_count"] < max_attempts:
                                    self.logger.info(
                                        f"Auto-restarting {component_name}..."
                                    )
                                    if self.restart_component(component_name):
                                        component_info["restart_count"] = 0
                                        self.logger.info(
                                            f"Successfully restarted {component_name}"
                                        )
                                    else:
                                        component_info["restart_count"] += 1
                                        self.logger.error(
                                            f"Failed to restart {component_name} (attempt {component_info['restart_count']})"
                                        )
                                else:
                                    self.logger.error(
                                        f"Component {component_name} failed too many times"
                                    )
                ...
                if (
                    ...
                    > self.config["performance_thresholds"]["cpu_usage"]
                ):
                    self.logger.warning(f"High CPU usage: {system['cpu_percent']}%")

                if (
                    system.get("memory_percent", 0)
                    > self.config["performance_thresholds"]["memory_usage"]
                ):
                    self.logger.warning(
                        f"High memory usage: {system['memory_percent']}%"
                    )

                if (
                    system.get("disk_percent", 0)
                    > self.config["performance_thresholds"]["disk_usage"]
                ):
        ...
        try:
            ...
            for component_name, component_info in self.components.items():
                ...
                report["components"][component_name] = {
                    ...
                    "last_health_check": (
                        component_info["last_health_check"].isoformat()
                        if component_info["last_health_check"]
                        else None
                    ),
                }
            ...
            if report["system_status"]["components_failed"] > 0:
                ...
                report["system_status"]["components_running"]
                < len(self.components) * 0.9
            ):
        ...
        try:
            ...
            while self.system_active:
                ...
                if self.system_active:
                    ...
                    self.logger.info(
                        f"System Status: {status.get('overall_health', 'unknown')}, "
