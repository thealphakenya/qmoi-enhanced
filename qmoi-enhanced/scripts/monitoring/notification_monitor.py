...
QMOI Backup Monitor
    ...
    def send_discord_notification(self, notification: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Send to webhook
            response = requests.post(
                discord_config["webhook_url"], json=payload, timeout=10
            )
            ...
            for url in webhook_config["urls"]:
                ...
                try:
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        self.logger.warning(
                            f"Webhook notification failed for {url}: {response.status_code}"
                        )
        ...
        try:
            ...
            for channel in notification["channels"]:
                ...
                if channel_success:
                    success = True
                    self.logger.info(
                        f"Notification sent via {channel}: {notification['id']}"
                    )
                else:
                    self.logger.warning(
                        f"Failed to send notification via {channel}: {notification['id']}"
                    )
        ...
        while True:
            try:
                ...
                if self.should_send_notification(notification):
                    ...
                    self.logger.info(
                        f"Notification skipped due to cooldown: {notification['id']}"
                    )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_notifications = len(
                [n for n in self.notification_history if n["status"] == "sent"]
            )
            failed_notifications = len(
                [n for n in self.notification_history if n["status"] == "failed"]
            )
            ...
            report = {
                ...
                "recent_notifications": self.notification_history[
                    -10:
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                self.logger.info(
                    f"Notification Status: {summary.get('successful_notifications', 0)}/{summary.get('total_notifications', 0)} "
