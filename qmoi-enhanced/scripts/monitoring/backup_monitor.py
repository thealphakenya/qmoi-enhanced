...
class BackupMonitor:
        ...
        try:
            for backup_type, schedule_time in self.config["backup_schedule"].items():
                schedule.every().day.at(
                    schedule_time.split()[1] + ":" + schedule_time.split()[0]
                ).do(self.create_backup, backup_type)
    ...
    def create_backup(self, backup_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not source_dirs:
                ...
                self.logger.error(
                    f"No source directories found for {backup_type} backup"
                )
                return backup_info
            ...
            if self.config["integrity_check"]:
                backup_info["integrity_check"] = self.check_backup_integrity(
                    backup_path
                )
            ...
            self.logger.info(
                f"Created {backup_type} backup: {backup_filename} ({backup_info['size']} bytes)"
            )
            ...
            try:
                ...
                for backup_type, status in self.backup_status.items():
                    if status.get("status") == "failed":
                        self.logger.warning(
                            f"Backup {backup_type} failed: {status.get('error', 'Unknown error')}"
                        )
                ...
                for backup_type in self.config["backup_directories"].keys():
                    ...
                    if last_backup:
                        hours_since_backup = (
                            datetime.now() - last_backup
                        ).total_seconds() / 3600

                        # Alert if backup is overdue
                        if hours_since_backup > 24:
                            self.logger.warning(
                                f"Backup {backup_type} is overdue ({hours_since_backup:.1f} hours)"
                            )
                ...
                for backup_type, directory in self.config["backup_directories"].items():
                    if os.path.exists(directory):
                        ...
                        if usage_percentage > self.config["storage_threshold"]:
                            self.logger.warning(
                                f"High storage usage for {backup_type}: {usage_percentage:.1f}%"
                            )
    ...
    def monitor_backup_integrity(self):
        ...
        while self.monitoring_active:
                        ...
                        for filename in os.listdir(directory):
                            if filename.startswith(f"{backup_type}_backup_"):
                                ...
                                if file_path not in self.integrity_checks:
                                    ...
                                    if not is_valid:
                                        self.logger.error(
                                            f"Backup integrity check failed: {filename}"
                                        )
            ...
            time.sleep(
                self.config["monitoring_interval"] * 4
            )  # Check integrity less frequently
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for storage_info in self.storage_usage.values():
                report["summary"]["total_storage_used"] += storage_info.get(
                    "total_size", 0
                )

                if (
                    storage_info.get("disk_usage_percentage", 0)
                    > self.config["storage_threshold"]
                ):
                    report["summary"]["alerts"].append(
                        {
                            "type": "high_storage_usage",
                            "backup_type": storage_info.get("directory", ""),
                            "usage_percentage": storage_info.get(
            ...
            if (
                report["summary"]["total_storage_used"] > 1024 * 1024 * 1024 * 10
            ):  # 10GB
                report["recommendations"].append(
                    {
