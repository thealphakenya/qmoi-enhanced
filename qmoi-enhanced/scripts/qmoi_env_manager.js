...
function checkAndCreateEnv() {
  ...
  if (missing.length && fs.existsSync(configPath)) {
    ...
    if (config.whatsapp_integration) {
      if (
        missing.includes("WHATSAPP_API_TOKEN") &&
        config.whatsapp_integration.api_token
      ) {
        envVars["WHATSAPP_API_TOKEN"] = config.whatsapp_integration.api_token;
        missing = missing.filter((k) => k !== "WHATSAPP_API_TOKEN");
      }
      if (
        missing.includes("WHATSAPP_WEBHOOK_URL") &&
        config.whatsapp_integration.webhook_url
      ) {
        envVars["WHATSAPP_WEBHOOK_URL"] =
          config.whatsapp_integration.webhook_url;
  ...
  if (missing.length) {
    ...
    if (Object.keys(exampleDefaults).length) {
      logEnvManager(
        `Filled from .env.example: ${Object.keys(exampleDefaults).join(", ")}`,
      );
    }
  }
  ...
  if (missing.length) {
    ...
    if (Object.keys(safeDefaults).length) {
      logEnvManager(
        `Filled from safe defaults: ${Object.keys(safeDefaults).join(", ")}`,
      );
    }
  }
  // Write .env if needed
  ...
  if (missing.length) {
    logEnvManager(
      `\u26a0\ufe0f Missing required environment variables after all attempts: ${missing.join(", ")}`,
    );
    logEnvManager(
      "Please set them in your GitHub secrets, config, or .env file.",
    );
