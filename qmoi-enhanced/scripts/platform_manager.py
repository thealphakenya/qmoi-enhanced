...
class PlatformManager:
    ...
    def load_config(self, config_path: str):
        ...
        try:
            ...
            self.logger.warning(
                f"Platform config not found at {config_path}, using defaults"
            )
            self.config = {
                "platforms": {"colab": True, "cloud": True, "local": True},
                "sync_interval": 60,
    ...
    def _update_platform_status(self):
        ...
        try:
            ...
            status_file = (
                Path("data/platform")
                / f"platform_status_{datetime.now().strftime('%Y%m%d')}.json"
            )
    ...
    def execute_on_platform(
        self, platform_id: str, command: str
