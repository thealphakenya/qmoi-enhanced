...
model_server = QMOIServer(
    model_path="deployed_model/model.onnx", config_path="deployed_model/config.json"
)
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        attention_mask = (
            torch.tensor([request.attention_mask]) if request.attention_mask else None
        )
        ...
        return PredictionResponse(
            predictions=predictions, probabilities=probabilities, task=request.task
        )
...
async def predict_batch(request: BatchPredictionRequest):
    ...
    try:
        ...
        for input_request in request.inputs:
            ...
            attention_mask = (
                torch.tensor([input_request.attention_mask])
                if input_request.attention_mask
                else None
            )
