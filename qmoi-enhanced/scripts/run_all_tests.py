...
class MasterTestRunner:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for category, test_func in test_categories.items():
            try:
                ...
                if not result or all(
                    "error" in v and "not found" in v["error"] for v in result.values()
                ):
        ...
        self.logger.info(
            f"✅ Master test suite completed in {execution_time:.2f} seconds"
        )
        return self.results
    ...
    async def run_multi_user_tests(self) -> Dict[str, Any]:
        ...
        tests = ["tests/unit/test_multi_user_session.py"]
    ...
    async def run_e2e_tests(self) -> Dict[str, Any]:
        ...
        tests = ["tests/e2e/test_full_workflow.py", "tests/e2e/test_performance.py"]
    ...
    def generate_summary_report(self, filepath: Path):
        ...
        with open(filepath, "w") as f:
            ...
            f.write(
                f"Execution Time: {summary.get('total_execution_time', 0):.2f} seconds\n\n"
            )
            ...
            for category, results in self.results["test_results"].items():
                if isinstance(results, dict):
                    ...
                    total_count = len(
                        [r for r in results.values() if isinstance(r, dict)]
                    )
                    success_rate = (
                        (success_count / total_count * 100) if total_count > 0 else 0
                    )
                    f.write(
                        f"{category}: {success_rate:.1f}% success ({success_count}/{total_count})\n"
                    )
    ...
    def print_results(self):
        ...
        for category, results in self.results["test_results"].items():
            if isinstance(results, dict):
                ...
                print(
                    f"{status} {category}: {success_rate:.1f}% ({success_count}/{total_count})"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Master Test Runner for Alpha-Q AI System"
    )
    parser.add_argument("--config", type=str, help="Path to test configuration file")
    parser.add_argument(
        "--category",
        choices=["error_fixing", "multi_user", "all"],
        default="all",
