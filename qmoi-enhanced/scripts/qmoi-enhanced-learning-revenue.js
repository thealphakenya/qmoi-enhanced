...
async function learnFromOrganizations() {
  try {
    ...
    const orgs = ['microsoft', 'google', 'facebook', 'netflix', 'uber', 'airbnb'];
    for (const org of orgs) {
      try {
        const response = await axios.get(`https://api.github.com/orgs/${org}/repos?sort=updated&per_page=10`);
  ...
  try {
    ...
    for (const file of learningFiles) {
      if (file.endsWith('.json')) {
        const data = JSON.parse(fs.readFileSync(path.join(LEARNING_DATA_PATH, file), 'utf8'));
        allData.push(data);
      }
    }
...
function analyzePatterns(data) {
  ...
  data.forEach(item => {
    if (item.language) {
      patterns.popularTechnologies[item.language] = (patterns.popularTechnologies[item.language] || 0) + 1;
    }
    if (item.topics) {
...
function generateInsights(data) {
  ...
  return {
    ...
    averageStars: data.filter(d => d.stars).reduce((acc, d) => acc + d.stars, 0) / data.filter(d => d.stars).length,
    mostPopularLanguage: Object.entries(analyzePatterns(data).popularTechnologies)
      .sort(([,a], [,b]) => b - a)[0]?.[0],
    topTrendingTopics: Object.entries(analyzePatterns(data).trendingTopics)
      .sort(([,a], [,b]) => b - a).slice(0, 5).map(([topic]) => topic)
  };
}
