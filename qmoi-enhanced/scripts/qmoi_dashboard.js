...
function requireAuth(req, res, next) {
  ...
  res.send(
    `<form method="POST" action="/login"><h2>QMOI Dashboard Login</h2><input name="user" placeholder="Username"/><br/><input name="pass" type="password" placeholder="Password"/><br/><button type="submit">Login</button></form>`,
  );
}
...
app.get("/", async (req, res) => {
  ...
  if (log.length > 1) {
    const points = log
      .map((s, i) => `${i * 30},${100 - (s.percentFixed || 0)}`)
      .join(" ");
  ...
  if (log.length > 0) {
    table =
      `<h3>Recent Error/Fix Runs</h3><table border="1" cellpadding="4" style="border-collapse:collapse;"><tr><th>Time</th><th>Found</th><th>Fixed</th><th>Manual</th><th>Remaining</th><th>% Auto Fixed</th><th>% Total Fixed</th><th>Types</th></tr>` +
      log
        .slice(-10)
        .reverse()
        .map(
          (s) =>
            `<tr><td>${s.timestamp}</td><td>${s.errorsFound}</td><td>${s.errorsFixed}</td><td>${s.manualCount}</td><td>${s.remaining}</td><td>${s.percentAutoFixed}%</td><td>${s.percentFixed}%</td><td>${Object.entries(
              s.errorTypeCounts,
            )
              .map(([t, c]) => `${t}:${c}`)
              .join(", ")}</td></tr>`,
  ...
  if (stats.manualErrors && stats.manualErrors.length > 0) {
    manualList =
      `<h3>Manual Errors Requiring Attention</h3><ul>` +
      stats.manualErrors
        .map(
          (e) =>
            `<li><b>${e.type}</b>: ${e.description}<br/><i>${e.manualInstructions}</i></li>`,
        )
        .join("") +
      "</ul>";
  }
      ...
      ${
        notificationHistory.length === 0
          ...
          : notificationHistory
              ...
              .map(
                (n) =>
                  `<li>[${n.type}] <b>${n.title}</b> - ${n.status} (${n.timestamp})</li>`,
              )
              .join("")
      }
    </ul>
      ...
      ${
        stats.errorTypeCounts
          ? Object.entries(stats.errorTypeCounts)
              .map(([type, count]) => `<li>${type}: <b>${count}</b></li>`)
              .join("")
          : "N/A"
      }
    </ul>
