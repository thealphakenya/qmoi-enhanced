...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/update_model_card.log"),
...
def update_model_card(
    repo_id, version, health, status, dashboard_url, status_url, token=None
):
    api = HfApi()
    if not token:
    ...
    if not token:
        logger.warning(
            "No Hugging Face token provided or found in env. Skipping model card update."
        )
        return False
    try:
        updated = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        version_badge = make_badge("version", version, "blue")
        health_badge = make_badge(
            "health",
            f"{health}%25",
            (
                "brightgreen"
                if status == "healthy"
                else "yellow" if status == "warning" else "red"
            ),
        )
        updated_badge = make_badge(
            "updated", updated.replace(":", "%3A"), "informational"
        )
        ...
        try:
            old_card = hf_hub_download(
                repo_id=repo_id, filename="README.md", repo_type="model", token=token
            )
...
def main():
    ...
    parser.add_argument(
        "--repo",
        required=True,
        help="Hugging Face repo id (e.g., alphaqmoi/qmoi-ai-system)",
    )
    parser.add_argument("--version", required=True, help="Latest model version")
    parser.add_argument(
        "--health", required=True, help="Health percentage (e.g., 99.8)"
    )
    parser.add_argument(
        "--status", required=True, help="Health status (e.g., healthy, warning, error)"
    )
