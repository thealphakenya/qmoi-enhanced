...
class FastGitCommit:
    ...
    def prepare_commit(self) -> bool:
        ...
        # Stage changes
        self.log("Staging all changes (git add -A)")
    ...
    def commit(self) -> Optional[str]:
        ...
        # Use a temporary file for commit message to avoid shell quoting issues
        ...
        try:
            # Use --no-verify to skip hooks; will fall back if hooks disabled
    ...
    def push_changes(self) -> bool:
        if not self.push:
            self.log("Push disabled by CLI option; skipping push")
            return True
