...
const handler = requireRole(['admin', 'master'])(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (method === 'POST') {
    ...
    if (!name || !host || !username) return res.status(400).json({ error: 'Missing fields' });
    const device = { id: `dev_${Date.now()}`, name, host, port: port || 22, username, password, privateKey, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    devices.push(device);
    saveDevices(devices);
  ...
  if (method === 'PUT') {
    ...
    devices[idx] = { ...devices[idx], ...update, updatedAt: new Date().toISOString() };
    saveDevices(devices);
