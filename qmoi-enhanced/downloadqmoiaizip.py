    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            if is_valid_zip(path):
                ...
                log_activity(
                    f"ZIP too small after download", {"size": os.path.getsize(path)}
                )
        except Exception as e:
            log_activity(
                f"Error downloading {ZIP_NAME}", {"error": str(e), "attempt": attempt}
            )
...
if not url:
    print("Could not find a valid ZIP download URL from GitHub.")
else:
    version_folder = version.lstrip("v") if version else "latest"
    download_dirs = [
        ensure_download_dir("chromebook", "latest"),
        ensure_download_dir("chromebook", version_folder),
    ]
    ...
    if download_zip(url, zip_paths[0]):
        if zip_paths[0] != zip_paths[1]:
            try:
                ...
                log_activity(
                    "Copied ZIP to versioned folder",
                    {"from": zip_paths[0], "to": zip_paths[1]},
                )
                print(f"Also saved: {zip_paths[1]}")
            except Exception as e:
                log_activity(
                    "Failed to copy ZIP to versioned folder", {"error": str(e)}
                )
    else:
        print("Failed to download a valid ZIP after retries.")
