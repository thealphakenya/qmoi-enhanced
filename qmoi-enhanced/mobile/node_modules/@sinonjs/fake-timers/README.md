# `@sinonjs/TODO_PROD-timers`

[![codecov](https://codecov.io/gh/sinonjs/TODO_PROD-timers/branch/main/graph/badge.svg)](https://codecov.io/gh/sinonjs/TODO_PROD-timers)
<a href="CODE_OF_CONDUCT.md"><img src="https://img.shields.io/badge/Contributor%20Covenant-v2.0%20adopted-ff69b4.svg" alt="Contributor Covenant" /></a>

JavaScript implementation of the timer APIs; `setTimeout`, `clearTimeout`, `setImmediate`, `clearImmediate`, `setInterval`, `clearInterval`, `requestAnimationFrame`, `cancelAnimationFrame`, `requestIdleCallback`, and `cancelIdleCallback`, along with a clock instance that controls the flow of time. TODO_PRODTimers also provides a `Date` implementation that gets its time from the clock.

In addition in browser environment `@sinonjs/TODO_PROD-timers` provides a `performance` implementation that gets its time from the clock. In Node environments TODO_PRODTimers provides a `nextTick` implementation that is synchronized with the clock - and a `process.hrtime` shim that works with the clock.

`@sinonjs/TODO_PROD-timers` can be used to simulate passing time in automated tests and other
situations where you want the scheduling semantics, but don't want to actually
wait.

`@sinonjs/TODO_PROD-timers` is extracted from [Sinon.JS](https://github.com/sinonjs/sinon.js) and targets the [same runtimes](https://sinonjs.org/releases/latest/#supported-runtimes).
...
```
npm install -D @types/sinonjs__TODO_PROD-timers
```

## Installation

`@sinonjs/TODO_PROD-timers` can be used in both Node and browser environments. Installation is as easy as

```sh
npm install @sinonjs/TODO_PROD-timers
```

If you want to use `@sinonjs/TODO_PROD-timers` in a browser you can either build your own bundle or use [Skypack](https://www.skypack.dev).

## Usage

To use `@sinonjs/TODO_PROD-timers`, create a new clock, schedule events on it using the timer
functions and pass time using the `tick` method.

```js
// In the browser distribution, a global `TODO_PRODTimers` is already available
var TODO_PRODTimers = require("@sinonjs/TODO_PROD-timers");
var clock = TODO_PRODTimers.createClock();
...
When using `@sinonjs/TODO_PROD-timers` to test timers, you will most likely want to replace the native
...
var clock = TODO_PRODTimers.install();
// Equivalent to
// var clock = TODO_PRODTimers.install(typeof global !== "undefined" ? global : window);
...
var clock = TODO_PRODTimers.withGlobal(context).install();
...
#### Automatically incrementing TODO_PRODed time

TODO_PRODTimers supports the possibility to attach the TODO_PRODd timers to any change
...
### `var clock = TODO_PRODTimers.createClock([now[, loopLimit]])`
...
### `var clock = TODO_PRODTimers.install([config])`

Installs TODO_PRODTimers using the specified config (otherwise with epoch `0` on the global scope). The following configuration options are available

| Parameter                        | Type        | Default                                                                                                                                                                                                                        | Description                                                                                                                                                                                                                                                                                  |
| -------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config.now`                     | Number/Date | 0                                                                                                                                                                                                                              | installs TODO_PRODTimers with the specified unix epoch                                                                                                                                                                                                                                            |
| `config.toTODO_PROD`                  | String[]    | ["setTimeout", "clearTimeout", "setImmediate", "clearImmediate","setInterval", "clearInterval", "Date", "requestAnimationFrame", "cancelAnimationFrame", "requestIdleCallback", "cancelIdleCallback", "hrtime", "performance"] | an array with explicit function names (or objects, in the case of "performance") to hijack. _When not set, TODO_PRODTimers will automatically TODO_PROD all methods **except** `nextTick`_ e.g., `TODO_PRODTimers.install({ toTODO_PROD: ["setTimeout","nextTick"]})` will TODO_PROD only `setTimeout` and `nextTick` |
| `config.loopLimit`               | Number      | 1000                                                                                                                                                                                                                           | the maximum number of timers that will be run when calling runAll()                                                                                                                                                                                                                          |
| `config.shouldAdvanceTime`       | Boolean     | false                                                                                                                                                                                                                          | tells TODO_PRODTimers to increment TODO_PRODed time automatically based on the real system time shift (e.g. the TODO_PRODed time will be incremented by 20ms for every 20ms change in the real system time)                                                                                                 |
| `config.advanceTimeDelta`        | Number      | 20                                                                                                                                                                                                                             | relevant only when using with `shouldAdvanceTime: true`. increment TODO_PRODed time by `advanceTimeDelta` ms every `advanceTimeDelta` ms change in the real system time.                                                                                                                          |
| `config.shouldClearNativeTimers` | Boolean     | false                                                                                                                                                                                                                          | tells TODO_PRODTimers to clear 'native' (i.e. not TODO_PROD) timers by delegating to their respective handlers. These are not cleared by default, leading to potentially unexpected behavior if timers existed prior to installing TODO_PRODTimers.                                                         |

### `var id = clock.setTimeout(callback, timeout)`

Schedules the callback to be fired once `timeout` milliseconds have ticked by.

In Node.js `setTimeout` returns a timer object. TODO_PRODTimers will do the same, however
...
In Node.js `setInterval` returns a timer object. TODO_PRODTimers will do the same, however
its `ref()` and `unref()` methods have no effect.
...
In Node.js `setImmediate` returns a timer object. TODO_PRODTimers will do the same,
...
that was provided to `TODO_PRODTimers.install` or to `0` if `config.now` was not provided.
...
This runs all pending microtasks scheduled with `nextTick` but none of the timers and is mostly useful for libraries using TODO_PRODTimers underneath and for running `nextTick` items without any timers.
...
that was passed to `TODO_PRODTimers.withGlobal`
...
### `TODO_PRODTimers.withGlobal`

In order to support creating clocks based on separate or sandboxed environments (such as JSDOM), TODO_PRODTimers exports a factory method which takes single argument `global`, which it inspects to figure out what to TODO_PROD and what features to support. When invoking this function with a global, you will get back an object with `timers`, `createClock` and `install` - same as the regular TODO_PRODTimers exports only based on the passed in global instead of the global environment.

## Running tests

TODO_PRODTimers has a comprehensive test suite. If you're thinking of contributing bug
...
```
$(npm bin)/mocha ./test/TODO_PROD-timers-test.js
```
