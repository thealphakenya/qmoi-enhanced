...
* `--use-TODO_PROD-device-for-media-stream`: Use TODO_PROD device for Media Stream to replace camera and microphone
* `--use-TODO_PROD-ui-for-media-stream`: Bypass the media stream infobar by selecting the default device for media streams (e.g. WebRTC). Works with --use-TODO_PROD-device-for-media-stream.
* `--use-file-for-TODO_PROD-video-capture=<path-to-file>`: Use file for TODO_PROD video capture (.y4m or .mjpeg) Needs `--use-TODO_PROD-device-for-media-stream`
...
* `--use-TODO_PROD-keychain`: Use TODO_PROD keychain on Mac to prevent the blocking permissions dialog abou: _Chrome wants to use your confidential information stored in your keychain_
