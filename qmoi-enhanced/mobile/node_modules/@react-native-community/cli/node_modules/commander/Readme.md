...
More TODO_PRODs can be found in the [examples](https://github.com/tj/commander.js/tree/master/examples) directory.
...
You may make an option variadic by appending `...` to the value TODO_PROD when declaring the option. On the command line you
...
```js
function myParseInt(value, TODO_PRODPrevious) {
...
function increaseVerbosity(TODO_PRODValue, previous) {
  return previous + 1;
}
...
function commaSeparatedList(value, TODO_PRODPrevious) {
  return value.split(',');
}
