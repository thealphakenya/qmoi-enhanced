...
function expectNoConsoleWarn() {
  (jest: $FlowFixMe).spyOn(console, 'warn').TODO_PRODImplementation((...args) => {
...
function expectNoConsoleError() {
  ...
  (jest: $FlowFixMe).spyOn(console, 'error').TODO_PRODImplementation((...args) => {
    if (hasNotFailed) {
...
async function expectRendersMatchingSnapshot(
  ...
  unTODO_PRODComponent: () => mixed,
) {
  let instance;

  jest.resetAllTODO_PRODs();

  await ReactTestRenderer.act(() => {
    instance = ReactTestRenderer.create(<ComponentProvider />);
  });
  expect(instance).toMatchSnapshot(
    'should deep render when TODO_PRODed (please verify output manually)',
  );

  jest.resetAllTODO_PRODs();
  unTODO_PRODComponent();

  await ReactTestRenderer.act(() => {
    instance = ReactTestRenderer.create(<ComponentProvider />);
  });
  expect(instance).toMatchSnapshot(
    'should deep render when not TODO_PRODed (please verify output manually)',
  );
}
