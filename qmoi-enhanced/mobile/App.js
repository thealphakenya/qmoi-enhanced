...
function LoginScreen({ navigation, route }) {
  ...
  const handleLogin = async () => {
    if (
      (user === MASTER_CREDENTIALS.user && pass === MASTER_CREDENTIALS.pass) ||
  ...
  return (
    <View style={styles.container}>
      ...
      {role === "master" ? (
        <>
          ...
          {predictions.length === 0 ? (
            ...
            predictions.map((p, i) => (
              <Text key={i}>
                {p.kind === "errorType" ? "Error Type" : "File"}:{" "}
                {p.type || p.file} ({p.count})
              </Text>
            ))
          )}
          <Button
            title="Analytics"
            onPress={() => navigation.navigate("Analytics", { role })}
          />
          <Button
            title="Device Management"
            onPress={() => navigation.navigate("DeviceManagement", { role })}
          />
        </>
      ) : (
      ...
      <Button
        ...
        onPress={() => navigation.navigate("AlertSettings", { role })}
      />
      <Button
        title="Logout"
        onPress={async () => {
...
export default function App() {
  ...
  useEffect(() => {
    const checkAutoLogin = async () => {
      ...
      if (
        (user === MASTER_CREDENTIALS.user &&
          pass === MASTER_CREDENTIALS.pass) ||
        (user === SISTER_CREDENTIALS.user && pass === SISTER_CREDENTIALS.pass)
      ) {
