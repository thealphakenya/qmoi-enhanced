...
export default function AlertSettingsScreen() {
  ...
  const savePrefs = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Offline",
        "Preferences saved locally and will sync when online.",
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        Custom Alert Rules {offline ? "(Offline)" : ""}
      </Text>
      <View style={styles.row}>
        <Text>Critical Errors Only</Text>
        <Switch value={criticalOnly} onValueChange={setCriticalOnly} />
      </View>
      <View style={styles.row}>
        <Text>Alert for Error Types (comma separated)</Text>
      </View>
      <TextInput
        value={errorTypes}
        onChangeText={setErrorTypes}
        placeholder="TypeError,ReferenceError"
        style={styles.input}
      />
      <View style={styles.row}>
        <Text>Quiet Hours (e.g. 22:00-07:00)</Text>
      </View>
      <TextInput
        value={quietHours}
