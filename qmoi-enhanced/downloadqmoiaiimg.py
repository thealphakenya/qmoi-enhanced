    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            if is_valid_img(path):
                ...
                log_activity(
                    f"IMG too small after download", {"size": os.path.getsize(path)}
                )
        except Exception as e:
            log_activity(
                f"Error downloading {IMG_NAME}", {"error": str(e), "attempt": attempt}
            )
...
if not url:
    ...
    download_dirs = [
        ensure_download_dir("raspberrypi", "latest"),
        ensure_download_dir("raspberrypi", version_folder),
    ]
    ...
    if download_img(url, img_paths[0]):
        if img_paths[0] != img_paths[1]:
            try:
                ...
                log_activity(
                    "Copied IMG to versioned folder",
                    {"from": img_paths[0], "to": img_paths[1]},
                )
                print(f"Also saved: {img_paths[1]}")
            except Exception as e:
                log_activity(
                    "Failed to copy IMG to versioned folder", {"error": str(e)}
                )
    else:
