    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            if is_valid_deb(path):
                ...
                log_activity(
                    f"DEB too small after download", {"size": os.path.getsize(path)}
                )
        except Exception as e:
            log_activity(
                f"Error downloading {DEB_NAME}", {"error": str(e), "attempt": attempt}
            )
    ...
    if download_deb(url, deb_paths[0]):
        if deb_paths[0] != deb_paths[1]:
            try:
                ...
                log_activity(
                    "Copied DEB to versioned folder",
                    {"from": deb_paths[0], "to": deb_paths[1]},
                )
                print(f"Also saved: {deb_paths[1]}")
            except Exception as e:
                log_activity(
                    "Failed to copy DEB to versioned folder", {"error": str(e)}
                )
    else:
