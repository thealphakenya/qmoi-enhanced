...
echo "Removing Ollama data at $OLLAMA_HOME (if present)..." | tee -a "$LOG"
rm -rf "$OLLAMA_HOME" || true

echo "Re-running Ollama installer..." | tee -a "$LOG"
if command -v curl >/dev/null 2>&1; then
  curl -fsSL https://ollama.com/install.sh | sh >> "$LOG" 2>&1 || true
else
  echo "Error: curl not available for Ollama install" | tee -a "$LOG"
  exit 1
fi
