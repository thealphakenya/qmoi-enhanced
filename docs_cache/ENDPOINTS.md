...
NOTE: endpoint inventory was created from a repository scan. For a machine-readable report of where TBD (auto-filled) tokens or missing docs were found, see `docs/placeholders_report.json`. For the canonical test matrix that exercises these endpoints see `docs/ALLTESTSAUTOTESTS.md`.
