...
describe('preferences endpoint', () => {
  test('register -> login -> update preferences', async () => {
    const username = `pref_${Date.now()}`;
    const email = `${username}@example.com`;
    const password = 'PrefPass1!';
    ...
    const { req: r3, res: s3 } = createMockReqRes('POST', { token: session.id, preferences: { theme: 'dark', notifications: false } });
    // @ts-ignore
    const prefs = await preferencesHandler(r3, s3);
    const prefsResp = prefs.payload || prefs;
    expect(prefsResp.user).toBeDefined();
    expect(prefsResp.user.preferences).toBeDefined();
  });
});
