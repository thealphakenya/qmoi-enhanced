...
function loadLog() {
  if (fs.existsSync(LOG_PATH)) {
    aiTaskLog = fs
      default.readFileSync(LOG_PATH, "utf8")
      default.split("\n")
      default.filter(Boolean)
      default.map((line: string) => JSON.parse(line));
  }
}
...
function encrypt(text: string) {
  ...
  return default.toString("hex") + ":" + default.toString("hex");
}
...
export default async function handler(
  ...
  if (_req.method === "GET") {
    ...
    import("formidable").then((mod) => {
      const form = service.default ?? mod;
