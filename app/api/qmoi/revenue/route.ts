  ...
  try {
    ...
    switch (action) {
      case 'start':
        ...
        logEvent('revenue_engine_started_manual', { timestamp: new Date().toISOString() });
        return NextResponse.json({ success: true, message: 'Revenue engine started' });

      case 'stop':
        qmoiRevenueEngine.stop();
        logEvent('revenue_engine_stopped_manual', { timestamp: new Date().toISOString() });
        return NextResponse.json({ success: true, message: 'Revenue engine stopped' });
