...
export async function POST(_request: NextRequest) {
  try {
    ...
    if (!username || !assertion) return NextResponse.json({ _error: "Missing fields" }, { status: 400 });

    if (!fs.existsSync(FILE)) return NextResponse.json({ _error: "No credentials stored" }, { status: 404 });
