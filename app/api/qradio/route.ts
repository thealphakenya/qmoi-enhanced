...
const channels = [
  { id: 1, name: 'Global Radio', type: 'QChannel', description: 'QMOI Global Channel', dj: 'QMOI', programs: [
    ...
    { time: '18:00', title: 'Evening Urban', presenter: 'QMOI', type: 'music' },
  ] },
  { id: 2, name: 'Urban Radio', type: 'QChannel', description: 'QMOI Urban Channel', dj: 'QMOI', programs: [
...
export async function GET_PROGRAMS(req: NextRequest) {
  return NextResponse.json({ programs: channels.map(c => ({ channel: c.name, programs: c.programs })) });
}
...
export async function POST_PLAY(req: NextRequest) {
  ...
  nowPlaying = { channel: channel.name, program: channel.programs[0], startedAt: new Date().toISOString() };
