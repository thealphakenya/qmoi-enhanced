...
export class TwitterAdapter implements SocialPlatformAdapter {
  ...
  async getAnalytics() { return { impressions: 0 }; }
  async createPost(content: unknown, approval: boolean = true) { const r = await this.requestApproval('createTweet', content); return `approval:${r.id}`; }
  async deletePost(postId: string) { return true; }
  async getEngagementMetrics(postId: string) { return { retweets: 0, likes: 0 }; }
}
