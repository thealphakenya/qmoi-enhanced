...
export class WhatsAppAdapter implements SocialPlatformAdapter {
  ...
  async initialize(config: PlatformConfig) { this.config = config; console.log('[WhatsAppAdapter] initialized'); }
  ...
  async getAnalytics() { return { messagesSent: 0 }; }
  ...
  async deletePost(postId: string) { return true; }
  async getEngagementMetrics() { return {}; }
}
...
export class WhatsAppAdapter implements SocialPlatformAdapter {
  ...
  async initialize(config: PlatformConfig): Promise<void> {
    ...
    if (this.config.sandboxMode) {
      console.log('[WhatsApp] Running in sandbox mode');
      return;
    }
    ...
    if (!this.config.phoneNumberId) {
      throw new Error('WhatsApp phone number ID is required in production mode');
    }
  ...
  async validateCredentials(): Promise<boolean> {
    ...
    return !!(this.config.credentials?.accessToken && this.config.phoneNumberId);
  }
  ...
  async createPost(content: unknown, requireApproval: boolean = true): Promise<string> {
    ...
    // Validate message content and template usage
    if (typeof content !== 'object' || !content || 
        !('recipient' in content) || !('templateName' in content)) {
      throw new Error('Invalid WhatsApp message content - must include recipient and templateName');
    }
    ...
    if (this.config.sandboxMode) {
      console.log('[WhatsApp Sandbox] Would send message:', content);
      return `mock-wa-msg-${Date.now()}`;
    }

    // In production mode, would send via WhatsApp Business API
    console.log('[WhatsApp] Sending message via Business API');
  ...
  async getEngagementMetrics(messageId: string): Promise<WhatsAppMessageMetrics> {
    if (!this.config) {
      throw new Error('WhatsApp adapter not initialized');
    }
  ...
  async getAnalytics(): Promise<unknown> {
    ...
    if (this.config.sandboxMode) {
      ...
      return {
        ...
        templatePerformance: this.config.messageTemplates.map(template => ({
          name: template.name,
