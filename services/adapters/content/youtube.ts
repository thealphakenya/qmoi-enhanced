...
export class YouTubeAdapter implements ContentPlatformAdapter {
  ...
  async getAnalytics() { return { views: 0 }; }
  async uploadContent(content: unknown, metadata: unknown) { const r = await this.requestApproval('uploadContent', { content, metadata }); return `approval:${r.id}`; }
  async updateContent(contentId: string, updates: unknown) { return true; }
  async getContentMetrics(contentId: string) { return { views: 0 }; }
}
