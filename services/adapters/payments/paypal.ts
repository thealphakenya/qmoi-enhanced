...
export class PayPalAdapter implements PaymentGatewayAdapter {
  ...
  async initialize(config: PlatformConfig) { this.config = config; console.log('[PayPalAdapter] sandbox=%s', !!config.sandboxMode); }
  ...
  async createPaymentIntent(amount: number, currency: string) {
    ...
    WalletManager.appendAudit({ event: 'payment_intent_created', gateway: 'paypal', id, amount, currency });
    return id;
  }
  async capturePayment(paymentId: string) { console.log('[PayPalAdapter] dry-capture', paymentId); return true; }
  async refundPayment(paymentId: string, amount?: number) { console.log('[PayPalAdapter] dry-refund', paymentId, amount); return true; }
