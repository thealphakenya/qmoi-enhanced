...
export class StripeAdapter implements PaymentGatewayAdapter {
  ...
  async initialize(config: PlatformConfig) { this.config = config; console.log('[StripeAdapter] sandbox=%s', !!config.sandboxMode); }
  ...
  async getAnalytics() { return { processed: 0 }; }
  ...
  async createPaymentIntent(amount: number, currency: string) {
    ...
    if (existing) {
      console.log('[StripeAdapter] returning existing idempotent payment intent', existing.record.id);
      return existing.record.id;
    }
    const pid = `pi_${Math.random().toString(36).slice(2)}`;
    console.log('[StripeAdapter] dry-createPaymentIntent', amount, currency, pid);
    markIdempotent(key, { id: pid, amount, currency });
    WalletManager.appendAudit({ event: 'payment_intent_created', gateway: 'stripe', id: pid, amount, currency });
    return pid;
  }

  async capturePayment(paymentId: string) { console.log('[StripeAdapter] dry-capture', paymentId); return true; }
  async refundPayment(paymentId: string, amount?: number) { console.log('[StripeAdapter] dry-refund', paymentId, amount); return true; }
  async getTransactionHistory(startDate: Date, endDate: Date) { return []; }
}
