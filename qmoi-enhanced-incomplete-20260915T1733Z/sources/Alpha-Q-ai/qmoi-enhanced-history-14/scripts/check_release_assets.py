...
if not releases_file.exists():
    print('Missing', releases_file, '— run scripts/audit_releases.py first')
    raise SystemExit(1)
...
for r in data:
    ...
    release_report = {'tag': tag, 'assets': assets, 'missing': {}}
    for app in APPS:
        ...
        # mark missing if any platform False
        missing = [p for p,v in per_platform.items() if not v]
        release_report['missing'][app] = missing
...
for r in report['releases']:
    ...
    for a in r['assets']:
        md_lines.append(f"  - {a}")
    md_lines.append('- Missing per app:')
    for app, miss in r['missing'].items():
        if len(miss) == 0:
            md_lines.append(f"  - **{app}**: all platforms present")
        else:
            md_lines.append(f"  - **{app}**: missing {', '.join(miss)}")
    md_lines.append('')
