...
for title, lines in sections:
    ...
    # Instead write a tiny draft marker so the operator knows generation was
    # attempted but blocked by billing settings.
    if not billing_allowed():
        stub = OUT_DIR / f"{fname}.payed-draft.md"
        stub.write_text('\n'.join([f'# {title} PAYED (draft)', '', 'Generation blocked: billing disabled.']), encoding='utf-8')
        print('Billing disabled; wrote draft', stub.name)
        continue
    # prepare body
