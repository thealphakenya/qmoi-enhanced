...
This file is a lightweight runner intended to be started as a long-running
...
def main(concurrency: int = 2, lease: int = 120, poll_interval: int = 3):
    ...
    if env_manager:
        try:
            ...
            if rc != 0:
                logger.error('Missing required secrets according to %s; aborting queue worker startup', env_manager.MANIFEST_DEFAULT)
                return
        except Exception:
