...
*Generated by QMOI GitHub Fallback System*`;
  ...
  async runFallbackPipeline() {
    try {
      ...
      if (!gitlabAvailable && githubAvailable) {
        ...
        await this.sendGitHubNotification("sync_completed", {
          direction: "to-github",
        });
  ...
  async monitorPlatforms() {
    try {
      ...
      this.log(
        `Platform status: GitLab=${status.gitlab}, GitHub=${status.github}`,
      );
  ...
  try {
    ...
    switch (command) {
      ...
      case "--notify":
        ...
        await fallback.sendGitHubNotification(
          args[2] || "fallback_activated",
          data,
        );
        break;
      default:
