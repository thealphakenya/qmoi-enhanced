...
- **Repository coverage**: the same owner-scoped credential is used for both `thealphakenya/qmoi-enhanced` and `thealphakenya/Alpha-Q-ai`; authorization must be verified independently for each repository
- **Scope**: least privilege covering the required contents, pull requests, actions, and metadata operations for both repositories
- **Security**: Token never logged or exposed in output

### Safe Terminal Synchronization Commands

The operator or hosted runner supplies the credential through the environment;
the value is never written into a file, command log, commit, or tracker:

```bash
export GH_TOKEN="$GITHUB_TOKEN"
gh auth status --hostname github.com
git fetch --all --prune
git ls-remote --heads https://github.com/thealphakenya/qmoi-enhanced.git
git ls-remote --heads https://github.com/thealphakenya/Alpha-Q-ai.git
python3 scripts/ollama_autonomous_agent.py autonomous
python3 scripts/ollama_autonomous_agent.py proof
python3 scripts/ollama_autonomous_agent.py checkpoint
python3 scripts/materialize_merged_repo.py \
	--workspace /workspaces/qmoi-enhanced \
	--alpha-repo /path/to/Alpha-Q-ai
```

The agent records command names, repository refs, checksums, branch results,
and validation outcomes, but redacts `GH_TOKEN`, `GITHUB_TOKEN`,
`MY_CUSTOM_TOKEN`, and all credential values. Sync publishes to
`autosync-backup` first, validates there, and only then proposes or applies a
reviewable `main` update.
