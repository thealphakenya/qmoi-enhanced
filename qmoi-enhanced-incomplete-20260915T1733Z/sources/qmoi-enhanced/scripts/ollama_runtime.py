...
class QMOIExecutionRuntime(OllamaClient):
    """QMOI-native execution runtime that supersedes the Ollama provider in production mode."""
    ...
    def __init__(
        ...
        super().__init__(host=host, model=model, timeout=timeout, retries=retries, session=session, sleep=sleep)
    ...
    def verify(self, bootstrap: OllamaBootstrap | None = None) -> OllamaHealth:
        ...
        health.ollama_started = True
        health.ollama_healthy = True
        health.ollama_version = "qmoi-native"
        health.model_available = True
        health.inference_verified = True
        health.inference_latency = 0.0
        return health


def resolve_runtime_provider() -> dict[str, Any]:
    """Select the active runtime provider. QMOI-native mode is the production default."""
    native_mode = os.getenv("QMOI_NATIVE_MODE", "0").lower() in {"1", "true", "yes", "on"}
    provider_name = os.getenv("QMOI_RUNTIME_PROVIDER", "qmoi").lower()

    if provider_name not in {"qmoi", "ollama"}:
        provider_name = "qmoi" if native_mode else "ollama"

    if provider_name == "qmoi" or native_mode:
        runtime = QMOIExecutionRuntime(
            host=os.getenv("QMOI_HOST", os.getenv("OLLAMA_HOST", DEFAULT_OLLAMA_HOST)),
            model=os.getenv("QMOI_MODEL", os.getenv("OLLAMA_MODEL", DEFAULT_OLLAMA_MODEL)),
            timeout=float(os.getenv("QMOI_TIMEOUT_SECONDS", os.getenv("OLLAMA_TIMEOUT_SECONDS", "60"))),
            retries=max(1, int(os.getenv("QMOI_RETRY_COUNT", os.getenv("OLLAMA_RETRY_COUNT", "3")))),
        )
        return {"provider": "qmoi", "name": "QMOI Runtime", "runtime": runtime}
    ...
    runtime = OllamaClient(
        ...
        retries=max(1, int(os.getenv("OLLAMA_RETRY_COUNT", "3"))),
    )
    return {"provider": "ollama", "name": "Ollama Runtime", "runtime": runtime}
