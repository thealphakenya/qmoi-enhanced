...
from ollama_runtime import resolve_runtime_provider
...
class TestQMOIRuntimeProvider:
    """Tests for the QMOI-native runtime replacement path."""

    def test_qmoi_native_runtime_provider_is_selected(self, monkeypatch):
        """The runtime layer must prefer the QMOI provider when configured as production mode."""
        monkeypatch.setenv("QMOI_RUNTIME_PROVIDER", "qmoi")
        monkeypatch.setenv("QMOI_NATIVE_MODE", "1")

        provider = resolve_runtime_provider()

        assert provider["provider"] == "qmoi"
        assert provider["name"] == "QMOI Runtime"
        assert provider["runtime"].__class__.__name__ == "QMOIExecutionRuntime"
...
class TestResumeCheckpoint:
    ...
    def test_autonomous_agent_trigger_workflow_exists(self):
        ...
        assert 'AUTO_CONTINUE: "1"' in content
        assert 'AUTO_PUSH: "1"' in content
        assert 'TARGET_BRANCH:' in content

    def test_autonomous_runner_defaults_to_continuous_loop(self):
        """The local terminal runner should default to a keepalive loop, not a one-shot task."""
        import autonomous_runner

        config_yaml = autonomous_runner.ESSENTIAL_FILES["config.yml"]
        assert "continuous_loop: true" in config_yaml
        assert "interval_seconds: 30" in config_yaml

    def test_autonomous_runner_refreshes_runtime_docs_and_status(self, tmp_path):
        """The production runner should refresh memory/model docs and write a health snapshot every cycle."""
        import autonomous_runner

        result = autonomous_runner.refresh_repository_docs(tmp_path)

        assert (tmp_path / "MEMORY_INDEX.md").exists()
        assert (tmp_path / "MODEL_CARD.md").exists()
        assert (tmp_path / "QMOI_MODEL_CARD.md").exists()
        assert (tmp_path / "oe2.txt").exists()
        assert "Tracking/validation markdown files: 0" in (tmp_path / "oe2.txt").read_text()
        assert "Required merge source: MERGE.md" in (tmp_path / "oe2.txt").read_text()
        status_file = tmp_path / "ollamatracks" / "production_status.json"
        assert status_file.exists()
        data = json.loads(status_file.read_text())
        assert data["status"] in {"updated", "ready", "healthy"}
        assert result["docs_updated"] is True
        assert (tmp_path / "ollamatracks" / "automation_monitoring.json").exists()

    def test_autonomous_runner_monitors_automation_independence_and_evolution(self, tmp_path):
        """The refresh records automation, independent-runtime, and evolution evidence."""
        import autonomous_runner

        (tmp_path / "ALLAUTO.md").write_text("# automation\n")
        (tmp_path / "INDEPENDENT_RUNTIME.md").write_text("# independent runtime\n")
        (tmp_path / "MODELEVOLUTIONO.md").write_text("# evolution\n")

        result = autonomous_runner.refresh_repository_docs(tmp_path)
        monitoring = json.loads((tmp_path / "ollamatracks" / "automation_monitoring.json").read_text())
        evolution = (tmp_path / "MODELEVOLUTIONO.md").read_text()
        oe2 = (tmp_path / "oe2.txt").read_text()

        assert result["automation_docs"] >= 1
        assert result["independent_docs"] >= 1
        assert monitoring["github_hosted_required"] is True
        assert monitoring["qmoi_primary_runtime"] is True
        assert "QMOI Runtime Transition Status" in evolution
        assert "2026-12-31T23:59:59Z" in evolution
        assert "QMOI evolution target: 2026-12-31T23:59:59Z" in oe2

    def test_autonomous_runner_refreshes_qmoi_card_and_category(self, tmp_path):
        """The realtime refresh keeps QMOI model-card and filename category coverage complete."""
        import autonomous_runner

        (tmp_path / "src").mkdir(parents=True)
        (tmp_path / "src" / "QMOIWidget.tsx").write_text("export {};\n")
        (tmp_path / "QMOI_FEATURES.md").write_text("# QMOI features\n")
        (tmp_path / "ALLMDFILESREFS.md").write_text("# refs\n")

        autonomous_runner.refresh_repository_docs(tmp_path)

        card = (tmp_path / "QMOI_MODEL_CARD.md").read_text()
        refs = (tmp_path / "ALLMDFILESREFS.md").read_text()
        assert "QVillage" in card
        assert "linked documents" in card
        assert "QMOI-native runtime selection" in card
        assert "## Category: QMOI" in refs
        assert "src/qmoiwidget.tsx" in refs
        assert "q moi" not in refs.lower()

    def test_autonomous_runner_indexes_tracking_validation_docs(self, tmp_path):
        """Tracking and validation markdown inventory covers active and historical paths."""
        import autonomous_runner

        (tmp_path / "qmoi-enhanced-history-14" / "qmoi_tracks").mkdir(parents=True)
        (tmp_path / "qmoi-enhanced-history-14" / "qmoi_tracks" / "HISTORY_VALIDATION.md").write_text("# history")
        (tmp_path / "TRACKS.md").write_text("# tracks")

        autonomous_runner.refresh_repository_docs(tmp_path)

        oe2 = (tmp_path / "oe2.txt").read_text()
        assert "qmoi-enhanced-history-14/qmoi_tracks/history_validation.md" in oe2
        assert "tracks.md" in oe2
