...
- Validation orchestrator: Python scripts under `scripts/` (e.g., `run_validations.py`) that coordinate link checks, artifact verification, and TBD (auto-filled) scanning.
...
- Use `scripts/run_validations.py` to orchestrate:
  ...
  - TBD (auto-filled) scanning report
...
- Replace any TBD (auto-filled) links in docs with real download locations for installers (no placeholders in final docs).
