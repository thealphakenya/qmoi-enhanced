...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/config | jq '.enableErrorDetection'

# 3. Scan not running
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/autoscan | jq '.status'
...
curl -X POST -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/scan
```
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/config | jq '.autoFixOnErrors'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/config | jq '.healthMonitoringEnabled'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/healthmonitor | jq '.thresholds'
```
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/config | jq '.config'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/config | jq '.config.autoScanInterval'
```
...
# Health monitor logs
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/healthmonitor | jq '.alerts'

# Bootstrap logs
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/bootstrap | jq '.logs'
```
...
# Export via curl
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/autoscan > scan-logs.json
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/admin/autofix/background-automation | jq '.status.services'
