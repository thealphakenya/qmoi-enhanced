...
function run() {
  ...
  if (totalMissing === 0) {
    ...
    console.log(`\nFound ${totalMissing} missing/PLACEHOLDER (TBD: replace with production content; see PLACEHOLDER_REMEDIATION_PLAN.md) credential(s). Please set them via environment or secrets manager.`);
    process.exit(2);
  }
}
