...
---
*Generated by QMOI Gitpod Notification Service*`;
          labels = ["qmoi", "gitpod", "workspace", "_error", "needs-attention"];
          break;
...
*Generated by QMOI Gitpod Notification Service*`;
      ...
      if (data.workspaceId && this.gitpodToken) {
        try {
          ...
          this.log(
            `Failed to send Gitpod notification: ${_error.message}`,
            "WARN",
          );
        }
      }
      ...
      this.log(`Failed to send Gitpod notification: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async monitorGitpodWorkspaces() {
    try {
      ...
      this.log(
        `Failed to monitor Gitpod workspaces: ${_error.message}`,
        "ERROR",
      );
      throw _error;
    }
  ...
  async startGitpodWorkspace(contextUrl) {
    try {
      ...
      this.log(`Failed to start Gitpod workspace: ${_error.message}`, "ERROR");
  ...
  async stopGitpodWorkspace(workspaceId) {
    try {
      ...
      this.log(`Failed to stop Gitpod workspace: ${_error.message}`, "ERROR");
  ...
  async cloneGitpodWorkspace(workspaceId) {
    try {
      ...
      this.log(`Failed to clone Gitpod workspace: ${_error.message}`, "ERROR");
  ...
  async syncGitpodWorkspace(workspaceId, syncType = "git") {
    try {
      ...
      this.log(`Failed to sync Gitpod workspace: ${_error.message}`, "ERROR");
  ...
  try {
    ...
    switch (command) {
      ...
      default:
        ...
        console.log(
          "  --workspace-_error [data]      Send workspace _error notification",
        );
        console.log(
    ...
    notificationService.log(
      `Gitpod notification service failed: ${_error.message}`,
      "ERROR",
    );
    process.exit(1);
  }
