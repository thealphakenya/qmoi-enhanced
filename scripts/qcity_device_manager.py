...
class QCityDeviceManager:
    ...
    def check_device_compatibility(self) -> Dict:
        ...
        if self.device_info["cpu_count"] < min_requirements["cpu_count"]:
            compatibility["issues"].append(
                f"Insufficient CPU cores: {self.device_info['cpu_count']} < {min_requirements['cpu_count']}"
            )
        ...
        if memory_gb < min_requirements["memory_gb"]:
            compatibility["issues"].append(
                f"Insufficient memory: {memory_gb:.1f}GB < {min_requirements['memory_gb']}GB"
            )
        ...
        if disk_gb < min_requirements["disk_gb"]:
            compatibility["issues"].append(
                f"Insufficient disk space: {disk_gb:.1f}GB < {min_requirements['disk_gb']}GB"
            )
        ...
        if python_version < min_python:
            compatibility["issues"].append(
                f"Python version too old: {python_version} < {min_python}"
            )
            ...
            try:
                ...
                if result.returncode == 0:
                    self.success_log.append(
                        f"Successfully installed {package} via conda"
                    )
                    return True
            except Exception as e:
    ...
    def try_system_package_manager(self, package: str) -> bool:
        ...
        if shutil.which("apt"):
            try:
                result = subprocess.run(
                    ["sudo", "apt", "install", "-y", system_package],
                    capture_output=True,
                    text=True,
                    timeout=300,
                )
        ...
        if shutil.which("yum"):
            try:
                result = subprocess.run(
                    ["sudo", "yum", "install", "-y", system_package],
                    capture_output=True,
                    text=True,
                    timeout=300,
                )
    ...
    def run_security_check(self) -> Dict:
        ...
        important_files = ["qcity_configs/qcity_config.json", "qcity_logs/qcity.log"]
...
def main():
    ...
    print(
        f"Installations: {report['install_results']['successful_installs']}/{report['install_results']['total_dependencies']}"
    )
