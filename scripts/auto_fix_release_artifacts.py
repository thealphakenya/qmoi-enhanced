...
if not MANIFEST_PATH.exists() or not RELEASES_REPORT.exists():
    print('Manifest or Report missing. Ensure you have run the checks first.')
    sys.exit(1)
...
for asset in manifest.get('assets', []):
    ...
    need_restore = False
    if entry and entry.get('status') in ('missing', 'mismatched'):
        need_restore = True
    ...
    if not found:
        ...
        if platform == 'android':
            ...
            steps = ["Use Mac/Xcode CI: build and export an IPA signed for distribution",
                     "Upload to GitHub release or TestFlight for staging"]
        elif platform == 'windows':
