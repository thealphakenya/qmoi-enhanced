...
class GitLabPushAutomation {
  ...
  async configureGit() {
    try {
      ...
      if (this.gitlabToken) {
        await this.runCommand(
          `git config --global url."https://oauth2:${this.gitlabToken}@${new URL(this.gitlabUrl).hostname}/".insteadOf "https://${new URL(this.gitlabUrl).hostname}/"`,
        );
      }

      this.log("Git configuration completed successfully");
    } catch (_error) {
      this.log(`Git configuration failed: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async checkGitStatus() {
    try {
      ...
      this.log(`Git status check failed: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async stageAllChanges() {
    try {
      ...
      this.log(`Staging changes failed: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
...
- Improved _error handling and recovery
...
Generated by QMOI Automation System`;
      ...
      this.log(`Commit failed: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async pushToGitLab(branch = null) {
    try {
      ...
      this.log(`Push to GitLab failed: ${_error.message}`, "ERROR");

      // Try to recover from push errors
      await this.handlePushError(_error);
      throw _error;
    }
  }

  async handlePushError(_error) {
    try {
      this.log("Attempting to recover from push _error...");

      // Check if it's a force push situation
      if (_error.stdout && _error.stdout.includes("rejected")) {
        this.log("Detected rejected push - attempting force push...");
        await this.runCommand(
          `git push origin ${this.branch} --force-with-lease`,
        );
      ...
      if (_error.stdout && _error.stdout.includes("conflict")) {
      ...
      if (_error.stdout && _error.stdout.includes("upstream")) {
      ...
      this.log("Could not automatically recover from push _error");
      return false;
    } catch (recoveryError) {
      this.log(`Push _error recovery failed: ${recoveryError.message}`, "ERROR");
      return false;
    }
  }
  ...
  async createMergeRequest(title = null, description = null) {
    try {
      this.log("Creating merge _request...");

      const defaultTitle = `🤖 QMOI Automation Update - ${new Date().toISOString()}`;
      const defaultDescription = `## QMOI Automation Merge Request

### Overview
This merge _request contains automated updates from the QMOI automation system.

### Changes Included:
- Enhanced GitLab integration and automation
- Improved _error handling and recovery mechanisms
...
*Generated by QMOI Automation System*`;
      ...
      // Create merge _request using GitLab API
      const _response = await fetch(
        `${this.gitlabUrl}/api/v4/projects/${this.projectId}/merge_requests`,
        {
          method: "POST",
          headers: {
      ...
      if (!_response.ok) {
        throw new Error(
          `Failed to create merge _request: ${_response.statusText}`,
        );
      }

      const mr = await _response.json();
      this.log(`Merge _request created successfully: ${mr.web_url}`);
      return mr;
    } catch (_error) {
      this.log(`Failed to create merge _request: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async tagRelease(version = null) {
    try {
      ...
      await this.runCommand(
        `git tag -a ${tagName} -m "QMOI Release ${tagName} - ${new Date().toISOString()}"`,
      );
      ...
      this.log(`Failed to create release tag: ${_error.message}`, "ERROR");
      throw _error;
    }
  }
  ...
  async runFullPushPipeline() {
    try {
      ...
      // Step 6: Create merge _request (if not on main branch)
      ...
      this.log(`Push pipeline failed: ${_error.message}`, "ERROR");

      // Create _error notification
      await this.createErrorNotification(_error);

      throw _error;
    }
  }

  async createErrorNotification(_error) {
    try {
      this.log("Creating _error notification...");
...
*Generated by QMOI Automation System*`;
      ...
      // Create GitLab issue for the _error
      const _response = await fetch(
        `${this.gitlabUrl}/api/v4/projects/${this.projectId}/issues`,
        {
          ...
          body: JSON.stringify({
            ...
            labels: "qmoi,_error,push-failed,needs-attention",
          }),
        },
      );

      if (_response.ok) {
        const issue = await _response.json();
        this.log(`Error notification created: ${issue.web_url}`);
      } else {
        this.log("Failed to create _error notification", "ERROR");
      }
    } catch (notificationError) {
      this.log(
        `Failed to create _error notification: ${notificationError.message}`,
        "ERROR",
      );
    }
  }
  ...
  try {
    ...
    switch (command) {
      ...
      default:
        ...
        console.log("  --merge-_request    Create merge _request");
        console.log("  --tag-release      Create release tag");
        break;
    }
  } catch (_error) {
    automation.log(`Push automation failed: ${_error.message}`, "ERROR");
    process.exit(1);
  }
}
