...
class DeploymentHealthCheck {
      ...
      if (latestDeployment) {
        ...
        if (url) {
          ...
          try {
            ...
            this.log(`✅ Vercel deployment healthy: ${url} (${response.status})`);
            return { status: 'healthy', url, statusCode: response.status };
          } catch (error) {
            this.log(`❌ Vercel deployment unhealthy: ${url} - ${error.message}`);
  ...
  checkEnvironment() {
    try {
      ...
      this.log(`✅ Environment configuration healthy (${requiredVars.length} variables)`);
  ...
  checkDependencies() {
    try {
      ...
      this.log(`✅ Dependencies healthy (${Object.keys(packageJson.dependencies || {}).length} packages)`);
  ...
  async runFullCheck() {
    ...
    this.log(`📊 Health check complete: ${results.overall.score} (${results.overall.percentage}%)`);
