...
class DeploymentMonitor {
  ...
  async checkDeploymentStatus() {
    try {
      ...
      const status = {
        ...
        overall: this.calculateOverallStatus([vercelStatus, buildStatus, envStatus, depStatus])
      };
  ...
  calculateOverallStatus(statuses) {
    const statusMap = { healthy: 3, incomplete: 2, missing: 1, error: 0, unknown: 1 };
  ...
  async redeploy() {
    ...
    try {
      ...
      await notifyMaster('QMOI deployment monitor: Redeployment successful after auto-fixes');
    } catch (error) {
      this.log(`Redeployment failed: ${error.message}`);
      await notifyMaster(`QMOI deployment monitor: Redeployment failed - ${error.message}`);
    }
  }

  async startMonitoring(interval = 300000) { // 5 minutes default
    this.log(`Starting deployment monitoring with ${interval/1000}s interval...`);
    ...
    const monitor = async () => {
      try {
        ...
        if (status.overall === 'critical') {
          this.log('Critical deployment issues detected, applying auto-fixes...');
          await this.autoFixIssues(status);
        } else if (status.overall === 'warning') {
          this.log('Warning: Some deployment issues detected');
          await notifyMaster('QMOI deployment monitor: Warning - Some issues detected');
        } else {
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'fix':
      ...
      break;
    default:
      console.log('Usage: node deployment-monitor.js [status|monitor|fix] [interval]');
