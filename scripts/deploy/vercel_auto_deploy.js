...
function injectDockerFallback() {
  ...
  try {
    ...
    execSync("docker run -d --name alpha-q-ai -p 3000:3000 alpha-q-ai", {
      stdio: "inherit",
    });
...
async function main() {
  ...
  if (hasVercelEnv()) {
    ...
    if (!hasGitHubEnv()) {
      log(
        "GitHub credentials also missing. Attempting static export and email notification.",
      );
      injectStaticExportFallback();
      await notifyByEmail(
        "All cloud credentials missing. Static export attempted.",
      );
      return;
    } else {
      injectDockerFallback();
      await notifyByEmail(
        "All cloud credentials missing. Docker fallback attempted.",
      );
    }
  }
