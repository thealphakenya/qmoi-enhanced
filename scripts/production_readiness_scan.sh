...
# Run a set of scans for TODO/FIXME/nonproduction markers and document results.
...
echo "Scanning for TODO/FIXME/nonproduction/production ready markers..."
SCAN_FILE="$OUT_DIR/production_readiness_scan.txt"

grep -RIn --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=reports --exclude='production_readiness_scan.txt' -e 'TODO' -e 'FIXME' -e 'nonproduction' -e 'production ready' "$ROOT" > "$SCAN_FILE" || true
