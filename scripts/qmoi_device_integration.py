...
class QMOIDeviceIntegration:
    ...
    def _queue_unlock_request(
        self, request_type: str, restrictions: List[DeviceRestriction]
    ):
        ...
        try:
            ...
            for restriction in restrictions:
                ...
                if restriction.type == "mkopa":
                    ...
                    result = self.unlock_system.unlock_generic_device(
                ...
                if result.success:
                    ...
                    logger.error(
                        f"❌ Failed to unlock {restriction.organization}: {result.message}"
                    )
            ...
            if self.config.master_mode_enabled:
                ...
                if master_result.success:
                    ...
                    logger.error(
                        f"❌ Failed to enable master mode: {master_result.message}"
                    )
            ...
            logger.info(
                f"✅ Unlock request completed: {len([r for r in unlock_results if r.success])}/{len(unlock_results)} successful"
            )
    ...
    def trigger_manual_unlock(
        self, restrictions: Optional[List[DeviceRestriction]] = None
    ):
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "device_status": {
                ...
                "last_unlock": (
                    self.device_status.last_unlock.isoformat()
                    if self.device_status.last_unlock
                    else None
                ),
            },
            "threads": {
                ...
                "unlock_thread_alive": (
                    self.unlock_thread.is_alive() if self.unlock_thread else False
                ),
            },
        }
...
def create_integration_api():
    ...
    try:
            ...
            return jsonify(
                {
                    ...
                    "last_unlock": (
                        status.last_unlock.isoformat() if status.last_unlock else None
                    ),
                }
            )
        ...
        def trigger_unlock():
            ...
            return jsonify({"success": True, "message": "Unlock request queued"})
    ...
    try:
        ...
        try:
            while True:
                ...
                if status["running"]:
                    logger.info(
                        f"✅ Integration running - Restrictions: {status['device_status']['restriction_count']}, Master Mode: {status['device_status']['qmoi_master_mode']}"
                    )
