...
class DeviceUnlockSystem:
    ...
    def unlock_mkopa_device(self) -> UnlockResult:
        ...
        try:
            ...
            if not admin_result["success"]:
                errors.append(
                    f"Failed to remove M-KOPA device admin: {admin_result['error']}"
                )
            else:
            ...
            if not payment_result["success"]:
                errors.append(
                    f"Failed to clear payment locks: {payment_result['error']}"
                )
            else:
            ...
            if not app_result["success"]:
                errors.append(
                    f"Failed to remove app restrictions: {app_result['error']}"
                )
            else:
            ...
            result = UnlockResult(
                success=success,
                message=(
                    "M-KOPA restrictions removed successfully"
                    if success
                    else f"Unlock failed with {len(errors)} errors"
                ),
    ...
    def unlock_watu_device(self) -> UnlockResult:
        ...
        try:
            ...
            if not admin_result["success"]:
                errors.append(
                    f"Failed to remove Watu device admin: {admin_result['error']}"
                )
            else:
            ...
            if not loan_result["success"]:
                errors.append(
                    f"Failed to clear loan restrictions: {loan_result['error']}"
                )
            else:
            ...
            if not monitor_result["success"]:
                errors.append(
                    f"Failed to remove usage monitoring: {monitor_result['error']}"
                )
            else:
            ...
            result = UnlockResult(
                success=success,
                message=(
                    "Watu Credit restrictions removed successfully"
                    if success
                    else f"Unlock failed with {len(errors)} errors"
                ),
    ...
    def unlock_generic_device(self, organization: str) -> UnlockResult:
        ...
        try:
            ...
            if not policy_result["success"]:
                errors.append(
                    f"Failed to clear device policies: {policy_result['error']}"
                )
            else:
            ...
            if not restriction_result["success"]:
                errors.append(
                    f"Failed to remove device restrictions: {restriction_result['error']}"
                )
            else:
            ...
            if not freedom_result["success"]:
                errors.append(
                    f"Failed to enable device freedoms: {freedom_result['error']}"
                )
            else:
            ...
            result = UnlockResult(
                success=success,
                message=(
                    f"{organization} restrictions removed successfully"
                    if success
                    else f"Unlock failed with {len(errors)} errors"
                ),
    ...
    def _remove_macos_device_admin(self, package_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["profiles", "remove", "-all"], capture_output=True, text=True
            )
            if result.returncode == 0:
                ...
                return {
                    "success": False,
                    "error": f"Failed to remove profiles: {result.stderr}",
                }
        except Exception as e:
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            for pm in package_managers:
                try:
                    ...
                    result = subprocess.run(
                        [pm, "remove", "-y", "device-manager", "mdm"],
                        capture_output=True,
                        text=True,
                    )
                    if result.returncode == 0:
                        return {
                            "success": True,
                            "message": f"Linux device admin removed via {pm}",
                        }
                except:
                    pass
            ...
            for path in registry_paths:
                try:
                    with winreg.OpenKey(
                        ...
                        winreg.SetValueEx(
                            key, "DisableAppInstall", 0, winreg.REG_DWORD, 0
                        )
                        winreg.SetValueEx(
                            key, "AllowAllTrustedApps", 0, winreg.REG_DWORD, 1
                        )
                except:
                    pass
    ...
    def _remove_macos_app_restrictions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ["sudo", "spctl", "--master-disable"], capture_output=True, text=True
            )
            if result.returncode == 0:
                ...
                return {
                    "success": False,
                    "error": f"Failed to disable Gatekeeper: {result.stderr}",
                }
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def _clear_macos_policies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {
                    "success": False,
                    "error": f"Failed to clear policies: {result.stderr}",
                }
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def enable_master_mode(self) -> UnlockResult:
        ...
        try:
            ...
            if not perm_result["success"]:
                errors.append(
                    f"Failed to enable master permissions: {perm_result['error']}"
                )
            else:
            ...
            if not control_result["success"]:
                errors.append(
                    f"Failed to enable full device control: {control_result['error']}"
                )
            else:
            ...
            result = UnlockResult(
                success=success,
                message=(
                    "QMOI master mode enabled successfully"
                    if success
                    else f"Master mode failed with {len(errors)} errors"
                ),
            ...
            report = {
                ...
                "summary": {
                    ...
                    "successful_unlocks": len(
                        [r for r in self.unlock_history if r.success]
                    ),
                    "failed_unlocks": len(
                        [r for r in self.unlock_history if not r.success]
                    ),
                    "average_duration": (
                        sum(r.duration_seconds for r in self.unlock_history)
                        / len(self.unlock_history)
...
def main():
    ...
    try:
        ...
        logger.info(
            f"M-KOPA unlock result: {mkopa_result.success} - {mkopa_result.message}"
        )
        ...
        logger.info(
            f"Watu Credit unlock result: {watu_result.success} - {watu_result.message}"
        )
        ...
        logger.info(
            f"Master mode result: {master_result.success} - {master_result.message}"
        )
