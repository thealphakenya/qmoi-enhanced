...
class WorkflowManager:
    ...
    def log_workflow_activity(self, name, status, details):
        ...
        tracks_log = f'[{now}] [ENHANCEMENT] [Workflow] [{name}] [{status}] - {details}\n'
    ...
    def run_workflow(self, workflow):
        ...
        try:
            ...
            result = subprocess.run(['python3', 'scripts/auto_lint_fix.py', '--autofix'], capture_output=True, text=True)
            if result.returncode == 0:
                print(f'✅ {name} succeeded.')
                self.stats[name] = 'success'
                self.log_workflow_activity(name, 'success', 'Build and validation succeeded in QCity runner.')
                return True
            else:
                print(f'❌ {name} failed. Attempting auto-fix...')
                print(result.stdout)
                print(result.stderr)
                self.log_workflow_activity(name, 'error', f'Initial run failed. Output: {result.stdout.strip()} {result.stderr.strip()}')
                ...
                for attempt in range(2):
                    ...
                    if result.returncode == 0:
                        ...
                        self.stats[name] = 'success'
                        self.log_workflow_activity(name, 'fix', f'Auto-fix attempt {attempt+1} succeeded.')
                        self.log_workflow_activity(name, 'success', 'Build and validation succeeded after auto-fix.')
                        return True
                    else:
                        self.log_workflow_activity(name, 'fix', f'Auto-fix attempt {attempt+1} failed. Output: {result.stdout.strip()} {result.stderr.strip()}')
                print(f'❌ {name} failed after auto-fix attempts.')
                self.stats[name] = 'failed'
                self.log_workflow_activity(name, 'error', 'Validation failed after 3 attempts.')
                return False
    ...
    def disable_github_workflows(self):
        ...
        self.log_workflow_activity('ALL', 'success', 'All workflows succeeded in QCity runners. GitHub workflows will be skipped.')
