...
class QMOIErrorHandler:
    ...
    async def handle_error(
        ...
        try:
            ...
            handler = self.error_handlers.get(
                error_info.error_type, self.handle_unknown_error
            )
    ...
    def analyze_error(
        ...
        return ErrorInfo(
            ...
            details={"error_class": type(error).__name__, "context": context or {}},
    ...
    def determine_error_type(
        ...
        if any(
            ...
            for keyword in ["npm", "package", "dependency"]
        ):
            return ErrorType.NPM_ERROR
        elif any(
            keyword in error_message_lower
            for keyword in ["build", "compile", "webpack"]
        ):
            return ErrorType.BUILD_ERROR
        elif any(
            keyword in error_message_lower for keyword in ["test", "jest", "cypress"]
        ):
            return ErrorType.TEST_ERROR
        elif any(
            keyword in error_message_lower for keyword in ["deploy", "vercel", "gitlab"]
        ):
            ...
            keyword in error_message_lower for keyword in ["memory", "cpu", "resource"]
        ):
    ...
    def log_error(self, error_info: ErrorInfo):
        ...
        # Log to console
        self.logger.error(
            f"🚨 {error_info.error_type.value.upper()}: {error_info.message}"
        )
    ...
    async def attempt_recovery(self, error_info: ErrorInfo) -> bool:
        ...
        try:
            self.logger.info(
                f"🔄 Attempting recovery for {error_info.error_type.value}..."
            )
            ...
            for strategy in strategies:
                try:
                    ...
                    self.logger.info(
                        f"✅ Recovery strategy {strategy.__name__} succeeded"
                    )
                    return True
                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Recovery strategy {strategy.__name__} failed: {e}"
                    )
                    continue
    ...
    async def clear_npm_cache(self, error_info: ErrorInfo):
        ...
        try:
            subprocess.run(
                ["npm", "cache", "clean", "--force"], cwd=self.project_root, check=True
            )
    ...
    async def update_npm_config(self, error_info: ErrorInfo):
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "--upgrade", "npm"], check=True
            )
