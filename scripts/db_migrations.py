...
class DatabaseMigrator:
    ...
    def _get_default_config(self) -> Dict:
        ...
        if self.db_type == "sqlite":
            return {"database": "app.db", "migrations_table": "schema_migrations"}
            ...
            for file in sorted(self.migrations_dir.glob("V*.sql")):
                ...
                if version not in applied:
                    with open(file, "r") as f:
                        content = f.read()
                        up_migration = (
                            content.split("-- Down Migration")[0]
    ...
    def apply_migration(self, migration: Dict) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error applying migration {migration['version']}: {str(e)}"
            )
            raise
    ...
    def rollback_migration(self, migration: Dict) -> None:
        ...
        try:
            ...
            self.connection.rollback()
            self.logger.error(
                f"Error rolling back migration {migration['version']}: {str(e)}"
            )
            raise
        ...
        try:
            ...
            for migration in reversed(to_rollback):
                ...
                migration_file = next(
                    (
                        f
                        for f in self.migrations_dir.glob(
                            f"{migration['version']}_*.sql"
                        )
                    ),
                    None,
                )
