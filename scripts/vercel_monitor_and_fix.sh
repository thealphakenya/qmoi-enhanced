...
if [ -z "$VERCEL_TOKEN" ] || [ -z "$VERCEL_PROJECT_ID" ]; then
  ...
  echo "Example: VERCEL_TOKEN=xxx VERCEL_PROJECT_ID=yyy ./scripts/vercel_monitor_and_fix.sh"
  exit 2
fi
...
echo "If known failure patterns are detected (missing deps, build-tool mismatch), the recommended safe actions are:"
echo "- Add missing dependencies to package.json and open a PR"
