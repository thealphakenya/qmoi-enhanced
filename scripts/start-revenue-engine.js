  ...
  async start() {
    try {
      ...
      if (!configValidation.success) {
        this.log(`❌ Configuration validation failed: ${configValidation.error}`);
      ...
      if (startResult.success) {
        ...
        this.log(`   Engine Status: ${status.engine.running ? 'Running' : 'Stopped'}`);
        this.log(`   Master Mode: ${status.engine.masterMode ? 'Enabled' : 'Disabled'}`);
        ...
        throw new Error(`Failed to start revenue engine: ${startResult.message}`);
      }
    ...
    setInterval(() => {
      try {
        ...
        if (Date.now() % 300000 < 30000) { // Every 5 minutes
          ...
          this.log(`   M-Pesa: ${today.mpesa.earned}/${today.mpesa.target} KES (${((today.mpesa.earned/today.mpesa.target)*100).toFixed(1)}%)`);
          this.log(`   Airtel: ${today.airtel.earned}/${today.airtel.target} KES (${((today.airtel.earned/today.airtel.target)*100).toFixed(1)}%)`);
          this.log(`   Combined: ${today.combined.earned}/${today.combined.target} KES (${((today.combined.earned/today.combined.target)*100).toFixed(1)}%)`);
        }
        
        // Check for target completion
        if (today.mpesa.earned >= today.mpesa.target && !this.mpesaTargetReached) {
          this.log('🎉 M-Pesa daily target reached!');
          this.mpesaTargetReached = true;
        }
        
        if (today.airtel.earned >= today.airtel.target && !this.airtelTargetReached) {
          this.log('🎉 Airtel Money daily target reached!');
          this.airtelTargetReached = true;
        }
        
        if (today.combined.earned >= today.combined.target && !this.combinedTargetReached) {
...
async function main() {
  ...
  if (args.includes('--config')) {
    ...
    starter.log(result.success ? '✅ Configuration completed' : `❌ Configuration failed: ${result.error}`);
    return;
  }
  ...
  if (args.includes('--validate')) {
    ...
    starter.log(result.success ? '✅ Configuration valid' : `❌ Configuration invalid: ${result.error}`);
    return;
  }
