...
async function checkDeployment() {
  try {
    ...
    console.log(`   Region: ${project.latestDeployments?.[0]?.regions?.[0] || 'N/A'}\n`);
    ...
    deployments.deployments.slice(0, 5).forEach((deploy, idx) => {
      ...
      const badge = deploy.state === 'READY' ? '✅' : deploy.state === 'ERROR' ? '❌' : '⏳';
      console.log(
        `${badge} ${(idx + 1).toString().padStart(2)}. ${deploy.state.padEnd(8)} - ${new Date(deploy.createdAt).toLocaleString()}`
      );
    });
