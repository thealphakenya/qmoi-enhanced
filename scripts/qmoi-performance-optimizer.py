...
class QMOIPerformanceOptimizer:
    ...
    async def optimize_processes(self):
        ...
        try:
            ...
            processes.sort(
                key=lambda x: (x["memory_percent"] or 0) + (x["cpu_percent"] or 0),
                reverse=True,
            )
            ...
            for proc in processes[:5]:
                self.logger.info(
                    f"  - {proc['name']}: CPU {proc['cpu_percent']:.1f}%, MEM {proc['memory_percent']:.1f}%"
                )
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if any(
                        keyword in name.lower() for keyword in ["cache", "temp", "tmp"]
                    ):
            ...
            self.logger.info(
                f"🔧 Terminated {len(unnecessary_processes[:10])} unnecessary processes"
            )
    ...
    async def generate_performance_report(self):
        ...
        try:
            ...
            report = {
                ...
                // 10,  # Rough count
                "status": (
                    "healthy" if avg_cpu < 70 and avg_memory < 80 else "needs_attention"
                ),
            }
            ...
            # Log summary
            self.logger.info(
                f"📊 Performance Report: CPU {avg_cpu:.1f}%, MEM {avg_memory:.1f}%, DISK {avg_disk:.1f}%"
            )
        ...
        try:
            ...
            if avg_cpu > 80:
                recommendations.append(
                    "Consider CPU optimization: Reduce concurrent processes"
                )

            if avg_memory > 85:
                recommendations.append(
                    "Consider memory optimization: Clear caches and unused data"
                )

            if avg_disk > 90:
                recommendations.append(
                    "Consider disk optimization: Clean temporary files and logs"
                )
