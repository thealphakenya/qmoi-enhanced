...
class QMOIRegistryManager {
  constructor() {
    this.registryPath = path.join(process.cwd(), 'config', 'qmoi-registry.json');
    this.registry = {
      version: '2.0.0',
  ...
  async backupRegistry() {
    const backupPath = this.registryPath.replace('.json', `.backup.${Date.now()}.json`);
    ...
    for (const [name, rule] of this.autoEnhancementRules) {
      if (rule.condition(metrics)) {
        try {
          ...
          enhancements.push({ name, result, timestamp: new Date().toISOString() });
