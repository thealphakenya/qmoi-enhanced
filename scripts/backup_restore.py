    ...
    def _upload_to_s3(self, backup_path: Path) -> None:
        ...
        try:
            ...
            self.s3_client.upload_file(
                str(backup_path), self.config["s3_bucket"], s3_key
            )
            ...
            for backup_path in self.backup_dir.glob("backup_*"):
                if backup_path.is_dir() or backup_path.suffix == ".tar.gz":
                    try:
                        if backup_path.suffix == ".tar.gz":
                            with tarfile.open(backup_path, "r:gz") as tar:
                                metadata_file = tar.extractfile(
                                    f"{backup_path.stem}/metadata.json"
                                )
                                if metadata_file:
                        ...
                        self.logger.warning(
                            f"Error reading backup {backup_path.name}: {str(e)}"
                        )
                            ...
                            backups.append(
                                {
                                    ...
                                    "timestamp": obj["LastModified"].strftime(
                                        "%Y%m%d_%H%M%S"
                                    ),
                                    "size": obj["Size"],
                                    "location": "s3",
