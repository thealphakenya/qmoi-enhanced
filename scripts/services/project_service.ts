...
export class ProjectService {
  ...
  private async loadConfig(): Promise<void> {
    try {
      this.config = {
        ...
        defaultPriority: (process.env.DEFAULT_PROJECT_PRIORITY || 'medium') as 'low' | 'medium' | 'high' | 'critical',
        ...
        notificationSettings: {
          ...
          onResourceAllocation: process.env.NOTIFY_RESOURCE_ALLOCATION === 'true'
        },
        defaultTags: (process.env.DEFAULT_PROJECT_TAGS || '').split(','),
        maxTeamSize: parseInt(process.env.MAX_TEAM_SIZE || '10'),
        maxConcurrentProjects: parseInt(process.env.MAX_CONCURRENT_PROJECTS || '5')
      };
  ...
  public async addTask(projectId: string, taskData: Omit<Task, 'id' | 'projectId' | 'createdAt' | 'updatedAt'>): Promise<Task> {
    try {
      const project = this.projects.find(p => p.id === projectId);
      if (!project) {
        throw new Error('Project not found');
  ...
  public async updateTask(projectId: string, taskId: string, updates: Partial<Task>): Promise<Task> {
    try {
      const project = this.projects.find(p => p.id === projectId);
      if (!project) {
        throw new Error('Project not found');
      ...
      if (updates.status === 'completed' && this.config.notificationSettings.onTaskCompleted) {
        await this.notificationService.sendNotification(
          'Task Completed',
  ...
  public async allocateResource(projectId: string, resourceData: Omit<Resource, 'id' | 'projectId'>): Promise<Resource> {
    try {
      const project = this.projects.find(p => p.id === projectId);
      if (!project) {
        throw new Error('Project not found');
