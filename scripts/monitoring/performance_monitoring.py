...
class QMOIPerformanceMonitor:
    ...
    def get_top_processes(self, limit: int = 5) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            top_memory = sorted(processes, key=lambda x: x["memory_mb"], reverse=True)[
                :limit
            ]
    ...
    def check_performance_alerts(self, metrics: Dict[str, Any]) -> List[Dict]:
        ...
        try:
            ...
            if (
                ...
                > self.monitoring_config["process_threshold"]
            ):
                alerts.append(
                    {
    ...
    def analyze_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_memory = [
                m["memory"]["usage_percent"] for m in recent if "memory" in m
            ]
            older_memory = [
                m["memory"]["usage_percent"] for m in older if "memory" in m
            ]

            trends = {
                "cpu_trend": (
                    "increasing"
                    if sum(recent_cpu) > sum(older_cpu)
                    else "decreasing" if sum(recent_cpu) < sum(older_cpu) else "stable"
                ),
                "memory_trend": (
                    "increasing"
                    if sum(recent_memory) > sum(older_memory)
                    else (
                        "decreasing"
                        if sum(recent_memory) < sum(older_memory)
                        else "stable"
    ...
    def calculate_stability_score(self) -> float:
        ...
        try:
            ...
            cpu_values = [
                m["cpu"]["usage_percent"]
                for m in self.performance_history[-20:]
                if "cpu" in m
            ]
            memory_values = [
                m["memory"]["usage_percent"]
    ...
    def send_notification(self, alert: Dict) -> None:
        ...
        try:
            ...
            notification_script = (
                self.root_dir / "scripts" / "utils" / "send_notification.py"
            )
    ...
    def monitor_once(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Monitoring cycle complete. Alerts: {len(alerts)}, Recommendations: {len(recommendations)}"
        )
        return cycle_result
    ...
    def start_continuous_monitoring(self, interval: int = 60) -> None:
        ...
        logger.info(
            f"Starting continuous performance monitoring (interval: {interval}s)"
        )
...
def main():
    ...
    monitor = QMOIPerformanceMonitor(
        continuous=args.continuous, alert_threshold=args.alert_threshold
    )
