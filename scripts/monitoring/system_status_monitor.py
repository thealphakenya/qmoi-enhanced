...
def __init__(self, continuous: bool = False, interval: int = 60) -> Any:
        ...
        self.monitoring_config = {
            'components': {
                'qmoi_core': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/health',
                    'timeout': 10,
                    'critical': True
                },
                'qcity_prodice': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/qcity/status',
                    'timeout': 15,
                    'critical': True
                },
                'database': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/database/health',
                    'timeout': 10,
                    'critical': True
                },
                'trading_engine': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/trading/status',
                    'timeout': 10,
                    'critical': False
                },
                'ai_models': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/ai/health',
                    'timeout': 15,
                    'critical': False
                },
                'voice_system': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/qmoi/voice-profiles',
                    'timeout': 10,
                    'critical': False
                },
                'avatar_system': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/qmoi/avatars',
                    'timeout': 10,
                    'critical': False
                },
                'automation_engine': {
                    'enabled': True,
                    'health_check_url': 'http:process.env.API_HOST || "qmoi.ai:3000"/api/automation/status',
                    'timeout': 10,
                    'critical': False
                }
            },
...
def check_all_components(self) -> Dict[str, Any]:
        ...
        for component_name, config in self.monitoring_config['components'].items():
            ...
            if config['critical'] and result['status'] not in ['healthy', 'enabled']:
                critical_failures.append(component_name)
                overall_status = 'critical'
            elif result['status'] not in ['healthy', 'enabled'] and overall_status == 'healthy':
...
def generate_system_status_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_health = self.calculate_overall_health(
                system_overview, component_health, process_health, 
                filesystem_health, network_health
            )
...
def calculate_overall_health(self, system_overview: Dict, component_health: Dict, 
        ...
        network_reachable = sum(
            1 for url, status in network_health.items() 
            if isinstance(status, dict) and status.get('status') == 'reachable'
        )
        network_score = (network_reachable / len([k for k in network_health.keys() if k != 'interfaces'])) * 100
            ...
            if resources:
                ...
                if cpu_usage > self.monitoring_config['thresholds']['cpu_usage']:
                    alerts.append({
                        ...
                        'value': cpu_usage,
                        'threshold': self.monitoring_config['thresholds']['cpu_usage']
                    })
                ...
                if memory_usage > self.monitoring_config['thresholds']['memory_usage']:
                    alerts.append({
                        ...
                        'value': memory_usage,
                        'threshold': self.monitoring_config['thresholds']['memory_usage']
                    })
                ...
                if disk_usage > self.monitoring_config['thresholds']['disk_usage']:
                    alerts.append({
                        ...
                        'value': disk_usage,
                        'threshold': self.monitoring_config['thresholds']['disk_usage']
                    })
            
            # Component alerts
            for component_name, component_status in component_health.get('components', {}).items():
                if component_status.get('status') not in ['healthy', 'enabled']:
                    alerts.append({
                        'type': 'component_unhealthy',
                        'severity': 'critical' if component_status.get('critical') else 'warning',
            ...
            if resources:
                ...
                if cpu_usage > 70:
                    recommendations.append({
                        ...
                        'actions': ['Review high-CPU processes', 'Consider process optimization']
                    })
                ...
                if memory_usage > 80:
                    recommendations.append({
                        ...
                        'actions': ['Review memory-intensive processes', 'Consider memory optimization']
                    })
                ...
                if disk_usage > 85:
                    recommendations.append({
                        ...
                        'actions': ['Clean up permanent files', 'Remove unused files', 'Consider disk expansion']
                    })
            
            # Component recommendations
            unhealthy_components = [
                name for name, status in component_health.get('components', {}).items()
                if status.get('status') not in ['healthy', 'enabled']
            ]
...
def monitor_once(self) -> Dict[str, Any]:
        ...
        logger.info(f"Monitoring cycle complete. Health: {overall_health}, Alerts: {alerts_count}, Recommendations: {recommendations_count}")
