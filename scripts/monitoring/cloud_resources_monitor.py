...
class CloudResourcesMonitor:
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.logger.info(
                f"Cloud resources monitoring started with {len(threads)} threads"
            )
    ...
    def monitor_aws_resources(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # EC2 instances
                try:
                    ec2_response = self.cloud_providers["aws"][
                        "ec2"
                    ].describe_instances()
                    instances = []
                ...
                try:
                    ...
                    for bucket in s3_response["Buckets"]:
                        try:
                            # Get bucket size
                            size = 0
                            objects = self.cloud_providers["aws"]["s3"].list_objects_v2(
                                Bucket=bucket["Name"]
                            )
                            ...
                            self.logger.warning(
                                f"Error getting S3 bucket details for {bucket['Name']}: {e}"
                            )
                ...
                # RDS instances
                try:
                    rds_response = self.cloud_providers["aws"][
                        "rds"
                ...
                # Lambda functions
                try:
                    lambda_response = self.cloud_providers["aws"][
                        "lambda"
                    ].list_functions()
                    functions = []
        ...
        while self.monitoring_active:
            try:
                ...
                try:
                    ...
                    for instance in self.cloud_providers["gcp"]["compute"].list(
                        request=request
                    ):
                        instances.append(
                            {
    ...
    def monitor_costs(self):
        ...
        while self.monitoring_active:
            try:
                cost_data = {"timestamp": datetime.now().isoformat(), "costs": {}}
                ...
                if "aws" in self.cloud_providers:
                    try:
                        ...
                        cost_response = self.cloud_providers["aws"][
                            "ce"
                        ].get_cost_and_usage(
                            TimePeriod={
                        ...
                        if cost_response["ResultsByTime"]:
                            cost = cost_response["ResultsByTime"][0]["Total"][
                                "UnblendedCost"
                            ]["Amount"]
                            cost_data["costs"]["aws"] = {
                                "amount": float(cost),
                                "currency": cost_response["ResultsByTime"][0]["Total"][
                                    "UnblendedCost"
                                ]["Unit"],
                            }
                            ...
                            if float(cost) > self.config["alerts"]["cost_threshold"]:
                                self.logger.warning(
                                    f"AWS cost threshold exceeded: ${cost}"
                                )
            ...
            try:
                ...
                if "aws" in self.cloud_providers:
                    try:
                        ...
                        cw_response = self.cloud_providers["aws"][
                            "cloudwatch"
                        ...
                        if cw_response["Datapoints"]:
                            ...
                            if cpu_avg > self.config["alerts"]["cpu_threshold"]:
                                self.logger.warning(
                                    f"AWS CPU threshold exceeded: {cpu_avg}%"
                                )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "aws" in self.resource_metrics:
                ...
                report["summary"]["total_instances"] += report["providers"]["aws"][
                    "instances"
                ]
            ...
            if "azure" in self.resource_metrics:
                ...
                report["providers"]["azure"] = {"instances": total_azure_vms}
            ...
            if "gcp" in self.resource_metrics:
                ...
                report["summary"]["total_instances"] += report["providers"]["gcp"][
                    "instances"
                ]
            ...
            if (
                ...
                > self.config["alerts"]["cost_threshold"]
            ):
