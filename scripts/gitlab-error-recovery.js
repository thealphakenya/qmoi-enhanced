...
class GitLabErrorRecovery {
  constructor() {
    ...
    this.logFile = path.join(process.cwd(), 'logs', 'gitlab-error-recovery.log');
    this.ensureLogDir();
  }
    ...
    try {
      ...
      const fixes = [
        ...
        },
        {
          ...
          command: 'npm install --save-dev @testing-library/react @testing-library/jest-dom @testing-library/user-event jest jest-environment-jsdom',
          continueOnError: true
        },
        {
      ...
      const fixes = [
        {
          ...
          command: 'git config --global user.name "QMOI Automation" && git config --global user.email "qmoi-automation@gitlab.com"',
          continueOnError: true
        },
        {
    ...
    try {
      ...
      for (const fix of fixes) {
        try {
          ...
          this.log(`Environment fix failed: ${fix.name} - ${error.message}`, 'WARN');
          if (!fix.continueOnError) {
            throw error;
          }
        }
  ...
  async runFullRecovery() {
    try {
      ...
      try {
        ...
        this.log(`Health check failed after recovery: ${error.message}`, 'WARN');
      }
      ...
      try {
        const { GitLabNotificationService } = require('./gitlab-notification-service');
