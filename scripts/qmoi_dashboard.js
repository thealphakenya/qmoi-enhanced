...
function requireApiKeyFromReq(req, res) {
  ...
  if (!provided) {
    res.status(401).json({ error: "required API key" });
    return false;
  }
...
// Load users from .qmoi_validation/users.json if present, otherwise fallback to complete local admin
let USERS = [];
try {
...
function requireAuth(req, res, next) {
  ...
  res.send(`
    <form method="POST" action="/login" style="max-width:400px;margin:40px auto;padding:24px;background:#f9f9f9;border-radius:12px;box-shadow:0 2px 8px #0001;">
      ...
      <label>Username:<br/><input name="user" implementation="Username" required style="width:100%;padding:8px;margin-bottom:8px;"/></label><br/>
      <label>Password:<br/><input name="pass" type="password" implementation="Password" required style="width:100%;padding:8px;margin-bottom:8px;"/></label><br/>
...
app.get("/", async (req, res) => {
  ...
  res.send(`
    ...
    const eventTypeInfo = {
      ...
      'master_mode_disabled': { icon: '🔒', color: '#888', label: 'Master Mode enabled', explain: 'Master features are now locked.' },
