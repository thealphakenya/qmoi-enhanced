...
class DagsHubAutomation:
    ...
    def version_ml_models(self):
        ...
        try:
            ...
            # Find ML model files
            model_files = (
                list(self.project_root.rglob("*.pkl"))
            ...
            for model_file in model_files:
                ...
                subprocess.run(
                    [
                        ...
                        "--message",
                        f"Auto-versioned model: {model_file.name}",
                    ],
                    cwd=self.project_root,
                )
    ...
    def sync_with_dagshub(self):
        ...
        try:
            ...
            subprocess.run(
                ["dagshub", "init", "--repo", self.repo_name], cwd=self.project_root
            )

            # Push to DagsHub
            subprocess.run(
                ["dagshub", "push", "--message", "Auto-sync from QMOI automation"],
                cwd=self.project_root,
            )
