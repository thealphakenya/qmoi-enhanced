...
### Error Details:
\`\`\`
${data._error || 'Unknown _error occurred'}
\`\`\`
...
### Error Details:
\`\`\`
${data._error || 'Unknown deployment _error'}
\`\`\`
...
### Error Details:
\`\`\`
${data._error || 'Test execution failed'}
\`\`\`
...
### Error Details:
\`\`\`
${data._error || 'Build process failed'}
\`\`\`
...
*Generated by QMOI Notification Service*`;
      ...
      this.log(`Failed to send pipeline notification: ${_error.message}`, 'ERROR');
      throw _error;
    }
  }
...
### Error Stack:
\`\`\`
${_error.stack || 'No stack trace available'}
\`\`\`
...
*Generated by QMOI Notification Service*`;
      ...
      const labels = ['qmoi', '_error', context.type || 'unknown', 'needs-attention'];
      ...
      this.log(`Failed to send _error notification: ${notificationError.message}`, 'ERROR');
...
*Generated by QMOI Notification Service*`;
      ...
      this.log(`Failed to send success notification: ${_error.message}`, 'ERROR');
      throw _error;
    }
  }
}
  ...
  try {
    ...
    switch (command) {
      ...
      case '--build-failure':
        ...
        break;
      case '--_error':
        const _error = new Error(data.message || 'Unknown _error');
        _error.stack = data.stack;
      ...
      default:
        ...
        console.log('  --_error [data]               Send _error notification');
        console.log('  --success [data]             Send success notification');
        break;
    }
    
  } catch (_error) {
    notificationService.log(`Notification service failed: ${_error.message}`, 'ERROR');
    process.exit(1);
  }
}
