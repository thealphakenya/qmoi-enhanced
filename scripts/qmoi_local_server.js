#!/usr/bin/env node
// Minimal Node-based helper server used by tests when Python+Flask is unavailable.
// Implements endpoints compatible with scripts/qmoi_local_server.py:
//  - GET  /            -> health
//  - POST /v1/chat/completions -> minimal chat-completion (echo)
//  - POST /sync/push  -> save JSON memory (optional auth via QMOI_SYNC_API_KEY)
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    ...
    console.error('[qmoi_local_server] handler error', e && e.stack ? e.stack : String(e));
    try {
      return sendJson(res, { error: 'server error', details: String(e) }, 500);
    } catch (_e2) {
...
server.listen(PORT, HOST, () => {
  console.log(
    `qmoi_local_server.js listening on http://${HOST}:${PORT} (memory=${MEMORY_FILE})`
  );
});
