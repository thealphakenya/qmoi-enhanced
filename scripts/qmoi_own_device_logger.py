...
class QMOIOwnDeviceLogger:
    ...
    def log_ownership_detection(
        self,
        device_info: Dict[str, Any],
        restriction: Dict[str, Any],
        master_user: str = "system",
        session_id: str = "auto",
    ) -> str:
    ...
    def log_unlock_attempt(
        ...
        try:
            ...
            logger.info(
                f"🔓 Logged unlock attempt for device {device_id} - Success: {success}"
            )
            return timestamp
        ...
        try:
            ...
            """,
                (
                    ...
                    log_entry.action,
                    log_entry.device_id,
                    json.dumps(log_entry.device_info),
                    log_entry.restriction_type,
                    log_entry.organization,
                    log_entry.severity,
                    log_entry.success,
                    log_entry.duration_ms,
                    log_entry.error_message,
                    log_entry.master_user,
    ...
    def save_unlock_log(self, unlock_log: DeviceUnlockAttempt):
        ...
        try:
            ...
            """,
                (
                    unlock_log.timestamp,
                    unlock_log.device_id,
                    unlock_log.unlock_method,
                    unlock_log.attempt_number,
                    unlock_log.success,
                    unlock_log.duration_ms,
                    unlock_log.error_details,
                    json.dumps(unlock_log.bypass_techniques),
        ...
        try:
            ...
            """,
                (
                    ...
                    master_log["master_user"],
                    master_log["action"],
                    master_log["target_device"],
                    master_log["success"],
                    json.dumps(master_log["details"]),
                    master_log["session_id"],
                ),
            )
    ...
    def get_logs_for_master(
        ...
        try:
            ...
            if log_type in ["all", "ownership"]:
                logs["ownership_logs"] = self.get_ownership_logs(
                    limit, device_id, date_from, date_to
                )

            if log_type in ["all", "unlock"]:
                logs["unlock_logs"] = self.get_unlock_logs(
                    limit, device_id, date_from, date_to
                )

            if log_type in ["all", "master"]:
                logs["master_logs"] = self.get_master_logs(
                    limit, device_id, date_from, date_to
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get organizations
            cursor.execute(
                "SELECT DISTINCT organization FROM ownership_logs WHERE organization IS NOT NULL"
            )
            ...
            return {
                ...
                "success_rate": (
                    (successful_unlocks / total_unlock_attempts * 100)
                    if total_unlock_attempts > 0
                    else 0
                ),
            }
    ...
    def _cleanup_old_logs(self):
        ...
        try:
            ...
            # Clean up old logs
            cursor.execute(
                "DELETE FROM ownership_logs WHERE timestamp < ?", (cutoff_date,)
            )
            cursor.execute(
                "DELETE FROM unlock_logs WHERE timestamp < ?", (cutoff_date,)
            )
            cursor.execute(
                "DELETE FROM master_logs WHERE timestamp < ?", (cutoff_date,)
            )
