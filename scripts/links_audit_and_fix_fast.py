#!/usr/bin/env python3
"""Fast, conservative Markdown link auditor/fixer (clean separate file).

This is a safe replacement runner to avoid the corrupted original file. Use
--max-files to bound the scan. Writes reports to .qmoi_validation like the original.
"""
...
def find_md_files() -> list[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        if any(p in dirpath for p in ("/.git/", "/node_modules/", "/.qmoi_validation/", "/.venv/")):
            continue
        for fn in filenames:
            p = Path(dirpath) / fn
...
def main():
    ap = argparse.ArgumentParser(description='Conservative Markdown link auditor/fixer (fast)')
