        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.base_url, json={"authorize": self.api_key}
                ) as response:
                    data = await response.json()
                    if data.get("error"):
                        logger.error(
                            f"Deriv connection error: {data['error']['message']}"
                        )
                        return False
...
class EnhancedTradingSystem:
    ...
    async def connect_all_platforms(self) -> bool:
        ...
        logger.info(
            f"Connected to {successful_connections}/{total_platforms} platforms"
        )
                ...
                if platform.enabled:
                    ...
                    for symbol in symbols:
                        try:
                            ...
                            logger.error(
                                f"Error getting market data for {symbol} on {name}: {e}"
                            )
    ...
    async def execute_signals(self, signals: List[TradeSignal]) -> List[TradeResult]:
        ...
        for signal in signals:
            try:
                ...
                if balances:
                    available = max(
                        b.available for b in balances if b.currency in signal.symbol
                    )
        ...
        try:
            ...
            if signals:
                ...
                if self.current_daily_profit >= self.daily_profit_target:
                    logger.info(
                        f"Daily profit target reached: KSH {self.current_daily_profit}"
                    )
                    return True
    ...
    def get_trading_statistics(self) -> Dict:
        ...
        return {
            ...
            "successful_trades": len(
                [t for t in self.trade_history if t.status == "filled"]
            ),
            "platforms_connected": len(
                [p for p in self.platforms.values() if p.enabled]
            ),
            "last_trade": (
                self.trade_history[-1].__dict__ if self.trade_history else None
            ),
        }
