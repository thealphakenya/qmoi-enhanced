...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("qmoi_space_backend.log"), logging.StreamHandler()],
)
        ...
        try:
            ...
            """,
                (
                    ...
                    (
                        json.dumps(revenue_data.metadata)
                        if revenue_data.metadata
                        else None
                    ),
                ),
            )
...
class NotificationManager:
    ...
    async def send_notification(
        self, notification: NotificationData, user_id: Optional[int] = None
    ):
        """Send notification"""
        try:
            # Save to database
...
async def get_analytics_overview():
    ...
    try:
        ...
        return {
            "users": {"total": 1250, "active": 890, "new_today": 45},
            "revenue": {"today": 45230, "this_month": 1245600, "growth": 23.5},
            "performance": {"uptime": 99.9, "response_time": 120, "error_rate": 0.1},
        }
