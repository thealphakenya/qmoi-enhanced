...
`;
  ...
  async setupMissingEnvVars() {
    ...
    const lines = envContent
      .split("\n")
      .filter(
        (line) => !line.includes("NODE_ENV") && !line.includes("NEXT_PUBLIC"),
      );
  ...
  async setupNoInternet() {
    ...
    const newHostsContent = hostsContent.replace(
...
`;
  ...
  getTestCommands(category) {
    const commands = {
      ...
      lint: ["npm run lint"],
      config: [
        "node -e \"JSON.parse(require('fs').readFileSync('test-config.json'))\"",
      ],
      env: ["npm run build"],
  ...
  generateReport(totalTime) {
    const report = {
      ...
      summary: {
        ...
        autoFixAttempted: this.testResults.filter((r) => r.autoFixAttempted)
          .length,
    ...
    console.log(
      `   Setup success: ${report.summary.setupSuccess}/${report.summary.totalTests}`,
    );
    console.log(
      `   Test success: ${report.summary.testSuccess}/${report.summary.totalTests}`,
    );
    console.log(
      `   Cleanup success: ${report.summary.cleanupSuccess}/${report.summary.totalTests}`,
    );
    ...
    Object.entries(report.categories).forEach(([category, results]) => {
      ...
      console.log(
        `   ${category}: ${successCount}/${results.length} tests passed`,
      );
    });
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      console.log(
        "Usage: node qmoi_self_test_runner.js [all|test|list] [test-name]",
      );
  }
}
