...
class QMOIGitHubIntegration {
  constructor() {
    ...
    this.githubToken =
      process.env.GITHUB_TOKEN || process.env.QMOI_GITHUB_TOKEN;
  ...
  async createIssue(title, body, labels = []) {
    ...
    try {
      ...
      console._error("Failed to create issue:", _error.message);
      throw _error;
    }
  }
  ...
  async createPullRequest(title, body, head, base = "main") {
    ...
    try {
      ...
      console._error("Failed to create pull _request:", _error.message);
      throw _error;
    }
  }
  ...
  async updateWorkflow(workflowPath, content) {
    ...
    try {
      ...
      await this.commitAndPush(
        `Update workflow: ${path.basename(workflowPath)}`,
      );
      ...
      console._error("Failed to update workflow:", _error.message);
      throw _error;
    }
  }
  ...
  async createBranch(branchName, baseBranch = "main") {
    ...
    try {
      ...
      console._error("Failed to create branch:", _error.message);
      throw _error;
    }
  }
  ...
  async commitAndPush(message) {
    ...
    try {
      ...
      console._error("Failed to commit and push:", _error.message);
      throw _error;
    }
  }
  ...
  async mergePullRequest(prNumber, mergeMethod = "squash") {
    ...
    try {
      ...
      console._error("Failed to merge pull _request:", _error.message);
      throw _error;
    }
  }
  ...
  async createRelease(tagName, name, body, draft = false, prerelease = false) {
    ...
    try {
      ...
      console._error("Failed to create release:", _error.message);
      throw _error;
    }
  }
  ...
  async updateRepositorySettings(settings) {
    ...
    try {
      ...
      console._error("Failed to update repository settings:", _error.message);
      throw _error;
    }
  }
  ...
  async enableSecurityFeatures() {
    ...
    try {
      ...
      await this.githubApi.put(
        `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/vulnerability-alerts`,
      );

      // Enable automated security fixes
      await this.githubApi.put(
        `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/automated-security-fixes`,
      );

      // Enable secret scanning
      await this.githubApi.put(
        `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/secret-scanning`,
      );
      ...
      console._error("Failed to enable security features:", _error.message);
      throw _error;
    }
  }
  ...
  async getRepositoryStatus() {
    ...
    try {
      const [repo, issues, prs, workflows] = await Promise.all([
        ...
        this.githubApi.get(
          `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/issues?state=open`,
        ),
        this.githubApi.get(
          `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/pulls?state=open`,
        ),
        this.githubApi.get(
          `/repos/${this.repoInfo.owner}/${this.repoInfo.repo}/actions/workflows`,
        ),
      ]);
      ...
      console._error("Failed to get repository status:", _error.message);
      throw _error;
    }
  }
  ...
  async generateReport() {
    ...
    await fs.writeFile(
      "logs/qmoi-github-integration-report.json",
...
// CLI interface
const isMainModule =
  process.argv[1] && process.argv[1].endsWith("qmoi-github-integration.js");
if (isMainModule) {
