...
out.push('');
out.push('_Auto-generated by scripts/generate-builds-doc.js_');
out.push('');
...
try {
  ...
  const files = fs.existsSync(scriptsDir) ? fs.readdirSync(scriptsDir).filter(f => f.endsWith('.js') || f.endsWith('.sh') || f.endsWith('.ps1')) : [];
