...
def sync_model(repo_id, model_path, token=None):
    ...
    if not token:
        logger.warning(
            "No Hugging Face token provided or found in env. Skipping model sync."
        )
        return False
    try:
...
def main():
    ...
    parser.add_argument(
        "--model-path", required=True, help="Path to model directory or file"
    )
    ...
    for attempt in range(1, RETRY_LIMIT + 1):
        ...
        logger.error(
            "All attempts to sync model failed. Continuing workflow (non-fatal)."
        )
