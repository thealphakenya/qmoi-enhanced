...
async function validateTradingEngine() {
  try {
    ...
    if (!response.ok) {
      console.error('❌ Trading engine health check failed:', response.statusText);
      return false;
    }
...
async function validatePesapal() {
  ...
  try {
    const response = await fetch(`https://${process.env.PESAPAL_ENVIRONMENT === 'live' ? 'api' : 'sandbox'}.pesapal.com/v3/api/Auth/RequestToken`, {
      method: 'POST',
      headers: {
