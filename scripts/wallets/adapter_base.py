"""Wallet adapter base and sample testnet adapters.

This module provides a small adapter interface and a few mocked/testnet adapters
you can extend. All adapters are mock-first and must be explicitly enabled for
...
class TestnetAdapter(AdapterBase):
    """Simple testnet adapter that returns deterministic mock values."""
...
class CashonAdapter(AdapterBase):
    ...
    def check_balance(self, config=None, real=False):
        ...
        return {'balance': '0.00 (cashon-mock)', 'currency': cfg.get('currency','KES'), 'last_checked': now_iso(), 'status': 'mocked', 'meta': {'adapter': 'cashon'}}
...
class MegavaultAdapter(AdapterBase):
    ...
    def check_balance(self, config=None, real=False):
        ...
        return {'balance': '0.00 (megavault-mock)', 'currency': cfg.get('currency','USD'), 'last_checked': now_iso(), 'status': 'mocked', 'meta': {'adapter': 'megavault'}}
