...
def build_proposals():
    ...
    for r in summary.get('results', []):
        ...
        for f in r.get('files', []):
            body_lines.append(f"- {f.get('path')}: {'OK' if f.get('ok') else 'MISSING'}")
        body = '\n'.join(body_lines)
...
def publish(proposals, upload=False):
    ...
    if not token or not repo:
        print('Missing GITHUB_TOKEN or GITHUB_REPO; publish disabled. Proposals available for manual review.')
        return
    owner_repo = repo.strip()
    created = []
    for p in proposals:
        url = f'https://api.github.com/repos/{owner_repo}/releases'
        payload = json.dumps({'tag_name': p['tag'], 'name': p['name'], 'body': p['body'], 'draft': False, 'prerelease': False}).encode('utf-8')
        try:
