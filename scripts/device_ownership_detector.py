...
class DeviceOwnershipDetector:
    ...
    def detect_all_restrictions(self) -> List[DeviceRestriction]:
        ...
        logger.info(
            f"🔍 Detection complete. Found {len(all_restrictions)} restrictions."
        )

        return all_restrictions

    def _detect_organization_restrictions(
        self, org_key: str, org_info: Dict[str, Any]
    ) -> List[DeviceRestriction]:
        ...
        try:
            ...
            for proc in self.device_info.get("running_processes", []):
                ...
                for signature in org_info["signatures"]:
                    if (
                        signature.lower() in proc_name
                        or signature.lower() in proc_cmdline
                    ):
                        restrictions.append(
                            DeviceRestriction(
                                type=org_key,
            ...
            if platform.system() == "Darwin":
                try:
                    result = subprocess.run(
                        ["profiles", "list"], capture_output=True, text=True
                    )
            ...
            if self._check_app_store_restrictions():
                restrictions.append(
                    DeviceRestriction(
                        ...
                        restrictions=[
                            "app_installation_block",
                            "developer_account_lock",
                        ],
                        device_info=self.device_info,
                        unlock_methods=[
                            "bypass_app_store",
        ...
        try:
            report = {
                ...
                "summary": {
                    ...
                    "high_severity": len(
                        [r for r in self.detection_results if r.severity == "high"]
                    ),
                    "critical_severity": len(
                        [r for r in self.detection_results if r.severity == "critical"]
                    ),
                    "organizations": list(
                        set(r.organization for r in self.detection_results)
                    ),
                },
            }
            return report
    ...
    try:
        ...
        if restrictions:
            ...
            for restriction in restrictions:
                logger.warning(
                    f"  - {restriction.organization}: {restriction.description} ({restriction.severity})"
                )
