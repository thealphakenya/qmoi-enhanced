#!/usr/bin/env node
// Lightweight autoheal script for bulk safe fixes
const fs = require('fs');
...
function dedupeErrorBoundary(file) {
  ...
  if (!/import\s+ErrorBoundary\s+from\s+['\"]@\/components\/ErrorBoundary['\"]/g.test(cleaned)) {
    safeWrite(file, `import ErrorBoundary from '@/components/ErrorBoundary';\n` + cleaned);
  } else {
    safeWrite(file, cleaned);
  }
  return count;
}
...
function ensureReact(file) {
  ...
  safeWrite(file, `import React from 'react';\n` + src);
  return true;
}
...
function run() {
  ...
  // run tsc to update diagnostics
  try {
    execSync('npx tsc --noEmit 2> /tmp/tsc_output.txt', { stdio: 'inherit' });
  } catch (e) {
    // tsc exits non-zero on errors, that's expected
  }
