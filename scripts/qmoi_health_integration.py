...
class QMOIHealthIntegration:
        ...
        try:
            ...
            if 'npm ERR!' in result.stderr:
                self.errors.append({
                    ...
                    'severity': 'critical',
                    'message': 'Some npm dependencies are missing or broken',
                    'timestamp': datetime.now().isoformat(),
                    'fixed': False,
                })
        except Exception as e:
            ...
            for file in required_files:
                if not os.path.exists(file):
                    self.errors.append({
                        ...
                        'severity': 'warning',
                        'message': f"Configuration file missing: {file}",
                        'file': file,
                        'timestamp': datetime.now().isoformat(),
                        'fixed': False,
                    })
        ...
        try:
            ...
            if os.path.exists('.env') and not os.path.exists('.gitignore'):
                self.errors.append({
                    ...
                    'type': 'Security Issue',
                    'severity': 'critical',
                    'message': '.env file may be exposed - .gitignore missing',
                    'timestamp': datetime.now().isoformat(),
                    'fixed': False,
                })
    ...
    def _apply_fix_for_error(self, error: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            elif 'Missing Dependencies' in error_type:
                return self._fix_missing_dependencies(error)
            elif 'Missing Configuration' in error_type:
