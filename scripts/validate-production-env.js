...
class EnvironmentValidator {
    ...
    validateEnvFile() {
        ...
        if (missing.length === 0) {
            ...
            missing.forEach(v => {
                this.errors.push(`Missing environment variable: ${v}`);
                log.error(`Missing environment variable: ${v}`);
            });
        }
    }
    ...
    validateBuildArtifacts() {
        ...
        if (fs.existsSync('.next')) {
            ...
            this.warnings.push('Next.js build not found - will be created during deployment');
        ...
        if (fs.existsSync('node_modules')) {
            ...
            this.warnings.push('Dependencies not installed - will install during deployment');
    ...
    validateDirectories() {
        ...
        if (missing.length === 0) {
            ...
            this.errors.push(`Missing directories: ${missing.join(', ')}`);
            log.error(`Missing directories: ${missing.join(', ')}`);
        }
    }
    ...
    report() {
        ...
        if (this.errors.length === 0 && this.warnings.length === 0) {
            ...
            if (this.errors.length > 0) {
                console.log(`${colors.red}❌ ${this.errors.length} ERRORS FOUND${colors.reset}`);
