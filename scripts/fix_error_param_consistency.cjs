...
for (const file of files) {
  ...
  out = out.replace(/\be\s*instanceof\s*Error\s*\?\s*e\.message\s*:\s*String\(_e\)/g, '_e instanceof Error ? _e.message : String(_e)');
  // Also 'error: e instanceof Error ? e.message : String(_e)'
  out = out.replace(/error:\s*e\s*instanceof\s*Error\s*\?\s*e\.message\s*:\s*String\(_e\)/g, 'error: _e instanceof Error ? _e.message : String(_e)');
