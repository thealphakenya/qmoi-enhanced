...
class QMOIRealTimeMonitor:
    ...
    def collect_file_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"file_counts": file_counts, "recent_logs": log_files}
    ...
    def print_summary(self, stats: Dict[str, Any]):
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print(
                f"QMOI Real-Time Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            print(f"{'='*60}")
            ...
            if "qmoi" in qmoi:
                ...
                print(
                    f"Deployments Successful: {qmoi_stats.get('deployments_successful', 0)}"
                )
                print(f"Notifications Sent: {qmoi_stats.get('notifications_sent', 0)}")
                print(
                    f"Health Checks Passed: {qmoi_stats.get('health_checks_passed', 0)}"
                )
...
"""
            ...
            dashboard_thread = threading.Thread(
                target=run_dashboard_server, daemon=True
            )
