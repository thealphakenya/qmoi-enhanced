...
class QCityDeviceManager {
  ...
  loadConfig() {
    try {
      return JSON.parse(
        fs.readFileSync("config/qcity-device-config.json", "utf8"),
      );
    } catch (error) {
  ...
  async executeInQCity(command, options = {}) {
    ...
    if (this.multiDevice.enabled) {
      result.multiDevice = await this.executeOnMultipleDevices(
        command,
        options,
      );
    }
  ...
  async npmInstall(packages = []) {
    const command =
      packages.length > 0 ? `npm install ${packages.join(" ")}` : "npm install";
  ...
  async status() {
    ...
    console.log(
      `- Unlimited Resources: ${status.unlimitedResources ? "Enabled" : "Disabled"}`,
    );
  ...
  getQCityStatus() {
    return {
      ...
      resourceOffloading:
        this.config.qcity_device.resource_offloading?.enabled || true,
      storageInQCity:
        this.config.qcity_device.storage?.node_modules_in_qcity || true,
    };
  }
  ...
  async handleCommand(command, args = []) {
    switch (command) {
      ...
      default:
        ...
        console.log(
          "Available commands: npm-install, build, test, lint, deploy, upgrade, optimize, cluster, security-audit, tune, status, monitor, auto-fix",
        );
    }
  }
  ...
  async atomicNpmInstall(packages = []) {
    ...
    const command =
      packages.length > 0
        ? `npm install ${packages.join(" ")} --prefix ${tempDir}`
        : `npm install --prefix ${tempDir}`;
    await this.executeInQCity(command, {
      storage: "unlimited_qcity",
      nodeModules: "unlimited_qcity",
      unlimitedResources: true,
    });
  ...
  // Deduplication
  async dedupe() {
    return this.executeInQCity("npm dedupe", {
      nodeModules: "unlimited_qcity",
    });
  }
    ...
    for (const env of envs) {
      switch (env) {
        case "node":
          ...
          break;
        case "python":
          await this.executeInQCity("pip install -r requirements.txt", {
            isolated: true,
          });
          break;
        case "java":
  ...
  async getEnvironmentsStatus() {
    return this.detectEnvironments().map((env) => ({
      env,
      status: "detected",
    }));
  }
