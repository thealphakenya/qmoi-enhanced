...
class NetworkManager:
    ...
    def schedule_monitoring(self) -> None:
        """Schedule next monitoring cycle."""
        interval = (
            self.config["monitoring"]["interval"] * 1000
    ...
    def check_alerts(self, interface: str) -> None:
        ...
        try:
            ...
            if stats["latency"] > alerts["latency_threshold"]:
                self.logger.warning(
                    f"High latency on {interface}: " f"{stats['latency']:.2f} ms"
                )

            # Check packet loss
            if stats["packet_loss"] > alerts["packet_loss_threshold"]:
                self.logger.warning(
                    f"High packet loss on {interface}: " f"{stats['packet_loss']:.2f}%"
                )
        ...
        try:
            if sys.platform == "win32":
                ...
                if password:
                    ...
                    subprocess.run(
                        ["netsh", "wlan", "connect", "name=" + ssid], check=True
                    )
                ...
                if password:
                    ...
                    subprocess.run(
                        ["nmcli", "device", "wifi", "connect", ssid], check=True
                    )

            # Add to connection history
            self.connection_history.append(
                {"ssid": ssid, "timestamp": datetime.now().isoformat(), "success": True}
            )
        ...
        try:
            if self.config["optimization"]["enabled"]:
                ...
                if self.config["optimization"]["dns_servers"]:
                    self.set_dns_servers(self.config["optimization"]["dns_servers"])
    ...
    def optimize_mtu(self) -> None:
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                # Linux
                subprocess.run(
                    ["ip", "link", "set", "wlan0", "mtu", str(optimal_mtu)], check=True
                )
    ...
    def enable_qos(self) -> None:
        ...
        try:
            if sys.platform == "win32":
                ...
                # Linux
                subprocess.run(
                    ["tc", "qdisc", "add", "dev", "wlan0", "root", "tbf"], check=True
                )
    ...
    def scan_network(self) -> Dict:
        ...
        try:
            if self.config["security"]["enabled"]:
                results = {"devices": [], "vulnerabilities": [], "ports": []}
