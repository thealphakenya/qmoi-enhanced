...
[ -f "package.json" ] && print_check "pass" "package.json exists" || print_check "fail" "package.json missing"
[ -f "next.config.js" ] && print_check "pass" "next.config.js exists" || print_check "fail" "next.config.js missing"
[ -f "vercel.json" ] && print_check "pass" "vercel.json exists" || print_check "fail" "vercel.json missing"
[ -d "app/api" ] && print_check "pass" "app/api directory exists" || print_check "fail" "app/api directory missing"
[ -d "lib" ] && print_check "pass" "lib directory exists" || print_check "fail" "lib directory missing"

echo ""
echo -e "${BLUE}3. Checking critical library files${NC}"

[ -f "lib/auth/service.ts" ] && print_check "pass" "Authentication service exists" || print_check "fail" "Authentication service missing"
[ -f "lib/db/prisma.ts" ] && print_check "pass" "Database service exists" || print_check "fail" "Database service missing"
[ -f "lib/email/service.ts" ] && print_check "pass" "Email service exists" || print_check "fail" "Email service missing"
[ -f "lib/payments/service.ts" ] && print_check "pass" "Payment service exists" || print_check "fail" "Payment service missing"
[ -f "lib/monitoring/error-tracker.ts" ] && print_check "pass" "Error tracker exists" || print_check "fail" "Error tracker missing"
...
[ -f ".env.production.example" ] && print_check "pass" "Environment template exists" || print_check "fail" "Environment template missing"
...
[ -f "PRODUCTION_DEPLOYMENT_CHECKLIST.md" ] && print_check "pass" "Deployment checklist exists" || print_check "fail" "Deployment checklist missing"
[ -f "PRODUCTION_API_REFERENCE.md" ] && print_check "pass" "API reference exists" || print_check "fail" "API reference missing"
[ -f ".github/workflows/deploy.yml" ] && print_check "pass" "CI/CD pipeline configured" || print_check "fail" "CI/CD pipeline missing"
...
if [ $CHECKS_FAILED -eq 0 ]; then
  ...
  echo ""
  echo "Next steps:"
  echo "  1. Configure environment variables: cp .env.production.example .env.production"
