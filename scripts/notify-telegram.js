...
if (!TELEGRAM_BOT_TOKEN || CHAT_IDS.length === 0) {
  console.warn('⚠️ Telegram notification skipped: missing TELEGRAM_BOT_TOKEN or CHAT_IDS');
  process.exit(0);
}
...
async function sendMessageToAll() {
  for (const chat_id of CHAT_IDS) {
    try {
      ...
      console.warn(`❌ Failed to send message to ${chat_id}:`, err?.response?.data || err.message);
    }
  }
}
  ...
  for (const platform of platforms) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.warn(`❌ Error uploading ${file}:`, err?.response?.data || err.message);
      }
    }
  }
}
...
async function sendImagePreview() {
  ...
  try {
    ...
    console.warn(`⚠️ Failed to send preview image:`, err?.response?.data || err.message);
  }
}
