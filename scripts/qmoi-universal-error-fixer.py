...
class QMOIUniversalErrorFixer:
        ...
        try:
            ...
            for error_type, patterns in self.error_patterns.items():
                for pattern in patterns:
                    ...
                    if matches:
                        logger.warning(
                            f"⚠️ Found {error_type} in {file_path}: {matches}"
                        )
    ...
    def scan_npm_output(self):
        ...
        try:
            result = subprocess.run(
                "npm list", shell=True, capture_output=True, text=True
            )
    ...
    def apply_fix_strategies(self, error_type: str, strategies: List[str]):
        ...
        try:
            logger.info(
                f"🔧 Applying {len(strategies)} fix strategies for {error_type}"
            )
            ...
            for strategy in strategies:
                try:
                    ...
                    if result is None:
                        logger.error(
                            f"❌ Subprocess failed to start or returned None: {strategy}"
                        )
    ...
    def fix_npm_errors(self):
        ...
        try:
            ...
            for fix in npm_fixes:
                try:
                    result = subprocess.run(
                        fix, shell=True, capture_output=True, text=True, timeout=300
                    )
                    if result is None:
                        logger.error(
                            f"❌ Subprocess failed to start or returned None: {fix}"
                        )
                        self.fix_stats["failed_fixes"] += 1
                        continue
                    if result.returncode == 0:
    ...
    def fix_build_errors(self):
        ...
        try:
            ...
            for fix in build_fixes:
                try:
                    result = subprocess.run(
                        fix, shell=True, capture_output=True, text=True, timeout=600
                    )
                    if result is None:
                        logger.error(
                            f"❌ Subprocess failed to start or returned None: {fix}"
