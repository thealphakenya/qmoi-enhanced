...
- For each release, upload missing assets and replace mismatched assets in-place.
    ...
    for rel in releases:
        ...
        for a in assets:
            ...
            if not os.path.exists(local):
                print('  local file missing for', name, '- skipping for', tag)
                continue
            ...
            if name in existing:
                ...
                print('  uploading missing', name, 'to', tag)
                try:
                    # upload missing asset to this release (create backup dir for symmetry)
                    backup_dir = os.path.join(ROOT, 'reports', 'releases_backup', tag)
                    os.makedirs(backup_dir, exist_ok=True)
                    upload_asset_to_release(upload_tpl, local, name)
                    print('    uploaded', name)
