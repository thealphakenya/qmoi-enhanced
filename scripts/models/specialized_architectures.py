...
class QMOILongformerAttention(nn.Module):
    ...
    def compute_global_attention(
        ...
        full_scores = torch.zeros(
            batch_size, seq_len, num_heads, seq_len, device=scores.device
        )
...
class QMOIPerformerAttention(nn.Module):
    ...
    def generate_random_features(self) -> torch.Tensor:
        ...
        if self.kernel_type == "relu":
            ...
            return torch.randn(self.num_random_features, self.head_dim) / math.sqrt(
                self.head_dim
            )

    def compute_performer_attention(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,
...
class QMOIReformerAttention(nn.Module):
    ...
    def compute_reformer_attention(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,
    ...
    def compute_lsh_buckets(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        ...
        # Compute buckets
        buckets = (q_proj > 0).long() * (
            2 ** torch.arange(self.num_hashes, device=q.device)
        )
    ...
    def sort_by_bucket(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, buckets: torch.Tensor
