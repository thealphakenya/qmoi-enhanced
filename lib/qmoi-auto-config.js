...
`;
      ...
      logger.error('❌ QMOI Auto-Configuration failed:', error.message);
      return {
        success: false,
        error: error.message
      };
  ...
  async validateConfiguration() {
    try {
      ...
      if (revenue.mpesa.target < 10000 || revenue.airtel.target < 10000) {
        logger.warning('⚠️  Warning: Daily targets are below required minimum of 10,000 KES');
      }

      if (revenue.mpesa.autoTransfer < 2000 || revenue.airtel.autoTransfer < 2000) {
        logger.warning('⚠️  Warning: Auto-transfer amounts are below required minimum of 2,000 KES');
      }
  ...
  async testMpesaConnectivity() {
    try {
      ...
      logger.error('❌ M-Pesa connectivity test failed:', error.message);
      return {
        success: false,
        error: error.message
      };
  ...
  async testAirtelConnectivity() {
    try {
      ...
      logger.error('❌ Airtel Money connectivity test failed:', error.message);
      return {
        success: false,
        error: error.message
      };
