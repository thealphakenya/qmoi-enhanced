...
class EmitVisitor(asdl.VisitorBase):
    ...
    def metadata(self):
        if self._metadata is None:
            raise ValueError(
                "%s was expecting to be annnotated with metadata" % type(self).__name__
            )
...
class Obj2ModVisitor(PickleVisitor):
    ...
    def recursive_call(self, node, level):
        self.emit(
            "if (_Py_EnterRecursiveCall(\" while traversing '%s' node\")) {" % node,
            level,
            reflow=False,
        )
    ...
    def visitField(self, field, name, sum=None, prod=None, depth=0):
            ...
            if not field.opt:
                ...
                if self.isNumeric(field):
                    if field.name in self.attribute_special_defaults:
                        self.emit(
                            "%s = %s;"
                            % (field.name, self.attribute_special_defaults[field.name]),
                            depth + 1,
                        )
                    else:
        ...
        if field.seq:
            ...
            if self.isSimpleType(field):
                ...
                self.emit(
                    "%s = _Py_asdl_%s_seq_new(len, arena);" % (field.name, field.type),
                    depth + 1,
                )
...
class PyTypesDeclareVisitor(PickleVisitor):
    ...
    def visitProduct(self, prod, name):
        self.emit(
            "static PyObject* ast2obj_%s(struct ast_state *state, struct validator *vstate, void*);"
            % name,
            0,
        )
        if prod.attributes:
    ...
    def visitSum(self, sum, name):
        ...
        self.emit(
            "static PyObject* ast2obj_%s(struct ast_state *state, struct validator *vstate, %s);"
            % (name, ptype),
            0,
        )
        for t in sum.types:
...
""",
    ...
    def visitProduct(self, prod, name):
        ...
        if prod.attributes:
            ...
            self.emit(
                "if (!add_attributes(state, state->%s_type, NULL, 0)) return 0;" % name,
                1,
            )
...
class ObjVisitor(PickleVisitor):
    ...
    def func_begin(self, name):
        ...
        self.emit(
            "ast2obj_%s(struct ast_state *state, struct validator *vstate, void* _o)"
            % (name),
            0,
        )
        self.emit("{", 0)
    ...
    def simpleSum(self, sum, name):
        self.emit(
            "PyObject* ast2obj_%s(struct ast_state *state, struct validator *vstate, %s_ty o)"
            % (name, name),
            0,
        )
        self.emit("{", 0)
    ...
    def set(self, field, value, depth):
        if field.seq:
            if field.type in self.metadata.simple_sums:
                ...
                self.emit(
                    "value = ast2obj_list(state, vstate, (asdl_seq*)%s, ast2obj_%s);"
                    % (value, field.type),
                    depth,
                )
        else:
            self.emit(
                "value = ast2obj_%s(state, vstate, %s);" % (field.type, value),
                depth,
                reflow=False,
            )
...
def generate_module_def(mod, metadata, f, internal_h):
    ...
    module_state.update("%s_singleton" % singleton for singleton in metadata.singletons)
...
def write_header(mod, metadata, f):
    ...
    c = ChainOfVisitors(
        TypeDefVisitor(f), SequenceDefVisitor(f), StructVisitor(f), metadata=metadata
    )
    c.visit(mod)
