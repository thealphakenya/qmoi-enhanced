...
def main_tw():
    ...
    for c1, m in list(cp950encmap.items()):
        for c2, code in list(m.items()):
            if c1 in big5encmap and c2 in big5encmap[c1] and big5encmap[c1][c2] == code:
                del cp950encmap[c1][c2]
    for c1, m in list(cp950decmap.items()):
        for c2, code in list(m.items()):
            if c1 in big5decmap and c2 in big5decmap[c1] and big5decmap[c1][c2] == code:
...
def main_hkscs():
    ...
    hkscsdecmap, hkscsencmap_bmp, hkscsencmap_nonbmp, isbmpmap = make_hkscs_map(table)
