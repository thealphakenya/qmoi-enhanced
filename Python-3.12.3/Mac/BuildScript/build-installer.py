            ...
            [
                dict(
                    ...
                    % {
                        ...
                        "TCL_LIBRARY": shellQuote(
                            "/Library/Frameworks/Python.framework/Versions/%s/lib/tcl8.6"
                            % (getVersion())
                        ),
                    },
                ),
                dict(
                    ...
                    % {
                        ...
                        "TK_LIBRARY": shellQuote(
                            "/Library/Frameworks/Python.framework/Versions/%s/lib/tk8.6"
                            % (getVersion())
                        ),
                    },
                ),
...
def checkEnvironment():
    ...
    if not internalTk():
        ...
        EXPECTED_SHARED_LIBS["_tkinter.so"] = [
            "/Library/Frameworks/Tcl.framework/Versions/%s/Tcl" % frameworks["Tcl"],
            "/Library/Frameworks/Tk.framework/Versions/%s/Tk" % frameworks["Tk"],
        ]
    else:
    ...
    for ev in list(os.environ):
        for prefix in environ_var_prefixes:
            if ev.startswith(prefix):
                print(
                    "INFO: deleting environment variable %s=%s" % (ev, os.environ[ev])
                )
                del os.environ[ev]
...
def parseOptions(args=None):
    ...
    print("")
    print(
        " -- Building a Python %s framework at patch level %s"
...
def extractArchive(builddir, archiveName):
    ...
    try:
        ...
        if archiveName.endswith(".tar.gz"):
            ...
            if (
                retval.startswith("tcl") or retval.startswith("tk")
...
def buildRecipe(recipe, basedir, archList):
    ...
    install = recipe.get(
        "install", "make && make install DESTDIR=%s" % (shellQuote(basedir))
    )
        ...
        if recipe.get("useLDFlags", 1):
            configure_args.extend(
                [
                    ...
                    % (DEPTARGET, shellQuote(basedir)[1:-1], " -arch ".join(archList)),
                ]
            )
        else:
            configure_args.extend(
...
def buildPython():
    ...
    if os.path.exists(os.path.join(WORKDIR, "libraries", "Library")):
        ...
        if internalTk():
            ...
            runCommand(
                "mv %s/pkgconfig/* %s/pkgconfig"
                % (shellQuote(build_lib_dir), shellQuote(fw_lib_dir))
            )
...
def installSize(clear=False, _saved=[]):
    ...
    if not _saved:
        data = captureCommand(
            "du -ks %s" % (shellQuote(os.path.join(WORKDIR, "_root")))
        )
...
def buildDMG():
    ...
    imagepath = os.path.join(
        outdir, "python-%s-%s%s" % (getFullVersion(), os_name, build_system_version)
    )
    ...
    runCommand(
        "SetFile -a C %s/" % (shellQuote(os.path.join(WORKDIR, "mnt", volname)),)
    )

    runCommand(
        "hdiutil detach %s" % (shellQuote(os.path.join(WORKDIR, "mnt", volname)))
    )

    setIcon(imagepath + ".tmp.dmg", "../Icons/Disk Image.icns")
    runCommand(
        "hdiutil convert %s -format UDZO -o %s"
...
def setIcon(filePath, icnsPath):
    ...
    if (
        not os.path.exists(toolPath)
        or os.stat(toolPath).st_mtime < os.stat(dirPath + "/seticon.m").st_mtime
    ):
    ...
    runCommand(
        "%s %s %s"
        % (
            shellQuote(os.path.abspath(toolPath)),
            shellQuote(icnsPath),
            shellQuote(filePath),
        )
    )
...
def main():
    ...
    folder = os.path.join(
        WORKDIR, "_root", "Applications", "Python %s" % (getVersion(),)
    )
