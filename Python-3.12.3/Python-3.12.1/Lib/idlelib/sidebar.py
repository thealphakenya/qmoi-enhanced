...
def get_displaylines(text, index):
    ...
    res = text.count(f"{index} linestart", f"{index} lineend", "displaylines")
...
class BaseSideBar:
    ...
    def redirect_mousewheel_event(self, event):
        ...
        self.text.event_generate("<MouseWheel>", x=0, y=event.y, delta=event.delta)
        return "break"
    ...
    def bind_events(self):
        ...
        def bind_mouse_event(event_name, target_event_name):
            handler = functools.partial(
                self.redirect_mousebutton_event, event_name=target_event_name
            )
        ...
        for button in [2, 3, 4, 5]:
            ...
            for event_name in (
                ...
                bind_mouse_event(event_name, target_event_name=f"<Button-{button}>")
        ...
        def b1_leave_handler(event):
            ...
            if auto_scrolling_after_id is None:
                nonlocal last_y
                last_y = event.y
                auto_scrolling_after_id = self.main_widget.after(0, text_auto_scroll)
...
class LineNumbers(BaseSideBar):
    ...
    def __init__(self, editwin):
        ...
        self.editwin.per.insertfilterafter(end_line_delegator, after=self.editwin.undo)

    def init_widgets(self):
        _padx, pady = get_widget_padding(self.text)
        self.sidebar_text = tk.Text(
            self.parent,
            width=1,
            wrap=tk.NONE,
            padx=2,
...
class ShellSidebar(BaseSideBar):
    ...
    def __init__(self, editwin):
        ...
        change_delegator = WrappedLineHeightChangeDelegator(self.change_callback)
    ...
    def init_widgets(self):
        self.canvas = tk.Canvas(
            self.parent, width=30, borderwidth=0, highlightthickness=0, takefocus=False
        )
        self.update_sidebar()
        self.grid()
        return self.canvas
    ...
    def update_sidebar(self):
        ...
        while (lineinfo := text.dlineinfo(index)) is not None:
            ...
            prompt = (
                ">>>"
                if "console" in prev_newline_tagnames
                else "..." if "stdin" in prev_newline_tagnames else None
            )
            if prompt:
                canvas.create_text(
                    2, y, anchor=tk.NW, text=prompt, font=self.font, fill=self.colors[0]
                )
