...
class LineNumbersTest(unittest.TestCase):
    ...
    def simulate_drag(self, start_line, end_line):
        ...
        self.linenumber.sidebar_text.event_generate("<Button-1>", x=start_x, y=start_y)
        self.root.update()
    ...
    def test_scroll(self):
        ...
        # any change.
        self.linenumber.sidebar_text.event_generate("<MouseWheel>", x=0, y=0, delta=10)
        self.root.update()
...
class ShellSidebarTest(unittest.TestCase):
    ...
    def test_very_long_wrapped_line(self):
        with support.adjust_int_max_str_digits(11_111), swap_attr(
            self.shell, "squeezer", None
        ):
    ...
    def test_copy_with_prompts(self):
        ...
        expected_prompts = [">>>"] + ["..."] * (len(selected_lines) - 2) + [None]
        selected_text_with_prompts = (
            "\n".join(
                line if prompt is None else prompt + " " + line
                for prompt, line in zip(expected_prompts, selected_lines, strict=True)
            )
            + "\n"
        )
