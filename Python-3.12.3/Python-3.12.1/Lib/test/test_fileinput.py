...
class FileInputTests(BaseTests, unittest.TestCase):
    ...
    def test_inplace_encoding_errors(self):
        ...
        with FileInput(
            temp_file, inplace=True, encoding="ascii", errors="replace"
        ) as fobj:
            line = fobj.readline()
    ...
    def test_fileno_when_ValueError_raised(self):
        ...
        # sanity check to make sure that our test scenario was actually hit
        self.assertTrue(
            unconditionally_raise_ValueError.invoked, "_file.fileno() was not invoked"
        )
...
class Test_hook_encoded(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        def check(errors, expected_lines):
            with FileInput(
                files=TESTFN, mode="r", openhook=hook_encoded("utf-8", errors=errors)
            ) as fi:
                lines = list(fi)
    ...
    def test_modes(self):
        ...
        def check(mode, expected_lines):
            with FileInput(
                files=TESTFN, mode=mode, openhook=hook_encoded("utf-7")
            ) as fi:
                lines = list(fi)
            self.assertEqual(lines, expected_lines)
