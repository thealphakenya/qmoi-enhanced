...
def requires_singlephase_init(meth):
    ...
    return unittest.skipIf(
        _testsinglephase is None, "test requires _testsinglephase module"
    )(meth)
...
class ImportTests(unittest.TestCase):
    ...
    def test_issue105979(self):
        ...
        self.assertIn("Frozen object named 'x' is invalid", str(cm.exception))
...
class SubinterpImportTests(unittest.TestCase):
    ...
    def run_here(
        ...
        *,
        ...
        r, w = self.pipe()
        script = self.import_script(name, w, filename, check_singlephase_override)

        ret = run_in_subinterp_with_config(script, **kwargs)
        self.assertEqual(ret, 0)
        return os.read(r, 100)

    def check_compatible_here(
        self,
        name,
        filename=None,
        *,
    ...
    def test_multi_init_extension_per_interpreter_gil_compat(self):
        ...
        with self.subTest(f"{modname}: not isolated, strict"):
            self.check_compatible_here(modname, filename, strict=True, isolated=False)
        with self.subTest(f"{modname}: not isolated, not strict"):
            self.check_compatible_here(modname, filename, strict=False, isolated=False)
...
class SinglephaseInitTests(unittest.TestCase):
    ...
    def check_fresh(self, loaded):
        ...
        self.assertEqual(snap.module._module_initialized, snap.state_initialized)
    ...
    def check_semi_fresh(self, loaded, base, prev):
        ...
        # The module attrs were initialized from that state.
        self.assertEqual(snap.module._module_initialized, snap.state_initialized)
        self.assertNotEqual(snap.state_initialized, base.snapshot.state_initialized)
        self.assertNotEqual(snap.state_initialized, prev.snapshot.state_initialized)
    ...
    def check_copied(self, loaded, base):
        ...
        self.assertEqual(snap.module._module_initialized, snap.state_initialized)
        self.assertEqual(snap.state_initialized, base.snapshot.state_initialized)
    ...
    def test_basic_reloaded(self):
        ...
        for name in [
            ...
            with self.subTest(name):
                ...
                self.assertEqual(id(loaded.module.__dict__), loaded.snapshot.ns_id)
                self.assertEqual(loaded.snapshot.ns.__dict__, loaded.module.__dict__)

                self.assertEqual(reloaded.module.__spec__.name, reloaded.name)
                self.assertEqual(
                    reloaded.module.__name__, reloaded.snapshot.ns.__name__
                )
                ...
                # didn't change.
                self.assertIs(reloaded.snapshot.lookedup, loaded.snapshot.lookedup)
                self.assertEqual(
                    reloaded.snapshot.state_initialized,
                    loaded.snapshot.state_initialized,
                )
                self.assertEqual(
                    reloaded.snapshot.init_count, loaded.snapshot.init_count
                )
    ...
    def test_with_reinit_reloaded(self):
        ...
        for name, has_state in [
            ...
            with self.subTest(name=name, has_state=has_state):
                ...
                self.assertNotEqual(reloaded.module.__dict__, loaded.module.__dict__)
    ...
    @no_rerun(
        reason="rerun not possible; module state is never cleared (see gh-102251)"
    )
    @requires_subinterpreters
