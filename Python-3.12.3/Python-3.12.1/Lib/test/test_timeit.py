...
class TestTimeit(unittest.TestCase):
    ...
    def test_timer_invalid_stmt(self):
        ...
        self.assertRaises(
            SyntaxError, timeit.Timer, setup="while False:\n  pass", stmt="  break"
        )
    ...
    def test_timeit_function_zero_iters(self):
        delta_time = timeit.timeit(
            self.fake_stmt, self.fake_setup, number=0, timer=FakeTimer()
        )
    ...
    def test_timeit_globals_args(self):
        ...
        timeit.timeit(
            stmt="_global_timer.inc()", timer=_global_timer, globals=globals(), number=3
        )
        local_timer = FakeTimer()
        timeit.timeit(
            stmt="local_timer.inc()", timer=local_timer, globals=locals(), number=3
        )
    ...
    def test_repeat_callable_stmt_and_setup(self):
        self.repeat(
            self.fake_callable_stmt, self.fake_callable_setup, repeat=3, number=5
        )
    ...
    def test_repeat_function_zero_reps(self):
        delta_times = timeit.repeat(
            self.fake_stmt, self.fake_setup, repeat=0, timer=FakeTimer()
        )
    ...
    def test_main_setup(self):
        s = self.run_main(
            seconds_per_increment=2.0, switches=["-n35", "-s", 'print("CustomSetup")']
        )
        self.assertEqual(
            s,
