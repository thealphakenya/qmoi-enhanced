...
class LongTest(unittest.TestCase):
    ...
    def test_division(self):
        digits = list(range(1, MAXDIGITS + 1)) + list(
        ...
        self.check_division(
            1115141373653752303710932756325578065, 1038556335171453937726882627
        )
        # 30-bit cases that require the post-subtraction correction step
        self.check_division(
            922498905405436751940989320930368494, 949985870686786135626943396
        )
        self.check_division(
            768235853328091167204009652174031844, 1091555541180371554426545266
        )
    ...
    def test_karatsuba(self):
        digits = list(range(1, 5)) + list(
            range(KARATSUBA_CUTOFF, KARATSUBA_CUTOFF + 10)
        )
    ...
    def test_float_overflow(self):
        ...
        for test in [
            ...
            "math.sin(mhuge)",
            "math.sqrt(huge)",
            "math.sqrt(mhuge)",  # should do better
        ...
        self.assertNotEqual(
            float(shuge), int(shuge), "float(shuge) should not equal int(shuge)"
        )
    ...
    def test_mixed_compares(self):
        ...
        for t in 2.0**48, 2.0**50, 2.0**53:
            cases.extend(
                [t - 1.0, t - 0.3, t, t + 0.3, t + 1.0, int(t - 1), int(t), int(t + 1)]
            )
    ...
    def test__format__(self):
        ...
        self.assertRaisesRegex(
            ValueError, "Cannot specify ',' with 's'", format, 3, ",s"
        )
        self.assertRaisesRegex(
            ValueError, "Cannot specify '_' with 's'", format, 3, "_s"
        )
        ...
        for format_spec in "eEfFgG%":
            for value in [0, 1, -1, 100, -100, 1234567890, -1234567890]:
                self.assertEqual(
                    format(value, format_spec), format(float(value), format_spec)
                )
    ...
    def test_correctly_rounded_true_division(self):
        ...
        for n in range(250):
            ...
            # up instead.
            self.check_truediv(
                (2**DBL_MANT_DIG + 1) * 12345 * 2**200 + 2**n, 2**DBL_MANT_DIG * 12345
            )
    ...
    def test_bit_length(self):
        ...
        for x in range(-65000, 65000):
            ...
            if x != 0:
                ...
                self.assertEqual(
                    k, 1 + math.floor(math.log(abs(x)) / math.log(2) + tiny)
                )
            ...
            for test, expected in tests.items():
                try:
                    ...
                    raise AssertionError(
                        "failed to convert {} with byteorder={} and signed={}".format(
                ...
                if len(expected) == 1 and byteorder == "big" and not signed:
                    try:
                        ...
                        raise AssertionError(
                            "failed to convert {} with default arguments".format(test)
                        ) from err
                ...
                try:
                    ...
                    raise AssertionError(
                        ...
                        "signed={2}".format(test, byteorder, signed)
                    ) from err
    ...
    def test_from_bytes(self):
        def check(tests, byteorder, signed=False):
            ...
            for test, expected in tests.items():
                try:
                    ...
                    raise AssertionError(
                        "failed to convert {} with byteorder={!r} and signed={}".format(
                            test, byteorder, signed
                        )
                    ) from err
                ...
                if byteorder == "big" and not signed:
                    try:
                        self.assertEqual(int.from_bytes(test), expected)
                    except Exception as err:
                        raise AssertionError(
                            "failed to convert {} with default arguments".format(test)
                        ) from err
                ...
                try:
                    ...
                    raise AssertionError(
                        "Code equivalent from docs is not equivalent for "
                        "conversion of {0} with byteorder={1!r} and signed={2}".format(
                            test, byteorder, signed
                        )
                    ) from err
        ...
        self.assertEqual(int.from_bytes((255, 0, 0), "big", signed=True), -65536)
        self.assertEqual(
            int.from_bytes(bytearray(b"\xff\x00\x00"), "big", signed=True), -65536
        )
        self.assertEqual(
            int.from_bytes(bytearray(b"\xff\x00\x00"), "big", signed=True), -65536
        )
        ...
        self.assertEqual(
            int.from_bytes(memoryview(b"\xff\x00\x00"), "big", signed=True), -65536
        )
    ...
    def test_square(self):
        ...
        for bitlen in range(1, 400):
            ...
            with self.subTest(bitlen=bitlen, n=n):
                ...
                self.assertEqual(n**2, (1 << (2 * bitlen)) - (1 << (bitlen + 1)) + 1)
    ...
    def test___sizeof__(self):
        ...
        for value, ndigits in test_values:
            with self.subTest(value):
                self.assertEqual(
                    value.__sizeof__(), int.__basicsize__ + int.__itemsize__ * ndigits
                )
