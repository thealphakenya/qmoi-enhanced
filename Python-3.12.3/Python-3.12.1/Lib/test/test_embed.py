...
class EmbeddingTestsMixin:
    ...
    def run_embedded_interpreter(
        self, *args, env=None, timeout=None, returncode=0, input=None, cwd=None
    ):
...
class EmbeddingTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    def test_initialize_pymain(self):
        ...
        self.assertEqual(
            out.rstrip(), "Py_Main() after Py_Initialize: sys.argv=['-c', 'arg2']"
        )
        self.assertEqual(err, "")
...
class InitConfigTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    def check_all_configs(
        ...
        *,
        ...
        self.get_expected_config(
            expected_preconfig, expected_config, env, api, modify_path_cb
        )
    ...
    def test_init_global_config(self):
        ...
        self.check_all_configs(
            "test_init_global_config", config, preconfig, api=API_COMPAT
        )
    ...
    def test_init_from_config(self):
        ...
        self.check_all_configs(
            "test_init_from_config", config, preconfig, api=API_COMPAT
        )
    ...
    def test_init_compat_env(self):
        ...
        self.check_all_configs(
            "test_init_compat_env", config, preconfig, api=API_COMPAT
        )
    ...
    def test_init_python_env(self):
        ...
        self.check_all_configs(
            "test_init_python_env", config, preconfig, api=API_PYTHON
        )
    ...
    def test_preinit_parse_argv(self):
        ...
        self.check_all_configs(
            "test_preinit_parse_argv", config, preconfig, api=API_PYTHON
        )
    ...
    def test_preinit_dont_parse_argv(self):
        ...
        argv = ["python3", "-E", "-I", "-P", "-X", "dev", "-X", "utf8", "script.py"]
        config = {
            "argv": argv,
            "orig_argv": argv,
            "isolated": 0,
        ...
        self.check_all_configs(
            "test_preinit_dont_parse_argv", config, preconfig, api=API_ISOLATED
        )
    ...
    def test_init_dont_configure_locale(self):
        ...
        self.check_all_configs(
            "test_init_dont_configure_locale", {}, preconfig, api=API_PYTHON
        )

    @unittest.skip(
        "as of 3.11 this test no longer works because "
    ...
    def test_init_read_set(self):
        ...
        self.check_all_configs(
            "test_init_read_set", config, api=API_PYTHON, modify_path_cb=modify_path
        )
    ...
    def test_init_dont_parse_argv(self):
        ...
        self.check_all_configs(
            "test_init_dont_parse_argv", config, pre_config, api=API_PYTHON
        )
    ...
    def test_init_setpath(self):
        ...
        self.check_all_configs(
            "test_init_setpath", config, api=API_COMPAT, env=env, ignore_stderr=True
        )
    ...
    def module_search_paths(self, prefix=None, exec_prefix=None):
        ...
        if MS_WINDOWS:
            ...
            return [
                os.path.join(
                    prefix, sys.platlibdir, f"python{ver.major}{ver.minor}.zip"
                ),
                os.path.join(prefix, sys.platlibdir, f"python{ver.major}.{ver.minor}"),
                os.path.join(
                    exec_prefix,
                    sys.platlibdir,
            ...
            if MS_WINDOWS:
                ...
                for f in os.listdir(exedir):
                    if fnmatch.fnmatch(f, "*.dll"):
                        shutil.copyfile(
                            os.path.join(exedir, f), os.path.join(tmpdir, f)
                        )
    ...
    def test_init_setpythonhome(self):
        ...
        self.check_all_configs(
            "test_init_setpythonhome", config, api=API_COMPAT, env=env
        )
    ...
    def test_init_is_python_build_with_home(self):
        ...
        self.check_all_configs(
            "test_init_is_python_build", config, api=API_COMPAT, env=env
        )
    ...
    def test_init_pybuilddir_win32(self):
        ...
        with self.tmpdir_with_python(subdir) as tmpdir:
            ...
            module_search_paths = self.module_search_paths()
            module_search_paths[-3] = os.path.join(
                tmpdir, os.path.basename(module_search_paths[-3])
            )
    ...
    def test_global_pathconfig(self):
        ...
        self.assertEqual(
            Py_GetPath().split(os.path.pathsep), config["module_search_paths"]
        )
    ...
    def test_init_warnoptions(self):
        ...
        config = {
            ...
            "orig_argv": ["python3", "-Wignore:::cmdline1", "-Wignore:::cmdline2"],
        }
        self.check_all_configs(
            "test_init_warnoptions", config, preconfig, api=API_PYTHON
        )
    ...
    def test_init_use_frozen_modules(self):
        ...
        for raw, expected in tests:
            ...
            with self.subTest(repr(raw)):
                self.check_all_configs(
                    "test_init_use_frozen_modules", config, api=API_PYTHON, env=env
                )
    ...
    def test_init_main_interpreter_settings(self):
        ...
        expected = {
            ...
            "feature_flags": OBMALLOC | FORK | EXEC | THREADS | DAEMON_THREADS,
            "own_gil": True,
        }
        out, err = self.run_embedded_interpreter(
...
class AuditingTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    def test_audit_run_command(self):
        self.run_embedded_interpreter(
            "test_audit_run_command", timeout=support.SHORT_TIMEOUT, returncode=1
        )

    def test_audit_run_file(self):
        self.run_embedded_interpreter(
            "test_audit_run_file", timeout=support.SHORT_TIMEOUT, returncode=1
        )
    ...
    def test_audit_run_stdin(self):
        self.run_embedded_interpreter(
            "test_audit_run_stdin", timeout=support.SHORT_TIMEOUT, returncode=1
        )
...
class MiscTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    # See bpo-44133
    @unittest.skipIf(os.name == "nt", "Py_FrozenMain is not exported on Windows")
    ...
    @unittest.skipUnless(
        support.Py_DEBUG, "-X showrefcount requires a Python debug build"
    )
