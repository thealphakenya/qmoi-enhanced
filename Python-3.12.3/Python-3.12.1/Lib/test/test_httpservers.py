...
class BaseHTTPServerTestCase(BaseTestCase):
    ...
    def test_latin1_header(self):
        self.con.request(
            "LATINONEHEADER", "/", headers={"X-Special-Incoming": "Ärger mit Unicode"}
        )
        res = self.con.getresponse()
...
class SimpleHTTPServerTestCase(BaseTestCase):
    ...
    @unittest.skipIf(
        sys.platform == "darwin", "undecodable name cannot always be decoded on macOS"
    )
    @unittest.skipIf(
        sys.platform == "win32", "undecodable name cannot be decoded on win32"
    )
    @unittest.skipUnless(
        os_helper.TESTFN_UNDECODABLE, "need os_helper.TESTFN_UNDECODABLE"
    )
    def test_undecodable_filename(self):
        ...
        self.check_status_and_reason(
            response, HTTPStatus.OK, data=os_helper.TESTFN_UNDECODABLE
        )
    ...
    def test_undecodable_parameter(self):
        ...
        self.assertRegex(
            response, rf"listing for {self.base_url}/\?x=123".encode("latin1")
        )
        # now the bogus encoding
        response = self.request(self.base_url + "/?x=%bb").read()
        self.assertRegex(
            response, rf"listing for {self.base_url}/\?x=\xef\xbf\xbd".encode("latin1")
        )
    ...
    def test_get_dir_redirect_location_domain_injection_bug(self):
        ...
        expected_location = (
            f"{url}/"  # /python.org.../ single slash single prefix, trailing slash
        )
