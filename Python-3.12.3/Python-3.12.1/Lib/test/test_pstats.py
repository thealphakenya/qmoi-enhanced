...
class StatsTestCase(unittest.TestCase):
    ...
    def test_sort_stats_int(self):
        ...
        for arg_int, arg_str in valid_args.items():
            ...
            self.assertEqual(
                self.stats.sort_type, self.stats.sort_arg_dict_default[arg_str][-1]
            )
    ...
    def test_sort_stats_string(self):
        for sort_name in [
            ...
            self.assertEqual(
                self.stats.sort_type, self.stats.sort_arg_dict_default[sort_name][-1]
            )
    ...
    def test_sort_stats_partial(self):
        ...
        for sort_name in [
            ...
            self.stats.sort_stats(sort_name)
            self.assertEqual(
                self.stats.sort_type, self.stats.sort_arg_dict_default[sortkey][-1]
            )
    ...
    def test_sort_stats_enum(self):
        for member in SortKey:
            ...
            self.assertEqual(
                self.stats.sort_type, self.stats.sort_arg_dict_default[member.value][-1]
            )
