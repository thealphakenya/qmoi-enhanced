...
def unsorted_keyword_only_parameters_fn(
    *, throw, out, the, baby, with_, the_, bathwater
):
    pass
...
class TestPredicates(IsTestBase):
    ...
    def test_iscoroutine(self):
        ...
        self.assertFalse(
            inspect.iscoroutinefunction(
                functools.partial(functools.partial(gen_coroutine_function_example))
            )
        )
        ...
        self.assertTrue(
            inspect.iscoroutinefunction(inspect.markcoroutinefunction(lambda: _fn3()))
        )
        ...
        self.assertTrue(
            inspect.iscoroutinefunction(inspect.markcoroutinefunction(Cl()))
        )
        ...
        # unless explicitly marked.
        self.assertTrue(
            inspect.iscoroutinefunction(inspect.markcoroutinefunction(Cl2()))
        )
        ...
        self.assertTrue(
            inspect.isasyncgenfunction(
                functools.partial(functools.partial(async_generator_function_example))
            )
        )
    ...
    def test_isawaitable(self):
        ...
        coro.close()
        gen_coro.close()  # silence warnings
...
class TestInterpreterStack(IsTestBase):
    ...
    def test_trace(self):
        ...
        self.assertEqual(
            revise(*frame2[1:]), (modfile, 9, "spam", ["    eggs(b + d, c + f)\n"], 0)
        )
        self.assertEqual(frame2.positions, dis.Positions(9, 9, 4, 22))
        self.assertEqual(
            revise(*frame3[1:]), (modfile, 18, "eggs", ["    q = y / 0\n"], 0)
        )
    ...
    def test_frame(self):
        ...
        self.assertEqual(
            inspect.formatargvalues(args, varargs, varkw, locals), "(x=11, y=14)"
        )
...
class TestRetrievingSourceCode(GetSourceBase):
                ...
                [
                    ...
                    [
                        ...
                        [
                            (
                                mod.FesteringGob,
                                (mod.MalodorousPervert, mod.ParrotDroppings),
                            )
                        ],
                    ],
                    (
    ...
    def test_getfunctions(self):
        ...
        self.assertEqual(
            functions,
            [
                ("after_closing", mod.after_closing),
                ("eggs", mod.eggs),
                ("lobbest", mod.lobbest),
                ("spam", mod.spam),
            ],
    ...
    def test_getdoc_inherited(self):
        ...
        self.assertEqual(
            inspect.getdoc(mod.FesteringGob.abuse),
            "Another\n\ndocstring\n\ncontaining\n\ntabs",
        )
        self.assertEqual(
            inspect.getdoc(mod.FesteringGob().abuse),
        ...
        self.assertEqual(
            inspect.getdoc(mod.FesteringGob.contradiction), "The automatic gainsaying."
        )
...
class TestBuggyCases(GetSourceBase):
    ...
    @unittest.skipIf(
        not hasattr(unicodedata, "__file__") or unicodedata.__file__.endswith(".py"),
    ...
    @unittest.skipIf(
        support.is_emscripten or support.is_wasi, "socket.accept is broken"
    )
...
class TestNoEOL(GetSourceBase):
    def setUp(self):
        ...
        with open(
            os.path.join(self.tempdir, "inspect_fodder3%spy" % os.extsep),
            "w",
            encoding="utf-8",
        ) as f:
            f.write("class X:\n    pass # No EOL")
...
class TestClassesAndFunctions(unittest.TestCase):
    ...
    def test_getfullargspec(self):
        ...
        self.assertFullArgSpecEquals(mod2.annotated, ["arg1"], ann_e={"arg1": list})
        self.assertFullArgSpecEquals(mod2.keyword_only_arg, [], kwonlyargs_e=["arg"])

        self.assertFullArgSpecEquals(
            mod2.all_markers, ["a", "b", "c", "d"], kwonlyargs_e=["e", "f"]
        )

        self.assertFullArgSpecEquals(
            mod2.all_markers_with_args_and_kwargs,
            ["a", "b", "c", "d"],
            varargs_e="args",
            varkw_e="kwargs",
            kwonlyargs_e=["e", "f"],
        ...
        self.assertFullArgSpecEquals(
            mod2.all_markers_with_defaults,
            ["a", "b", "c", "d"],
            defaults_e=(1, 2, 3),
            kwonlyargs_e=["e", "f"],
    ...
    def test_argspec_api_ignores_wrapped(self):
        ...
        self.assertFullArgSpecEquals(functools.partial(ham), ["x", "y"])
    ...
    def test_getfullargspec_builtin_methods(self):
        ...
        self.assertFullArgSpecEquals(
            _pickle.Pickler(io.BytesIO()).dump, ["self", "obj"]
        )
    ...
    def test_classify_builtin_types(self):
        ...
        self.assertIn(
            ("from_bytes", "class method", int), attrs, "missing class method"
        )
    ...
    def test_getmembers_descriptors(self):
        ...
        self.assertEqual(
            inspect.getmembers(A, ismethoddescriptor), [("md", A.__dict__["md"])]
        )
        self.assertEqual(
            inspect.getmembers(A, isdatadescriptor), [("dd", A.__dict__["dd"])]
        )
        ...
        self.assertEqual(
            inspect.getmembers(B, ismethoddescriptor), [("md", A.__dict__["md"])]
        )
        self.assertEqual(
            inspect.getmembers(B, isdatadescriptor), [("dd", A.__dict__["dd"])]
        )
    ...
    def test_get_annotations_with_stock_annotations(self):
        ...
        self.assertEqual(
            inspect.get_annotations(foo, eval_str=True, locals=locals()),
            {"a": foo, "b": str},
        )
        self.assertEqual(
            inspect.get_annotations(foo, eval_str=True, globals=locals()),
            {"a": foo, "b": str},
        )
        ...
        self.assertEqual(
            inspect.get_annotations(isa.function),
            {"a": int, "b": str, "return": isa.MyClass},
        )
        self.assertEqual(
            inspect.get_annotations(isa.function2),
            {"a": int, "b": "str", "c": isa.MyClass, "return": isa.MyClass},
        )
        self.assertEqual(
            inspect.get_annotations(isa.function3),
            {"a": "int", "b": "str", "c": "MyClass"},
        )
        self.assertEqual(
            inspect.get_annotations(inspect), {}
        ...
        self.assertEqual(
            inspect.get_annotations(isa, eval_str=True), {"a": int, "b": str}
        )
        self.assertEqual(
            inspect.get_annotations(isa.MyClass, eval_str=True), {"a": int, "b": str}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            inspect.get_annotations(isa, eval_str=False), {"a": int, "b": str}
        )
        self.assertEqual(
            inspect.get_annotations(isa.MyClass, eval_str=False), {"a": int, "b": str}
        )
        self.assertEqual(
            inspect.get_annotations(isa.function, eval_str=False),
            {"a": int, "b": str, "return": isa.MyClass},
        )
        self.assertEqual(
            inspect.get_annotations(isa.function2, eval_str=False),
            {"a": int, "b": "str", "c": isa.MyClass, "return": isa.MyClass},
        )
        self.assertEqual(
            inspect.get_annotations(isa.function3, eval_str=False),
            {"a": "int", "b": "str", "c": "MyClass"},
        )
        self.assertEqual(inspect.get_annotations(inspect, eval_str=False), {})
        self.assertEqual(
            inspect.get_annotations(isa.UnannotatedClass, eval_str=False), {}
        )
        self.assertEqual(
            inspect.get_annotations(isa.unannotated_function, eval_str=False), {}
        )
        ...
        self.assertEqual(
            inspect.get_annotations(wrapped, eval_str=False),
            {"a": int, "b": str, "return": isa.MyClass},
        )
    ...
    def test_get_annotations_with_stringized_annotations(self):
        ...
        self.assertEqual(
            inspect.get_annotations(isa.MyClass, eval_str=False),
            {"a": "int", "b": "str"},
        )
        self.assertEqual(
            inspect.get_annotations(isa.function, eval_str=False),
        ...
        self.assertEqual(
            inspect.get_annotations(isa.MyClassWithLocalAnnotations), {"x": "mytype"}
        )
        self.assertEqual(
...
class TestFormatAnnotation(unittest.TestCase):
    def test_typing_replacement(self):
        ...
        self.assertEqual(
            inspect.formatannotation(ann1), "Union[List[testModule.typing.A], int]"
        )
...
class TestIsDataDescriptor(unittest.TestCase):
    ...
    def test_custom_descriptors(self):
        ...
        self.assertFalse(
            inspect.isdatadescriptor(NonDataDescriptor()),
            "class with only __get__ not a data descriptor",
        )
        self.assertTrue(
            inspect.isdatadescriptor(DataDescriptor0()),
            "class with __set__ is a data descriptor",
        )
        self.assertTrue(
            inspect.isdatadescriptor(DataDescriptor1()),
            "class with __delete__ is a data descriptor",
        )
        self.assertTrue(
            inspect.isdatadescriptor(DataDescriptor2()),
    ...
    def test_slot(self):
        ...
        self.assertTrue(
            inspect.isdatadescriptor(Slotted.foo), "a slot is a data descriptor"
        )
    ...
    def test_property(self):
        ...
        self.assertTrue(
            inspect.isdatadescriptor(Propertied.a_property),
    ...
    def test_functions(self):
        ...
        self.assertFalse(
            inspect.isdatadescriptor(Test().instance_method),
            "a instance method is not a data descriptor",
        )
        self.assertFalse(
            inspect.isdatadescriptor(Test().class_method),
            "a class method is not a data descriptor",
        )
        self.assertFalse(
            inspect.isdatadescriptor(Test().static_method),
            "a static method is not a data descriptor",
        )
        self.assertFalse(
            inspect.isdatadescriptor(function), "a function is not a data descriptor"
        )
        self.assertFalse(
            inspect.isdatadescriptor(a_lambda), "a lambda is not a data descriptor"
        )
...
class TestGetcallargsFunctions(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "value 5 is not a valid Parameter.kind"
        ):
...
class TestGetGeneratorState(unittest.TestCase):
    ...
    def test_getgeneratorlocals(self):
        ...
        numbers = each([1, 2, 3])
        self.assertEqual(
            inspect.getgeneratorlocals(numbers), {"a": None, "lst": [1, 2, 3]}
        )
        next(numbers)
        self.assertEqual(
            inspect.getgeneratorlocals(numbers),
            {"a": None, "lst": [1, 2, 3], "v": 1, "b": (1, 2, 3)},
        )
        next(numbers)
        self.assertEqual(
            inspect.getgeneratorlocals(numbers),
            {"a": None, "lst": [1, 2, 3], "v": 2, "b": (1, 2, 3)},
        )
        next(numbers)
        self.assertEqual(
            inspect.getgeneratorlocals(numbers),
            {"a": None, "lst": [1, 2, 3], "v": 3, "b": (1, 2, 3), "c": 12},
        )
        try:
            next(numbers)
...
class TestGetCoroutineState(unittest.TestCase):
    ...
    def test_getcoroutinelocals(self):
        ...
        coro = func()
        self.assertEqual(
            inspect.getcoroutinelocals(coro), {"a": None, "gencoro": gencoro}
        )
        coro.send(None)
        self.assertEqual(
...
class TestGetAsyncGenState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_getasyncgenlocals(self):
        ...
        numbers = each([1, 2, 3])
        self.assertEqual(
            inspect.getasyncgenlocals(numbers), {"a": None, "lst": [1, 2, 3]}
        )
        await anext(numbers)
        self.assertEqual(
            inspect.getasyncgenlocals(numbers),
            {"a": None, "lst": [1, 2, 3], "v": 1, "b": (1, 2, 3)},
        )
        await anext(numbers)
        self.assertEqual(
...
class TestSignatureObject(unittest.TestCase):
        ...
        return (
            tuple(
                (
                    ...
                    (... if param.annotation is param.empty else param.annotation),
                    str(param.kind).lower(),
                )
                for param in sig.parameters.values()
            ),
            (... if sig.return_annotation is sig.empty else sig.return_annotation),
        )
    ...
    def test_signature_object_pickle(self):
        ...
        mysig = MySignature().replace(
            parameters=myparams.values(), return_annotation=sig.return_annotation
        )
    ...
    def test_signature_on_wkwonly(self):
        ...
        self.assertEqual(
            self.signature(test),
            ((("a", ..., float, "keyword_only"), ("b", ..., str, "keyword_only")), int),
        )
    ...
    def test_signature_without_self(self):
        ...
        self.assertEqual(
            self.signature(test_args_only),
            ((("args", ..., ..., "var_positional"),), ...),
        )
        self.assertEqual(
            self.signature(test_args_kwargs_only),
            (
                (
                    ("args", ..., ..., "var_positional"),
                    ("kwargs", ..., ..., "var_keyword"),
        ...
        self.assertEqual(
            self.signature(A.f1), ((("args", ..., ..., "var_positional"),), ...)
        )
        self.assertEqual(
            self.signature(A.f2), ((("args", ..., ..., "var_positional"),), ...)
        )
        self.assertEqual(
            self.signature(A.f3), ((("args", ..., ..., "var_positional"),), ...)
        )
        self.assertEqual(
            self.signature(A.f4),
            (
    ...
    def test_signature_on_decorated_builtins(self):
        ...
        self.assertEqual(
            inspect.signature(decorated_func, follow_wrapped=False),
    ...
    def test_signature_on_method(self):
        ...
        self.assertEqual(
            self.signature(Test().m2), ((("args", ..., ..., "var_positional"),), ...)
        )

        self.assertEqual(
            self.signature(Test), ((("args", ..., ..., "var_positional"),), ...)
        )
    ...
    def test_signature_on_partial(self):
        ...
        self.assertEqual(
            self.signature(partial(test, 1, c=2)),
            (
                (
                    ("b", ..., ..., "positional_or_keyword"),
                    ("c", 2, ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, b=1, c=2)),
            (
                (
                    ("a", ..., ..., "positional_or_keyword"),
                    ("b", 1, ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, 0, b=1, c=2)),
            (
                (
                    ("b", 1, ..., "keyword_only"),
                    ("c", 2, ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, a=1)),
            (
                (
                    ("a", 1, ..., "keyword_only"),
                    ("b", ..., ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, 1, 2, 3)),
            (
                (
                    ("args", ..., ..., "var_positional"),
                    ("b", ..., ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, 1, 2, 3, test=True)),
            (
                (
                    ("args", ..., ..., "var_positional"),
                    ("b", ..., ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, 1, 2, 3, test=1, b=0)),
            (
                (
                    ("args", ..., ..., "var_positional"),
                    ("b", 0, ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(test, b=0)),
            (
                (
                    ("a", ..., ..., "positional_or_keyword"),
                    ("args", ..., ..., "var_positional"),
        ...
        self.assertEqual(
            self.signature(partial(test, b=0, test=1)),
            (
                (
                    ("a", ..., ..., "positional_or_keyword"),
                    ("args", ..., ..., "var_positional"),
        ...
        self.assertEqual(
            self.signature(partial(partial(test, 1))),
            (
                (
                    ("b", ..., ..., "positional_or_keyword"),
        ...
        self.assertEqual(
            self.signature(partial(partial(test, 1), 2)),
        ...
        self.assertEqual(
            self.signature(_foo),
            ((("b", 30, ..., "keyword_only"), ("c", ..., ..., "keyword_only")), ...),
        )
        ...
        self.assertEqual(
            self.signature(partial(foo, 1, 2)),
            (
                (
                    ("c", ..., ..., "positional_or_keyword"),
                    ("d", ..., ..., "positional_or_keyword"),
        ...
        self.assertEqual(
            self.signature(partial(foo, 1, 2, 3)),
            (
                (
                    ("d", ..., ..., "positional_or_keyword"),
                    ("kwargs", ..., ..., "var_keyword"),
        ...
        self.assertEqual(
            self.signature(partial(foo, 1, 2, c=3)),
            (
                (
                    ("c", 3, ..., "keyword_only"),
                    ("d", ..., ..., "keyword_only"),
        ...
        self.assertEqual(
            self.signature(partial(foo, 1, c=3)),
            (
                (
                    ("b", ..., ..., "positional_only"),
                    ("c", 3, ..., "keyword_only"),
    ...
    def test_signature_on_partialmethod(self):
        ...
        self.assertEqual(
            self.signature(Spam.ham, eval_str=False),
            (
                (
        ...
        self.assertEqual(
            self.signature(Spam.g, eval_str=False),
    ...
    def test_signature_on_decorated(self):
        ...
        self.assertEqual(
            self.signature(Foo.bar, follow_wrapped=False),
            (
                (
                    ("args", ..., ..., "var_positional"),
                    ("kwargs", ..., ..., "var_keyword"),
        ...
        self.assertEqual(
            self.signature(wrapped_foo_call),
            (
                (
                    ("a", ..., ..., "positional_or_keyword"),
                    ("b", ..., ..., "positional_or_keyword"),
    ...
    def test_signature_on_class(self):
        ...
        self.assertEqual(
            self.signature(C), ((("a", ..., ..., "positional_or_keyword"),), ...)
        )
        ...
        self.assertEqual(
            self.signature(C), ((("b", ..., ..., "positional_or_keyword"),), ...)
        )
    ...
    def test_signature_on_subclass(self):
        ...
        self.assertEqual(
            self.signature(B), ((("b", ..., ..., "positional_or_keyword"),), ...)
        )
        self.assertEqual(
            self.signature(C),
            (
    ...
    def test_signature_on_callable_objects(self):
        ...
        self.assertEqual(
            self.signature(Foo()), ((("a", ..., ..., "positional_or_keyword"),), ...)
        )
        ...
        self.assertEqual(
            self.signature(Bar()), ((("a", ..., ..., "positional_or_keyword"),), ...)
        )
        ...
        self.assertEqual(
            self.signature(Wrapped), ((("a", ..., ..., "positional_or_keyword"),), ...)
        )
        # wrapper loop:
    ...
    def test_signature_hashable(self):
        ...
        self.assertNotEqual(
            hash(foo_sig), hash(manual_sig.replace(return_annotation="spam"))
        )
    ...
    def test_signature_str(self):
        ...
        self.assertEqual(
            str(inspect.signature(foo)), "(a: int = 1, *, b, c=None, **kwargs) -> 42"
        )
        ...
        self.assertEqual(
            str(inspect.signature(foo)), "(a: list[str]) -> tuple[str, float]"
        )
    ...
    def test_signature_str_positional_only(self):
        ...
        self.assertEqual(
            str(
                S(parameters=[P("foo", P.POSITIONAL_ONLY), P("bar", P.VAR_POSITIONAL)])
            ),
            "(foo, /, *bar)",
        )
    ...
    def test_signature_eval_str(self):
        ...
        for signature_func in (inspect.signature, inspect.Signature.from_callable):
            with self.subTest(signature_func=signature_func):
                ...
                self.assertEqual(
                    signature_func(isa.unannotated_function),
                    sig(
                        parameters=(
                            par("a", PORK),
                            par("b", PORK),
                ...
                self.assertEqual(
                    signature_func(
                        isa.function, eval_str=True, globals=globalns, locals=localns
                    ),
                    sig(
                        return_annotation=dict,
                        parameters=(
...
class TestParameterObject(unittest.TestCase):
    ...
    def test_signature_parameter_object(self):
        ...
        p = inspect.Parameter(
            "a", default=42, kind=inspect.Parameter.POSITIONAL_OR_KEYWORD
        )
        with self.assertRaisesRegex(ValueError, "cannot have default values"):
    ...
    def test_signature_parameter_hashable(self):
        ...
        self.assertNotEqual(
            hash(foo), hash(P("foo", kind=P.POSITIONAL_ONLY, default=42))
        )
...
class TestSignatureBind(unittest.TestCase):
    ...
    def test_signature_bind_just_kwargs(self):
        ...
        self.assertEqual(self.call(test), {})
        self.assertEqual(
            self.call(test, foo="bar", spam="ham"), {"foo": "bar", "spam": "ham"}
        )
    ...
    def test_signature_bind_args_and_kwargs(self):
        ...
        self.assertEqual(self.call(test, 1, 2), (1, 2, 3, {}))
        self.assertEqual(
            self.call(test, 1, 2, foo="bar", spam="ham"),
            (1, 2, 3, {"foo": "bar", "spam": "ham"}),
        )
        self.assertEqual(
            self.call(test, b=2, a=1, foo="bar", spam="ham"),
            (1, 2, 3, {"foo": "bar", "spam": "ham"}),
        )
        self.assertEqual(
            self.call(test, a=1, b=2, foo="bar", spam="ham"),
            (1, 2, 3, {"foo": "bar", "spam": "ham"}),
        )
        self.assertEqual(
            self.call(test, 1, b=2, foo="bar", spam="ham"),
            (1, 2, 3, {"foo": "bar", "spam": "ham"}),
        )
        self.assertEqual(
    ...
    def test_signature_bind_kwonly(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "missing a required keyword-only " "argument: 'bar'"
        ):
            self.call(test, 1)
        ...
        self.assertEqual(
            self.call(test, spam="ham", foo=1, bar=2), (1, 2, {"spam": "ham"})
        )
        with self.assertRaisesRegex(TypeError, "missing a required argument: 'foo'"):
            self.call(test, spam="ham", bar=2)
        self.assertEqual(
            self.call(test, 1, bar=2, bin=1, spam=10), (1, 2, {"bin": 1, "spam": 10})
        )
...
class TestSignaturePrivateHelpers(unittest.TestCase):
    def _strip_non_python_syntax(self, input, clean_signature, self_parameter):
        computed_clean_signature, computed_self_parameter = (
    ...
    def test_signature_strip_non_python_syntax(self):
        ...
        self._strip_non_python_syntax(
            "($module, word, salt, /)", "(module, word, salt, /)", 0
        )

        self._strip_non_python_syntax(
            "(x, y=None, z=None, /)", "(x, y=None, z=None, /)", None
        )
        ...
        self._strip_non_python_syntax("", "", None)

        self._strip_non_python_syntax(None, None, None)
...
def foo():
    ...
    def assertInspectEqual(self, path, source):
        ...
        with open(path, encoding="utf-8") as src:
            self.assertEqual(
                src.read().splitlines(True), inspected_src.splitlines(True)
            )
