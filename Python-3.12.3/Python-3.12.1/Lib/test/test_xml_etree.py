...
class ElementTreeTest(unittest.TestCase):
    ...
    def test_simpleops(self):
        ...
        self.serialize_check(
            element, '<tag key="value"><subtag /><subtag /></tag>'
        )  # 3
    ...
    def test_cdata(self):
        ...
        self.serialize_check(
            ET.XML("<tag>&#104;&#101;&#108;&#108;&#111;</tag>"), "<tag>hello</tag>"
        )
    ...
    def test_attrib(self):
        ...
        self.assertEqual(
            ET.tostring(elem), b'<test testa="testval" testb="test2">aa</test>'
        )
    ...
    def test_parseliteral(self):
        ...
        self.assertEqual(
            ET.tostring(element, encoding="unicode"), "<html><body>text</body></html>"
        )
        ...
        self.assertEqual(
            b"".join(ET.tostringlist(element)), b"<html><body>text</body></html>"
        )
        self.assertEqual(
    ...
    def test_iterparse(self):
        ...
        self.assertEqual(
            [(action, elem.tag) for action, elem in context],
            [
                ("end", "element"),
                ("end", "empty-element"),
                ("end", "root"),
        ...
        self.assertEqual(
            [
                (action, elem.tag) if action in ("start", "end") else (action, elem)
        ...
        self.assertEqual(
            [(action, elem) for action, elem in context],
            [
        ...
        with self.assertRaises(ET.ParseError) as cm:
            next(it)
        self.assertEqual(
            str(cm.exception), "junk after document element: line 1, column 12"
        )
    ...
    def test_writefile(self):
        ...
        self.serialize_check(
            elem, "text<!--comment--><subtag>subtext</subtag>"
        )  # assumes 1.3
    ...
    def test_children(self):
        ...
        with open(SIMPLE_XMLFILE, "rb") as f:
            tree = ET.parse(f)
        self.assertEqual(
            [summarize_list(elem) for elem in tree.getroot().iter()],
            [
        ...
        self.assertEqual(
            [summarize_list(elem) for elem in tree.iter()],
            [
                ["element", "element", "empty-element"],
                [],
                [],
    ...
    def test_indent_space_caching(self):
        ...
        ET.indent(elem)
        self.assertEqual(
            {el.tail for el in elem.iter()}, {None, "\n", "\n  ", "\n    "}
        )
        self.assertEqual(
    ...
    def test_tostring_default_namespace(self):
        ...
        self.assertEqual(
            ET.tostring(
                elem, encoding="unicode", default_namespace="http://effbot.org/ns"
            ),
    ...
    def test_tostringlist_xml_declaration(self):
        elem = ET.XML("<body><tag/></body>")
        self.assertEqual(
            "".join(ET.tostringlist(elem, encoding="unicode")), "<body><tag /></body>"
        )
        self.assertEqual(
    ...
    def test_issue18347(self):
        ...
        self.assertEqual(
            serialize(e, method="html"), "<html><CamelCase>text</CamelCase></html>"
        )
    ...
    def test_entity(self):
        ...
        with self.assertRaises(ET.ParseError) as cm:
            ET.XML(ENTITY_XML)
        self.assertEqual(
            str(cm.exception), "undefined entity &entity;: line 5, column 10"
        )
        ...
        self.assertEqual(
            str(cm.exception), "undefined entity &entity;: line 4, column 10"
        )
    ...
    def test_namespace(self):
        ...
        self.serialize_check(
            elem, '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" />'
        )  # 2.1

        elem = ET.XML("<html:html xmlns:html='http://www.w3.org/1999/xhtml' />")
        self.serialize_check(
            elem, '<html:html xmlns:html="http://www.w3.org/1999/xhtml" />'
        )  # 2.2
    ...
    def test_qname(self):
        ...
        self.serialize_check(
            elem, '<ns0:tag xmlns:ns0="uri"><ns0:tag1 /><ns0:tag2 /></ns0:tag>'
        )  # 1.4
        ...
        self.serialize_check(elem, '<ns0:tag xmlns:ns0="uri" ns0:key="value" />')  # 2.1

        elem.clear()
        elem.attrib[ET.QName("{uri}key")] = "value"
        self.serialize_check(elem, '<ns0:tag xmlns:ns0="uri" ns0:key="value" />')  # 2.2
        ...
        self.serialize_check(
            elem, '<ns0:tag xmlns:ns0="uri" ns0:key="{uri}value" />'
        )  # 3.1

        elem.clear()
        elem.attrib["{uri}key"] = ET.QName("{uri}value")
        self.serialize_check(
            elem, '<ns0:tag xmlns:ns0="uri" ns0:key="ns0:value" />'
        )  # 3.2
    ...
    def test_xpath_tokenizer(self):
        ...
        def check(p, expected, namespaces=None):
            self.assertEqual(
                [op or tag for op, tag in ElementPath.xpath_tokenizer(p, namespaces)],
                expected,
            )
    ...
    def test_processinginstruction(self):
        ...
        self.assertEqual(
            ET.tostring(ET.PI("test", "instruction")), b"<?test instruction?>"
        )

        # Issue #2746

        self.assertEqual(
            ET.tostring(ET.PI("test", "<testing&>")), b"<?test <testing&>?>"
        )
        self.assertEqual(
    ...
    def test_tree_write_attribute_order(self):
        ...
        self.assertEqual(
            serialize(root), '<cirriculum status="public" company="example" />'
        )
        self.assertEqual(
            serialize(root, method="html"),
    ...
    def test_attlist_default(self):
        ...
        self.assertEqual(
            root[0].attrib, {"{http://www.w3.org/XML/1998/namespace}lang": "eng"}
        )
...
""".format(
        html.escape(SIMPLE_XMLFILE, True)
    )
)

XINCLUDE[
    "include_c1_repeated.xml"
] = """\
<?xml version='1.0'?>
<document xmlns:xi="http://www.w3.org/2001/XInclude">
...
class XIncludeTest(unittest.TestCase):
    ...
    def test_xinclude_failures(self):
        ...
        with self.assertRaises(ElementInclude.FatalIncludeError) as cm:
            ElementInclude.include(document, loader=self.none_loader)
        self.assertEqual(
            str(cm.exception), "unknown parse type in xi:include tag ('BAD_TYPE')"
        )
...
class BugsTest(unittest.TestCase):
    ...
    def test_bug_xmltoolkit39(self):
        ...
        self.assertEqual(
            ET.tostring(tree, "utf-8"), b'<tag \xc3\xa4ttr="v\xc3\xa4lue" />'
        )
...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    def test_bug_200708_newline(self):
        ...
        self.assertEqual(
            ET.tostring(e), b'<SomeTag text="def _f():&#10;  return 3&#10;" />'
        )
    ...
    def test_bug_200709_register_namespace(self):
        ...
        self.assertEqual(
            ET.tostring(e), b'<dc:title xmlns:dc="http://purl.org/dc/elements/1.1/" />'
        )
    ...
    def test_pickle(self):
        ...
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            for dumper, loader in product(self.modules, repeat=2):
                ...
                e2 = self.pickleRoundTrip(
                    e, "xml.etree.ElementTree", dumper, loader, proto
                )
    ...
    def test_pickle_issue18997(self):
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            for dumper, loader in product(self.modules, repeat=2):
                ...
                e2 = self.pickleRoundTrip(
                    e1, "xml.etree.ElementTree", dumper, loader, proto
                )
...
class ElementFindTest(unittest.TestCase):
    ...
    def test_findall(self):
        ...
        self.assertEqual(
            summarize_list(e.findall("section/*")), ["tag", "nexttag", "nextsection"]
        )
        self.assertEqual(
            summarize_list(e.findall("section//*")),
        ...
        self.assertEqual(
            summarize_list(e.findall("*/*")), ["tag", "nexttag", "nextsection"]
        )
        self.assertEqual(
            summarize_list(e.findall("*//*")), ["tag", "nexttag", "nextsection", "tag"]
        )
        ...
        self.assertEqual(
            summarize_list(e.findall(".//section[tag='subtext']")), ["section"]
        )
        self.assertEqual(
            summarize_list(e.findall(".//section[tag ='subtext']")), ["section"]
        )
        ...
        self.assertEqual(summarize_list(e.findall(".//section[tag != 'subtext']")), [])
        self.assertEqual(
            summarize_list(e.findall(".//section[ tag != 'subtext' ]")), []
        )
        ...
        self.assertEqual(
            summarize_list(e.findall(".//tag[. !='subtext']")), ["tag"] * 3
        )
        self.assertEqual(
            summarize_list(e.findall('.//tag[.!= "subtext"]')), ["tag"] * 3
        )
        ...
        # Matches all tags.
        self.assertEqual(
            summarize_list(e.findall(".//tag[. != 'subtext ']")), ["tag"] * 4
        )
        self.assertEqual(
            summarize_list(e.findall(".//tag[.!= ' subtext']")), ["tag"] * 4
        )
        ...
        self.assertEqual(
            summarize_list(e.findall(".//tag[. = 'subtext']")), ["tag", "tag"]
        )
    ...
    def test_findall_wildcard(self):
        ...
        self.assertEqual(
            summarize_list(root.findall("{*}*")), ["{X}b", "b", "c", "{Y}b"]
        )
        # This is an unfortunate difference, but that's how find('*') works.
        self.assertEqual(
            summarize_list(root.findall("{*}*") + [root[-1]]),
            summarize_list(root.findall("*")),
        )

        self.assertEqual(
            summarize_list(root.findall(".//{*}b")), ["{X}b", "b", "{X}b", "b", "{Y}b"]
        )
...
class IOTest(unittest.TestCase):
    def test_encoding(self):
        ...
        for enc in ("utf-8", "us-ascii"):
            with self.subTest(enc):
                ...
                self.assertEqual(
                    serialize(elem, encoding=enc.upper()), b"<tag>abc</tag>"
                )
        ...
        self.assertEqual(
            serialize(elem, encoding="utf-8"), b"<tag>&lt;&amp;\"'&gt;</tag>"
        )
        self.assertEqual(
            serialize(elem, encoding="us-ascii"), b"<tag>&lt;&amp;\"'&gt;</tag>"
        )
        for enc in ("iso-8859-1", "utf-16", "utf-32"):
            self.assertEqual(
                serialize(elem, encoding=enc),
        ...
        self.assertEqual(
            serialize(elem, encoding="utf-8"), b'<tag key="&lt;&amp;&quot;\'&gt;" />'
        )
        self.assertEqual(
            serialize(elem, encoding="us-ascii"), b'<tag key="&lt;&amp;&quot;\'&gt;" />'
        )
        for enc in ("iso-8859-1", "utf-16", "utf-32"):
            self.assertEqual(
                serialize(elem, encoding=enc),
    ...
    def test_tostringlist_invariant(self):
        ...
        self.assertEqual(
            ET.tostring(root, "unicode"), "".join(ET.tostringlist(root, "unicode"))
        )
        self.assertEqual(
            ET.tostring(root, "utf-16"), b"".join(ET.tostringlist(root, "utf-16"))
        )
...
class ParseErrorTest(unittest.TestCase):
    ...
    def test_error_code(self):
        ...
        self.assertEqual(
            self._get_error("foo").code, ERRORS.codes[ERRORS.XML_ERROR_SYNTAX]
        )
...
class C14NTest(unittest.TestCase):
    ...
    def test_simple_roundtrip(self):
        ...
        self.assertEqual(
            c14n_roundtrip("<doc xmlns:prefix='uri'><prefix:bar/></doc>"),
            '<doc><prefix:bar xmlns:prefix="uri"></prefix:bar></doc>',
        )
        self.assertEqual(
            c14n_roundtrip(
                "<elem xmlns:wsu='http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd' xmlns:SOAP-ENV='http://schemas.xmlsoap.org/soap/envelope/' />"
            ),
            "<elem></elem>",
        )
        ...
        # C14N spec
        self.assertEqual(
            c14n_roundtrip("<doc>Hello, world!<!-- Comment 1 --></doc>"),
            "<doc>Hello, world!</doc>",
        )
        ...
        self.assertEqual(
            c14n_roundtrip("<normNames attr='   A   &#x20;&#13;&#xa;&#9;   B   '/>"),
            '<normNames attr="   A    &#xD;&#xA;&#x9;   B   "></normNames>',
        )
        self.assertEqual(
            c14n_roundtrip("<normId id=' &apos;   &#x20;&#13;&#xa;&#9;   &apos; '/>"),
            "<normId id=\" '    &#xD;&#xA;&#x9;   ' \"></normId>",
        )
        ...
        xml = (
            '<X xmlns="http://nps/a"><Y xmlns="http://nsp/b" targets="abc,xyz"></Y></X>'
        )
        self.assertEqual(c14n_roundtrip(xml), xml)
    ...
    def test_c14n_exclusion(self):
        ...
        self.assertEqual(
            c14n_roundtrip(
                xml, strip_text=True, exclude_attrs=["{http://example.com/x}attr"]
            ),
            "<root>"
            "<a><b>abtext</b></a>"
            "<b>btext</b>"
        ...
        self.assertEqual(
            c14n_roundtrip(
                xml, strip_text=True, exclude_tags=["{http://example.com/x}d"]
            ),
            "<root>"
            '<a xmlns:x="http://example.com/x" x:attr="attrx"><b>abtext</b></a>'
            "<b>btext</b>"
    ...
    def test_xml_c14n2(self):
        ...
        self.assertEqual(
            ...
            len(
                [
                    output_file
                    for output_files in tests.values()
        ...
        for input_file, output_files in tests.items():
            for output_file, config in output_files:
                keep_comments = (
                    get_option(config, "IgnoreComments") == "true"
                )  # no, it's right :)
                ...
                with self.subTest(f"{output_file}({config_descr})"):
                    ...
                    if (
                        "QNameAware" in config
                        and config["QNameAware"][1].find(
                            "{http://www.w3.org/2010/xml-c14n2}XPathElement"
                        )
                        is not None
                    ):
                        self.skipTest(
                    ...
                    text = ET.canonicalize(
                        ...
                        qname_aware_tags=qtags,
                        qname_aware_attrs=qattrs,
                    )

                    with open(
                        full_path(output_file + ".xml"), "r", encoding="utf8"
                    ) as f:
                        expected = f.read()
