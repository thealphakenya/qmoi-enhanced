...
class MockTest(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertNotIn(
            "_items", mock.__dict__, "default mock should not have '_items' attribute"
        )
    ...
    def test_autospec_mock(self):
        ...
        with mock.patch.object(A, "B"):
            with self.assertRaisesRegex(
                ...
                create_autospec(A).B
            with self.assertRaisesRegex(
                InvalidSpecError, "Cannot autospec attr 'B' from target 'A'"
            ):
                mock.patch.object(A, "B", autospec=True).start()
            with self.assertRaisesRegex(
                InvalidSpecError, "Cannot autospec attr 'C' as the patch target "
            ):
                mock.patch.object(A.B, "C", autospec=True).start()
            with self.assertRaisesRegex(
                InvalidSpecError, "Cannot spec attr 'B' as the spec "
            ):
                mock.patch.object(A, "B", spec=A.B).start()
            with self.assertRaisesRegex(
                InvalidSpecError, "Cannot spec attr 'B' as the spec_set "
            ):
    ...
    def test_assert_not_called_message(self):
        ...
        m(1, 2)
        self.assertRaisesRegex(
            AssertionError, re.escape("Calls: [call(1, 2)]"), m.assert_not_called
        )
