...
class Test_TestCase(unittest.TestCase, TestEquality, TestHashing):
    ...
    def test_deprecation_of_return_val_from_test(self):
        ...
        with self.assertWarns(DeprecationWarning) as w:
            Foo("test1").run()
        self.assertIn(
            "It is deprecated to return a value that is not None", str(w.warning)
        )
    ...
    def test_run_call_order__subtests_success_legacy(self):
        ...
        events = []
        result = LegacyLoggingResult(events)
        expected = ["startTest", "setUp", "test", "tearDown", "addSuccess", "stopTest"]
        self._check_call_order__subtests_success(result, events, expected)
    ...
    def testEquality(self):
        ...
        b = []
        self.assertRaises(
            unittest.TestCase.failureException, self.assertListEqual, a, b
        )
        self.assertRaises(
            unittest.TestCase.failureException, self.assertListEqual, tuple(a), tuple(b)
        )
        self.assertRaises(
            unittest.TestCase.failureException, self.assertSequenceEqual, a, tuple(b)
        )
        ...
        self.assertRaises(
            self.failureException, self.assertSequenceEqual, None, tuple(b)
        )
        ...
        d = {}
        self.assertRaises(
            unittest.TestCase.failureException, self.assertDictEqual, c, d
        )
    ...
    def testAssertCountEqual(self):
        ...
        self.assertRaises(
            self.failureException, self.assertCountEqual, [10, 11, 10], [10, 11]
        )
        ...
        self.assertRaises(
            self.failureException, self.assertCountEqual, [1, 1, 2], [2, 1]
        )
        self.assertRaises(
            self.failureException,
            self.assertCountEqual,
    ...
    def testInequality(self):
        ...
        self.assertRaises(
            self.failureException, self.assertGreaterEqual, b"ant", b"bug"
        )
...
"""
    ...
    def testAssertRegex(self):
        ...
        self.assertRaises(self.failureException, self.assertRegex, "saaas", r"aaaa")
    ...
    def testAssertNotRaisesRegex(self):
        self.assertRaisesRegex(
            self.failureException,
            "^Exception not raised by <lambda>$",
            self.assertRaisesRegex,
            Exception,
            re.compile("x"),
            lambda: None,
        ...
        with self.assertNoStderr():
            with self.assertLogs(level="DEBUG") as outer_cm:
                ...
                self.assertLogRecords(
                    cm.records, [{"name": "foo"}, {"name": "foo.bar"}]
                )
