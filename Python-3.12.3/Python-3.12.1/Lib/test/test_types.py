...
class UnionTests(unittest.TestCase):
    ...
    def test_or_type_operator_with_forward(self):
        ...
        self.assertEqual(
            typing.get_args(typing.get_type_hints(forward_after)["x"]), (int, Forward)
        )
        self.assertEqual(
            typing.get_args(typing.get_type_hints(forward_before)["x"]), (int, Forward)
        )
    ...
    def test_or_type_operator_with_Literal(self):
        ...
        self.assertEqual((Literal[1] | Literal[2]).__args__, (Literal[1], Literal[2]))

        self.assertEqual(
            (Literal[0] | Literal[False]).__args__, (Literal[0], Literal[False])
        )
        self.assertEqual(
            (Literal[1] | Literal[True]).__args__, (Literal[1], Literal[True])
        )
        ...
        self.assertEqual(
            (Literal[Ints.B] | Literal[Ints.A]).__args__,
            (Literal[Ints.B], Literal[Ints.A]),
        )

        self.assertEqual(
            (Literal[0] | Literal[Ints.A]).__args__, (Literal[0], Literal[Ints.A])
        )
        self.assertEqual(
            (Literal[1] | Literal[Ints.B]).__args__, (Literal[1], Literal[Ints.B])
        )
...
class ClassCreationTests(unittest.TestCase):
    ...
    def test_get_original_bases(self):
        ...
        self.assertIs(
            types.get_original_bases(ClassBasedTypedDict)[0], typing.TypedDict
        )
        self.assertEqual(
            types.get_original_bases(GenericTypedDict),
            (typing.TypedDict, typing.Generic[T]),
        )
        self.assertIs(types.get_original_bases(CallBasedTypedDict)[0], typing.TypedDict)
    ...
    def test_resolve_bases_with_mro_entry(self):
        self.assertEqual(
            types.resolve_bases((typing.List[int],)), (list, typing.Generic)
        )
