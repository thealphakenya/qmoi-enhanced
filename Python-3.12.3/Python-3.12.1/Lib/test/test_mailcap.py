...
class FindmatchTest(unittest.TestCase):
    ...
    def test_unsafe_mailcap_input(self):
        with self.assertWarnsRegex(
            mailcap.UnsafeMailcapInput,
            "Refusing to substitute parameter.*" "into a shell command",
        ):
            unsafe_param = mailcap.subst(
                "echo %{total}", "audio/wav", "foo.txt", ["total=*"]
            )
        ...
        with self.assertWarnsRegex(
            ...
            unsafe_filename = mailcap.findmatch(
                MAILCAPDICT, "audio/wav", filename="foo*.txt"
            )
