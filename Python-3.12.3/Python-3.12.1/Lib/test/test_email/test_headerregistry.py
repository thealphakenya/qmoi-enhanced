...
class TestUnstructuredHeader(TestHeaderBase):
    ...
    string_params = {
        ...
        "rfc2047_simple_nonascii_quopri": ("=?utf-8?q?=C3=89ric?=", "Éric"),
...
class TestContentTypeHeader(TestHeaderBase):
    ...
    content_type_params = {
        ...
        "unknown_content_type": ("bad/names", "bad/names", "bad", "names"),
        # The content type is case insensitive, and CFWS is ignored.
        "mixed_case_content_type": ("ImAge/JPeg", "image/jpeg", "image", "jpeg"),
        "spaces_in_content_type": ("  text  /  plain  ", "text/plain", "text", "plain"),
        ...
        "non_ascii_rfc2231_value": (
            ...
            .encode("latin-1")
            .decode("us-ascii", "surrogateescape"),
            "text/plain",
            "text",
            "plain",
...
class TestContentDisposition(TestHeaderBase):
    ...
    content_disp_params = {
        ...
        "invalid_value": ("ab./k", "ab.", {}, [errors.InvalidHeaderDefect]),
...
class TestMIMEVersionHeader(TestHeaderBase):
    ...
    version_string_params = {
        ...
        "1_1": ("1.1", "1.1", "1.1", 1, 1, []),
        "2_1": ("2.1", "2.1", "2.1", 2, 1, []),
        "whitespace": ("1 .0", "1 .0", "1.0", 1, 0, []),
        "leading_trailing_whitespace_ignored": ("  1.0  ", "  1.0  ", "1.0", 1, 0, []),
        ...
        "two_periods": ("1..0", "1..0", None, None, None, [errors.InvalidHeaderDefect]),
        "2_x": (
            "2.x",
            "2.x",
        ...
        "foo": ("foo", "foo", None, None, None, [errors.InvalidHeaderDefect]),
        "missing": ("", "", None, None, None, [errors.HeaderMissingRequiredValue]),
    }
...
class TestAddressHeader(TestHeaderBase):
    ...
    def test_simple_address_list(self):
        value = (
            "Fred <dinsdale@python.org>, foo@example.com, "
    ...
    def test_set_from_Address_list(self):
        h = self.make_header(
            "to",
            [Address("me", "foo", "example.com"), Address("you", "bar", "example.com")],
        )
    ...
    def test_set_from_Address_and_Group_list(self):
        h = self.make_header(
            "to",
            [
                Address("me", "foo", "example.com"),
                Group(
                    "bing", [Address("fiz", "z", "b.com"), Address("zif", "f", "c.com")]
                ),
        ...
        self.assertEqual(
            h,
            "me <foo@example.com>, bing: fiz <z@b.com>, "
            "zif <f@c.com>;, you <bar@example.com>",
        )
        self.assertEqual(
            h.fold(policy=policy.default.clone(max_line_length=40)),
...
class TestFolding(TestHeaderBase):
    ...
    def test_fold_unstructured_with_two_overlong_words(self):
        h = self.make_header(
            "Subject",
            "thisisaverylonglineconsistingofa"
