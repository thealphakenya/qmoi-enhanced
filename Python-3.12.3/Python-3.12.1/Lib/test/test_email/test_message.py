...
class TestEmailMessageBase:
    ...
    def message_as_get_body(self, body_parts, attachments, parts, msg):
        ...
        self.assertEqual(
            m.get_body(preferencelist=("related", "html", "plain")), first(expected)
        )
        self.assertEqual(
            m.get_body(preferencelist=("related", "html")),
            first(expected[related : html + 1]),
        )
        self.assertEqual(
            m.get_body(preferencelist=("related", "plain")),
            first([expected[related], expected[plain]]),
        )
        self.assertEqual(
            m.get_body(preferencelist=("html", "plain")),
            first(expected[html : plain + 1]),
        )
        self.assertEqual(m.get_body(preferencelist=["related"]), expected[related])
        ...
        self.assertEqual(
            m.get_body(preferencelist=("plain", "html")),
            first(expected[plain : html - 1 : -1]),
        )
        self.assertEqual(
            m.get_body(preferencelist=("plain", "related")),
            first([expected[plain], expected[related]]),
        )
        self.assertEqual(
            m.get_body(preferencelist=("html", "related")), first(expected[html::-1])
        )
        self.assertEqual(
            m.get_body(preferencelist=("plain", "html", "related")),
            first(expected[::-1]),
        )
        self.assertEqual(
            m.get_body(preferencelist=("html", "plain", "related")),
            first([expected[html], expected[plain], expected[related]]),
        )
    ...
    def subtype_as_make(self, method, subtype, outcome):
        ...
        if subtype == "no_content":
            ...
            self.assertEqual(
                m.items(), msg_headers + [("Content-Type", "multipart/" + method)]
            )
        else:
    ...
    def test_default_content_manager_for_add_comes_from_policy(self):
        ...
        for method in ("add_related", "add_alternative", "add_attachment"):
            with self.assertRaises(
                self._TestSetRaisingContentManager.CustomError
            ) as ar:
                getattr(m, method)("")
...
class TestEmailMessage(TestEmailMessageBase, TestEmailBase):
    ...
    def test_as_string_allows_maxheaderlen(self):
        m = self._str_msg("Subject: long line" + " ab" * 50 + "\n\n")
        self.assertEqual(len(m.as_string(maxheaderlen=0).strip().splitlines()), 1)
        self.assertEqual(len(m.as_string(maxheaderlen=34).strip().splitlines()), 6)
    ...
    def test_folding_with_utf8_encoding_6(self):
        ...
        m = EmailMessage()
        m["Subject"] = (
            "123456789 123456789 123456789 123456789 Hello Wörld!"
            " 123456789 123456789 123456789 123456789 123456789"
            " 123456789"
        )
        self.assertEqual(
    ...
    def test_folding_with_utf8_encoding_7(self):
        ...
        m = EmailMessage()
        m["Subject"] = (
            "123456789 123456789 Hello Wörld! Hello Wörld! "
