...
def adjust_rlimit_nofile():
    ...
    if fd_limit < desired_fds and fd_limit < max_fds:
        ...
        try:
            resource.setrlimit(resource.RLIMIT_NOFILE, (new_fd_limit, max_fds))
            print(f"Raised RLIMIT_NOFILE: {fd_limit} -> {new_fd_limit}")
        except (ValueError, OSError) as err:
            print_warning(
                f"Unable to raise RLIMIT_NOFILE from {fd_limit} to "
                f"{new_fd_limit}: {err}."
            )
...
def display_header(use_resources: tuple[str, ...], python_cmd: tuple[str, ...] | None):
    ...
    if use_resources:
        ...
        print(
            "== resources: all test resources are disabled, "
    ...
    if python_cmd:
        ...
        proc = subprocess.run(
            get_cmd, stdout=subprocess.PIPE, text=True, cwd=os_helper.SAVEDCWD
        )
        ...
        if stdout:
            ...
            print(
                f"== host platform: <command failed with exit code {proc.returncode}>"
            )
    else:
