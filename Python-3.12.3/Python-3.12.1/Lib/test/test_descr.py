...
class OperatorsTest(unittest.TestCase):
    ...
    def test_lists(self):
        ...
        self.setsliceop_test(
            [1, 2, 3, 4], 1, 3, [5, 6], [1, 5, 6, 4], "a[b:c]=d", "__setitem__"
        )
    ...
    def test_dicts(self):
        ...
        self.set2op_test(
            {1: 2, 3: 4}, 2, 3, {1: 2, 2: 3, 3: 4}, "a[b]=c", "__setitem__"
        )
    ...
    def test_complexes(self):
        ...
        self.number_operators(
            100.0j,
            3.0j,
            skip=["lt", "le", "gt", "ge", "int", "float", "floordiv", "divmod", "mod"],
        )
    ...
    def test_spam_lists(self):
        ...
        self.binop_test(
            spamlist([1]), spamlist([2]), spamlist([1, 2]), "a+b", "__add__"
        )
        ...
        self.sliceop_test(
            spamlist([1, 2, 3]), 0, 2, spamlist([1, 2]), "a[b:c]", "__getitem__"
        )
        self.setop_test(
            spamlist([1]), spamlist([2]), spamlist([1, 2]), "a+=b", "__iadd__"
        )
        self.setop_test(
            spamlist([1, 2]), 3, spamlist([1, 2, 1, 2, 1, 2]), "a*=b", "__imul__"
        )
        self.unop_test(spamlist([1, 2, 3]), 3, "len(a)", "__len__")
        self.binop_test(
            spamlist([1, 2]), 3, spamlist([1, 2, 1, 2, 1, 2]), "a*b", "__mul__"
        )
        self.binop_test(
            spamlist([1, 2]), 3, spamlist([1, 2, 1, 2, 1, 2]), "b*a", "__rmul__"
        )
        self.set2op_test(
            spamlist([1, 2]), 1, 3, spamlist([1, 3]), "a[b]=c", "__setitem__"
        )
        self.setsliceop_test(
            spamlist([1, 2, 3, 4]),
            1,
...
class ClassPropertiesAndMethods(unittest.TestCase):
    ...
    def test_metaclass(self):
        ...
        class autoproperty(type):
            ...
            def __new__(metaclass, name, bases, dict):
                ...
                return super(autoproperty, metaclass).__new__(
    ...
    def test_monotonicity(self):
        ...
        self.assertEqual(
            PedalWheelBoat.__mro__,
            (PedalWheelBoat, EngineLess, DayBoat, WheelBoat, Boat, object),
        )
        self.assertEqual(
            SmallCatamaran.__mro__,
...
order (MRO) for bases """
    ...
    def test_basic_inheritance(self):
        ...
        self.assertEqual(
            int(a), -1
        )  # self.assertTrue PyNumber_Long() copies the sign bit
    ...
    def test_keyword_arguments(self):
        ...
        ba = bytearray()
        bytearray.__init__(ba, "abc\xbd\u20ac", encoding="latin1", errors="replace")
    ...
    def test_builtin_bases(self):
        ...
        builtin_types = [
            tp for tp in builtins.__dict__.values() if isinstance(tp, type)
        ]
    ...
    def test_set_doc(self):
        ...
        self.assertIn(
            "cannot set '__doc__' attribute of immutable type 'list'", str(cm.exception)
        )

        with self.assertRaises(TypeError) as cm:
            type(X).__dict__["__doc__"].__delete__(X)
        self.assertIn(
            "cannot delete '__doc__' attribute of immutable type 'X'", str(cm.exception)
        )
        self.assertEqual(X.__doc__, "banana")
    ...
    def test_bound_method_repr(self):
        ...
        self.assertRegex(
            repr(Foo().method), r"<bound method .*Foo\.method of <.*Foo object at .*>>"
        )
        ...
        self.assertRegex(
            repr(base.method), r"<bound method .*Base\.method of <.*Base object at .*>>"
        )
        self.assertRegex(
            repr(derived1.method),
            r"<bound method .*Base\.method of <.*Derived1 object at .*>>",
        )
        self.assertRegex(
            repr(derived2.method),
            r"<bound method .*Derived2\.method of <.*Derived2 object at .*>>",
        )
        self.assertRegex(
            repr(super_d2.method),
        ...
        foo = Foo()
        self.assertRegex(
            repr(foo.method),  # access via instance
            r"<bound method .*Foo\.method of <class '.*Foo'>>",
        )
        self.assertRegex(
            repr(Foo.method),  # access via the class
            r"<bound method .*Foo\.method of <class '.*Foo'>>",
        )
        ...
        self.assertRegex(repr(method), r"<bound method \? of <object object at .*>>")
        func.__name__ = "name"
        self.assertRegex(repr(method), r"<bound method name of <object object at .*>>")
        func.__qualname__ = "qualname"
        self.assertRegex(
            repr(method), r"<bound method qualname of <object object at .*>>"
        )
...
class DictProxyTests(unittest.TestCase):
    ...
    def test_iter_keys(self):
        ...
        keys.sort()
        self.assertEqual(
            keys, ["__dict__", "__doc__", "__module__", "__weakref__", "meth"]
        )
...
class PicklingTests(unittest.TestCase):
    ...
    def _check_reduce(
        ...
        if proto >= 2:
            ...
            reduce_value = (
                ...
                (type(obj), base_type, None if base_type is object else base_type(obj)),
            )
    ...
    def _assert_is_copy(self, obj, objcopy, msg=None):
        ...
        if type(obj).__repr__ is object.__repr__:
            ...
            # themselves.
            raise ValueError(
                "object passed to _assert_is_copy must "
