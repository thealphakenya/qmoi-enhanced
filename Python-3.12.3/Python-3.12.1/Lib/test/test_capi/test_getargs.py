...
class ParseTupleAndKeywords_Test(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "function missing required argument 'a'"
        ):
            parse((), {}, "O", ["a"])
        with self.assertRaisesRegex(TypeError, "'b' is an invalid keyword argument"):
            parse((), {"b": 1}, "|O", ["a"])
        with self.assertRaisesRegex(
            TypeError,
            rf"argument for function given by name \('a'\) " rf"and position \(1\)",
        ):
            parse((1,), {"a": 2}, "O|O", ["a", "b"])
    ...
    def test_positional_only(self):
        ...
        with self.assertRaisesRegex(
            ...
            parse((1,), {"a": 3}, "OOO", ["", "", "a"])
        self.assertEqual(parse((1,), {}, "O|OO", ["", "", "a"]), (1, NULL, NULL))
        with self.assertRaisesRegex(
            TypeError, r"function takes at least 1 positional argument \(0 given\)"
        ):
            parse((), {}, "O|OO", ["", "", "a"])
        self.assertEqual(parse((1, 2), {"a": 3}, "OO$O", ["", "", "a"]), (1, 2, 3))
        with self.assertRaisesRegex(
            TypeError, r"function takes exactly 2 positional arguments \(1 given\)"
        ):
            parse((1,), {"a": 3}, "OO$O", ["", "", "a"])
        self.assertEqual(parse((1,), {}, "O|O$O", ["", "", "a"]), (1, NULL, NULL))
        with self.assertRaisesRegex(
            TypeError, r"function takes at least 1 positional argument \(0 given\)"
        ):
            parse((), {}, "O|O$O", ["", "", "a"])
...
class Test_testcapi(unittest.TestCase):
    ...
    def test_nested_tuple(self):
        ...
        self.assertEqual(parse(((1, 2, 3),), {}, "(OOO)", ["a"]), (1, 2, 3))
        self.assertEqual(
            parse((1, (2, 3), 4), {}, "O(OO)O", ["a", "b", "c"]), (1, 2, 3, 4)
        )
        parse(((1, 2, 3),), {}, "(iii)", ["a"])
        ...
        with self.assertRaisesRegex(
            ...
            parse(((1, 2, 3),), {}, "(ii)", ["a"])
        with self.assertRaisesRegex(
            TypeError, "argument 1 must be sequence of length 2, not 1"
        ):
            parse(((1,),), {}, "(ii)", ["a"])
        with self.assertRaisesRegex(
            ...
            parse((1,), {}, "(ii)", ["a"])
        with self.assertRaisesRegex(
            TypeError, "argument 1 must be 2-item sequence, not bytes"
        ):
            parse((b"ab",), {}, "(ii)", ["a"])
        ...
        for f in "es", "et", "es#", "et#":
            with self.assertRaises(LookupError):  # empty encoding ""
                parse((("a",),), {}, "(" + f + ")", ["a"])
            with self.assertRaisesRegex(
                TypeError, "argument 1 must be sequence of length 1, not 0"
            ):
                parse(((),), {}, "(" + f + ")", ["a"])
