...
class CAPITest(unittest.TestCase):
    ...
    def test_mapping_keys_valuesitems(self):
        ...
        for mapping in [dict_obj, DictSubclass(dict_obj), BadMapping(dict_obj)]:
            self.assertListEqual(_testcapi.dict_keys(mapping), list(dict_obj.keys()))
            self.assertListEqual(
                _testcapi.dict_values(mapping), list(dict_obj.values())
            )
            self.assertListEqual(_testcapi.dict_items(mapping), list(dict_obj.items()))
