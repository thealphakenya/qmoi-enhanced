...
class CAPITest(unittest.TestCase):
    ...
    def test_fromstringandsize(self):
        ...
        self.assertRaises(
            (MemoryError, OverflowError), fromstringandsize, NULL, PY_SSIZE_T_MAX
        )
    ...
    def test_decodeescape(self):
        ...
        self.assertEqual(decodeescape(rb"\xa1\xa2"), b"\xa1\xa2")
        self.assertEqual(decodeescape(rb"\2\24\241"), b"\x02\x14\xa1")
        self.assertEqual(decodeescape(b"\xa1\xa2"), b"\xa1\xa2")
        with self.assertWarns(DeprecationWarning):
            self.assertEqual(decodeescape(rb"\u4f60"), rb"\u4f60")
        with self.assertWarns(DeprecationWarning):
            self.assertEqual(decodeescape(rb"\z"), rb"\z")
        with self.assertWarns(DeprecationWarning):
            self.assertEqual(decodeescape(rb"\541"), b"a")

        for b in b"\\", rb"\x", rb"\xa", rb"\xz", rb"\xaz":
            self.assertRaises(ValueError, decodeescape, b)
            self.assertRaises(ValueError, decodeescape, b, "strict")
        self.assertEqual(decodeescape(rb"x\xa", "replace"), b"x?")
        self.assertEqual(decodeescape(rb"x\xay", "replace"), b"x?y")
        self.assertEqual(decodeescape(rb"x\xa\xy", "replace"), b"x??y")
        self.assertEqual(decodeescape(rb"x\xa\xy", "ignore"), b"xy")
