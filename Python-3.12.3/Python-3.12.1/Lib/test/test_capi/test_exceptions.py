    ...
    def test_exc_info(self):
        ...
        try:
            ...
            self.assertSequenceEqual(
                orig_exc_info, (raised_exception.__class__, raised_exception, tb)
            )
...
class Test_FatalError(unittest.TestCase):
    ...
    def check_fatal_error(self, code, expected, not_expected=()):
        ...
        match = re.search(
            r"^Extension modules:(.*) \(total: ([0-9]+)\)$", err, re.MULTILINE
        )
        if not match:
...
class Test_ErrSetAndRestore(unittest.TestCase):
    ...
    def test_set_object_and_fetch(self):
        ...
        self.assertEqual(
            exc.__notes__[0], "Normalization failed: type=Broken args='abcd'"
        )
        ...
        self.assertIsInstance(exc, ValueError)
        self.assertEqual(
            exc.__notes__[0], "Normalization failed: type=Broken args=<unknown>"
        )
    ...
    def test_setfromerrnowithfilename(self):
        ...
        self.assertEqual(e.exception.args, (ENOENT, "No such file or directory"))
        ...
        if TESTFN_UNDECODABLE:
            ...
            self.assertEqual(e.exception.filename, os.fsdecode(TESTFN_UNDECODABLE))
        ...
        self.assertEqual(
            e.exception.args, (ENOENT, "No such file or directory", "file")
        )
    ...
    def test_err_writeunraisable(self):
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertEqual(cm.unraisable.exc_traceback.tb_lineno, firstline + 6)
        ...
        with support.catch_unraisable_exception() as cm:
            ...
            self.assertEqual(cm.unraisable.exc_type, CustomError)
            self.assertEqual(str(cm.unraisable.exc_value), "oops!")
            self.assertEqual(cm.unraisable.exc_traceback.tb_lineno, firstline + 15)
            self.assertIsNone(cm.unraisable.err_msg)
            self.assertIsNone(cm.unraisable.object)

        with (
            support.swap_attr(sys, "unraisablehook", None),
...
class Test_PyUnstable_Exc_PrepReraiseStar(ExceptionIsLikeMixin, unittest.TestCase):
    ...
    def test_invalid_args(self):
        ...
        with self.assertRaisesRegex(ValueError, "orig must be a raised exception"):
            _testcapi.unstable_exc_prep_reraise_star(
                ExceptionGroup("eg", [ValueError(42)]), [TypeError(42)]
            )
    ...
    def test_nothing_to_reraise(self):
        ...
        try:
            ...
            orig = e
        self.assertEqual(_testcapi.unstable_exc_prep_reraise_star(orig, [None]), None)
