...
class CAPITest(unittest.TestCase):
                    ...
                    for length in [
                        *range(-1, 7 - start),
                        ...
                        if filled == 5 and to != strings[idx]:
                            ...
                            continue
                        expected = (
                            to[:start] + chr(fill_char) * filled + to[start + filled :]
                        )
                        self.assertEqual(
                            fill(to, start, length, fill_char), (expected, filled)
                        )
    ...
    def test_writechar(self):
        ...
        for i, s in enumerate(strings):
            for j, c in enumerate(chars):
                if j <= i:
                    self.assertEqual(writechar(s, 1, c), (s[:1] + chr(c) + s[2:], 0))
                else:
    ...
    def test_from_format(self):
        ...
        for conv, signed, value, expected in [
            ...
            for mod, ctype in [
                ...
                with self.subTest(format=b"%" + mod + conv):
                    check_format(expected, b"%" + mod + conv, ctype(value))
        ...
        # test %V
        check_format("abc", b"%V", "abc", b"xyz")
        check_format("xyz", b"%V", None, b"xyz")

        # test %ls
        check_format("abc", b"%ls", c_wchar_p("abc"))
        check_format("\u4eba\u6c11", b"%ls", c_wchar_p("\u4eba\u6c11"))
        check_format(
            "\U0001f4bb+\U0001f40d", b"%ls", c_wchar_p("\U0001f4bb+\U0001f40d")
        )
        check_format("   ab", b"%5.2ls", c_wchar_p("abc"))
        check_format("   \u4eba\u6c11", b"%5ls", c_wchar_p("\u4eba\u6c11"))
        check_format(
            "  \U0001f4bb+\U0001f40d", b"%5ls", c_wchar_p("\U0001f4bb+\U0001f40d")
        )
        check_format("\u4eba", b"%.1ls", c_wchar_p("\u4eba\u6c11"))
        check_format(
            "\U0001f4bb" if sizeof(c_wchar) > 2 else "\ud83d",
            b"%.1ls",
            c_wchar_p("\U0001f4bb+\U0001f40d"),
        )
        check_format(
            "\U0001f4bb+" if sizeof(c_wchar) > 2 else "\U0001f4bb",
            b"%.2ls",
            c_wchar_p("\U0001f4bb+\U0001f40d"),
        )

        # test %lV
        check_format("abc", b"%lV", "abc", c_wchar_p("xyz"))
        check_format("xyz", b"%lV", None, c_wchar_p("xyz"))
        check_format("\u4eba\u6c11", b"%lV", None, c_wchar_p("\u4eba\u6c11"))
        check_format(
            "\U0001f4bb+\U0001f40d", b"%lV", None, c_wchar_p("\U0001f4bb+\U0001f40d")
        )
        check_format("   ab", b"%5.2lV", None, c_wchar_p("abc"))
        check_format("   \u4eba\u6c11", b"%5lV", None, c_wchar_p("\u4eba\u6c11"))
        check_format(
            "  \U0001f4bb+\U0001f40d", b"%5lV", None, c_wchar_p("\U0001f4bb+\U0001f40d")
        )
        check_format("\u4eba", b"%.1lV", None, c_wchar_p("\u4eba\u6c11"))
        check_format(
            "\U0001f4bb" if sizeof(c_wchar) > 2 else "\ud83d",
            b"%.1lV",
            None,
        ...
        check_format("   ab", b"%*.*lV", c_int(5), c_int(2), None, c_wchar_p("abc"))
        ...
        self.assertRegex(p_format2, r"0x[a-zA-Z0-9]{3,} 0x[a-zA-Z0-9]{3,}")
    ...
    def test_fromwidechar(self):
        ...
        self.assertRaises(
            SystemError, fromwidechar, b"\0" * SIZEOF_WCHAR_T, PY_SSIZE_T_MIN
        )
    ...
    def test_asutf8andsize(self):
        ...
        self.assertEqual(
            unicode_asutf8andsize("абв", 7), (b"\xd0\xb0\xd0\xb1\xd0\xb2\0", 6)
        )
        self.assertEqual(
            unicode_asutf8andsize("\U0001f600", 5), (b"\xf0\x9f\x98\x80\0", 4)
        )
    ...
    def test_transform_decimal_and_space(self):
        ...
        from _testcapi import (
            unicode_transformdecimalandspacetoascii as transform_decimal,
        )

        self.assertEqual(transform_decimal("123"), "123")
        self.assertEqual(transform_decimal("\u0663.\u0661\u0664"), "3.14")
        self.assertEqual(transform_decimal("\N{EM SPACE}3.14\N{EN SPACE}"), " 3.14 ")
        self.assertEqual(transform_decimal("12\u20ac3"), "12?")
    ...
    def test_split(self):
        ...
        self.assertEqual(split("a|b|c|d", "|", PY_SSIZE_T_MAX), ["a", "b", "c", "d"])
        self.assertEqual(split("a|b|c|d", "|", -1), ["a", "b", "c", "d"])
        self.assertEqual(split("a|b|c|d", "|", PY_SSIZE_T_MIN), ["a", "b", "c", "d"])
        self.assertEqual(split("a|b|c|d", "\u20ac"), ["a|b|c|d"])
        self.assertEqual(split("a||b|c||d", "||"), ["a", "b|c", "d"])
        self.assertEqual(split("а|б|в|г", "|"), ["а", "б", "в", "г"])
        self.assertEqual(
            split("абабагаламага", "а"), ["", "б", "б", "г", "л", "м", "г", ""]
        )
        self.assertEqual(split(" a\tb\nc\rd\ve\f", NULL), ["a", "b", "c", "d", "e"])
        self.assertEqual(
            split("a\x85b\xa0c\u1680d\u2000e", NULL), ["a", "b", "c", "d", "e"]
        )
    ...
    def test_rsplit(self):
        ...
        self.assertEqual(rsplit("a|b|c|d", "|", PY_SSIZE_T_MAX), ["a", "b", "c", "d"])
        self.assertEqual(rsplit("a|b|c|d", "|", -1), ["a", "b", "c", "d"])
        self.assertEqual(rsplit("a|b|c|d", "|", PY_SSIZE_T_MIN), ["a", "b", "c", "d"])
        self.assertEqual(rsplit("a|b|c|d", "\u20ac"), ["a|b|c|d"])
        self.assertEqual(rsplit("a||b|c||d", "||"), ["a", "b|c", "d"])
        self.assertEqual(rsplit("а|б|в|г", "|"), ["а", "б", "в", "г"])
        self.assertEqual(
            rsplit("абабагаламага", "а"), ["", "б", "б", "г", "л", "м", "г", ""]
        )
        self.assertEqual(rsplit("aжbжcжd", "ж"), ["a", "b", "c", "d"])
        self.assertEqual(rsplit(" a\tb\nc\rd\ve\f", NULL), ["a", "b", "c", "d", "e"])
        self.assertEqual(
            rsplit("a\x85b\xa0c\u1680d\u2000e", NULL), ["a", "b", "c", "d", "e"]
        )
    ...
    def test_splitlines(self):
        ...
        self.assertEqual(splitlines("a\nb\rc\r\nd", True), ["a\n", "b\r", "c\r\n", "d"])
        self.assertEqual(splitlines("a\x85b\u2028c\u2029d"), ["a", "b", "c", "d"])
        self.assertEqual(
            splitlines("a\x85b\u2028c\u2029d", True),
            ["a\x85", "b\u2028", "c\u2029", "d"],
        )
    ...
    def test_translate(self):
        ...
        self.assertEqual(
            translate("abcd", {ord("a"): "A", ord("b"): ord("B"), ord("c"): "<>"}),
            "AB<>d",
        )
        self.assertEqual(
            translate("абвг", {ord("а"): "А", ord("б"): ord("Б"), ord("в"): "<>"}),
            "АБ<>г",
        )
        ...
        self.assertRaises(
            UnicodeTranslateError, translate, "abc", {ord("b"): None}, "strict"
        )
        ...
        self.assertEqual(
            translate("abc", {ord("b"): None}, "backslashreplace"), r"a\x62c"
        )
    ...
    def test_findchar(self):
        ...
        self.assertEqual(unicode_findchar(str, ord("!"), PY_SSIZE_T_MIN, -1, -1), 0)
        self.assertEqual(
            unicode_findchar(str, ord("!"), PY_SSIZE_T_MIN, PY_SSIZE_T_MAX, 1), 0
        )
        self.assertEqual(
            unicode_findchar(str, ord("!"), PY_SSIZE_T_MIN, PY_SSIZE_T_MAX, -1), 4
        )
        # bad arguments
    ...
    def test_copycharacters(self):
        ...
        self.assertRaises(
            IndexError, unicode_copycharacters, s, PY_SSIZE_T_MAX, s, 0, 5
        )
        self.assertRaises(IndexError, unicode_copycharacters, s, -1, s, 0, 5)
        self.assertRaises(
            IndexError, unicode_copycharacters, s, PY_SSIZE_T_MIN, s, 0, 5
        )
        self.assertRaises(IndexError, unicode_copycharacters, s, 0, s, 6, 5)
        self.assertRaises(
            IndexError, unicode_copycharacters, s, 0, s, PY_SSIZE_T_MAX, 5
        )
        self.assertRaises(IndexError, unicode_copycharacters, s, 0, s, -1, 5)
        self.assertRaises(
            IndexError, unicode_copycharacters, s, 0, s, PY_SSIZE_T_MIN, 5
        )
        self.assertRaises(SystemError, unicode_copycharacters, s, 1, s, 0, 5)
        self.assertRaises(
            SystemError, unicode_copycharacters, s, 1, s, 0, PY_SSIZE_T_MAX
        )
        self.assertRaises(SystemError, unicode_copycharacters, s, 0, s, 0, -1)
        self.assertRaises(
            SystemError, unicode_copycharacters, s, 0, s, 0, PY_SSIZE_T_MIN
        )
