...
from test.support.os_helper import TESTFN, unlink, rmtree, temp_dir, temp_cwd, fd_count
...
class AbstractTestsWithSourceFile:
    ...
    def test_writestr_compresslevel(self):
        ...
        zipfp.writestr(
            "b.txt", "hello world", compress_type=self.compression, compresslevel=2
        )
...
class StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.TestCase):
    ...
    def large_file_exception_test2(self, f, compression):
        with zipfile.ZipFile(f, "w", compression, allowZip64=False) as zipfp:
            self.assertRaises(
                zipfile.LargeZipFile, zipfp.writestr, "another.name", self.data
            )
    ...
    def test_absolute_arcnames(self):
        with zipfile.ZipFile(
            TESTFN2, "w", zipfile.ZIP_STORED, allowZip64=True
        ) as zipfp:
            zipfp.write(TESTFN, "/absolute")
    ...
    def test_force_zip64(self):
        ...
        self.assertGreaterEqual(
            vers, zipfile.ZIP64_VERSION
        )  # requires zip64 to extract
        ...
        self.assertEqual(
            cd_sig, b"PK\x01\x02"
        )  # ensure the central directory header is next

        z = zipfile.ZipFile(io.BytesIO(zipdata))
        zinfos = z.infolist()
        self.assertEqual(len(zinfos), 1)
        self.assertGreaterEqual(
            zinfos[0].extract_version, zipfile.ZIP64_VERSION
        )  # requires zip64 to extract
    ...
    def test_unseekable_zip_known_filesize(self):
        ...
        (
            ...
            "<4sBBHH8xIIHH8shhQQ{}x4s".format(file_size),
            seekable_data[: 62 + file_size],
        )
        ...
        (
            ...
            "<4sBBHH8xIIHH8shhQQ{}x4s4xQQ4s".format(file_size),
            unseekable_data[: 86 + file_size],
        )
        ...
        self.assertEqual(
            ex_usize, 0
        )  # uncompressed size - 0 to defer to data descriptor
    ...
    def _test_extract_with_target(self, target):
        ...
        with zipfile.ZipFile(TESTFN2, "r") as zipfp:
            for fpath, fdata in SMALL_TEST_DATA:
                ...
                correctfile = os.path.normpath(correctfile)
                self.assertTrue(
                    os.path.samefile(writtenfile, correctfile), (writtenfile, target)
                )
...
class OtherTests(unittest.TestCase):
    ...
    def test_create_zipinfo_before_1980(self):
        self.assertRaises(
            ValueError, zipfile.ZipInfo, "seventies", (1979, 1, 1, 0, 0, 0)
        )
...
class AbstractTestsWithRandomBinaryFiles:
    ...
    def setUpClass(cls):
        ...
        cls.data = b"".join(
            struct.pack("<f", random() * randint(-1000, 1000)) for i in range(datacount)
        )
...
class StoredTestsWithRandomBinaryFiles(
    AbstractTestsWithRandomBinaryFiles, unittest.TestCase
):
    compression = zipfile.ZIP_STORED


@requires_zlib()
class DeflateTestsWithRandomBinaryFiles(
    AbstractTestsWithRandomBinaryFiles, unittest.TestCase
):
    compression = zipfile.ZIP_DEFLATED
...
@requires_bz2()
class Bzip2TestsWithRandomBinaryFiles(
    AbstractTestsWithRandomBinaryFiles, unittest.TestCase
):
    compression = zipfile.ZIP_BZIP2
...
@requires_lzma()
class LzmaTestsWithRandomBinaryFiles(
    AbstractTestsWithRandomBinaryFiles, unittest.TestCase
):
    compression = zipfile.ZIP_LZMA
...
class TestWithDirectory(unittest.TestCase):
    ...
    def test_mkdir(self):
        with zipfile.ZipFile(TESTFN, "w") as zf:
            ...
            self.assertEqual(
                set(os.listdir(target)),
                {"directory", "directory2", "directory3", "directory4"},
            )
...
class CommandLineTest(unittest.TestCase):
    ...
    def test_list_command(self):
        ...
        for opt in "-l", "--list":
            out = self.zipfilecmd(
                opt, zip_name, PYTHONIOENCODING="ascii:backslashreplace"
            )
...
class TestExecutablePrependedZip(unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.access("/bin/bash", os.X_OK), "Test relies on #!/bin/bash working."
    )
    @requires_subprocess()
...
class EncodedMetadataTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name, temp in zip(self.file_names, placeholders[:2]):
            data = data.replace(temp.encode("ascii"), name.encode("shift_jis"))
    ...
    def test_read_without_metadata_encoding(self):
        ...
        expected_names = [
            name.encode("shift_jis").decode("cp437") for name in self.file_names[:2]
        ] + self.file_names[2:]
    ...
    def test_read_with_incorrect_metadata_encoding(self):
        ...
        expected_names = [
            name.encode("shift_jis").decode("koi8-u") for name in self.file_names[:2]
        ] + self.file_names[2:]
    ...
    def test_write_with_metadata_encoding(self):
        ZF = zipfile.ZipFile
        for mode in ("w", "x", "a"):
            with self.assertRaisesRegex(ValueError, "^metadata_encoding is only"):
