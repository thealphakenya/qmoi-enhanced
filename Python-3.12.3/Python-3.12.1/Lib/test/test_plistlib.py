...
TESTDATA = {
    plistlib.FMT_XML: binascii.a2b_base64(
        b"""
    ...
    plistlib.FMT_BINARY: binascii.a2b_base64(
        b"""
    ...
    "KEYED_ARCHIVE": binascii.a2b_base64(
        b"""
...
class TestPlistlib(unittest.TestCase):
    ...
    def test_uid(self):
        data = UID(1)
        self.assertEqual(
            plistlib.loads(plistlib.dumps(data, fmt=plistlib.FMT_BINARY)), data
        )
        dict_data = {
            "uid0": UID(0),
            "uid2": UID(2),
        ...
        self.assertEqual(
            plistlib.loads(plistlib.dumps(dict_data, fmt=plistlib.FMT_BINARY)),
            dict_data,
        )
    ...
    def test_invalidinteger(self):
        self.assertRaises(
            ValueError, plistlib.loads, b"<plist><integer>not integer</integer></plist>"
        )

    def test_invalidreal(self):
        self.assertRaises(
            ValueError, plistlib.loads, b"<plist><integer>not real</integer></plist>"
        )
    ...
    def test_xml_plist_with_entity_decl(self):
        with self.assertRaisesRegex(
            plistlib.InvalidFileException, "XML entity declarations are not supported"
        ):
...
class TestBinaryPlistlib(unittest.TestCase):
    ...
    def test_large_timestamp(self):
        ...
        for ts in -(2**31) - 1, 2**31:
            with self.subTest(ts=ts):
                d = datetime.datetime(1970, 1, 1, 0, 0) + datetime.timedelta(seconds=ts)
    ...
    def test_load_int(self):
        ...
        self.assertEqual(
            self.decode(b"\x13\x01\x23\x45\x67\x89\xab\xcd\xef"), 0x0123456789ABCDEF
        )
        self.assertEqual(
            self.decode(b"\x13\xfe\xdc\xba\x98\x76\x54\x32\x10"), -0x123456789ABCDF0
        )
...
class TestKeyedArchive(unittest.TestCase):
    def test_keyed_archive_data(self):
        ...
        data = {
            ...
            "$objects": [
                "$null",
                {"pytype": 1, "$class": UID(2), "NS.string": "KeyArchive UID Test"},
                {
...
class TestPlutil(unittest.TestCase):
    ...
    properties = {"fname": "H", "lname": "A", "marks": {"a": 100, "b": 0x10}}
    exptected_properties = {"fname": "H", "lname": "A", "marks": {"a": 100, "b": 16}}
    pl = {"HexType": 0x0100000C, "IntType": 0o123}
    ...
    def get_lint_status(self):
        return subprocess.run(
            ["plutil", "-lint", self.file_name], capture_output=True, text=True
        ).stdout
