...
no_chdir = unittest.mock.patch(
    "os.chdir", side_effect=AssertionError("shouldn't call os.chdir()")
)
...
class TestRmTree(BaseTest, unittest.TestCase):
    ...
    def test_on_exc(self):
        ...
        self.assertEqual(
            self.errorState, 3, "Expected call to onexc function did not happen."
        )
...
class TestArchives(BaseTest, unittest.TestCase):
    ...
    def check_unpack_archive(self, format, **kwargs):
        self.check_unpack_archive_with_converter(format, lambda path: path, **kwargs)
        self.check_unpack_archive_with_converter(format, pathlib.Path, **kwargs)
...
class TestWhich(BaseTest, unittest.TestCase):
    ...
    @unittest.skipUnless(sys.platform != "win32", "test is for non win32")
    ...
    @unittest.skipUnless(sys.platform == "win32", "test is for win32")
    def test_cwd_win32(self):
        base_dir = os.path.dirname(self.dir)
        with os_helper.change_cwd(path=self.dir):
            with unittest.mock.patch(
                "shutil._win_path_needs_curdir", return_value=True
            ):
                rv = shutil.which(self.file, path=base_dir)
                # Current directory implicitly on PATH
                self.assertEqual(rv, os.path.join(self.curdir, self.file))
            with unittest.mock.patch(
                "shutil._win_path_needs_curdir", return_value=False
            ):
                rv = shutil.which(self.file, path=base_dir)
    ...
    def test_pathext_match_before_path_full_match(self):
        ...
        assert (
            os.path.basename(
                shutil.which("hello.com", path=test_path, mode=os.F_OK)
            ).lower()
            == "hello.com.exe"
        )
    ...
    def test_pathext(self):
        ...
        temp_filexyz = tempfile.NamedTemporaryFile(
            dir=self.temp_dir, prefix=self.to_text_type("Tmp2"), suffix=ext
        )
    ...
    def test_pathext_with_empty_str(self):
        ...
        with os_helper.EnvironmentVarGuard() as env:
            env["PATHEXT"] = (
                f"{ext if isinstance(ext, str) else ext.decode()};"  # note the ;
            )
            rv = shutil.which(program, path=self.temp_dir)
            self.assertEqual(rv, temp_filexyz.name)
    ...
    def test_pathext_applied_on_files_in_path(self):
        with os_helper.EnvironmentVarGuard() as env:
            env["PATH"] = (
                ...
                else self.temp_dir.decode()
            )
            env["PATHEXT"] = ".test"

            test_path = os.path.join(
                self.temp_dir, self.to_text_type("test_program.test")
            )
    ...
    def test_win_path_needs_curdir(self):
        with unittest.mock.patch(
            "_winapi.NeedCurrentDirectoryForExePath", return_value=True
        ) as need_curdir_mock:
        ...
        with unittest.mock.patch(
            "_winapi.NeedCurrentDirectoryForExePath", return_value=False
        ) as need_curdir_mock:
    ...
    def test_pathext_preferred_for_execute(self):
        with os_helper.EnvironmentVarGuard() as env:
            ...
            self.assertEqual(
                shutil.which(self.to_text_type("test.exe"), mode=os.F_OK), exe
            )
