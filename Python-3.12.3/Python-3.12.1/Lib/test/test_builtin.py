...
test_conv_no_sign = [
    ...
    (" ", ValueError),
    ("  \t\t  ", ValueError),
    (str(rb"\u0663\u0661\u0664 ", "raw-unicode-escape"), 314),
...
class BuiltinTest(unittest.TestCase):
    ...
    def test_chr(self):
        ...
        self.assertEqual(
            chr(sys.maxunicode), str("\\U0010ffff".encode("ascii"), "unicode-escape")
        )
    ...
    def test_compile(self):
        ...
        values = [
            (-1, __debug__, f.__doc__, __debug__, __debug__),
    ...
    def test_exec_globals(self):
        ...
        self.assertRaisesRegex(
            NameError, "name 'print' is not defined", exec, code, {"__builtins__": {}}
        )
    ...
    def test_exec_globals_frozen(self):
        ...
        code = compile(
            "__builtins__['superglobal']=2; print(superglobal)", "test", "exec"
        )
        self.assertRaises(
            frozendict_error, exec, code, {"__builtins__": frozen_builtins}
        )

        # no __build_class__ function
        code = compile("class A: pass", "", "exec")
        self.assertRaisesRegex(
            NameError, "__build_class__ not found", exec, code, {"__builtins__": {}}
        )
    ...
    def test_exec_globals_error_on_get(self):
        ...
        self.assertRaises(setonlyerror, exec, code, setonlydict({"globalname": 1}))

        # builtins' `__getitem__` raises
        code = compile("superglobal", "test", "exec")
        self.assertRaises(
            setonlyerror, exec, code, {"__builtins__": setonlydict({"superglobal": 1})}
        )
    ...
    def test_exec_globals_dict_subclass(self):
        ...
        self.assertRaisesRegex(
            NameError,
            "name 'superglobal' is not defined",
            exec,
            code,
    ...
    def test_exec_closure(self):
        ...
        exec(three_freevars.__code__, three_freevars.__globals__, closure=my_closure)
    ...
    def test_filter(self):
        self.assertEqual(
            list(filter(lambda c: "a" <= c <= "z", "Hello World")), list("elloorld")
        )
        self.assertEqual(
            list(filter(None, [1, "hello", [], [3], "", None, 9, 0])),
            [1, "hello", [3], 9],
        )
        self.assertEqual(list(filter(lambda x: x > 0, [1, -3, 9, 0, 2])), [1, 9, 2])
        self.assertEqual(
            list(filter(None, Squares(10))), [1, 4, 9, 16, 25, 36, 49, 64, 81]
        )
    ...
    def test_map(self):
        ...
        self.assertEqual(
            list(map(lambda x, y: x + y, [1, 3, 2], [9, 1, 4])), [10, 4, 6]
        )
        ...
        self.assertEqual(
            list(map(int, Squares(10))), [0, 1, 4, 9, 16, 25, 36, 49, 64, 81]
        )
    ...
    def test_max(self):
        ...
        with self.assertRaisesRegex(ValueError, r"max\(\) iterable argument is empty"):
            max(())
    ...
    def test_min(self):
        ...
        with self.assertRaisesRegex(ValueError, r"min\(\) iterable argument is empty"):
            min(())
    ...
    def test_open(self):
        ...
        with fp:
            ...
            self.assertEqual(
                fp.readline(), "The quick brown fox jumps over the lazy dog.\n"
            )
    ...
    linux_alpha = platform.system().startswith(
        "Linux"
    ) and platform.machine().startswith("alpha")
    ...
    def test_zip(self):
        ...
        self.assertEqual(
            list(zip(SequenceWithoutALength(), range(2**30))), list(enumerate(range(5)))
        )
    ...
    def test_zip_strict(self):
        self.assertEqual(
            tuple(zip((1, 2, 3), "abc", strict=True)), ((1, "a"), (2, "b"), (3, "c"))
        )
        ...
        def child(wpipe):
            ...
            if stdio_encoding:
                ...
                sys.stdout = io.TextIOWrapper(
                    sys.stdout.detach(), encoding=stdio_encoding, errors="replace"
                )
