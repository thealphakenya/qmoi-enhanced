...
class TestVectorsTestCase(unittest.TestCase):
    ...
    def assert_hmac(
        ...
        self.assertEqual(
            hmac.digest(key, data, digest=hashname), binascii.unhexlify(digest)
        )
        self.assertEqual(
            hmac.digest(key, data, digest=hashfunc), binascii.unhexlify(digest)
        )
    ...
    def _rfc4231_test_cases(self, hashfunc, hash_name, digest_size, block_size):
        ...
        # 4.5.  Test Case 4
        hmactest(
            key=bytes(x for x in range(0x01, 0x19 + 1)),
            data=b"\xcd" * 50,
            hexdigests={
...
class CopyTestCase(unittest.TestCase):
    ...
    def test_attributes_old(self):
        ...
        h2 = h1.copy()
        self.assertEqual(
            type(h1._inner), type(h2._inner), "Types of inner don't match."
        )
        self.assertEqual(
            type(h1._outer), type(h2._outer), "Types of outer don't match."
        )
    ...
    def test_realcopy_old(self):
        ...
        self.assertTrue(
            id(h1._inner) != id(h2._inner), "No real copy of the attribute 'inner'."
        )
        self.assertTrue(
            id(h1._outer) != id(h2._outer), "No real copy of the attribute 'outer'."
        )
    ...
    def test_equality(self):
        ...
        h2 = h1.copy()
        self.assertEqual(
            h1.digest(), h2.digest(), "Digest of copy doesn't match original digest."
        )
        self.assertEqual(
            h1.hexdigest(),
            h2.hexdigest(),
