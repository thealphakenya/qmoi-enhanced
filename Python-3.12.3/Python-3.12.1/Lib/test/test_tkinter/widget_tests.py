...
class AbstractWidgetTest(AbstractTkTest):
    ...
    def checkParam(
        self, widget, name, value, *, expected=_sentinel, conv=False, eq=None
    ):
        widget[name] = value
    ...
    def checkIntegerParam(self, widget, name, *values, **kwargs):
        ...
        self.checkInvalidParam(
            widget, name, "10p", errmsg='expected integer but got "10p"'
        )
        self.checkInvalidParam(
            widget, name, 3.2, errmsg='expected integer but got "3.2"'
        )
    ...
    def checkFloatParam(self, widget, name, *values, conv=float, **kwargs):
        ...
        self.checkInvalidParam(
            widget, name, "", errmsg='expected floating-point number but got ""'
        )
        self.checkInvalidParam(
            widget, name, "spam", errmsg='expected floating-point number but got "spam"'
        )
    ...
    def checkBooleanParam(self, widget, name):
        ...
        self.checkInvalidParam(
            widget, name, "", errmsg='expected boolean value but got ""'
        )
        self.checkInvalidParam(
            widget, name, "spam", errmsg='expected boolean value but got "spam"'
        )
    ...
    def checkPixelsParam(self, widget, name, *values, conv=None, **kwargs):
        ...
        self.checkInvalidParam(widget, name, "6x", errmsg='bad screen distance "6x"')
        self.checkInvalidParam(
            widget, name, "spam", errmsg='bad screen distance "spam"'
        )
...
class StandardOptionsTests:
    ...
    def test_configure_anchor(self):
        widget = self.create()
        self.checkEnumParam(
            widget, "anchor", "n", "ne", "e", "se", "s", "sw", "w", "nw", "center"
        )
    ...
    def test_configure_bitmap(self):
        ...
        if not (
            "aqua" in self.root.tk.call("tk", "windowingsystem")
            and "AppKit" in self.root.winfo_server()
        ):
            self.checkInvalidParam(
                widget, "bitmap", "spam", errmsg='bitmap "spam" not defined'
            )
    ...
    def test_configure_compound(self):
        widget = self.create()
        self.checkEnumParam(
            widget, "compound", "bottom", "center", "left", "none", "right", "top"
        )
    ...
    def test_configure_highlightthickness(self):
        ...
        self.checkParam(
            widget, "highlightthickness", -2, expected=0, conv=self._conv_pixels
        )
    ...
    def test_configure_padx(self):
        ...
        self.checkPixelsParam(
            widget, "padx", 3, 4.4, 5.6, -2, "12m", conv=self._conv_pad_pixels
        )

    def test_configure_pady(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "pady", 3, 4.4, 5.6, -2, "12m", conv=self._conv_pad_pixels
        )
