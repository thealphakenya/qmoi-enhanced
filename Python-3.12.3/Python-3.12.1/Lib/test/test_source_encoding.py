    ...
    def test_20731(self):
        sub = subprocess.Popen(
            [
                sys.executable,
                os.path.join(
                    os.path.dirname(__file__), "tokenizedata", "coding20731.py"
                ),
            ],
...
class AbstractSourceEncodingTest:
    ...
    def test_crlf(self):
        src = b'print(ascii("""\r\n"""))\n'
        out = self.check_script_output(src, rb"'\n'")
...
class UTF8ValidatorTest(unittest.TestCase):
    @unittest.skipIf(
        not sys.platform.startswith("linux"), "Too slow to run on non-Linux platforms"
    )
