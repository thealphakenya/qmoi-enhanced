...
--JfISa01"""
        ...
        env = {
            "boundary": BOUNDARY.encode("latin1"),
            "CONTENT-LENGTH": str(len(POSTDATA.encode("utf8"))),
        }
        result = cgi.parse_multipart(
            fp, env, encoding="ascii", errors="surrogateescape"
        )
...
""".replace(
            ...
            self.assertEqual(
                gen_result(data, environ), {"upload": content.encode("latin1")}
            )
...
"""
        ...
        result.update({"upload": b"this is the content of the fake file\n"})
        v = gen_result(data, environ)
