...
class BaseEventLoopTests(test_utils.TestCase):
    ...
    def test_not_implemented(self):
        ...
        self.assertRaises(
            NotImplementedError, self.loop._make_ssl_transport, m, m, m, m
        )
        ...
        self.assertRaises(
            NotImplementedError, self.loop._make_read_pipe_transport, m, m
        )
        self.assertRaises(
            NotImplementedError, self.loop._make_write_pipe_transport, m, m
        )
    ...
    def test__run_once(self):
        h1 = asyncio.TimerHandle(
            time.monotonic() + 5.0, lambda: True, (), self.loop, None
        )
        h2 = asyncio.TimerHandle(
            time.monotonic() + 10.0, lambda: True, (), self.loop, None
        )
    ...
    def test__run_once_cancelled_event_cleanup(self):
        ...
        add_not_cancel_count = max(
            base_events._MIN_SCHEDULED_TIMER_HANDLES - add_cancel_count, 0
        )
...
class BaseEventLoopWithSelectorTests(test_utils.TestCase):
    ...
    def test_create_datagram_endpoint_addr_error(self):
        ...
        self.assertRaises(TypeError, self.loop.run_until_complete, coro)
        coro = self.loop.create_datagram_endpoint(
    ...
    def test_create_datagram_endpoint_allow_broadcast(self):
        ...
        coro = self.loop.create_datagram_endpoint(
            lambda: protocol, remote_addr=("127.0.0.1", 0), allow_broadcast=True
        )
    ...
    @unittest.skipIf(
        sys.platform == "vxworks", "SO_BROADCAST is enabled by default on VxWorks"
    )
...
class BaseLoopSockSendfileTests(test_utils.TestCase):
    ...
    def test__sock_sendfile_native_failure(self):
        ...
        with self.assertRaisesRegex(
            asyncio.SendfileNotAvailableError, "sendfile is not available"
        ):
