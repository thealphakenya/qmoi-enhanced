...
"""
    ...
    def test_unclosed_resource_warnings(self):
        async def inner(httpd):
            ...
            with self.assertWarns(ResourceWarning) as cm:
                ...
                self.assertTrue(
                    str(cm.warnings[0].message).startswith("unclosed <StreamWriter")
                )
    ...
    def test_unhandled_exceptions(self) -> None:
        ...
        async def main():
            ...
            server = await asyncio.start_server(handle_echo, "localhost", port)
        ...
        self.assertEqual(
            messages[0]["message"], "Unhandled exception in client_connected_cb"
        )
