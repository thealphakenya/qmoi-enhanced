...
class WidgetTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_identify(self):
        self.widget.update()
        self.assertEqual(
            self.widget.identify(
                int(self.widget.winfo_width() / 2), int(self.widget.winfo_height() / 2)
            ),
            "label",
        )
    ...
    def test_widget_state(self):
        ...
        self.assertEqual(
            self.widget.state(["!disabled", "active"]), ("!active", "disabled")
        )
        ...
        self.assertRaises(
            tkinter.TclError, self.widget.instate, ["disabled", "badstate"]
        )
...
class AbstractLabelTest(AbstractWidgetTest):
    ...
    def checkImageParam(self, widget, name):
        ...
        self.checkInvalidParam(
            widget, name, "spam", errmsg='image "spam" doesn\'t exist'
        )

    def test_configure_compound(self):
        options = "none text image center top bottom left right".split()
        errmsg = (
            'bad compound "{}": must be' f' {", ".join(options[:-1])}, or {options[-1]}'
        )
        widget = self.create()
...
class LabelTest(AbstractLabelTest, unittest.TestCase):
    ...
    def test_configure_font(self):
        widget = self.create()
        self.checkParam(
            widget, "font", "-Adobe-Helvetica-Medium-R-Normal--*-120-*-*-*-*-*-*"
        )
...
class EntryTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_configure_validate(self):
        widget = self.create()
        self.checkEnumParam(
            widget, "validate", "all", "key", "focus", "focusin", "focusout", "none"
        )
...
class ComboboxTest(EntryTest, unittest.TestCase):
    ...
    def test_configure_values(self):
        ...
        self.assertEqual(
            self.combo["values"], ("1", "", "2") if self.wantobjects else "1 {} 2"
        )
        ...
        self.assertEqual(
            self.combo["values"],
            (r"a\tb", '"a"', "} {") if self.wantobjects else r'a\\tb {"a"} \}\ \{',
        )

        # out of range
        self.assertRaises(
            tkinter.TclError, self.combo.current, len(self.combo["values"])
        )
        ...
        self.assertEqual(
            combo2["values"], ("1", "2", "") if self.wantobjects else "1 2 {}"
        )
        combo2.destroy()
...
class PanedWindowTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_pane(self):
        ...
        self.assertEqual(
            self.paned.pane(0, weight=None), 0 if self.wantobjects else "0"
        )
...
class MenubuttonTest(AbstractLabelTest, unittest.TestCase):
    ...
    def test_direction(self):
        widget = self.create()
        self.checkEnumParam(
            widget, "direction", "above", "below", "left", "right", "flush"
        )
...
class NotebookTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_add_and_hidden(self):
        ...
        self.assertRaises(
            tkinter.TclError, self.nb.add, ttk.Label(self.root), unknown="option"
        )
...
class SpinboxTest(EntryTest, unittest.TestCase):
    ...
    def test_configure_values(self):
        ...
        self.assertEqual(
            self.spin["values"], ("1", "", "2") if self.wantobjects else "1 {} 2"
        )
        ...
        self.assertEqual(
            spin2["values"], ("1", "2", "") if self.wantobjects else "1 2 {}"
        )
        spin2.destroy()
...
class TreeviewTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_configure_displaycolumns(self):
        ...
        self.checkInvalidParam(
            widget, "displaycolumns", ("a", "b", "d"), errmsg="Invalid column index d"
        )
        self.checkInvalidParam(
            widget, "displaycolumns", (1, 2, 3), errmsg="Column index 3 out of bounds"
        )
        self.checkInvalidParam(
            widget, "displaycolumns", (1, -2), errmsg="Column index -2 out of bounds"
        )
    ...
    def test_column(self):
        ...
        self.assertEqual(
            self.tv.column("#0", "width"), 10 if self.wantobjects else "10"
        )
        self.assertEqual(
            self.tv.column("#0", width=None), 10 if self.wantobjects else "10"
        )
    ...
    def test_detach_reattach(self):
        ...
        self.assertRaises(
            tkinter.TclError, self.tv.reattach, item2, "otherparent", "end"
        )
    ...
    def test_insert_item(self):
        ...
        self.assertRaises(
            tkinter.TclError, self.tv.insert, "", "end", MockTclObj("first-item")
        )
        ...
        self.assertEqual(
            self.tv.item(item, "values"), (value,) if self.wantobjects else value
        )
        self.assertEqual(
            self.tv.item(item, values=None), (value,) if self.wantobjects else value
        )
        ...
        # text
        self.assertEqual(
            self.tv.item(self.tv.insert("", "end", text="Label here"), text=None),
            "Label here",
        )
        self.assertEqual(
            self.tv.item(self.tv.insert("", "end", text=value), text=None), value
        )
    ...
    def test_set(self):
        ...
        self.assertEqual(
            self.tv.item(item, values=None), ("a", "a") if self.wantobjects else "a a"
        )
        ...
        self.assertEqual(
            self.tv.item(item, values=None), ("b", "a") if self.wantobjects else "b a"
        )

        self.tv.set(item, "B", 123)
        self.assertEqual(self.tv.set(item, "B"), 123 if self.wantobjects else "123")
        self.assertEqual(
            self.tv.item(item, values=None), (123, "a") if self.wantobjects else "123 a"
        )
        self.assertEqual(
            self.tv.set(item), {"B": 123} if self.wantobjects else {"B": "123"}
        )
