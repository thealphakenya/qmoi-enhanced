...
class BaseTest(unittest.TestCase):
    ...
    for fname in glob.glob(
        os.path.join(glob.escape(os.path.dirname(__file__)), "*.py")
    ):
...
class BZ2DecompressorTest(BaseTest):
    ...
    def test_uninitialized_BZ2Decompressor_crash(self):
        self.assertEqual(
            BZ2Decompressor.__new__(BZ2Decompressor).decompress(bytes()), b""
        )
...
class OpenTest(BaseTest):
    ...
    def test_bad_params(self):
        ...
        self.assertRaises(ValueError, self.open, self.filename, "wbt")
        self.assertRaises(ValueError, self.open, self.filename, "xbt")
        self.assertRaises(ValueError, self.open, self.filename, "rb", encoding="utf-8")
        self.assertRaises(ValueError, self.open, self.filename, "rb", errors="ignore")
        self.assertRaises(ValueError, self.open, self.filename, "rb", newline="\n")
    ...
    def test_encoding_error_handler(self):
        ...
        with self.open(self.filename, "rt", encoding="ascii", errors="ignore") as f:
