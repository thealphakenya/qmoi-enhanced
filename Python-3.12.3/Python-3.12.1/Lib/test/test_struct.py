...
class StructTest(unittest.TestCase):
        ...
        class IntTester(unittest.TestCase):
            ...
            def test_one(
                ...
                if self.min_value <= x <= self.max_value:
                    ...
                    self.assertRaises(
                        (struct.error, TypeError), unpack, format, b"\x01" + got
                    )
                else:
                    # x is out of range -- verify pack realizes that.
                    self.assertRaises(
                        (OverflowError, ValueError, struct.error), pack, format, x
                    )
            ...
            def run(self):
                ...
                self.assertRaises(
                    (TypeError, struct.error), struct.pack, self.format, "a string"
                )
                self.assertRaises(
                    (TypeError, struct.error), struct.pack, self.format, randrange
                )
                self.assertRaises(
                    (TypeError, struct.error), struct.pack, self.format, 3 + 42j
                )
                self.assertRaises(
                    (TypeError, struct.error), struct.pack, self.format, NotAnInt()
                )
                self.assertRaises(
                    (TypeError, struct.error), struct.pack, self.format, BadIndex()
                )
                ...
                for obj in (
                    ...
                    self.assertRaises(
                        (TypeError, struct.error), struct.pack, self.format, obj
                    )
    ...
    def test_pack_into(self):
        ...
        self.assertRaises(
            (ValueError, struct.error), s.pack_into, small_buf, 0, test_string
        )
        self.assertRaises(
            (ValueError, struct.error), s.pack_into, small_buf, 2, test_string
        )
    ...
    def test_pack_into_fn(self):
        ...
        small_buf = array.array("b", b" " * 10)
        self.assertRaises(
            (ValueError, struct.error), pack_into, small_buf, 0, test_string
        )
        self.assertRaises(
            (ValueError, struct.error), pack_into, small_buf, 2, test_string
        )
    ...
    def test_trailing_counter(self):
        ...
        self.assertRaises(
            struct.error, struct.pack_into, "14s42", store, 0, "spam and eggs"
        )
    ...
    def test_boundary_error_message_with_negative_offset(self):
        ...
        with self.assertRaisesRegex(
            struct.error, r"no space to pack 4 bytes at offset -2"
        ):
            struct.pack_into("<I", byte_list, -2, 123)

        with self.assertRaisesRegex(
            struct.error, "offset -11 out of range for 10-byte buffer"
        ):
            struct.pack_into("<B", byte_list, -11, 123)

        with self.assertRaisesRegex(
            struct.error, r"not enough data to unpack 4 bytes at offset -2"
        ):
    ...
    def test_boundary_error_message_with_large_offset(self):
        ...
        regex2 = (
            r"unpack_from requires a buffer of at least "
            + str(sys.maxsize + 4)
            + r" bytes for unpacking 4 bytes at offset "
            + str(sys.maxsize)
            + r" \(actual buffer size is 10\)"
        )
        with self.assertRaisesRegex(struct.error, regex2):
    ...
    def test_half_float(self):
        ...
        format_bits_float__doubleRoundingError_list = [
            (
                ...
                b"\x67\xff",
                0x1FFDFFFFFF * 2**-26,
            ),  # should be 2047, if double-rounded 64>32>16, becomes 2048
        ]
