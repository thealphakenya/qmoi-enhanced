...
class DemoWindow(object):
    ...
    def __init__(self, filename=None):
        ...
        if darwin:  # Leave Mac button colors alone - #44254.
            self.start_btn = Button(
                root, text=" START ", font=btnfont, fg="#00cc22", command=self.startDemo
            )
            self.stop_btn = Button(
                root, text=" STOP ", font=btnfont, fg="#00cc22", command=self.stopIt
            )
    ...
    def makeLoadDemoMenu(self, master):
        ...
        for entry in getExampleEntries():
            ...
            menu.add_command(label=entry, underline=0, font=menufont, command=load)
        return menu
    ...
    def loadfile(self, filename):
        ...
        self.configGUI(NORMAL, DISABLED, DISABLED, "Press start button", "red")
        self.state = READY
    ...
    def startDemo(self):
        ...
        self.configGUI(DISABLED, NORMAL, DISABLED, "demo running...", "black")
        self.screen.clear()
        ...
        try:
            ...
            result = "stopped!"
        if self.state == DONE:
            self.configGUI(NORMAL, DISABLED, NORMAL, result)
        elif self.state == EVENTDRIVEN:
            self.exitflag = True
            self.configGUI(DISABLED, NORMAL, DISABLED, "use mouse/keys or STOP", "red")
