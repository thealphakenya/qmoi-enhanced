...
class CodecCallbackTest(unittest.TestCase):
    ...
    def test_nameescape(self):
        ...
        sout = (
            b"a\xac\\N{ETHIOPIC SYLLABLE SEE}\\N{EURO SIGN}"
            b"\\N{CJK UNIFIED IDEOGRAPH-8000}\\U0010ffff"
        )
        self.assertEqual(sin.encode("latin-1", "namereplace"), sout)

        sout = (
            b"a\xac\\N{ETHIOPIC SYLLABLE SEE}\xa4"
            b"\\N{CJK UNIFIED IDEOGRAPH-8000}\\U0010ffff"
        )
    ...
    def test_callbacks(self):
        ...
        self.assertEqual(
            s.decode("ascii", "test.handler1"), "\x00[<129>]\x7f[<128>][<255>]"
        )
        ...
        self.assertEqual(
            "g\xfc\xdfrk".encode("ascii", "test.handler1"), b"g[<252><223>]rk"
        )
    ...
    def test_badandgoodignoreexceptions(self):
        ...
        self.assertEqual(
            codecs.ignore_errors(UnicodeEncodeError("ascii", "a\u3042b", 1, 2, "ouch")),
            ("", 2),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            codecs.ignore_errors(UnicodeTranslateError("a\u3042b", 1, 2, "ouch")),
            ("", 2),
        )
    ...
    def test_badandgoodreplaceexceptions(self):
        ...
        self.assertRaises(
            TypeError, codecs.replace_errors, BadObjectUnicodeEncodeError()
        )
        self.assertRaises(
            TypeError, codecs.replace_errors, BadObjectUnicodeDecodeError()
        )
        ...
        self.assertEqual(
            codecs.replace_errors(UnicodeTranslateError("a\u3042b", 1, 2, "ouch")),
            ("\ufffd", 2),
        )
    ...
    def test_badandgoodxmlcharrefreplaceexceptions(self):
        ...
        self.assertRaises(
            TypeError, codecs.xmlcharrefreplace_errors, UnicodeError("ouch")
        )
    ...
    def test_badandgoodbackslashreplaceexceptions(self):
        ...
        self.assertRaises(
            TypeError, codecs.backslashreplace_errors, UnicodeError("ouch")
        )
        # Use the correct exception
        tests = [
            ("\u3042", "\\u3042"),
        ...
        for s, r in tests:
            with self.subTest(str=s):
                ...
                self.assertEqual(
                    codecs.backslashreplace_errors(
                        UnicodeTranslateError("a" + s + "b", 1, 1 + len(s), "ouch")
                    ),
                    (r, 1 + len(s)),
                )
        tests = [
        ...
        for b, r in tests:
            with self.subTest(bytes=b):
                self.assertEqual(
                    codecs.backslashreplace_errors(
                        UnicodeDecodeError(
                            "ascii", bytearray(b"a" + b + b"b"), 1, 2, "ouch"
                        )
                    ),
                    (r, 2),
                )
    ...
    def test_lookup(self):
        ...
        self.assertEqual(codecs.strict_errors, codecs.lookup_error("strict"))
        self.assertEqual(
            codecs.xmlcharrefreplace_errors, codecs.lookup_error("xmlcharrefreplace")
        )
        self.assertEqual(
    ...
    def test_encode_odd_bytes_replacement(self):
        ...
        # of code units.
        for enc, repl in (
            *itertools.product(("utf-16le", "utf-16be"), [b"a", b"abc"]),
            *itertools.product(
                ("utf-32le", "utf-32be"), [b"a", b"ab", b"abc", b"abcde"]
            ),
        ):
    ...
    def test_decodehelper(self):
        ...
        self.assertRaises(
            TypeError, b"\\".decode, "unicode-escape", "test.baddecodereturn1"
        )
        self.assertRaises(
            TypeError, b"\\x0".decode, "unicode-escape", "test.baddecodereturn1"
        )
        self.assertRaises(
            TypeError, b"\\x0y".decode, "unicode-escape", "test.baddecodereturn1"
        )
        self.assertRaises(
            TypeError, b"\\Uffffeeee".decode, "unicode-escape", "test.baddecodereturn1"
        )
        self.assertRaises(
            TypeError, b"\\uyyyy".decode, "raw-unicode-escape", "test.baddecodereturn1"
        )
        ...
        self.assertRaises(
            UnicodeError, codecs.charmap_decode, b"\xff", "strict", {0xFF: None}
        )
    ...
    def test_encodehelper(self):
        ...
        for err in (
            ...
            self.assertRaises(
                UnicodeError, codecs.charmap_encode, "\xff", err, {0xFF: None}
            )
            self.assertRaises(ValueError, codecs.charmap_encode, "\xff", err, D())
            self.assertRaises(
                TypeError, codecs.charmap_encode, "\xff", err, {0xFF: 300}
            )
    ...
    def test_crashing_decode_handler(self):
        ...
        self.assertEqual(
            b"\x00".decode("utf-16", "test.replace_with_long"), "\ufffd\x00\x00\x00\x00"
        )
        self.assertEqual(
            b"\x00".decode("utf-32", "test.replace_with_long"), "\ufffd\x00\x00"
        )
