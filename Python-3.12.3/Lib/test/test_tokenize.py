...
"""
        ...
        self.assertEqual(
            e.exception.msg, "unindent does not match any outer indentation level"
        )
...
class TestMisc(TestCase):
    ...
    def test_decistmt(self):
        ...
        s = "+21.3e-5*-.1234/81.7"
        self.assertEqual(
            decistmt(s), "+Decimal ('21.3e-5')*-Decimal ('.1234')/Decimal ('81.7')"
        )
        ...
        # platforms.
        self.assertEqual(
            eval(decistmt(s)), Decimal("-3.217160342717258261933904529E-7")
        )
...
class Test_Tokenize(TestCase):
    ...
    def test__tokenize_decodes_with_specified_encoding(self):
        ...
        tokens = list(
            _generate_tokens_from_c_tokenizer(
                readline().__next__, encoding="utf-8", extra_tokens=True
            )
        )[:-2]
...
class TestDetectEncoding(TestCase):
    ...
    def test_syntaxerror_latin1(self):
        ...
        lines = (b'print("\xdf")',)  # Latin-1: LATIN SMALL LETTER SHARP S
        readline = self.get_readline(lines)
        self.assertRaises(SyntaxError, detect_encoding, readline)
...
class TestTokenize(TestCase):
    ...
    def test_tokenize(self):
        ...
        try:
            ...
            self.assertEqual(
                list(results)[1:], [b"first", b"second", b"1", b"2", b"3", b"4"]
            )
        finally:
    ...
    def assertExactTypeEqual(self, opstr, *optypes):
        ...
        for i in range(num_optypes):
            self.assertEqual(tok_name[tokens[i + 1].exact_type], tok_name[optypes[i]])
        self.assertEqual(
            tok_name[tokens[1 + num_optypes].exact_type], tok_name[token.NEWLINE]
        )
        self.assertEqual(
            tok_name[tokens[2 + num_optypes].exact_type], tok_name[token.ENDMARKER]
        )
    ...
    def test_comment_at_the_end_of_the_source_without_newline(self):
        ...
        expected_tokens = [
            TokenInfo(
                type=token.ENCODING, string="utf-8", start=(0, 0), end=(0, 0), line=""
            ),
            TokenInfo(
                type=token.NAME, string="b", start=(1, 0), end=(1, 1), line="b = 1\n"
            ),
            TokenInfo(
                type=token.OP, string="=", start=(1, 2), end=(1, 3), line="b = 1\n"
            ),
            TokenInfo(
                type=token.NUMBER, string="1", start=(1, 4), end=(1, 5), line="b = 1\n"
            ),
            TokenInfo(
                type=token.NEWLINE,
                string="\n",
            ...
            TokenInfo(
                type=token.ENDMARKER, string="", start=(4, 0), end=(4, 0), line=""
            ),
        ]
    ...
    def test_newline_and_space_at_the_end_of_the_source_without_newline(self):
        ...
        expected_tokens = [
            TokenInfo(
                token.ENCODING, string="utf-8", start=(0, 0), end=(0, 0), line=""
            ),
...
class UntokenizeTest(TestCase):
    ...
    def test_bad_input_order(self):
        ...
        self.assertEqual(
            cm.exception.args[0], "start (1,3) precedes previous end (2,2)"
        )
...
class InvalidPythonTests(TestCase):
    def test_number_followed_by_name(self):
        ...
        expected_tokens = [
            TokenInfo(
                type=token.NUMBER, string="2", start=(1, 0), end=(1, 1), line="2sin(x)"
            ),
            TokenInfo(
                type=token.NAME, string="sin", start=(1, 1), end=(1, 4), line="2sin(x)"
            ),
            TokenInfo(
                type=token.OP, string="(", start=(1, 4), end=(1, 5), line="2sin(x)"
            ),
            TokenInfo(
                type=token.NAME, string="x", start=(1, 5), end=(1, 6), line="2sin(x)"
            ),
            TokenInfo(
                type=token.OP, string=")", start=(1, 6), end=(1, 7), line="2sin(x)"
            ),
            TokenInfo(
                type=token.NEWLINE, string="", start=(1, 7), end=(1, 8), line="2sin(x)"
            ),
            TokenInfo(
                type=token.ENDMARKER, string="", start=(2, 0), end=(2, 0), line=""
            ),
        ]
    ...
    def test_number_starting_with_zero(self):
        ...
        expected_tokens = [
            ...
            TokenInfo(
                type=token.NEWLINE, string="", start=(1, 5), end=(1, 6), line="01234"
            ),
            TokenInfo(
                type=token.ENDMARKER, string="", start=(2, 0), end=(2, 0), line=""
            ),
...
async def f():
  ...
  await = 2""",
    ...
    def test_max_indent(self):
        ...
        self.assertRaises(IndentationError, compile, invalid, "<string>", "exec")
    ...
    def test_continuation_lines_indentation(self):
        def get_tokens(string):
            ...
            return [
                (kind, string)
                for (kind, string, *_) in _generate_tokens_from_c_tokenizer(
                    the_string.readline
                )
            ]
