...
class SimpleTest(abc.LoaderTests):
    ...
    def test_package(self):
        with util.create_modules("_pkg.__init__") as mapping:
            ...
            check = {
                "__name__": "_pkg",
                "__file__": mapping["_pkg.__init__"],
...
class SourceDateEpochTestMeta(SourceDateEpochTestMeta, type(Source_SimpleTest)):
    pass


class SourceDateEpoch_SimpleTest(
    Source_SimpleTest, metaclass=SourceDateEpochTestMeta, source_date_epoch=True
):
    pass
...
class BadBytecodeTest:
    ...
    def _test_empty_file(self, test, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: b"", del_source=del_source
            )
    ...
    def _test_partial_magic(self, test, *, del_source=False):
        ...
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:3], del_source=del_source
            )
            test("_temp", mapping, bc_path)
    ...
    def _test_magic_only(self, test, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:4], del_source=del_source
            )
            test("_temp", mapping, bc_path)
    ...
    def _test_partial_flags(self, test, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:7], del_source=del_source
            )
            test("_temp", mapping, bc_path)
    ...
    def _test_partial_timestamp(self, test, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:11], del_source=del_source
            )
            test("_temp", mapping, bc_path)
    ...
    def _test_partial_size(self, test, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:15], del_source=del_source
            )
            test("_temp", mapping, bc_path)
    ...
    def _test_no_marshal(self, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bc_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:16], del_source=del_source
            )
    ...
    def _test_bad_marshal(self, *, del_source=False):
        with util.create_modules("_temp") as mapping:
            bytecode_path = self.manipulate_bytecode(
                "_temp", mapping, lambda bc: bc[:16] + b"<test>", del_source=del_source
            )
            file_path = mapping["_temp"] if not del_source else bytecode_path
            with self.assertRaises(EOFError):
                self.import_(file_path, "_temp")
...
class SourcelessLoaderBadBytecodeTestPEP451(
    SourcelessLoaderBadBytecodeTest, BadBytecodeTestPEP451
):
    pass


(Frozen_SourcelessBadBytecodePEP451, Source_SourcelessBadBytecodePEP451) = (
    util.test_both(
...
class SourcelessLoaderBadBytecodeTestPEP302(
    SourcelessLoaderBadBytecodeTest, BadBytecodeTestPEP302
):
    pass


(Frozen_SourcelessBadBytecodePEP302, Source_SourcelessBadBytecodePEP302) = (
    util.test_both(
