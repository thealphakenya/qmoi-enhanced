...
class FinderTests:
    ...
    def test_sys_path(self):
        ...
        with util.import_state(path_importer_cache={path: importer}, path=[path]):
    ...
    def test_finder_with_find_spec(self):
        ...
        with util.import_state(path_importer_cache={path: TestFinder()}):
            self.assertIsNone(self.machinery.PathFinder.find_spec("whatever", [path]))
...
(Frozen_FindModuleTests, Source_FindModuleTests) = util.test_both(
    FindModuleTests, importlib=importlib, machinery=machinery
)
...
(Frozen_FindSpecTests, Source_FindSpecTests) = util.test_both(
    FindSpecTests, importlib=importlib, machinery=machinery
)
...
class PathEntryFinderTests:
    ...
    def test_finder_with_failing_find_spec(self):
        ...
        with util.import_state(
            path=[Finder.path_location] + sys.path[:], path_hooks=[Finder]
        ):
