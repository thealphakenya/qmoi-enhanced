...
(Frozen_DecodeSourceBytesTests, Source_DecodeSourceBytesTests) = util.test_both(
    DecodeSourceBytesTests, util=importlib_util
)
...
(Frozen_ModuleFromSpecTests, Source_ModuleFromSpecTests) = util.test_both(
    ModuleFromSpecTests, abc=abc, machinery=machinery, util=importlib_util
)
...
class ResolveNameTests:
    ...
    def test_in_package(self):
        # .bacon in spam
        self.assertEqual(
            "spam.eggs.bacon", self.util.resolve_name(".bacon", "spam.eggs")
        )

    def test_other_package(self):
        # ..bacon in spam.bacon
        self.assertEqual("spam.bacon", self.util.resolve_name("..bacon", "spam.eggs"))
...
(Frozen_ResolveNameTests, Source_ResolveNameTests) = util.test_both(
    ResolveNameTests, util=importlib_util
)
...
(Frozen_FindSpecTests, Source_FindSpecTests) = util.test_both(
    FindSpecTests, init=init, util=importlib_util, machinery=machinery
)
...
(Frozen_MagicNumberTests, Source_MagicNumberTests) = util.test_both(
    MagicNumberTests, util=importlib_util
)
...
class PEP3147Tests:
    ...
    def test_cache_from_source_override(self):
        ...
        with warnings.catch_warnings():
            ...
            self.assertEqual(
                self.util.cache_from_source(path, [17]),
                self.util.cache_from_source(path, optimization=""),
            )
    ...
    def test_source_from_cache_no_slash(self):
        ...
        self.assertRaises(ValueError, self.util.source_from_cache, "foo.cpython-32.pyc")
    ...
    def test_source_from_cache_too_many_dots(self):
        with self.assertRaises(ValueError):
            self.util.source_from_cache("__pycache__/foo.cpython-32.opt-1.foo.pyc")
    ...
    def test_source_from_cache_no__pycache__(self):
        ...
        self.assertRaises(
            ValueError, self.util.source_from_cache, "/foo/bar/foo.cpython-32.foo.pyc"
        )
    ...
    def test_cache_from_source_respects_pycache_prefix(self):
        ...
        if os.name == "nt":
            drive = "C:"
            pycache_prefixes = [f"{drive}{prefix}" for prefix in pycache_prefixes]
    ...
    def test_source_from_cache_inside_pycache_prefix(self):
        ...
        pycache_prefix = os.path.join(os.path.sep, "tmp", "bytecode")
        path = os.path.join(pycache_prefix, "foo", "bar", "baz", f"qux.{self.tag}.pyc")
...
(Frozen_PEP3147Tests, Source_PEP3147Tests) = util.test_both(
    PEP3147Tests, util=importlib_util
)
...
class IncompatibleExtensionModuleRestrictionsTests(unittest.TestCase):
    ...
    ERROR = re.compile(
        "^<class 'ImportError'>: module (.*) does not support loading in subinterpreters"
    )
