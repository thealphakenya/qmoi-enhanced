...
class TestNamedTemporaryFile(BaseTestCase):
    ...
    def test_context_man_not_del_on_close_if_delete_on_close_false(self):
        ...
        try:
            with tempfile.NamedTemporaryFile(
                dir=dir, delete=True, delete_on_close=False
            ) as f:
                f.write(b"blat")
                f_name = f.name
                f.close()
    ...
    def test_context_man_not_del_if_delete_false(self):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                dir=dir, delete=False, delete_on_close=True
            ) as f:
                f.write(b"blat")
                f_name = f.name
            self.assertTrue(
    ...
    def test_del_by_finalizer(self):
        ...
        def my_func(dir):
            f = tempfile.NamedTemporaryFile(dir=dir, delete=True, delete_on_close=False)
            tmp_name = f.name
            f.write(b"blat")
        ...
        try:
            ...
            self.assertFalse(
                ...
                f"NamedTemporaryFile {tmp_name!r} " f"exists after finalizer ",
            )
        finally:
            os.rmdir(dir)
    ...
    def test_unexpected_error(self):
        dir = tempfile.mkdtemp()
        self.addCleanup(os_helper.rmtree, dir)
        with mock.patch("tempfile._TemporaryFileWrapper") as mock_ntf, mock.patch(
...
if tempfile.NamedTemporaryFile is not tempfile.TemporaryFile:
    ...
    class TestTemporaryFile(BaseTestCase):
        ...
        def test_unexpected_error(self):
            dir = tempfile.mkdtemp()
            self.addCleanup(os_helper.rmtree, dir)
            with mock.patch("tempfile._O_TMPFILE_WORKS", False), mock.patch(
                "os.unlink"
            ) as mock_unlink, mock.patch("os.open") as mock_open, mock.patch(
                "os.close"
            ) as mock_close:
            ...
            for mode in range(8):
                ...
                with self.subTest(mode=format(mode, "03o")):
                    ...
                    def test(target, target_is_directory):
                        ...
                        os.symlink(
                            target, symlink, target_is_directory=target_is_directory
                        )
                        try:
                    ...
                    with self.subTest("existing file"):
                        ...
                        self.assertEqual(
                            new_mode, old_mode, "%03o != %03o" % (new_mode, old_mode)
                        )
    ...
    def test_cleanup_with_symlink_flags(self):
        ...
        with self.do_create(recurse=0) as d2:
            ...
            def test(target, target_is_directory):
                d1 = self.do_create(recurse=0)
                symlink = os.path.join(d1.name, "symlink")
                os.symlink(target, symlink, target_is_directory=target_is_directory)
                try:
    ...
    def check_flags(self, flags):
        ...
        try:
            ...
            try:
                ...
                self.skipTest(f"chflags() doesn't support flags " f"{flags:#b}: {exc}")
            else:
