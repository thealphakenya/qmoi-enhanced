...
class HandlerTests(unittest.TestCase):
    ...
    def test_winreg_proxy_bypass(self):
        ...
        for host in ("www.example.com", "www.example.net", "192.168.0.1"):
            self.assertTrue(
                proxy_bypass(host, proxy_override),
                "expected bypass of %s to be true" % host,
            )

        for host in ("example.com", "www.example.org", "example.net", "192.168.0.2"):
            self.assertFalse(
                proxy_bypass(host, proxy_override),
        ...
        self.assertTrue(
            proxy_bypass("example.com", proxy_override),
            "expected bypass of %s to be true" % host,
        )
        self.assertFalse(
            proxy_bypass("example.net", proxy_override),
            "expected bypass of %s to be False" % host,
        )
        for host in ("test", "localhost"):
            self.assertTrue(
                proxy_bypass(host, proxy_override),
                "expect <local> to bypass intranet address '%s'" % host,
            )
