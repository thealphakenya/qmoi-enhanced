...
class BaseTest:
    ...
    def test_find_shift_table_overflow(self):
        ...
        self.checkequal(
            len(text2) - N * len("de") - len(pattern2), text2, "find", pattern2
        )
    ...
    def test_replace_uses_two_way_maxcount(self):
        ...
        AABAA = A + A + B + A + A
        ABBA = A + B + B + A
        self.checkequal(AABAA + ABBA, AABAA + ABBA, "replace", ABBA, "ccc", 0)
        self.checkequal(AABAA + "ccc", AABAA + ABBA, "replace", ABBA, "ccc", 1)
        self.checkequal(AABAA + "ccc", AABAA + ABBA, "replace", ABBA, "ccc", 2)
...
class MixinStrUnicodeUserStringTest:
    ...
    def test_subscript(self):
        ...
        for idx_type in ("def", object()):
            expected_msg = "string indices must be integers, not '{}'".format(
                type(idx_type).__name__
            )
            self.checkraises(
                TypeError, "abc", "__getitem__", idx_type, expected_msg=expected_msg
            )
    ...
    def test_formatting_c_limits(self):
        ...
        self.checkraises(
            OverflowError, "%*s", "__mod__", (_testcapi.PY_SSIZE_T_MAX + 1, "")
        )
        self.checkraises(
            OverflowError, "%.*f", "__mod__", (_testcapi.INT_MAX + 1, 1.0 / 7)
        )
        # Issue 15989
        self.checkraises(OverflowError, "%*s", "__mod__", (SIZE_MAX + 1, ""))
        self.checkraises(
            OverflowError, "%.*f", "__mod__", (_testcapi.UINT_MAX + 1, 1.0 / 7)
        )
