...
class OrderedDictTests:
    ...
    def test_init(self):
        ...
        self.assertEqual(
            sorted(OrderedDict(**dict(pairs)).items()), pairs
        )  # kwds input
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(od), "OrderedDict({'c': 1, 'b': 2, 'a': 3, 'd': 4, 'e': 5, 'f': 6})"
        )
    ...
    def test_free_after_iterating(self):
        ...
        support.check_free_after_iterating(
            self, lambda d: iter(d.keys()), self.OrderedDict
        )
        support.check_free_after_iterating(
            self, lambda d: iter(d.values()), self.OrderedDict
        )
        support.check_free_after_iterating(
            self, lambda d: iter(d.items()), self.OrderedDict
        )
