...
class XMLRPCTestCase(unittest.TestCase):
    ...
    def test_load_standard_types(self):
        ...
        check(
            "<base64>AGJ5dGUgc3RyaW5n/w==</base64>",
    ...
    def test_load_extension_types(self):
        ...
        check(
            "<bigdecimal>9876543210.0123456789</bigdecimal>",
    ...
    def test_get_host_info(self):
        ...
        self.assertEqual(
            ...
            ("host.tld", [("Authorization", "Basic dXNlcg==")], {}),
        )
...
class FaultTestCase(unittest.TestCase):
    ...
    def test_dotted_attribute(self):
        ...
        # private methods
        self.assertRaises(
            AttributeError, xmlrpc.server.resolve_dotted_attribute, str, "__add"
        )
...
def http_multi_server(evt, numrequests, requestHandler=None):
    ...
    serv = MyXMLRPCServer(
        ("localhost", 0), MyRequestHandler, logRequests=False, bind_and_activate=False
    )
...
class GzipServerTestCase(BaseServerTestCase):
    ...
    def test_bad_gzip_request(self):
        ...
        t.fake_gzip = True
        p = xmlrpclib.ServerProxy(URL, transport=t)
        cm = self.assertRaisesRegex(xmlrpclib.ProtocolError, re.compile(r"\b400\b"))
        with cm:
            p.pow(6, 8)
        p("close")()
...
class CGIHandlerTestCase(unittest.TestCase):
    ...
    def test_cgi_xmlrpc_response(self):
        ...
        with os_helper.EnvironmentVarGuard() as env, captured_stdout(
        ...
        self.assertEqual(
            int(re.search(r"Content-Length: (\d+)", handle).group(1)), len(content)
        )
...
class UseBuiltinTypesTestCase(unittest.TestCase):
    ...
    def test_xmlrpcserver_has_use_builtin_types_flag(self):
        server = xmlrpc.server.SimpleXMLRPCServer(
            ("localhost", 0), use_builtin_types=True
        )
        server.server_close()
