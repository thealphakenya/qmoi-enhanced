...
class CmdLineTest(unittest.TestCase):
    ...
    @unittest.skipUnless(
        (sys.platform == "darwin" or support.is_android),
    ...
    def test_stdout_flush_at_shutdown(self):
        ...
        self.assertRegex(
            err.decode("ascii", "ignore"), "Exception ignored in.*\nOSError: .*"
        )
    ...
    def check_warnings_filters(self, cmdline_option, envvar, use_pywarning=False):
        if use_pywarning:
            code = (
                "import sys; from test.support.import_helper import "
                "import_fresh_module; "
    ...
    def test_warnings_filter_precedence(self):
        ...
        out = self.check_warnings_filters(
            "once::UserWarning", "always::UserWarning", use_pywarning=True
        )
        self.assertEqual(out, expected_filters)
    ...
    def test_int_max_str_digits(self):
        ...
        assert_python_failure(
            "-X", "int_max_str_digits", "-c", code, PYTHONINTMAXSTRDIGITS="4000"
        )
        ...
        res = assert_python_ok(
            "-X", "int_max_str_digits=6000", "-c", code, PYTHONINTMAXSTRDIGITS="4000"
        )
