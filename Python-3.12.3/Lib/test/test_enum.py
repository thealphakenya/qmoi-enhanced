    ...
    def setUp(self):
        if self.__class__.__name__[-5:] == "Class":
            ...
            self.NewStrFormatEnum = self.enum_type(
                "NewStrFormatEnum",
                dict(first=first, __format__=__format__, __str__=__str__),
            )
...
class TestPlainFlagClass(_EnumTests, _PlainOutputTests, _FlagTests, unittest.TestCase):
    ...
    def test_none_member(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            r"'FlagWithNoneMember.E' cannot be combined with other flags with |",
        ):
        ...
        with self.assertRaisesRegex(
            ...
            FlagWithNoneMember.A ^ FlagWithNoneMember.E
        with self.assertRaisesRegex(
            TypeError, r"'FlagWithNoneMember.E' cannot be inverted"
        ):
            ~FlagWithNoneMember.E


class TestPlainFlagFunction(
    _EnumTests, _PlainOutputTests, _FlagTests, unittest.TestCase
):
    enum_type = Flag
...
class TestIntFlagFunction(
    _EnumTests, _MinimalOutputTests, _FlagTests, unittest.TestCase
):
    enum_type = IntFlag
...
class TestMixedIntFlagClass(
    _EnumTests, _MixedOutputTests, _FlagTests, unittest.TestCase
):
...
class TestMixedDateFunction(_EnumTests, _MixedOutputTests, unittest.TestCase):
    ...
    #
    enum_type = Enum(
        "enum_type", {"_generate_next_value_": _generate_next_value_}, type=date
    )
...
class TestMixedFloatFunction(_EnumTests, _MixedOutputTests, unittest.TestCase):
    ...
    #
    enum_type = Enum(
        "enum_type", {"_generate_next_value_": _generate_next_value_}, type=float
    )
...
class TestSpecial(unittest.TestCase):
    ...
    def test_repr_with_dataclass(self):
        ...
        self.assertRegex(
            repr(Creature.DOG), "<Creature.DOG: .*CreatureDataMixin object at .*>"
        )
    ...
    def test_namedtuple_as_value(self):
        ...
        self.assertEqual(
            repr(NTEnum.NONE), "<NTEnum.NONE: TTuple(id=0, a=0, blist=[])>"
        )
        ...
        self.assertEqual(
            repr(NTCEnum.NONE), "<NTCEnum.NONE: TTuple(id=0, a=0, blist=[])>"
        )
        ...
        self.assertEqual(
            repr(NTDEnum.NONE), "<NTDEnum.NONE: TTuple(id=0, a=0, blist=[])>"
        )
...
class OldTestFlag(unittest.TestCase):
    ...
    def test_pickle(self):
        ...
        test_pickle_dump_load(
            self.assertEqual, IntFlagStooges.CURLY | IntFlagStooges.BIG
        )
...
class TestInternals(unittest.TestCase):
    ...
    def test_auto_garbage_corrected_ok(self):
        ...
        self.assertEqual(
            list(Color), [Color.red, Color.blue, Color.green, Color.yellow]
        )
        self.assertEqual(Color.red.value, "red")
        self.assertEqual(Color.blue.value, 2)
        self.assertEqual(Color.green.value, 3)
