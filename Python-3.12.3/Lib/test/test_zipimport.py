...
def make_pyc(co, mtime, size):
    ...
    pyc = (
        ...
        + struct.pack("<iLL", 0, int(mtime) & 0xFFFF_FFFF, size & 0xFFFF_FFFF)
        + data
    )
    return pyc
...
class UncompressedZipImportTestCase(ImportHooksBaseTestCase):
    ...
    def doDoctestFile(self, module):
        ...
        try:
            doctest.testfile(
                "xyz.txt", package=module, module_relative=True, globs=locals()
            )
        finally:
        ...
        try:
            ...
            self.assertTrue(
                s.getvalue().endswith(
                    ...
                    if support.has_no_debug_ranges()
                    else "                    ^^^^^^^^^^^^^^^\n"
                )
            )
        else:
    ...
    @unittest.skipIf(
        os_helper.TESTFN_UNENCODABLE is None, "need an unencodable filename"
    )
