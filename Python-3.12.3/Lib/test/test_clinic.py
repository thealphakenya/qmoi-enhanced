...
def save_restore_converters(testcase):
    testcase.addCleanup(restore_dict, clinic.converters, clinic.converters.copy())
    testcase.addCleanup(
        restore_dict, clinic.legacy_converters, clinic.legacy_converters.copy()
    )
    testcase.addCleanup(
        restore_dict, clinic.return_converters, clinic.return_converters.copy()
    )
...
class ClinicParserTest(_ParserBase):
    def checkDocstring(self, fn, expected):
        ...
        self.assertEqual(fn.docstring.strip(), dedent(expected).strip())
    ...
    def test_scaffolding(self):
        ...
        expected = 'Error in file "clown.txt" on line 69:\n' "The igloos are melting!\n"
        with support.captured_stdout() as stdout:
            with self.assertRaises(SystemExit):
                clinic.fail(
