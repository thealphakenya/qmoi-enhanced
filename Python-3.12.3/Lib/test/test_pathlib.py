...
class _BasePurePathTest(object):
    ...
    def test_equivalences(self):
        for k, tuples in self.equivalences.items():
            ...
            if canon != posix:
                tuples = tuples + [
                    tuple(part.replace("/", self.sep) for part in t) for t in tuples
                ]
                tuples.append((posix,))
    ...
    def test_stem_common(self):
        ...
        self.assertEqual(
            P("a/Some name. Ending with a dot.").stem, "Some name. Ending with a dot."
        )
...
class PureWindowsPathTest(_BasePurePathTest, unittest.TestCase):
    ...
    def test_stem(self):
        ...
        self.assertEqual(
            P("c:a/Some name. Ending with a dot.").stem, "Some name. Ending with a dot."
        )
    ...
    def test_relative_to(self):
        ...
        self.assertEqual(
            p.relative_to(P("C:Baz/Bar"), walk_up=True), P("../../Foo/Bar")
        )
        # Unrelated paths.
        self.assertRaises(ValueError, p.relative_to, P())
        self.assertRaises(ValueError, p.relative_to, "")
        ...
        self.assertEqual(
            p.relative_to(P("//sErver/sHare/bar"), walk_up=True), P("../Foo/Bar")
        )
        self.assertEqual(
            p.relative_to("//sErver/sHare/bar", walk_up=True), P("../Foo/Bar")
        )
        # Unrelated paths.
        ...
        self.assertRaises(
            ValueError, p.relative_to, P("/Server/Share/Foo"), walk_up=True
        )
        self.assertRaises(
            ValueError, p.relative_to, P("c:/Server/Share/Foo"), walk_up=True
        )
...
class _BasePathTest(object):
    ...
    @unittest.skipIf(pwd is None, reason="Test requires pwd module to get homedir.")
    def test_home(self):
    ...
    def test_read_write_text(self):
        ...
        self.assertEqual(
            (p / "fileA").read_text(encoding="utf-8", errors="ignore"), "bcdefg"
        )
    ...
    def test_glob_common(self):
        ...
        if not os_helper.can_symlink():
            ...
            _check(
                p.glob("*B/*"),
                ["dirB/fileB", "dirB/linkD", "linkB/fileB", "linkB/linkD"],
            )
        if not os_helper.can_symlink():
    ...
    def test_rglob_common(self):
        ...
        if not os_helper.can_symlink():
            ...
            _check(
                p.rglob("*/fileB"),
                ["dirB/fileB", "dirB/linkD/fileB", "linkB/fileB", "dirA/linkC/fileB"],
            )
        _check(
            p.rglob("file*"), ["fileA", "dirB/fileB", "dirC/fileC", "dirC/dirD/fileD"]
        )
        if not os_helper.can_symlink():
            _check(
                p.rglob("*/"),
        ...
        _check(
            p.rglob("*"),
            ["dirC/fileC", "dirC/novel.txt", "dirC/dirD", "dirC/dirD/fileD"],
        )
    ...
    def test_resolve_common(self):
        ...
        self.assertEqualNormCase(
            str(p.resolve(strict=False)), os.path.join(BASE, "foo")
        )
        p = P(BASE, "foo", "in", "spam")
        self.assertEqualNormCase(
            str(p.resolve(strict=False)), os.path.join(BASE, "foo", "in", "spam")
        )
        p = P(BASE, "..", "foo", "in", "spam")
        self.assertEqualNormCase(
    ...
    @unittest.skipIf(is_emscripten, "Unix sockets are not implemented on Emscripten.")
    @unittest.skipIf(is_wasi, "Cannot create socket on WASI.")
...
class PathTest(_BasePathTest, unittest.TestCase):
    ...
    def test_concrete_class(self):
        p = self.cls("a")
        self.assertIs(
            type(p), pathlib.WindowsPath if os.name == "nt" else pathlib.PosixPath
        )
...
class PosixPathTest(_BasePathTest, unittest.TestCase):
    ...
    @unittest.skipIf(
        is_emscripten or is_wasi, "umask is not implemented on Emscripten/WASI."
    )
    ...
    @unittest.skipUnless(
        hasattr(pwd, "getpwall"), "pwd module does not expose getpwall()"
    )
    ...
    @unittest.skipIf(
        sys.platform != "darwin", "Bad file descriptor in /dev/fd affects only macOS"
    )
...
class WindowsPathTest(_BasePathTest, unittest.TestCase):
    ...
    def test_absolute(self):
        ...
        with mock.patch("os.getcwd") as getcwd:
            ...
            self.assertEqual(
                str(P("a", "b", "c").absolute()), os.path.join(share, "a", "b", "c")
            )
        ...
        with os_helper.change_cwd(BASE):
            ...
            self.assertEqual(
                str(P(drive + "foo").absolute()), os.path.join(BASE, "foo")
            )
        ...
        with os_helper.subst_drive(BASE) as other_drive:
            ...
            self.assertEqual(
                str(P(other_drive + "foo").absolute()), other_cwd + "\\foo"
            )
