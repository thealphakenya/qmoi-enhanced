...
class Test_Csv(unittest.TestCase):
    ...
    def _test_arg_valid(self, ctor, arg):
        ...
        self.assertRaises(TypeError, ctor, arg, quoting=csv.QUOTE_ALL, quotechar="")
        self.assertRaises(TypeError, ctor, arg, quoting=csv.QUOTE_ALL, quotechar=None)
        self.assertRaises(TypeError, ctor, arg, quoting=csv.QUOTE_NONE, quotechar="")
        ctor(arg, delimiter=" ")
        ctor(arg, escapechar=" ")
        ctor(arg, quotechar=" ")
    ...
    def test_write_escape(self):
        ...
        self._write_error_test(
            csv.Error, ["a", 1, 'p,"q"'], escapechar=None, doublequote=False
        )
        self._write_test(
            ["a", 1, 'p,"q"'], 'a,1,"p,\\"q\\""', escapechar="\\", doublequote=False
        )
        self._write_test(['"'], '""""', escapechar="\\", quoting=csv.QUOTE_MINIMAL)
        self._write_test(
            ['"'], '\\"', escapechar="\\", quoting=csv.QUOTE_MINIMAL, doublequote=False
        )
        self._write_test(['"'], '\\"', escapechar="\\", quoting=csv.QUOTE_NONE)
        self._write_test(
            ["a", 1, "p,q"], "a,1,p\\,q", escapechar="\\", quoting=csv.QUOTE_NONE
        )
        self._write_test(["\\", "a"], "\\\\,a", escapechar="\\", quoting=csv.QUOTE_NONE)
        self._write_test(
            ["\\", "a"], "\\\\,a", escapechar="\\", quoting=csv.QUOTE_MINIMAL
        )
        self._write_test(
            ["\\", "a"], '"\\\\","a"', escapechar="\\", quoting=csv.QUOTE_ALL
        )
        self._write_test(
            ["\\ ", "a"], "\\\\ ,a", escapechar="\\", quoting=csv.QUOTE_MINIMAL
        )
        self._write_test(
            ["\\,", "a"], "\\\\\\,,a", escapechar="\\", quoting=csv.QUOTE_NONE
        )
        self._write_test(
            [",\\", "a"], '",\\\\",a', escapechar="\\", quoting=csv.QUOTE_MINIMAL
        )
    ...
    def test_write_empty_fields_space_delimiter(self):
        ...
        self._write_test(
            ["", ""], " ", delimiter=" ", skipinitialspace=False, quoting=csv.QUOTE_NONE
        )
        self._write_error_test(
            csv.Error,
            ["", ""],
        ...
        for quoting in csv.QUOTE_STRINGS, csv.QUOTE_NOTNULL:
            ...
            self._write_test(
                ["", ""], '"" ""', delimiter=" ", skipinitialspace=True, quoting=quoting
            )
    ...
    def test_read_oddinputs(self):
        ...
        self.assertRaises(csv.Error, self._read_test, [b"abc"], None)
    ...
    def test_read_eof(self):
        ...
        self.assertRaises(csv.Error, self._read_test, ['"a'], [], strict=True)
        self.assertRaises(
            csv.Error, self._read_test, ["^"], [], escapechar="^", strict=True
        )
    ...
    def test_read_quoting(self):
        ...
        self._read_test(
            ['1,",3,",5'], [["1", '"', "3", '"', "5"]], quotechar=None, escapechar="\\"
        )
        self._read_test(
            ['1,",3,",5'],
            [["1", '"', "3", '"', "5"]],
        ...
        self._read_test(
            [',3,"5",7.3, 9'], [["", 3, "5", 7.3, 9]], quoting=csv.QUOTE_NONNUMERIC
        )
        self._read_test(['"a\nb", 7'], [["a\nb", " 7"]])
        self.assertRaises(
            ValueError, self._read_test, ["abc,3"], [[]], quoting=csv.QUOTE_NONNUMERIC
        )
            ...
            with self.subTest(lineterminator=lineterminator):
                with TemporaryFile("w+", encoding="utf-8", newline="") as fileobj:
                    ...
                    for i, row in enumerate(
                        csv.reader(fileobj, quoting=csv.QUOTE_NONE, escapechar="\\")
                    ):
                        self.assertEqual(row, rows[i])
...
class TestDictFields(unittest.TestCase):
    ...
    def test_write_multiple_dict_rows(self):
        ...
        writer.writerows(
            [{"f1": 1, "f2": "abc", "f3": "f"}, {"f1": 2, "f2": 5, "f3": "xyz"}]
        )
        self.assertEqual(fileobj.getvalue(), "f1,f2,f3\r\n1,abc,f\r\n2,5,xyz\r\n")
    ...
    def test_typo_in_extrasaction_raises_error(self):
        fileobj = StringIO()
        self.assertRaises(
            ValueError, csv.DictWriter, fileobj, ["f1", "f2"], extrasaction="raised"
        )
...
class TestDialectValidity(unittest.TestCase):
    ...
    def test_escapechar(self):
        ...
        with self.assertRaisesRegex(
            csv.Error, '"escapechar" must be a 1-character string'
        ):
            mydialect()
        ...
        with self.assertRaisesRegex(
            csv.Error, '"escapechar" must be string or None, not bytes'
        ):
            mydialect()

        mydialect.escapechar = 4
        with self.assertRaisesRegex(
            csv.Error, '"escapechar" must be string or None, not int'
        ):
            mydialect()
...
@unittest.skipUnless(
    hasattr(sys, "gettotalrefcount"), "requires sys.gettotalrefcount()"
)
