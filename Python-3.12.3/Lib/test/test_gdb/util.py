...
def run_gdb(*args, exitcode=0, check=True, **env_vars):
    ...
    if GDB_VERSION >= (7, 4):
        cmd.extend(
            ("--init-eval-command", f"add-auto-load-safe-path {CHECKOUT_HOOK_PATH}")
        )
    cmd.extend(args)
    ...
    if check and proc.returncode != exitcode:
        ...
        raise Exception(
            f"{cmd_text} failed with exit code {proc.returncode}, "
...
def check_usable_gdb():
    ...
    if "auto-loading has been declined" in stderr:
        raise unittest.SkipTest(
            f"gdb security settings prevent use of custom hooks; " f"stderr: {stderr!r}"
        )

    if not stdout:
        raise unittest.SkipTest(
            f"gdb not built with embedded python support; " f"stderr: {stderr!r}"
        )
...
def cet_protection():
    ...
    return ("-mcet" in flags) and any(
        (flag.startswith("-fcf-protection") and not flag.endswith(("=none", "=return")))
        for flag in flags
    )
...
class DebuggerTests(unittest.TestCase):
    ...
    def get_stack_trace(
        ...
        if "PC not saved" in err:
            raise unittest.SkipTest(
                "gdb cannot walk the frame object"
