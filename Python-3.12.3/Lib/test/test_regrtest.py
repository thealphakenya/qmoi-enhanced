...
class BaseTestCase(unittest.TestCase):
    ...
    def check_executed_tests(
        ...
        *,
        ...
        if env_changed:
            regex = list_regex(
                r"%s test%s altered the execution environment " r"\(env changed\)",
                env_changed,
            )
            self.check_line(output, regex)
...
class ArgsTestCase(BaseTestCase):
    ...
    def test_random(self):
        ...
        output = self.run_tests(
            "-r", f"--randseed={randseed}", test, exitcode=EXITCODE_NO_TESTS_RAN
        )
        ...
        output = self.run_tests("-r", test, exitcode=EXITCODE_NO_TESTS_RAN, env=env)
        randseed = self.parse_random_seed(output)
    ...
    def test_interrupted(self):
        ...
        self.check_executed_tests(output, test, omitted=test, interrupted=True, stats=0)
    ...
    def test_forever(self):
        ...
        self.check_executed_tests(
            output, [test] * 3, failed=test, stats=TestStats(3, 1), forever=True
        )
    ...
    def test_env_changed(self):
        ...
        self.check_executed_tests(output, [testname], env_changed=testname, stats=1)
        ...
        self.check_executed_tests(
            output, [testname], env_changed=testname, fail_env_changed=True, stats=1
        )
    ...
    def test_no_tests_ran_skip(self):
        ...
        output = self.run_tests(testname)
        self.check_executed_tests(output, [testname], stats=TestStats(1, skipped=1))
    ...
    def test_no_tests_ran_multiple_tests_nonexistent(self):
        ...
        output = self.run_tests(
            testname, testname2, "-m", "nosuchtest", exitcode=EXITCODE_NO_TESTS_RAN
        )
        self.check_executed_tests(
            output,
            [testname, testname2],
    ...
    def test_no_test_ran_some_test_exist_some_not(self):
        ...
        output = self.run_tests(
            testname, testname2, "-m", "nosuchtest", "-m", "test_other_bug", exitcode=0
        )
        self.check_executed_tests(
            output,
            [testname, testname2],
    ...
    @unittest.skipIf(support.is_wasi, "checking temp files is not implemented on WASI")
    def test_leak_tmp_file(self):
        ...
        output = self.run_tests(
            "--fail-env-changed", "-v", "-j2", *testnames, exitcode=EXITCODE_ENV_CHANGED
        )
        self.check_executed_tests(
            output,
            testnames,
        ...
        for testname in testnames:
            self.assertIn(
                f"Warning -- {testname} leaked temporary " f"files (1): mytmpfile",
                output,
            )
    ...
    def test_doctest(self):
        ...
        output = self.run_tests(
            "--fail-env-changed", "-v", "-j1", testname, exitcode=EXITCODE_BAD_TEST
        )
        self.check_executed_tests(
            output,
            [testname],
    ...
    def _check_random_seed(self, run_workers: bool):
        ...
        tests = [
            self.create_test(name=f"test_random{i}", code=code) for i in range(1, 3 + 1)
        ]
    ...
    def test_python_command(self):
        ...
        self.check_executed_tests(output, tests, stats=len(tests), parallel=True)
    ...
    @unittest.skipIf(support.is_android, "raising SIGSEGV on Android is unreliable")
    def test_worker_output_on_failure(self):
        ...
        output = self.run_tests("-j1", testname, exitcode=EXITCODE_BAD_TEST, env=env)
        self.check_executed_tests(
...
class TestUtils(unittest.TestCase):
    ...
    def test_normalize_test_name(self):
        ...
        self.assertEqual(
            normalize("test_access (test.test_os.FileTests.test_access)"), "test_access"
        )
        self.assertEqual(
            normalize("setUpClass (test.test_os.ChownFileTests)", is_error=True),
            "ChownFileTests",
        )
        self.assertEqual(
            normalize(
                "test_success (test.test_bug.ExampleTests.test_success)", is_error=True
            ),
            "test_success",
        )
    ...
    def test_format_resources(self):
        ...
        self.assertEqual(format_resources(("network",)), "resources (1): network")
        self.assertEqual(
        ...
        self.assertEqual(
            format_resources(tuple(name for name in ALL_RESOURCES if name != "cpu")),
            "resources: all,-cpu",
        )
        self.assertEqual(
            format_resources((*ALL_RESOURCES, "tzdata")), "resources: all,tzdata"
        )
