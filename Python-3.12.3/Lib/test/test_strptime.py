...
class LocaleTime_Tests(unittest.TestCase):
    ...
    def test_weekday(self):
        ...
        self.compare_against_time(
            self.LT_ins.f_weekday, "%A", 6, "Testing of full weekday name failed"
        )
        self.compare_against_time(
            self.LT_ins.a_weekday, "%a", 6, "Testing of abbreviated weekday name failed"
        )
    ...
    def test_month(self):
        ...
        # within the tuple
        self.compare_against_time(
            self.LT_ins.f_month, "%B", 1, "Testing against full month name failed"
        )
        self.compare_against_time(
            self.LT_ins.a_month,
            "%b",
    ...
    def test_am_pm(self):
        ...
        self.assertIn(
            strftime_output, self.LT_ins.am_pm, "AM/PM representation not in tuple"
        )
        if self.time_tuple[3] < 12:
            ...
            position = 1
        self.assertEqual(
            self.LT_ins.am_pm[position],
            strftime_output,
            "AM/PM representation in the wrong position within the tuple",
        )

    @unittest.skipIf(support.is_emscripten, "musl libc issue on Emscripten, bpo-46390")
    ...
    def test_date_time(self):
        ...
        self.assertEqual(
            time.strftime(self.LT_ins.LC_date_time, magic_date),
            strftime_output,
            "LC_date_time incorrect",
        )
        strftime_output = time.strftime("%x", magic_date)
        self.assertEqual(
            time.strftime(self.LT_ins.LC_date, magic_date),
            strftime_output,
            "LC_date incorrect",
        )
        strftime_output = time.strftime("%X", magic_date)
        self.assertEqual(
            time.strftime(self.LT_ins.LC_time, magic_date),
            strftime_output,
            "LC_time incorrect",
        )
...
class TimeRETests(unittest.TestCase):
    ...
    def test_compile(self):
        ...
        self.assertTrue(
            found and found.group("A") == self.locale_time.f_weekday[6],
...
class StrptimeTests(unittest.TestCase):
    ...
    def test_ValueError(self):
        ...
        self.assertRaises(
            ValueError, _strptime._strptime_time, data_string="%d", format="%A"
        )
        for bad_format in ("%", "% ", "%e"):
            try:
        ...
        msg_week_no_year_or_weekday = (
            r"ISO week directive '%V' must be used with "
            r"the ISO year directive '%G' and a weekday directive "
            r"\('%A', '%a', '%w', or '%u'\)."
        )
        msg_week_not_compatible = (
            r"ISO week directive '%V' is incompatible with "
            r"the year directive '%Y'. Use the ISO year '%G' instead."
        )
        msg_julian_not_compatible = (
            r"Day of the year directive '%j' is not "
            r"compatible with ISO year directive '%G'. Use '%Y' instead."
        )
        msg_year_no_week_or_weekday = (
            r"ISO year directive '%G' must be used with "
        ...
        subtests = [
            ...
            (
                "1999 {}".format(locale_time.f_weekday[5]),
                "%G %A",
                msg_year_no_week_or_weekday,
            ),
            (
                "1999 {}".format(locale_time.a_weekday[5]),
                "%G %a",
                msg_year_no_week_or_weekday,
            ),
    ...
    @unittest.skipUnless(hasattr(time, "tzset"), "time module has no attribute tzset")
    ...
    def test_percent(self):
        ...
        self.assertTrue(
            strp_output[0] == self.time_tuple[0]
            and strp_output[1] == self.time_tuple[1],
    ...
    def test_caseinsensitive(self):
        ...
        self.assertTrue(
            _strptime._strptime_time(strf_output.upper(), "%B"),
            "strptime does not handle ALL-CAPS names properly",
        )
        self.assertTrue(
            _strptime._strptime_time(strf_output.lower(), "%B"),
            "strptime does not handle lowercase names properly",
        )
        self.assertTrue(
            _strptime._strptime_time(strf_output.capitalize(), "%B"),
    ...
    def test_mar1_comes_after_feb29_even_when_omitting_the_year(self):
        self.assertLess(
            time.strptime("Feb 29", "%b %d"), time.strptime("Mar 1", "%b %d")
        )
...
class CalculationTests(unittest.TestCase):
    ...
    def test_gregorian_calculation(self):
        ...
        self.assertTrue(
            result.tm_year == self.time_tuple.tm_year
    ...
    if support.is_android:
        ...
        _ymd_excluded = ((1905, 1, 1), (1906, 12, 31), (2008, 12, 29), (1917, 12, 31))
    ...
    def test_week_of_year_and_day_of_week_calculation(self):
        ...
        def test_helper(ymd_tuple, test_reason):
            for year_week_format in ("%Y %W", "%Y %U", "%G %V"):
                if (
                    year_week_format in self._formats_excluded
                ...
                for weekday_format in ("%w", "%u", "%a", "%A"):
                    ...
                    with self.subTest(
                        test_reason, date=ymd_tuple, format=format_string
                    ):
        ...
        test_helper(
            (1917, 12, 17),
            "year starting and ending on Monday with "
