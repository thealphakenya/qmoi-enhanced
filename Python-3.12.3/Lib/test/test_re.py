...
class ReTests(unittest.TestCase):
    ...
    def test_branching(self):
        ...
        self.assertEqual(re.match("(abc|bac|ca|cb)", "abc").span(), (0, 3))
        self.assertEqual(re.match("(abc|bac|ca|cb)", "bac").span(), (0, 3))
        self.assertEqual(re.match("(abc|bac|ca|cb)", "ca").span(), (0, 2))
        self.assertEqual(re.match("(abc|bac|ca|cb)", "cb").span(), (0, 2))
    ...
    def test_symbolic_groups_errors(self):
        ...
        self.checkPatternError(
            b"(?P<\xc2\xb5>x)", r"bad character in group name '\xc2\xb5'", 4
        )
        self.checkPatternError(
            b"(?P=\xc2\xb5)", r"bad character in group name '\xc2\xb5'", 4
        )
        self.checkPatternError(
            b"(?(\xc2\xb5)y)", r"bad character in group name '\xc2\xb5'", 3
        )

    def test_symbolic_refs(self):
        self.assertEqual(re.sub("(?P<a>x)|(?P<b>y)", r"\g<b>", "xx"), "")
        self.assertEqual(re.sub("(?P<a>x)|(?P<b>y)", r"\2", "xx"), "")
        self.assertEqual(re.sub(b"(?P<a1>x)", rb"\g<a1>", b"xx"), b"xx")
        # New valid identifiers in Python 3
    ...
    def test_symbolic_refs_errors(self):
        ...
        self.checkTemplateError(
            "()" * 10, r"\g<1_0>", "xx", "bad character in group name '1_0'", 3
        )
        self.checkTemplateError(
            "(?P<a>x)", r"\g< 1 >", "xx", "bad character in group name ' 1 '", 3
        )
        ...
        self.checkTemplateError(
            "(?P<a>x)", r"\g<१>", "xx", "bad character in group name '१'", 3
        )
    ...
    def test_re_groupref_exists_errors(self):
        ...
        self.checkPatternError(r"()(?(-1)a|b)", "bad character in group name '-1'", 5)
        self.checkPatternError(r"()(?(+1)a|b)", "bad character in group name '+1'", 5)
        self.checkPatternError(
            r"()" * 10 + r"(?(1_0)a|b)", "bad character in group name '1_0'", 23
        )
        self.checkPatternError(r"()(?( 1 )a|b)", "bad character in group name ' 1 '", 5)
        self.checkPatternError(r"()(?(㊀)a|b)", "bad character in group name '㊀'", 5)
        self.checkPatternError(r"()(?(¹)a|b)", "bad character in group name '¹'", 5)
        self.checkPatternError(r"()(?(१)a|b)", "bad character in group name '१'", 5)
        self.checkPatternError(r"()(?(1", "missing ), unterminated name", 5)
        self.checkPatternError(r"()(?(1)a", "missing ), unterminated subpattern", 2)
        ...
        self.checkPatternError(r"()(?(2)a)", "invalid group reference 2", 5)
    ...
    def test_special_escapes(self):
        ...
        self.assertEqual(
            re.search(rb"\b(b.)\b", b"abcd abc bcd bx", re.LOCALE).group(1), b"bx"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            re.search(rb"\d\D\w\W\s\S", b"1aa! a", re.LOCALE).group(0), b"1aa! a"
        )
    ...
    def test_named_unicode_escapes(self):
        ...
        self.checkPatternError(rb"\N{LESS-THAN SIGN}", r"bad escape \N", 0)
        self.checkPatternError(rb"[\N{LESS-THAN SIGN}]", r"bad escape \N", 1)
    ...
    def test_sre_byte_literals(self):
        ...
        self.assertTrue(re.match(rb"\08", b"\0008"))
        self.assertTrue(re.match(rb"\01", b"\001"))
        self.assertTrue(re.match(rb"\018", b"\0018"))
        self.checkPatternError(
            rb"\567", r"octal escape value \567 outside of " r"range 0-0o377", 0
        )
        self.checkPatternError(rb"\911", "invalid group reference 91", 1)
        self.checkPatternError(rb"\x1", r"incomplete escape \x1", 0)
        self.checkPatternError(rb"\x1z", r"incomplete escape \x1", 0)
    ...
    def test_sre_byte_class_literals(self):
        ...
        self.assertRaises(re.error, re.compile, rb"[\U00012345]")
        self.checkPatternError(
            rb"[\567]", r"octal escape value \567 outside of " r"range 0-0o377", 1
        )
        self.checkPatternError(rb"[\911]", r"bad escape \9", 1)
        self.checkPatternError(rb"[\x1z]", r"incomplete escape \x1", 1)
    ...
    def test_ascii_and_unicode_flag(self):
        ...
        for flags in (0, re.ASCII):
            ...
            self.assertIsNone(pat.match(b"\xe0"))
        # Incompatibilities
        self.assertRaises(ValueError, re.compile, rb"\w", re.UNICODE)
        self.assertRaises(re.error, re.compile, rb"(?u)\w")
    ...
    def test_locale_flag(self):
        ...
        pat = re.compile(rb"\w", re.LOCALE)
        if bletter:
            self.assertTrue(pat.match(bletter))
        pat = re.compile(rb"(?L)\w")
        if bletter:
            self.assertTrue(pat.match(bletter))
        pat = re.compile(rb"\w")
        if bletter:
    ...
    def test_scoped_flags(self):
        ...
        self.checkPatternError(
            rb"(?aL:a)", "bad inline flags: flags 'a', 'u' and 'L' are incompatible", 4
        )
    ...
    def test_look_behind_overflow(self):
        ...
        self.assertEqual(
            re.search(p1 % (2**7, 2**7, 2**7), string).span(), (2**21, 2**21)
        )
        self.assertEqual(re.search(p2 % (2**7, 2**7, 2**7), string).span(), (0, 0))
    ...
    @unittest.skipIf(
        is_emscripten or is_wasi, "musl libc issue on Emscripten/WASI, bpo-46390"
    )
    ...
    def test_possessive_quantifiers(self):
        ...
        self.assertEqual(re.match("([ae][ae])?+a", "aea").groups(), ("ae",))
        ...
        for name in deprecated:
            with self.subTest(module=name):
                ...
                with self.assertWarns(DeprecationWarning) as w:
                    __import__(name)
                self.assertEqual(str(w.warning), f"module {name!r} is deprecated")
