...
class BasicTest(BaseTest):
    ...
    def _check_output_of_default_create(self):
        ...
        self.assertIn("executable = %s" % os.path.realpath(sys.executable), data)
    ...
    def test_prefixes(self):
        ...
        for prefix, expected in (
            ...
            self.assertEqual(
                pathlib.Path(out.strip().decode()), pathlib.Path(expected), prefix
            )
    ...
    def test_zippath_from_non_installed_posix(self):
        ...
        for eachpath in sys.path:
            if eachpath.endswith(".zip"):
                if os.path.isfile(eachpath):
                    shutil.copyfile(
                        eachpath, os.path.join(non_installed_dir, platlibdir)
                    )
                ...
                additional_pythonpath_for_non_installed.append(eachpath)
        cmd = [
            os.path.join(non_installed_dir, self.bindir, self.exe),
            "-m",
            "venv",
            "--without-pip",
            self.env_dir,
        ...
        pythonpath = os.pathsep.join(additional_pythonpath_for_non_installed)
