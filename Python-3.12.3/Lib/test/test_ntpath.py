...
class TestNtpath(NtpathTestCase):
    ...
    def test_splitdrive(self):
        ...
        tester(
            'ntpath.splitdrive("\\\\?\\UNC\\server\\share\\dir")',
    ...
    def test_splitroot(self):
        ...
        tester("ntpath.splitroot('//')", ("//", "", ""))
        tester(
            'ntpath.splitroot("\\\\conky\\mountpoint\\foo\\bar")',
            ("\\\\conky\\mountpoint", "\\", "foo\\bar"),
        )
        tester(
            'ntpath.splitroot("//conky/mountpoint/foo/bar")',
        ...
        self.assertEqual(
            ntpath.splitroot("//conky/MOUNTPOİNT/foo/bar"),
        ...
        tester(
            'ntpath.splitroot("//?/UNC/server/share")', ("//?/UNC/server/share", "", "")
        )
        tester(
            'ntpath.splitroot("//?/UNC/server/share/dir")',
            ("//?/UNC/server/share", "/", "dir"),
        )
        tester(
            'ntpath.splitroot("//?/VOLUME{00000000-0000-0000-0000-000000000000}/spam")',
        ...
        tester('ntpath.splitroot("\\\\?\\UNC\\")', ("\\\\?\\UNC\\", "", ""))
        tester(
            'ntpath.splitroot("\\\\?\\UNC\\server\\")', ("\\\\?\\UNC\\server\\", "", "")
        )
        tester(
            'ntpath.splitroot("\\\\?\\UNC\\server\\share")',
            ("\\\\?\\UNC\\server\\share", "", ""),
        )
        tester(
            'ntpath.splitroot("\\\\?\\UNC\\server\\share\\dir")',
        ...
        tester(
            'ntpath.splitroot("\\\\?\\BootPartition\\")',
            ("\\\\?\\BootPartition", "\\", ""),
        )
        tester(
            'ntpath.splitroot("\\\\.\\BootPartition\\")',
        ...
        tester(
            'ntpath.splitroot("\\\\.\\PhysicalDrive0")',
            ("\\\\.\\PhysicalDrive0", "", ""),
        )
        tester('ntpath.splitroot("\\\\.\\nul")', ("\\\\.\\nul", "", ""))
        ...
        tester(
            'ntpath.splitroot("///y")', ("///y", "", "")
        )  # empty server & non-empty share
        tester(
            'ntpath.splitroot("//x")', ("//x", "", "")
        )  # non-empty server & missing share
        tester(
            'ntpath.splitroot("//x/")', ("//x/", "", "")
        )  # non-empty server & empty share
    ...
    def test_normpath(self):
        ...
        tester(
            "ntpath.normpath('handbook/../../Tests/image.png')", r"..\Tests\image.png"
        )
        tester(
            "ntpath.normpath('handbook/../../../Tests/image.png')",
            r"..\..\Tests\image.png",
        )
    ...
    def test_realpath_basic(self):
        ...
        self.assertPathEqual(
            ntpath.realpath(os.fsencode(ABSTFN + "1")), os.fsencode(ABSTFN)
        )
    ...
    def test_realpath_broken_symlinks(self):
        ...
        with os_helper.change_cwd(ABSTFN):
            ...
            self.assertPathEqual(ntpath.realpath("broken"), ABSTFN + r"\missing")
            self.assertPathEqual(
                ntpath.realpath(r"broken\foo"), ABSTFN + r"\missing\foo"
            )
            # bpo-38453: We no longer recursively resolve segments of relative
            # symlinks that the OS cannot resolve.
            self.assertPathEqual(ntpath.realpath(r"broken1"), ABSTFN + r"\broken\bar")
            self.assertPathEqual(
                ntpath.realpath(r"broken1\baz"), ABSTFN + r"\broken\bar\baz"
            )
            self.assertPathEqual(
                ntpath.realpath("broken2"), ABSTFN + r"\self\self\missing"
            )
            self.assertPathEqual(
                ntpath.realpath("broken3"),
                ABSTFN + r"\subdir\parent\subdir\parent\missing",
            )
            self.assertPathEqual(ntpath.realpath("broken4"), ABSTFN + r"\missing")
            self.assertPathEqual(ntpath.realpath("broken5"), ABSTFN + r"\missing")

            self.assertPathEqual(
                ntpath.realpath(b"broken"), os.fsencode(ABSTFN + r"\missing")
            )
            self.assertPathEqual(
                ntpath.realpath(rb"broken\foo"), os.fsencode(ABSTFN + r"\missing\foo")
            )
            self.assertPathEqual(
                ntpath.realpath(rb"broken1"), os.fsencode(ABSTFN + r"\broken\bar")
            )
            self.assertPathEqual(
                ntpath.realpath(rb"broken1\baz"),
                os.fsencode(ABSTFN + r"\broken\bar\baz"),
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken2"), os.fsencode(ABSTFN + r"\self\self\missing")
            )
            self.assertPathEqual(
            ...
            self.assertPathEqual(
                ntpath.realpath(b"broken4"), os.fsencode(ABSTFN + r"\missing")
            )
            self.assertPathEqual(
                ntpath.realpath(b"broken5"), os.fsencode(ABSTFN + r"\missing")
            )
    ...
    def test_realpath_symlink_loops(self):
        ...
        self.assertPathIn(
            ntpath.realpath(ABSTFN + "1\\x"), (ntpath.join(r, "x") for r in expected)
        )
        self.assertPathEqual(ntpath.realpath(ABSTFN + "1\\.."), ntpath.dirname(ABSTFN))
        self.assertPathEqual(
            ntpath.realpath(ABSTFN + "1\\..\\x"), ntpath.dirname(ABSTFN) + "\\x"
        )
        os.symlink(ABSTFN + "x", ABSTFN + "y")
        self.assertPathEqual(
            ntpath.realpath(ABSTFN + "1\\..\\" + ntpath.basename(ABSTFN) + "y"),
            ABSTFN + "x",
        )
        self.assertPathIn(
            ntpath.realpath(ABSTFN + "1\\..\\" + ntpath.basename(ABSTFN) + "1"),
            expected,
        )
    ...
    def test_realpath_symlink_loops_strict(self):
        ...
        self.assertPathEqual(
            ntpath.realpath(ABSTFN + "1\\..", strict=True), ntpath.dirname(ABSTFN)
        )
        self.assertRaises(OSError, ntpath.realpath, ABSTFN + "1\\..\\x", strict=True)
        os.symlink(ABSTFN + "x", ABSTFN + "y")
        self.assertRaises(
        ...
        # Test using relative path as well.
        self.assertRaises(
            OSError, ntpath.realpath, ntpath.basename(ABSTFN), strict=True
        )
    ...
    def test_realpath_symlink_prefix(self):
        ...
        self.assertPathEqual(
            ntpath.realpath(ABSTFN + "3.link"), "\\\\?\\" + ABSTFN + "3."
        )
        ...
        self.assertPathEqual(
            ntpath.realpath("\\\\?\\" + ABSTFN + "3link"), "\\\\?\\" + ABSTFN + "3"
        )
        self.assertPathEqual(
            ntpath.realpath("\\\\?\\" + ABSTFN + "3.link"), "\\\\?\\" + ABSTFN + "3."
        )
    ...
    def test_abspath(self):
        ...
        tester(
            'ntpath.abspath("\\\\?\\C:////spam////eggs. . .")', "\\\\?\\C:\\spam\\eggs"
        )
        tester(
            'ntpath.abspath("\\\\.\\C:////spam////eggs. . .")', "\\\\.\\C:\\spam\\eggs"
        )
    ...
    def test_isjunction(self):
        with os_helper.temp_dir() as d:
            with os_helper.change_cwd(d):
                ...
                self.assertPathEqual(
                    ntpath.realpath("testjunc"), ntpath.realpath("tmpdir")
                )
    ...
    def test_isdevdrive(self):
        ...
        self.assertFalse(
            ntpath.isdevdrive(r"\\?\Volume{00000000-0000-0000-0000-000000000000}\\")
        )
