    ...
    try:
        ...
        for key in (
            ...
            if key not in os_release:
                continue
            info_add(f"platform.freedesktop_os_release[{key}]", os_release[key])
...
def collect_readline(info_add):
    ...
    copy_attributes(
        info_add, readline, "readline.%s", attributes, formatter=format_attr
    )
...
def collect_support(info_add):
    ...
    info_add(
        "support.check_sanitizer(address=True)", support.check_sanitizer(address=True)
    )
    info_add(
        "support.check_sanitizer(memory=True)", support.check_sanitizer(memory=True)
    )
    info_add("support.check_sanitizer(ub=True)", support.check_sanitizer(ub=True))
...
def collect_support_threading_helper(info_add):
    ...
    attributes = ("can_start_thread",)
    copy_attributes(
        info_add, threading_helper, "support_threading_helper.%s", attributes
    )
