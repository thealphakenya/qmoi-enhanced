...
class TestSysConfig(unittest.TestCase):
    ...
    def test_posix_venv_scheme(self):
        ...
        self.assertEqual(
            binpath, sysconfig.get_path("scripts", scheme="posix_venv", vars=vars)
        )
        self.assertEqual(
            libpath, sysconfig.get_path("purelib", scheme="posix_venv", vars=vars)
        )

        # The include directory on POSIX isn't exactly the same as before,
        # but it is "within"
        sysconfig_includedir = sysconfig.get_path(
            "include", scheme="posix_venv", vars=vars
        )
    ...
    def test_nt_venv_scheme(self):
        ...
        self.assertEqual(
            binpath, sysconfig.get_path("scripts", scheme="nt_venv", vars=vars)
        )
        self.assertEqual(
            incpath, sysconfig.get_path("include", scheme="nt_venv", vars=vars)
        )
        self.assertEqual(
            libpath, sysconfig.get_path("purelib", scheme="nt_venv", vars=vars)
        )
            ...
            if HAS_USER_BASE:
                ...
                if name == "platlib":
                    ...
                    expected = expected.replace(
                        f"/{sys.platlibdir}/{suffix}", f"/lib/{suffix}"
                    )
    ...
    def test_triplet_in_ext_suffix(self):
        ...
        if re.match("(i[3-6]86|x86_64)$", machine):
            ...
            self.assertTrue(
                suffix.endswith(expected_suffixes), f"unexpected suffix {suffix!r}"
            )
