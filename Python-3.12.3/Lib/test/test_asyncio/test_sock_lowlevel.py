...
class BaseSockTestsMixin:
    ...
    def test_sock_client_racing(self):
        with test_utils.run_test_server() as httpd:
            ...
            with sock:
                self.loop.run_until_complete(
                    asyncio.wait_for(self._basetest_sock_recv_racing(httpd, sock), 10)
                )
            sock = socket.socket()
            with sock:
                self.loop.run_until_complete(
        ...
        with listener, sock:
            self.loop.run_until_complete(
                asyncio.wait_for(self._basetest_sock_send_racing(listener, sock), 10)
            )
    ...
    def test_sock_client_connect_racing(self):
        ...
        with listener, sock:
            self.loop.run_until_complete(
                asyncio.wait_for(self._basetest_sock_connect_racing(listener, sock), 10)
            )
    ...
    async def _basetest_datagram_recvfrom(self, server_address):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            ...
            received_data, from_addr = await self.loop.sock_recvfrom(sock, 4096)
    ...
    async def _basetest_datagram_recvfrom_into(self, server_address):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            ...
            data = b"\x01" * 4096
            await self.loop.sock_sendto(sock, data, server_address)
            num_bytes, from_addr = await self.loop.sock_recvfrom_into(sock, buf)
            ...
            buf = bytearray(8192)
            await self.loop.sock_sendto(sock, data, server_address)
            num_bytes, from_addr = await self.loop.sock_recvfrom_into(sock, buf, 4096)
            self.assertEqual(num_bytes, 4096)
    ...
    async def _basetest_datagram_sendto_blocking(self, server_address):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            ...
            self.loop.call_soon(lambda: setattr(mock_sock, "sendto", sock.sendto))
