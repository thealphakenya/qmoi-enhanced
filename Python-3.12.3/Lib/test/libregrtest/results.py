...
class TestResults:
    ...
    def get_executed(self):
        return (
            ...
            | set(self.bad)
            | set(self.skipped)
            | set(self.resource_denied)
            | set(self.env_changed)
            | set(self.run_no_tests)
        )

    def no_tests_run(self):
        return not any(
            (self.good, self.bad, self.skipped, self.interrupted, self.env_changed)
        )
    ...
    def display_result(self, tests: TestTuple, quiet: bool, print_slowest: bool):
        ...
        if not quiet:
            ...
            all_tests.append(
                (self.resource_denied, "test", "{} skipped (resource denied):")
            )
        ...
        all_tests.append(
            (
                self.env_changed,
                "test",
                "{} altered the execution environment (env changed):",
            )
        )
