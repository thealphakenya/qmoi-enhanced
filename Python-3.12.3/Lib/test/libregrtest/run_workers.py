...
class WorkerError(Exception):
    def __init__(
        self, test_name: TestName, err_msg: str | None, stdout: str | None, state: str
    ):
...
class WorkerThread(threading.Thread):
    ...
    def _run_process(
        self, runtests: WorkerRunTests, output_fd: int, tmp_dir: StrPath | None = None
    ) -> int | None:
    ...
    def create_stdout(self, stack: contextlib.ExitStack) -> TextIO:
        ...
        stdout_file = tempfile.TemporaryFile(
            "w+", encoding=encoding, errors="backslashreplace"
        )
    ...
    def create_worker_runtests(
        ...
        return self.runtests.create_worker_runtests(
            tests=tests, json_file=json_file, **kwargs
        )

    def run_tmp_files(
        self, worker_runtests: WorkerRunTests, stdout_fd: int
    ) -> tuple[int | None, list[StrPath]]:
        ...
        if not support.is_wasi:
            ...
            try:
                retcode = self._run_process(worker_runtests, stdout_fd, tmp_dir)
            finally:
    ...
    def read_json(
        self, json_file: JsonFile, json_tmpfile: TextIO | None, stdout: str
        ...
        try:
            ...
            raise WorkerError(self.test_name, err_msg, stdout, state=State.WORKER_BUG)

        if not worker_json:
            raise WorkerError(
                self.test_name, "empty JSON", stdout, state=State.WORKER_BUG
            )
    ...
    def _runtest(self, test_name: TestName) -> MultiprocessResult:
        with contextlib.ExitStack() as stack:
            ...
            retcode, tmp_files = self.run_tmp_files(
                worker_runtests, stdout_file.fileno()
            )
            ...
            if retcode is None:
                raise WorkerError(
                    self.test_name, stdout=stdout, err_msg=None, state=State.TIMEOUT
                )
            if retcode != 0:
...
class RunWorkers:
    def __init__(
        self, num_workers: int, runtests: RunTests, logger: Logger, results: TestResults
    ) -> None:
        ...
        if self.timeout is not None:
            ...
            self.worker_timeout: float | None = min(
                self.timeout * 1.5, self.timeout + 5 * 60
            )
        else:
    ...
    def start_workers(self) -> None:
        ...
        msg = f"Run {tests} in parallel using " f"{nworkers} worker {processes}"
        if self.timeout:
