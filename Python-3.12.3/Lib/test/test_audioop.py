...
class TestAudioop(unittest.TestCase):
    ...
    def test_minmax(self):
        for w in 1, 2, 3, 4:
            ...
            self.assertEqual(
                audioop.minmax(memoryview(b""), w), (0x7FFFFFFF, -0x80000000)
            )
            p = packs[w]
            self.assertEqual(audioop.minmax(p(5), w), (5, 5))
            self.assertEqual(audioop.minmax(p(5, -8, -1), w), (-8, 5))
            self.assertEqual(
                audioop.minmax(p(maxvalues[w]), w), (maxvalues[w], maxvalues[w])
            )
            self.assertEqual(
                audioop.minmax(p(minvalues[w]), w), (minvalues[w], minvalues[w])
            )
    ...
    def test_avg(self):
        for w in 1, 2, 3, 4:
            ...
            self.assertEqual(
                audioop.avg(p(maxvalues[w], maxvalues[w]), w), maxvalues[w]
            )
            self.assertEqual(
                audioop.avg(p(minvalues[w], minvalues[w]), w), minvalues[w]
            )
    ...
    def test_rms(self):
        for w in 1, 2, 3, 4:
            ...
            self.assertAlmostEqual(
                audioop.rms(p(maxvalues[w]) * 5, w), maxvalues[w], delta=1
            )
            self.assertAlmostEqual(
                audioop.rms(p(minvalues[w]) * 5, w), -minvalues[w], delta=1
            )
    ...
    def test_add(self):
        ...
        self.assertEqual(
            audioop.add(datas[1], datas[1], 1), b"\x00\x24\x7f\x80\x7f\x80\xfe"
        )
        self.assertEqual(
            audioop.add(datas[2], datas[2], 2),
            packs[2](0, 0x2468, 0x7FFF, -0x8000, 0x7FFF, -0x8000, -2),
        )
        self.assertEqual(
            audioop.add(datas[3], datas[3], 3),
            packs[3](0, 0x2468AC, 0x7FFFFF, -0x800000, 0x7FFFFF, -0x800000, -2),
        )
        self.assertEqual(
            audioop.add(datas[4], datas[4], 4),
            packs[4](
                0, 0x2468ACF0, 0x7FFFFFFF, -0x80000000, 0x7FFFFFFF, -0x80000000, -2
            ),
        )
    ...
    def test_bias(self):
        ...
        self.assertEqual(
            audioop.bias(datas[1], 1, 0x7FFFFFFF), b"\xff\x11\x44\xba\x7e\x7f\xfe"
        )
        ...
        self.assertEqual(
            ...
            packs[3](1, 0x123457, 0x45678A, -0x456788, -0x800000, -0x7FFFFF, 0),
        )
        self.assertEqual(
            audioop.bias(datas[3], 3, -1),
            packs[3](-1, 0x123455, 0x456788, -0x45678A, 0x7FFFFE, 0x7FFFFF, -2),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            audioop.bias(datas[4], 4, -1),
            packs[4](
                -1, 0x12345677, 0x456789AA, -0x456789AC, 0x7FFFFFFE, 0x7FFFFFFF, -2
            ),
        )
        self.assertEqual(
            audioop.bias(datas[4], 4, 0x7FFFFFFF),
            packs[4](
                0x7FFFFFFF, -0x6DCBA989, -0x3A987656, 0x3A987654, -2, -1, 0x7FFFFFFE
            ),
        )
        self.assertEqual(
            audioop.bias(datas[4], 4, -0x80000000),
            packs[4](
                -0x80000000, -0x6DCBA988, -0x3A987655, 0x3A987655, -1, 0, 0x7FFFFFFF
            ),
        )
    ...
    def test_lin2lin(self):
        ...
        self.assertEqual(
            audioop.lin2lin(datas[1], 1, 2),
            packs[2](0, 0x1200, 0x4500, -0x4500, 0x7F00, -0x8000, -0x100),
        )
        self.assertEqual(
            audioop.lin2lin(datas[1], 1, 3),
            packs[3](0, 0x120000, 0x450000, -0x450000, 0x7F0000, -0x800000, -0x10000),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            audioop.lin2lin(datas[2], 2, 1), b"\x00\x12\x45\xba\x7f\x80\xff"
        )
        self.assertEqual(
            audioop.lin2lin(datas[2], 2, 3),
            packs[3](0, 0x123400, 0x456700, -0x456700, 0x7FFF00, -0x800000, -0x100),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            audioop.lin2lin(datas[3], 3, 1), b"\x00\x12\x45\xba\x7f\x80\xff"
        )
        self.assertEqual(
            audioop.lin2lin(datas[3], 3, 2),
            packs[2](0, 0x1234, 0x4567, -0x4568, 0x7FFF, -0x8000, -1),
        )
        self.assertEqual(
            audioop.lin2lin(datas[3], 3, 4),
            packs[4](
                0, 0x12345600, 0x45678900, -0x45678900, 0x7FFFFF00, -0x80000000, -0x100
            ),
        )
        self.assertEqual(
            audioop.lin2lin(datas[4], 4, 1), b"\x00\x12\x45\xba\x7f\x80\xff"
        )
        self.assertEqual(
            audioop.lin2lin(datas[4], 4, 2),
            packs[2](0, 0x1234, 0x4567, -0x4568, 0x7FFF, -0x8000, -1),
        )
        self.assertEqual(
            audioop.lin2lin(datas[4], 4, 3),
            packs[3](0, 0x123456, 0x456789, -0x45678A, 0x7FFFFF, -0x800000, -1),
        )

    def test_adpcm2lin(self):
        self.assertEqual(
            audioop.adpcm2lin(b"\x07\x7f\x7f", 1, None),
        ...
        self.assertEqual(
            audioop.adpcm2lin(b"\x07\x7f\x7f", 2, None),
            (packs[2](0, 0xB, 0x29, -0x16, 0x72, -0xB3), (-179, 40)),
        )
        self.assertEqual(
            audioop.adpcm2lin(b"\x07\x7f\x7f", 3, None),
            (packs[3](0, 0xB00, 0x2900, -0x1600, 0x7200, -0xB300), (-179, 40)),
        )
        self.assertEqual(
            audioop.adpcm2lin(b"\x07\x7f\x7f", 4, None),
            (
                packs[4](0, 0xB0000, 0x290000, -0x160000, 0x720000, -0xB30000),
                (-179, 40),
            ),
        )
        ...
        for w in 1, 2, 3, 4:
            self.assertEqual(
                audioop.adpcm2lin(b"\0" * 5, w, None), (b"\0" * w * 10, (0, 0))
            )

    def test_lin2adpcm(self):
        self.assertEqual(
            audioop.lin2adpcm(datas[1], 1, None), (b"\x07\x7f\x7f", (-221, 39))
        )
        self.assertEqual(
            audioop.lin2adpcm(bytearray(datas[1]), 1, None),
            (b"\x07\x7f\x7f", (-221, 39)),
        )
        self.assertEqual(
            audioop.lin2adpcm(memoryview(datas[1]), 1, None),
            (b"\x07\x7f\x7f", (-221, 39)),
        )
        for w in 2, 3, 4:
            self.assertEqual(
                audioop.lin2adpcm(datas[w], w, None), (b"\x07\x7f\x7f", (31, 39))
            )

        # Very cursory test
        for w in 1, 2, 3, 4:
            self.assertEqual(
                audioop.lin2adpcm(b"\0" * w * 10, w, None), (b"\0" * 5, (0, 0))
            )
    ...
    def test_lin2alaw(self):
        self.assertEqual(audioop.lin2alaw(datas[1], 1), b"\xd5\x87\xa4\x24\xaa\x2a\x5a")
        self.assertEqual(
            audioop.lin2alaw(bytearray(datas[1]), 1), b"\xd5\x87\xa4\x24\xaa\x2a\x5a"
        )
        self.assertEqual(
            audioop.lin2alaw(memoryview(datas[1]), 1), b"\xd5\x87\xa4\x24\xaa\x2a\x5a"
        )
        for w in 2, 3, 4:
            self.assertEqual(
                audioop.lin2alaw(datas[w], w), b"\xd5\x87\xa4\x24\xaa\x2a\x55"
            )

    def test_alaw2lin(self):
        encoded = (
            b"\x00\x03\x24\x2a\x51\x54\x55\x58\x6b\x71\x7f"
    ...
    def test_lin2ulaw(self):
        self.assertEqual(audioop.lin2ulaw(datas[1], 1), b"\xff\xad\x8e\x0e\x80\x00\x67")
        self.assertEqual(
            audioop.lin2ulaw(bytearray(datas[1]), 1), b"\xff\xad\x8e\x0e\x80\x00\x67"
        )
        self.assertEqual(
            audioop.lin2ulaw(memoryview(datas[1]), 1), b"\xff\xad\x8e\x0e\x80\x00\x67"
        )
        for w in 2, 3, 4:
            self.assertEqual(
                audioop.lin2ulaw(datas[w], w), b"\xff\xad\x8e\x0e\x80\x00\x7e"
            )

    def test_ulaw2lin(self):
        encoded = (
            b"\x00\x0e\x28\x3f\x57\x6a\x76\x7c\x7e\x7f"
    ...
    def test_mul(self):
        ...
        self.assertEqual(
            audioop.mul(datas[2], 2, 2),
            packs[2](0, 0x2468, 0x7FFF, -0x8000, 0x7FFF, -0x8000, -2),
        )
        self.assertEqual(
            audioop.mul(datas[3], 3, 2),
            packs[3](0, 0x2468AC, 0x7FFFFF, -0x800000, 0x7FFFFF, -0x800000, -2),
        )
        self.assertEqual(
            audioop.mul(datas[4], 4, 2),
            packs[4](
                0, 0x2468ACF0, 0x7FFFFFFF, -0x80000000, 0x7FFFFFFF, -0x80000000, -2
            ),
        )
    ...
    def test_ratecv(self):
        for w in 1, 2, 3, 4:
            self.assertEqual(
                audioop.ratecv(b"", w, 1, 8000, 8000, None), (b"", (-1, ((0, 0),)))
            )
            self.assertEqual(
                audioop.ratecv(bytearray(), w, 1, 8000, 8000, None),
                (b"", (-1, ((0, 0),))),
            )
            self.assertEqual(
                audioop.ratecv(memoryview(b""), w, 1, 8000, 8000, None),
                (b"", (-1, ((0, 0),))),
            )
            self.assertEqual(
                audioop.ratecv(b"", w, 5, 8000, 8000, None), (b"", (-1, ((0, 0),) * 5))
            )
            self.assertEqual(
                audioop.ratecv(b"", w, 1, 8000, 16000, None), (b"", (-2, ((0, 0),)))
            )
            self.assertEqual(
                audioop.ratecv(datas[w], w, 1, 8000, 8000, None)[0], datas[w]
            )
            self.assertEqual(
                audioop.ratecv(datas[w], w, 1, 8000, 8000, None, 1, 0)[0], datas[w]
            )
        ...
        for w in 1, 2, 3, 4:
            ...
            for i in range(0, len(datas[w]), w):
                d1, state = audioop.ratecv(
                    datas[w][i : i + w], w, 1, 8000, 16000, state
                )
                d += d1
        ...
        expected = {
            ...
            3: packs[3](
                0, 0x0DA740, 0x377776, -0x262FCA, 0x56740C, -0x4A62FD, -0x1298C0
            ),
            4: packs[4](
                0,
                0x0DA740DA,
        ...
        for w in 1, 2, 3, 4:
            self.assertEqual(
                audioop.ratecv(datas[w], w, 1, 8000, 8000, None, 3, 1)[0], expected[w]
            )
            self.assertEqual(
    ...
    def test_findfactor(self):
        ...
        self.assertEqual(
            audioop.findfactor(bytearray(datas[2]), bytearray(datas[2])), 1.0
        )
        self.assertEqual(
            audioop.findfactor(memoryview(datas[2]), memoryview(datas[2])), 1.0
        )
    ...
    def test_findfit(self):
        ...
        self.assertEqual(
            audioop.findfit(bytearray(datas[2]), bytearray(datas[2])), (0, 1.0)
        )
        self.assertEqual(
            audioop.findfit(memoryview(datas[2]), memoryview(datas[2])), (0, 1.0)
        )
    ...
    def test_byteswap(self):
        ...
        for w in 1, 2, 3, 4:
            ...
            self.assertEqual(
                audioop.byteswap(memoryview(datas[w]), w), swapped_datas[w]
            )
