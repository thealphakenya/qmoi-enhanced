...
class CAPITest(unittest.TestCase):
    ...
    def test_fromencodedobject(self):
        ...
        self.assertEqual(fromencodedobject(b, "ascii", "replace"), "a" + "\ufffd" * 9)
    ...
    def test_decodeutf16stateful(self):
        ...
        for s in ["abc", "\xa1\xa2", "\u4f60\u597d", "a\U0001f600"]:
            ...
            self.assertEqual(
                decodeutf16stateful(0, b"\xff\xfe" + b), (-1, s, len(b) + 2)
            )
            b = s.encode("utf-16be")
            self.assertEqual(decodeutf16stateful(1, b), (1, s, len(b)))
            self.assertEqual(
                decodeutf16stateful(0, b"\xfe\xff" + b), (1, s, len(b) + 2)
            )
        ...
        self.assertEqual(
            decodeutf16stateful(0, b"\xff\xfe\x61\x00\x3d\xd8\x00"), (-1, "a", 4)
        )
        self.assertEqual(
            decodeutf16stateful(0, b"\xfe\xff\x00\x61\xd8\x3d\xde"), (1, "a", 4)
        )
        ...
        self.assertEqual(
            decodeutf16stateful(-1, b"\x00\xde", "replace"), (-1, "\ufffd", 2)
        )
        self.assertEqual(
            decodeutf16stateful(1, b"\xde\x00", "replace"), (1, "\ufffd", 2)
        )
        self.assertEqual(
            decodeutf16stateful(0, b"\xde\xde", "replace"), (0, "\ufffd", 2)
        )
        self.assertEqual(
            decodeutf16stateful(0, b"\xff\xfe\x00\xde", "replace"), (-1, "\ufffd", 4)
        )
        self.assertEqual(
            decodeutf16stateful(0, b"\xfe\xff\xde\x00", "replace"), (1, "\ufffd", 4)
        )

        self.assertRaises(
            UnicodeDecodeError, decodeutf16stateful, -1, b"\x3d\xd8\x61\x00"
        )
        self.assertEqual(
            decodeutf16stateful(-1, b"\x3d\xd8\x61\x00", "replace"), (-1, "\ufffda", 4)
        )
        self.assertRaises(
            UnicodeDecodeError, decodeutf16stateful, 1, b"\xd8\x3d\x00\x61"
        )
        self.assertEqual(
            decodeutf16stateful(1, b"\xd8\x3d\x00\x61", "replace"), (1, "\ufffda", 4)
        )
    ...
    def test_decodeutf32(self):
        ...
        self.assertRaises(
            UnicodeDecodeError, decodeutf32, 0, b"\xff\xfe\x00\x00\x61\x00\x00\x00\x00"
        )
        self.assertRaises(
            UnicodeDecodeError, decodeutf32, 0, b"\x00\x00\xfe\xff\x00\x00\x00\x61\x00"
        )
        ...
        self.assertEqual(
            decodeutf32(-1, b"\xff\xff\xff\xff", "replace"), (-1, "\ufffd")
        )
        self.assertEqual(decodeutf32(1, b"\xff\xff\xff\xff", "replace"), (1, "\ufffd"))
        self.assertEqual(decodeutf32(0, b"\xff\xff\xff\xff", "replace"), (0, "\ufffd"))
        self.assertEqual(
            decodeutf32(0, b"\xff\xfe\x00\x00\xff\xff\xff\xff", "replace"),
            (-1, "\ufffd"),
        )
        self.assertEqual(
            decodeutf32(0, b"\x00\x00\xfe\xff\xff\xff\xff\xff", "replace"),
            (1, "\ufffd"),
        )

        self.assertRaises(UnicodeDecodeError, decodeutf32, -1, b"\x3d\xd8\x00\x00")
        self.assertEqual(
            decodeutf32(-1, b"\x3d\xd8\x00\x00", "replace"), (-1, "\ufffd")
        )
    ...
    def test_decodeutf32stateful(self):
        ...
        for s in ["abc", "\xa1\xa2", "\u4f60\u597d", "a\U0001f600"]:
            ...
            self.assertEqual(
                decodeutf32stateful(0, b"\xff\xfe\x00\x00" + b), (-1, s, len(b) + 4)
            )
            b = s.encode("utf-32be")
            self.assertEqual(decodeutf32stateful(1, b), (1, s, len(b)))
            self.assertEqual(
                decodeutf32stateful(0, b"\x00\x00\xfe\xff" + b), (1, s, len(b) + 4)
            )

        self.assertEqual(decodeutf32stateful(-1, b"\x61\x00\x00\x00\x00"), (-1, "a", 4))
        self.assertEqual(
            decodeutf32stateful(-1, b"\x61\x00\x00\x00\x00\xf6"), (-1, "a", 4)
        )
        self.assertEqual(
            decodeutf32stateful(-1, b"\x61\x00\x00\x00\x00\xf6\x01"), (-1, "a", 4)
        )
        self.assertEqual(decodeutf32stateful(1, b"\x00\x00\x00\x61\x00"), (1, "a", 4))
        self.assertEqual(
            decodeutf32stateful(1, b"\x00\x00\x00\x61\x00\x01"), (1, "a", 4)
        )
        self.assertEqual(
            decodeutf32stateful(1, b"\x00\x00\x00\x61\x00\x01\xf6"), (1, "a", 4)
        )
        self.assertEqual(
            decodeutf32stateful(0, b"\xff\xfe\x00\x00\x61\x00\x00\x00\x00\xf6\x01"),
            (-1, "a", 8),
        )
        self.assertEqual(
            decodeutf32stateful(0, b"\x00\x00\xfe\xff\x00\x00\x00\x61\x00\x01\xf6"),
            (1, "a", 8),
        )
        ...
        for b in b"\xff", b"\xff\xff", b"\xff\xff\xff":
            ...
            self.assertEqual(
                decodeutf32stateful(0, b"\xff\xfe\x00\x00" + b), (-1, "", 4)
            )
            self.assertEqual(
                decodeutf32stateful(0, b"\x00\x00\xfe\xff" + b), (1, "", 4)
            )
        self.assertRaises(
            UnicodeDecodeError, decodeutf32stateful, -1, b"\xff\xff\xff\xff"
        )
        self.assertRaises(
            UnicodeDecodeError, decodeutf32stateful, 1, b"\xff\xff\xff\xff"
        )
        self.assertRaises(
            UnicodeDecodeError, decodeutf32stateful, 0, b"\xff\xff\xff\xff"
        )
        self.assertEqual(
            decodeutf32stateful(-1, b"\xff\xff\xff\xff", "replace"), (-1, "\ufffd", 4)
        )
        self.assertEqual(
            decodeutf32stateful(1, b"\xff\xff\xff\xff", "replace"), (1, "\ufffd", 4)
        )
        self.assertEqual(
            decodeutf32stateful(0, b"\xff\xff\xff\xff", "replace"), (0, "\ufffd", 4)
        )
        self.assertEqual(
            decodeutf32stateful(0, b"\xff\xfe\x00\x00\xff\xff\xff\xff", "replace"),
            (-1, "\ufffd", 8),
        )
        self.assertEqual(
            decodeutf32stateful(0, b"\x00\x00\xfe\xff\xff\xff\xff\xff", "replace"),
            (1, "\ufffd", 8),
        )

        self.assertRaises(
            UnicodeDecodeError, decodeutf32stateful, -1, b"\x3d\xd8\x00\x00"
        )
        self.assertEqual(
            decodeutf32stateful(-1, b"\x3d\xd8\x00\x00", "replace"), (-1, "\ufffd", 4)
        )
        self.assertRaises(
            UnicodeDecodeError, decodeutf32stateful, 1, b"\x00\x00\xd8\x3d"
        )
        self.assertEqual(
            decodeutf32stateful(1, b"\x00\x00\xd8\x3d", "replace"), (1, "\ufffd", 4)
        )

        self.assertRaises(
            LookupError, decodeutf32stateful, -1, b"\xff\xff\xff\xff", "foo"
        )
        self.assertRaises(
            LookupError, decodeutf32stateful, 1, b"\xff\xff\xff\xff", "foo"
        )
        self.assertRaises(
            LookupError, decodeutf32stateful, 0, b"\xff\xff\xff\xff", "foo"
        )
    ...
    def test_decodecharmap(self):
        ...
        self.assertEqual(
            decodecharmap(b"\1\0\2", ["\xa1", "\xa2", "\xa3"]), "\xa2\xa1\xa3"
        )
        self.assertEqual(
            decodecharmap(b"\1\0\2", ["\u4f60", "\u597d", "\u4e16"]),
            "\u597d\u4f60\u4e16",
        )
        self.assertEqual(
            decodecharmap(b"\1\0\2", ["\U0001f600", "\U0001f601", "\U0001f602"]),
            "\U0001f601\U0001f600\U0001f602",
        )
        ...
        self.assertEqual(
            decodecharmap(b"\1\0\2", [None, "b", "c"], "replace"), "b\ufffdc"
        )
    ...
    def test_ascharmapstring(self):
        ...
        self.assertEqual(
            ascharmapstring("\u4f60\u597d\u4e16", {0x4F60: 3, 0x597D: 0, 0x4E16: 7}),
            b"\3\0\7",
        )
        self.assertEqual(
            ascharmapstring(
                "\U0001f600\U0001f601\U0001f602", {0x1F600: 3, 0x1F601: 0, 0x1F602: 7}
            ),
            b"\3\0\7",
        )
        self.assertEqual(
            ascharmapstring("abc", {97: 3, 98: b"", 99: b"spam"}), b"\3spam"
        )
    ...
    def test_decodeunicodeescape(self):
        ...
        self.assertEqual(
            decodeunicodeescape(rb"\t\n\r\x0b\x0c\x00\\"), "\t\n\r\v\f\0\\"
        )
        self.assertEqual(decodeunicodeescape(b"\t\n\r\x0b\x0c\x00"), "\t\n\r\v\f\0")
        self.assertEqual(decodeunicodeescape(rb"\xa1\xa2"), "\xa1\xa2")
        self.assertEqual(decodeunicodeescape(b"\xa1\xa2"), "\xa1\xa2")
        self.assertEqual(decodeunicodeescape(rb"\u4f60\u597d"), "\u4f60\u597d")
        self.assertEqual(decodeunicodeescape(rb"\U0001f600"), "\U0001f600")
        with self.assertWarns(DeprecationWarning):
            self.assertEqual(decodeunicodeescape(rb"\z"), r"\z")

        for b in b"\\", rb"\xa", rb"\u4f6", rb"\U0001f60":
        ...
        self.assertEqual(decodeunicodeescape(rb"x\U0001f60", "replace"), "x\ufffd")
        self.assertEqual(decodeunicodeescape(rb"x\U0001f60y", "replace"), "x\ufffdy")
    ...
    def test_asunicodeescapestring(self):
        ...
        self.assertEqual(asunicodeescapestring("\xa1\xa2"), rb"\xa1\xa2")
        self.assertEqual(asunicodeescapestring("\u4f60\u597d"), rb"\u4f60\u597d")
        self.assertEqual(asunicodeescapestring("\U0001f600"), rb"\U0001f600")
    ...
    def test_decoderawunicodeescape(self):
        ...
        self.assertEqual(decoderawunicodeescape(rb"\u4f60\u597d"), "\u4f60\u597d")
        self.assertEqual(decoderawunicodeescape(rb"\U0001f600"), "\U0001f600")
        self.assertEqual(decoderawunicodeescape(rb"\xa1\xa2"), r"\xa1\xa2")
        self.assertEqual(decoderawunicodeescape(rb"\z"), r"\z")

        for b in rb"\u4f6", rb"\U0001f60":
        ...
        self.assertEqual(decoderawunicodeescape(rb"x\U0001f60", "replace"), "x\ufffd")
        self.assertEqual(decoderawunicodeescape(rb"x\U0001f60y", "replace"), "x\ufffdy")

        self.assertRaises(LookupError, decoderawunicodeescape, rb"\U0001f60", "foo")
    ...
    def test_asrawunicodeescapestring(self):
        ...
        self.assertEqual(asrawunicodeescapestring("\u4f60\u597d"), rb"\u4f60\u597d")
        self.assertEqual(asrawunicodeescapestring("\U0001f600"), rb"\U0001f600")
