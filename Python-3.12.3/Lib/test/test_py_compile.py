...
class PyCompileTestsBase:
    ...
    @unittest.skipIf(
        not os.path.exists(os.devnull) or os.path.isfile(os.devnull),
    ...
    def test_cwd(self):
        with os_helper.change_cwd(self.directory):
            py_compile.compile(
                os.path.basename(self.source_path), os.path.basename(self.pyc_path)
            )
        self.assertTrue(os.path.exists(self.pyc_path))
        self.assertFalse(os.path.exists(self.cache_path))
    ...
    def test_relative_path(self):
        py_compile.compile(
            os.path.relpath(self.source_path), os.path.relpath(self.pyc_path)
        )
        self.assertTrue(os.path.exists(self.pyc_path))
        self.assertFalse(os.path.exists(self.cache_path))
    ...
    def test_bad_coding(self):
        bad_coding = os.path.join(
            os.path.dirname(__file__), "tokenizedata", "bad_coding2.py"
        )
...
class PyCompileTestsWithSourceEpoch(
    PyCompileTestsBase,
    unittest.TestCase,
...
class PyCompileTestsWithoutSourceEpoch(
    PyCompileTestsBase,
    unittest.TestCase,
    metaclass=SourceDateEpochTestMeta,
...
class PyCompileCLITestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.source_path = os.path.join(self.directory, "_test.py")
        self.cache_path = importlib.util.cache_from_source(
    ...
    def pycompilecmd(self, *args, **kwargs):
        ...
        if args and args[0] == "-" and "input" in kwargs:
            return subprocess.run(
                [sys.executable, opts, "py_compile", "-"],
    ...
    def test_bad_syntax(self):
        bad_syntax = os.path.join(
            os.path.dirname(__file__), "tokenizedata", "badsyntax_3131.py"
        )
    ...
    def test_file_not_exists(self):
        should_not_exists = os.path.join(
            os.path.dirname(__file__), "should_not_exists.py"
        )
        rc, stdout, stderr = self.pycompilecmd_failure(
            self.source_path, should_not_exists
        )
        self.assertEqual(rc, 1)
        self.assertEqual(stdout, b"")
    ...
    def test_file_not_exists_with_quiet(self):
        ...
        rc, stdout, stderr = self.pycompilecmd_failure(
            "-q", self.source_path, should_not_exists
        )
        self.assertEqual(rc, 1)
        self.assertEqual(stdout, b"")
        self.assertEqual(stderr, b"")
