...
class GeneralTests:
    ...
    def test_debuglevel_2(self):
        ...
        with support.captured_stderr() as stderr:
            client.connect(HOST, self.port)
        client.close()
        expected = re.compile(r"^\d{2}:\d{2}:\d{2}\.\d{6} connect: ", re.MULTILINE)
        self.assertRegex(stderr.getvalue(), expected)
...
class DebuggingServerTests(unittest.TestCase):
    ...
    def testVRFY(self):
        ...
        expected = (
            252,
            b"Cannot VRFY user, but will accept message " + b"and attempt delivery",
        )
    ...
    def testHELP(self):
        ...
        self.assertEqual(
            smtp.help(),
            b"Supported commands: EHLO HELO MAIL " + b"RCPT DATA RSET NOOP QUIT VRFY",
        )
        smtp.quit()
    ...
    def testSendMessageWithMultipleFrom(self):
        ...
        debugout = smtpd.DEBUGSTREAM.getvalue()
        sender = re.compile(
            "^sender: the_rescuers@Rescue-Aid-Society.com$", re.MULTILINE
        )
        self.assertRegex(debugout, sender)
        for addr in ("John", "Dinsdale"):
            to_addr = re.compile(r"^recips: .*'{}'.*$".format(addr), re.MULTILINE)
...
class DefaultArgumentsTests(unittest.TestCase):
    ...
    def testSendMessage(self):
        ...
        self.smtp.send_message(self.msg)
        self.assertEqual(
            self.smtp.sendmail.call_args_list[0][0][3], expected_mail_options
        )
        self.assertEqual(
            self.smtp.sendmail.call_args_list[1][0][3], expected_mail_options
        )
...
class BadHELOServerTests(unittest.TestCase):
    ...
    def testFailingHELO(self):
        self.assertRaises(
            smtplib.SMTPConnectError, smtplib.SMTP, HOST, self.port, "localhost", 3
        )
...
class TooLongLineTests(unittest.TestCase):
    ...
    def testLineTooLong(self):
        self.assertRaises(
            smtplib.SMTPResponseException, smtplib.SMTP, HOST, self.port, "localhost", 3
        )
...
class SimSMTPServer(smtpd.SMTPServer):
    ...
    def handle_accepted(self, conn, addr):
        self._SMTPchannel = self.channel_class(
            self._extra_features, self, conn, addr, decode_data=self._decode_data
        )
...
class SMTPUTF8SimTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client_evt = threading.Event()
        # Pick a random unused port by passing 0 for the port number
        self.serv = SimSMTPUTF8Server(
            (HOST, 0), ("nowhere", -1), decode_data=False, enable_SMTPUTF8=True
        )
        # Keep a note of what port was assigned
        self.port = self.serv.socket.getsockname()[1]
        serv_args = (self.serv, self.serv_evt, self.client_evt)
    ...
    def test_send_unicode_with_SMTPUTF8_via_low_level_API(self):
        ...
        smtp.ehlo()
        self.assertEqual(
            smtp.mail("Jő", options=["BODY=8BITMIME", "SMTPUTF8"]), (250, b"OK")
        )
