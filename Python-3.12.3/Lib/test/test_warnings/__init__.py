...
class FilterTests(BaseTest):
    ...
    def test_catchwarnings_with_simplefilter_ignore(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            with self.module.catch_warnings(module=self.module, action="ignore"):
        ...
        with original_warnings.catch_warnings(module=self.module):
            ...
            with self.module.catch_warnings(
                ...
                with support.captured_stderr() as stderr:
                    ...
                    self.assertRaises(
                        FutureWarning, self.module.warn, FutureWarning("msg")
                    )
                    stderr = stderr.getvalue()
        ...
        with warnings_state(self.module):
            with original_warnings.catch_warnings(record=True, module=self.module) as w:
                ...
                package_helper.inner_api(
                    "inner_api", stacklevel=2, warnings_module=warning_tests.warnings
                )
        ...
        for remove in test:
            ...
            with self.subTest(remove=remove):
                with warnings_helper.check_warnings(filter, quiet=False):
                    self.module._deprecated(
                        "test_warnings", remove=remove, _version=version
                    )

        version = (3, 11, 0, "alpha", 0)
        msg = r".*test_warnings.*3\.11"
        with warnings_helper.check_warnings((msg, DeprecationWarning), quiet=False):
            self.module._deprecated("test_warnings", remove=(3, 11), _version=version)
    ...
    def test_RuntimeError(self):
        ...
        for level in ["beta", "candidate", "final"]:
            ...
            with self.subTest(releaselevel=level):
                with self.assertRaises(RuntimeError):
                    self.module._deprecated(
                        "test_warnings", remove=(3, 11), _version=version
                    )
