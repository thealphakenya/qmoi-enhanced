...
class UnicodeTest(
    ...
    def test_ascii(self):
        ...
        self.assertTypedEqual(
            ascii(WithRepr(StrSubclass("<abc>"))), StrSubclass("<abc>")
        )
        self.assertTypedEqual(ascii(WithRepr("<\U0001f40d>")), r"<\U0001f40d>")
        self.assertTypedEqual(
            ascii(WithRepr(StrSubclass("<\U0001f40d>"))), r"<\U0001f40d>"
        )
    ...
    def test_repr(self):
        ...
        self.assertTypedEqual(
            repr(WithRepr(StrSubclass("<abc>"))), StrSubclass("<abc>")
        )
        self.assertTypedEqual(repr(WithRepr("<\U0001f40d>")), "<\U0001f40d>")
        self.assertTypedEqual(
            repr(WithRepr(StrSubclass("<\U0001f40d>"))), StrSubclass("<\U0001f40d>")
        )
    ...
    def test_rsplit(self):
        ...
        for left, right in ("ba", "юё", "\u0101\u0100", "\U00010301\U00010300"):
            ...
            # Check `None` as well:
            self.checkequal([left + right], left + right, "rsplit", None)
    ...
    def test_formatting(self):
        ...
        self.assertRaisesRegex(
            TypeError, "%c requires int or char", operator.mod, "%c", pi
        )
    ...
    def test_formatting_with_enum(self):
        ...
        self.assertEqual("...%(foo)r..." % {"foo": Int.IDES}, "...<Int.IDES: 15>...")
        self.assertEqual("...%(foo)s..." % {"foo": Int.IDES}, "...15...")
    ...
    def test_conversion(self):
        ...
        self.assertTypedEqual(
            StrSubclass(WithStr(StrSubclass("abc"))), StrSubclass("abc")
        )
        self.assertTypedEqual(
            StrSubclass(WithStr(OtherStrSubclass("abc"))), StrSubclass("abc")
        )
        ...
        self.assertTypedEqual(
            StrSubclass(StrWithStr(StrSubclass("abc"))), StrSubclass("abc")
        )
        self.assertTypedEqual(
            StrSubclass(StrWithStr(OtherStrSubclass("abc"))), StrSubclass("abc")
        )
        ...
        self.assertTypedEqual(
            StrSubclass(WithRepr(StrSubclass("<abc>"))), StrSubclass("<abc>")
        )
        self.assertTypedEqual(
            StrSubclass(WithRepr(OtherStrSubclass("<abc>"))), StrSubclass("<abc>")
        )
    ...
    def test_raiseMemError(self):
        ...
        for char in ("a", "\xe9", "\u20ac", "\U0010ffff"):
            ...
            with self.subTest(char=char, struct_size=struct_size, char_size=char_size):
                # self-check
                self.assertEqual(
                    sys.getsizeof(char * 42), struct_size + (char_size * (42 + 1))
                )
