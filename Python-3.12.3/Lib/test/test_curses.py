...
class TestCurses(unittest.TestCase):
    ...
    def get_pair_limit(self):
        ...
        if hasattr(curses, "ncurses_version"):
            ...
            if not curses.has_extended_color_support() or (
                6,
                1,
            ) <= curses.ncurses_version < (6, 2):
    ...
    def test_pair_content(self):
        if not hasattr(curses, "use_default_colors"):
            self.assertEqual(
                curses.pair_content(0), (curses.COLOR_WHITE, curses.COLOR_BLACK)
            )
    ...
    def test_userptr_without_set(self):
        ...
        with self.assertRaises(
            curses.panel.error, msg="userptr should fail since not set"
        ):
            p.userptr()
    ...
    @unittest.skipIf(
        getattr(curses, "ncurses_version", (99,)) < (5, 8),
