...
class InternalFunctionsTest(unittest.TestCase):
    ...
    def test_format_optdict(self):
        ...
        check_against(
            ttk._format_optdict({"test": (1, 2, "", 0)}), {"-test": "1 2 {} 0"}
        )
        ...
        check_against(
            ttk._format_optdict({"option": ("one two", "three")}),
        ...
        check_against(
            ttk._format_optdict({"option": ("", "one")}), {"-option": "{} one"}
        )
        ...
        check_against(
            ttk._format_optdict({"option": ('"one"', "two")}),
            {"-option": '{"one"} two'},
        )
        check_against(
            ttk._format_optdict({"option": ("{one}", "two")}),
    ...
    def test_format_mapdict(self):
        ...
        self.assertEqual(
            ttk._format_mapdict(opts, script=True),
        ...
        self.assertEqual(
            ttk._format_mapdict({"opt": [("value",)]}), ("-opt", "{} value")
        )
    ...
    def test_format_elemcreate(self):
        ...
        self.assertEqual(
            ttk._format_elemcreate("image", False, "test", ("", "a")), ("test {} a", ())
        )
        ...
        self.assertEqual(
            ttk._format_elemcreate(
                "image", True, "test", ("a", "b", "c", "d"), x=[2, 3]
            ),
            ("{test {a b c} d}", "-x {2 3}"),
        )
        ...
        self.assertEqual(
            ttk._format_elemcreate("vsapi", False, "a", "b", ("a", "b"), opt="x"),
            ("a b a b", ("-opt", "x")),
        )
        # format returned values as a tcl script
    ...
    def test_format_layoutlist(self):
        ...
        self.assertRaises(
            ValueError, ttk._format_layoutlist, [("name", {"children": {"a": None}})]
        )
    ...
    def test_script_from_settings(self):
        ...
        self.assertFalse(
            ttk._script_from_settings(
                {"name": {"configure": None, "map": None, "element create": None}}
            )
        )
        ...
        self.assertRaises(
            IndexError,
            ttk._script_from_settings,
            {"name": {"element create": ["image"]}},
        )

        # minimal valid image
        self.assertTrue(
            ttk._script_from_settings({"name": {"element create": ["image", "name"]}})
        )

        image = {
            "thing": {"element create": ["image", "name", ("state1", "state2", "val")]}
        }
        self.assertEqual(
    ...
    def test_tclobj_to_py(self):
        self.assertEqual(
            ttk._tclobj_to_py((MockStateSpec("a", "b"), "val")), [("a", "b", "val")]
        )
        self.assertEqual(
            ttk._tclobj_to_py([MockTclObj("1"), 2, MockTclObj("3m")]), [1, 2, "3m"]
        )
    ...
    def test_list_from_layouttuple(self):
        ...
        self.assertEqual(
            ttk._list_from_layouttuple(tk, sample_ltuple),
            [("name", {"option": "value"})],
        )

        # empty children
        self.assertEqual(
            ttk._list_from_layouttuple(tk, ("something", "-children", ())),
            [("something", {"children": []})],
        )
                ...
                (
                    "name",
                    {
                        ...
                        "children": [
                            (
                                "otherone",
                                {"otheropt": "othervalue", "children": [("child", {})]},
                            )
                        ],
                    },
                )
        ...
        # bad tuples
        self.assertRaises(
            ValueError, ttk._list_from_layouttuple, tk, ("name", "no_minus")
        )
        self.assertRaises(
            ValueError, ttk._list_from_layouttuple, tk, ("name", "no_minus", "value")
        )
        self.assertRaises(
            ValueError, ttk._list_from_layouttuple, tk, ("something", "-children")
        )  # no children
...
class TclObjsToPyTest(unittest.TestCase):
    ...
    def test_nosplit(self):
        self.assertEqual(
            ttk.tclobjs_to_py({"text": "some text"}), {"text": "some text"}
        )
