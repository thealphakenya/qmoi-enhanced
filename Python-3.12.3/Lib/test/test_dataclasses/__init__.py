...
class TestCase(unittest.TestCase):
    ...
    def test_compare_subclasses(self):
        ...
        for idx, (fn, expected) in enumerate(
            [(lambda a, b: a == b, False), (lambda a, b: a != b, True)]
        ):
            with self.subTest(idx=idx):
    ...
    def test_field_no_default(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"__init__\(\) missing 1 required " "positional argument: 'x'"
        ):
            C()
    ...
    def test_missing_default(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"__init__\(\) missing 1 required " "positional argument"
        ):
            C()
        self.assertNotIn("x", C.__dict__)
    ...
    def test_class_var_frozen(self):
        ...
        c = C(5)
        self.assertEqual(
            repr(C(5)), "TestCase.test_class_var_frozen.<locals>.C(x=5, y=10)"
        )
        self.assertEqual(len(fields(C)), 2)  # We have 2 fields
    ...
    def test_is_dataclass_when_getattr_always_returns(self):
        ...
        for obj in a, b:
            with self.subTest(obj=obj):
                ...
                with self.assertRaisesRegex(
                    TypeError, "should be called on dataclass instances"
                ):
                    asdict(obj)
                with self.assertRaisesRegex(
                    TypeError, "should be called on dataclass instances"
    ...
    def test_helper_astuple_builtin_containers(self):
        ...
        self.assertEqual(
            astuple(gd), (0, {"first": ("Alice", 1), "second": ("Bob", 2)})
        )
    ...
    def test_dataclasses_qualnames(self):
        ...
        for function in (
            ...
            self.assertEqual(
                getattr(A, function).__qualname__,
                f"TestCase.test_dataclasses_qualnames.<locals>.A.{function}",
            )
...
class TestFieldNoAnnotation(unittest.TestCase):
    def test_field_without_annotation(self):
        with self.assertRaisesRegex(
            TypeError, "'f' is a field but has no type annotation"
        ):
...
class TestRepr(unittest.TestCase):
    ...
    def test_no_repr(self):
        ...
        self.assertIn(
            f"{__name__}.TestRepr.test_no_repr.<locals>.C object at", repr(C(3))
        )
        ...
        def test(case, unsafe_hash, eq, frozen, with_hash, result):
            with self.subTest(case=case, unsafe_hash=unsafe_hash, eq=eq, frozen=frozen):
                ...
                if result == "fn":
                    ...
                    assert with_hash
                    with self.assertRaisesRegex(
                        TypeError, "Cannot overwrite attribute __hash__"
                    ):
...
class TestFrozen(unittest.TestCase):
    ...
    def test_overwriting_frozen(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "Cannot overwrite attribute __setattr__"
        ):
        ...
        with self.assertRaisesRegex(
            TypeError, "Cannot overwrite attribute __delattr__"
        ):
...
class TestSlots(unittest.TestCase):
    ...
    def test_weakref_slot_without_slot(self):
        with self.assertRaisesRegex(
            TypeError, "weakref_slot is True but slots is False"
        ):
...
class TestStringAnnotations(unittest.TestCase):
    ...
    def test_text_annotations(self):
        ...
        self.assertEqual(
            get_type_hints(dataclass_textanno.Bar), {"foo": dataclass_textanno.Foo}
        )
        self.assertEqual(
...
ManualModuleMakeDataClass = make_dataclass(
    "ManualModuleMakeDataClass", [("x", int)], module=__name__
)
WrongNameMakeDataclass = make_dataclass("Wrong", [("x", int)])
WrongModuleMakeDataclass = make_dataclass(
    "WrongModuleMakeDataclass", [("x", int)], module="custom"
)
...
class TestMakeDataclass(unittest.TestCase):
    ...
    def test_no_types(self):
        ...
        self.assertEqual(
            C.__annotations__, {"x": "typing.Any", "y": "typing.Any", "z": "typing.Any"}
        )
        ...
        self.assertEqual(vars(c), {"x": 1, "y": 2, "z": 3})
        self.assertEqual(
            C.__annotations__, {"x": "typing.Any", "y": int, "z": "typing.Any"}
        )
...
class TestReplace(unittest.TestCase):
    ...
    def test_frozen(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"__init__\(\) got an unexpected " "keyword argument 'a'"
        ):
    ...
    def test_classvar(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"__init__\(\) got an " "unexpected keyword argument 'y'"
        ):
            replace(c, y=30)
    ...
    def test_initvar_is_specified(self):
        ...
        self.assertEqual(c.x, 10)
        with self.assertRaisesRegex(
            ValueError, r"InitVar 'y' must be " "specified with replace()"
        ):
            replace(c, x=3)
        c = replace(c, x=3, y=5)
...
class TestMatchArgs(unittest.TestCase):
    ...
    def test_make_dataclasses(self):
        ...
        C = make_dataclass(
            "C", [("x", int), ("y", int)], namespace={"__match_args__": ("z",)}
        )
