...
class TestCase(unittest.TestCase):
    ...
    def test_open_template(self, filename=None, protocol=None):
        ...
        s = shelve.open(
            filename=filename if filename is not None else self.fn, protocol=protocol
        )
        try:
            s["key1"] = (1, 2, 3, 4)
            self.assertEqual(s["key1"], (1, 2, 3, 4))
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    ...
    globals()[name] = type(name, bases, {"_args": {"protocol": proto}})
    ...
    for dbm_mod in dbm_iterator():
        ...
        globals()[name] = type(
            name, bases, {"dbm_mod": dbm_mod, "_args": {"protocol": proto}}
        )
