...
class Query(Toplevel):
    ...
    def create_widgets(self, ok_text="OK"):  # Do not replace.
        ...
        self.button_ok = Button(frame, text=ok_text, default="active", command=self.ok)
...
class HelpSource(Query):
    ...
    def askfilename(self, filetypes, initdir, initfile):  # htest #
        ...
        return filedialog.Open(parent=self, filetypes=filetypes).show(
    ...
    def path_ok(self):
        ...
        if not path:  # no path specified
            ...
            if not os.path.exists(path):
                self.showerror("help file path does not exist.", self.path_error)
                return None
...
class CustomRun(Query):
    ...
    def create_extra(self):
        ...
        self.args_error = Label(frame, text=" ", foreground="red", font=self.error_font)

        restart.grid(column=0, row=10, columnspan=3, padx=5, sticky="w")
        self.args_error.grid(column=0, row=12, columnspan=3, padx=5, sticky="we")
