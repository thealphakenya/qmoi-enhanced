...
class BaseConfigurator(object):
    ...
    def convert(self, value):
        ...
        if not isinstance(value, ConvertingDict) and isinstance(value, dict):
            ...
            not isinstance(value, ConvertingTuple)
                ...
                if "listener" in config:
                    ...
                    if isinstance(lspec, type):
                        ...
                        if isinstance(lspec, str):
                            ...
                            if isinstance(listener, type) and not issubclass(
                    ...
                    try:
                        for hn in config["handlers"]:
                            ...
                            if not isinstance(h, logging.Handler):
                                config.update(config_copy)  # restore for deferred cfg
                                raise TypeError(
                                    "Required handler %r " "is not configured yet" % hn
                                )
                            hlist.append(h)
                    except Exception as e:
                        raise ValueError(
                            "Unable to set required handler %r" % hn
                        ) from e
...
def listen(port=DEFAULT_LOGGING_CONFIG_PORT, verify=None):
    ...
    class Server(threading.Thread):
        ...
        def run(self):
            server = self.rcvr(
                port=self.port, handler=self.hdlr, ready=self.ready, verify=self.verify
            )
            if self.port == 0:
