...
class Traceback(_Traceback):
    ...
    def __repr__(self):
        return (
            "Traceback(filename={!r}, lineno={!r}, function={!r}, "
...
class FrameInfo(_FrameInfo):
    ...
    def __repr__(self):
        return (
            "FrameInfo(frame={!r}, filename={!r}, lineno={!r}, function={!r}, "
            "code_context={!r}, index={!r}, positions={!r})".format(
                self.frame,
                self.filename,
                self.lineno,
...
def _signature_is_builtin(obj):
    ...
    return (
        ...
        or isinstance(obj, _NonUserDefinedCallables)
        or
...
def _signature_is_functionlike(obj):
    ...
    return (
        ...
        and (isinstance(annotations, (dict)) or annotations is None)
    )
    ...
    def p(name_node, default_node, default=empty):
        ...
        if default_node and default_node is not _empty:
            try:
                ...
                raise ValueError(
                    "{!r} builtin has invalid signature".format(obj)
                ) from None
...
def _signature_from_function(
    cls, func, skip_bound_arg=True, globals=None, locals=None, eval_str=False
):
    ...
    for offset, name in enumerate(positional[non_default_count:]):
        ...
        annotation = annotations.get(name, _empty)
        parameters.append(
            Parameter(name, annotation=annotation, kind=kind, default=defaults[offset])
        )
        if posonly_left:
            posonly_left -= 1
...
def _signature_from_callable(
    ...
    *,
    ...
    _get_signature_of = functools.partial(
    ...
    try:
        ...
        if sig is not None:
            ...
            if not isinstance(sig, Signature):
                raise TypeError(
                    "unexpected object {!r} in __signature__ " "attribute".format(o_sig)
                )
            return sig
    ...
    if isfunction(obj) or _signature_is_functionlike(obj):
        ...
        return _signature_from_function(
            sigcls,
            obj,
            skip_bound_arg=skip_bound_arg,
            globals=globals,
