...
2. Navigate to: `http://localhost:3000/admin/master/login`
...
```bash
# Login
curl -X POST http://localhost:3000/api/admin/master/auth \
...
```bash
# Get status
curl -X GET http://localhost:3000/api/admin/autofix/background-automation \
  -H "Authorization: Bearer admin_token_here"

# Start automation
curl -X POST http://localhost:3000/api/admin/autofix/background-automation \
  -H "Authorization: Bearer admin_token_here" \
  -H "Content-Type: application/json" \
  -d '{"action":"start"}'
```
...
curl -X GET http://localhost:3000/api/admin/financial/summary \
  -H "Authorization: Bearer admin_token_here"
```
