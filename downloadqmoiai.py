...
def download_file(url, path, app_name, platform):
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            log_activity(
                f"Attempt {attempt}: Downloading {app_name} for {platform}",
                {"url": url},
            )
            ...
            if is_valid_file(path):
                log_activity(
                    f"Successfully downloaded {app_name} for {platform}", {"path": path}
                )
                print(f"Success: {path}")
                return True
            else:
                log_activity(
                    f"File too small after download", {"size": os.path.getsize(path)}
                )
        except Exception as e:
            log_activity(
                f"Error downloading {app_name} for {platform}",
...
def check_links_reachability(apps, timeout=10):
    ...
    for app in apps:
        ...
        try:
            ...
            log_activity(
                "Broken download link",
                {"app": name, "platform": platform, "url": url, "error": str(e)},
            )
            broken.append(app)
    return broken
        ...
        if download_file(url, file_path_latest, name, platform):
            if file_path_latest != file_path_version:
                try:
                    ...
                    log_activity(
                        "Copied file to versioned folder",
                        {"from": file_path_latest, "to": file_path_version},
                    )
                except Exception as e:
                    log_activity(
                        "Failed to copy file to versioned folder", {"error": str(e)}
                    )
        else:
            print(f"Failed to download a valid {name} for {platform} after retries.")
            log_activity(
                "Failed to download after retries",
                {"app": name, "platform": platform, "url": url},
            )
