...
describe('QMOI Kernel Panel SSO/OAuth E2E - Device & Delegated Access', () => {
  it('allows device-based SSO login', () => {
    cy.visit('/auth/callback?provider=device&token=device-test-token&device_id=dev123');
    cy.setCookie('authToken', 'device-test-token');
  ...
  it('handles delegated access', () => {
    cy.visit('/auth/callback?provider=delegated&token=delegated-test-token&delegator=admin');
    cy.setCookie('authToken', 'delegated-test-token');
