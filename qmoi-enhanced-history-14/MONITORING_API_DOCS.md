# Monitoring & Master APIs Documentation

## Overview

This document describes the monitoring, alerting, and administrative APIs for the QMOI system. All endpoints except health check are master-only and require authentication.

## Base URL

- **Development**: `http://localhost:3000/api`
- **Staging**: `https://staging.qmoi.app/api`
...
```
GET /master/audit-logs?action=UPDATE&resource=user&startDate=2024-01-01&skip=0&take=50
```
...
curl -H "Authorization: Bearer YOUR_TOKEN" \
  https://api.qmoi.app/api/master/monitoring
```
...
```bash
curl -H "Authorization: Bearer YOUR_TOKEN" \
  https://api.qmoi.app/api/master/alerts
```
...
```bash
curl -H "Authorization: Bearer YOUR_TOKEN" \
  "https://api.qmoi.app/api/master/audit-logs?action=DELETE&resource=user&skip=0&take=50"
```
...
curl -X POST \
  ...
  https://api.qmoi.app/api/master/audit-logs \
...
## Webhook Integration (Planned v2.1.0)

Subscribe to alert notifications:

```bash
POST /master/webhooks
{
...
- **High Error Rates**: Check `/master/monitoring` for error types and frequency
- **Performance Issues**: Review p99Duration metrics to identify bottlenecks
- **Audit Trail**: Use `/master/audit-logs` to track changes for debugging
- **Memory Leaks**: Monitor heapUsed trends over time in monitoring dashboard

For additional support, contact: api-support@qmoi.app
