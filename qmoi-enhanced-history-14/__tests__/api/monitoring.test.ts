...
describe("Master Monitoring APIs", () => {
  ...
  beforeAll(async () => {
    ...
    const hashedPassword = await authService.hashPassword("Master@123456");
    adminUser = await userService.create({
      email: "master@qmoi.app",
      username: "master",
    ...
    adminToken = authService.generateToken(
      (adminUser as any).id,
      (adminUser as any).email || "master@qmoi.app",
    );
  ...
  describe("Monitoring Dashboard", () => {
    ...
    test("should return 403 for non-master users", async () => {
      const response = await fetch(
        "http://localhost:3000/api/master/monitoring",
        {
    ...
    test("should return monitoring data for master users", async () => {
      const response = await fetch(
        "http://localhost:3000/api/master/monitoring",
        {
  ...
  describe("Alerts Management", () => {
    test("should return alerts list for master", async () => {
      const response = await fetch("http://localhost:3000/api/master/alerts", {
        headers: { Authorization: `Bearer ${adminToken}` },
      });
  ...
  describe("Rate Limits", () => {
    test("should return rate limit config for master", async () => {
      const response = await fetch(
    ...
    test("should filter rate limits by userId", async () => {
      const response = await fetch(
        `http://localhost:3000/api/master/rate-limits?userId=${regularUser.id}`,
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
      );
  ...
  describe("Audit Logs", () => {
    ...
    test("should return audit logs for master", async () => {
      const response = await fetch(
    ...
    test("should filter by action", async () => {
      const response = await fetch(
        "http://localhost:3000/api/master/audit-logs?action=UPDATE",
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
      );
    ...
    test("should filter by resource", async () => {
      const response = await fetch(
        "http://localhost:3000/api/master/audit-logs?resource=user",
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
      );
    ...
    test("should support pagination", async () => {
      const response = await fetch(
        "http://localhost:3000/api/master/audit-logs?skip=0&take=10",
        {
          headers: { Authorization: `Bearer ${adminToken}` },
        },
      );
  ...
  describe("Authorization", () => {
    test("should require master role for all master endpoints", async () => {
      const endpoints = [
        "/api/master/monitoring",
        "/api/master/alerts",
        "/api/master/rate-limits",
