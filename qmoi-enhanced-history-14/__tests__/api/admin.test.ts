...
describe("Master Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    // Generate tokens
    adminToken = authService.generateToken(adminId, "master@test.com");
  ...
  describe("Dashboard Endpoint", () => {
    it("should deny access to non-master users", () => {
    ...
    it("should allow master users to access dashboard", () => {
      expect(() => {
  ...
  describe("User Management Endpoints", () => {
    ...
    it("should prevent master self-deletion", () => {
    ...
    it("should search users by email", async () => {
      const users = await userService.list(1000);
      const filtered = users.filter((u) => u.email.includes("master"));
