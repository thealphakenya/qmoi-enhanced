...
class AppVerifier:
    ...
    def check_apk(self, filepath):
        ...
        try:
            with zipfile.ZipFile(filepath, 'r') as zf:
                ...
                if not has_required:
                    self.log(f"  ✗ Missing AndroidManifest.xml or other required files", "ERROR")
                    return False
        ...
        try:
            with zipfile.ZipFile(filepath, 'r') as zf:
                ...
                if not (has_payload and has_app):
                    self.log(f"  ✗ Missing Payload or .app bundle", "ERROR")
                    return False
    ...
    def check_deb(self, filepath):
        ...
        try:
            ...
            if 'control' not in contents or 'data' not in contents:
                self.log(f"  ✗ Missing required deb components", "ERROR")
                return False
    ...
    def run_all_checks(self):
        ...
        apps_to_check = [
            ...
            ("web", os.path.join(APPS_DIR, "web/latest/master.zip")),
