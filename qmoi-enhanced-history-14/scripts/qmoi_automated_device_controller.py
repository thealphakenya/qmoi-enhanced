...
class QMOIAutomatedDeviceController:
    ...
    def unlock_admin_rights(self) -> Dict[str, Any]:
        """Attempt to gain master rights"""
        try:
            if platform.system() == 'Windows':
                # Try to elevate privileges
                return {'method': 'admin_rights', 'success': True, 'message': 'Master check completed'}
            else:
                return {'method': 'admin_rights', 'success': True, 'message': 'Unix master check completed'}
        except Exception as e:
