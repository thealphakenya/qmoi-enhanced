...
def run_tests():
    ...
    with app.test_client() as c:
        ...
        # Master backup DB
        r = c.post('/master/backup-db', headers={'Authorization': CONTROL_TOKEN})
        try:
            results.append(('/master/backup-db', r.status_code, r.get_json()))
        except Exception:
            results.append(('/master/backup-db', r.status_code, r.get_data(as_text=True)))

        # Master update ngrok (dry-run)
        r = c.post('/master/update-ngrok', headers={'Authorization': CONTROL_TOKEN}, json={})
        try:
            results.append(('/master/update-ngrok (dry-run)', r.status_code, r.get_json()))
        except Exception:
            results.append(('/master/update-ngrok (dry-run)', r.status_code, r.get_data(as_text=True)))
