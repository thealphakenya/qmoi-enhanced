...
- Dry-run / execute / simulate modes
...
def create_todo(task: Dict[str, Any], title: str, body: str):
    ...
    try:
        ...
        todo = add_todo(title=title, description=body, metadata={'lion_task': task.get('id')})
        logger.info('Created todo %s', todo.get('id'))
        return todo
    except Exception:
        # fallback: write minimal todo in history
        h = load_history()
        todos = h.get('todos', [])
...
def run(limit: int = None, dry_run: bool = True, simulate: bool = False, config_path: str = None):
    # backward-compatible wrapper; prefer passing use_queue via CLI
    return run_internal(limit=limit, dry_run=dry_run, simulate=simulate, config_path=config_path, use_queue=False)


def run_internal(limit: int = None, dry_run: bool = True, simulate: bool = False, config_path: str = None, use_queue: bool = False):
...
def _parse_args():
    ...
    p.add_argument('--simulate', action='store_true', help='Simulate outcomes')
...
def main():
    ...
    if env_manager:
        try:
            ...
            if rc != 0:
                logger.error('Missing required secrets according to %s; aborting startup', env_manager.MANIFEST_DEFAULT)
                raise SystemExit(2)
        except Exception:
    ...
    run_internal(limit=args.limit, dry_run=dry, simulate=args.simulate, config_path=config_path, use_queue=getattr(args, 'use_queue', False))
