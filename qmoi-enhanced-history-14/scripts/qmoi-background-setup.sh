...
if ! grep -q "ADMIN_TOKEN" .env.local; then
  ...
  echo "✓ Generated master token: $ADMIN_TOKEN"
fi
...
echo "2. Visit http://localhost:3000/master to view the dashboard"
...
echo ""
echo "💡 Tips:"
echo "   - Master Token: $(grep ADMIN_TOKEN .env.local | cut -d= -f2)"
