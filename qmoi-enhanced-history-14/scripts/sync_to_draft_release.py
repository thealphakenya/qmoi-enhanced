#!/usr/bin/env python3
"""
Sync assets from `release_assets_manifest.json` to a draft release.

Creates or updates a draft release tagged with the current commit SHA or a
specified tag, uploads all manifest assets to it, and leaves it in draft state
...
Options:
  ...
  --publish      Also publish the draft (move to published)
...
def get_or_create_draft_release(tag):
    """Get existing draft with this tag, or create a new draft release."""
    try:
        # Try to get existing release
        rel = gh_get(f'/repos/{OWNER}/{REPO}/releases/tags/{tag}')
        if rel.get('draft'):
            print('Found existing draft release for tag', tag)
            return rel
        else:
            print('Release exists but is published; will create a new draft with -draft suffix')
            tag = tag + '-draft'
    ...
    print('Creating draft release for tag', tag)
...
def publish_release(rel_id):
    ...
    gh_patch(f'/repos/{OWNER}/{REPO}/releases/{rel_id_num}', {'draft': False})
...
def main():
    ...
    print(f'Syncing {len(assets)} assets to draft release {tag}')
    for a in assets:
        ...
        if not os.path.exists(local):
            print(f'  {name}: local file missing — skipping')
            continue
