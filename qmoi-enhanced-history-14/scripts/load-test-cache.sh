...
get_cache_stats() {
  ...
  local response=$(curl -s -X GET "$BASE_URL/api/master/cache-stats" \
...
warm_cache() {
  ...
  curl -s -X GET "$BASE_URL/api/master/monitoring" \
    -H "Authorization: Bearer $ADMIN_TOKEN" > /dev/null

  # Cache alerts
  curl -s -X GET "$BASE_URL/api/master/alerts" \
    -H "Authorization: Bearer $ADMIN_TOKEN" > /dev/null
...
show_index_recommendations() {
  ...
  echo "Run these SQL commands to create recommended indexes:"
  echo ""
  echo "-- Users table"
...
ENVIRONMENT VARIABLES:
  ...
  ADMIN_TOKEN        Master JWT token for API requests
