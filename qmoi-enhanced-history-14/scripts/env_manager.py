...
- CLI: --check to validate and fail if required secrets missing; --generate-example to produce .env.example from manifest
...
def check_required(manifest_path: Path) -> int:
    ...
    if missing:
        ...
        return 1
    print('All required secrets present (or optional).')
    return 0
...
def generate_example(manifest_path: Path, out: Path = ENV_EXAMPLE) -> None:
    manifest = load_manifest(manifest_path)
    lines = []
    lines.append('# Generated .env.example from required_secrets.json\n')
    for s in manifest.get('secrets', []):
        name = s.get('name')
...
def interactive_fill(manifest_path: Path, out_path: Path = REPO_ENV) -> None:
    ...
    for s in manifest.get('secrets', []):
        ...
        cur = cfg.get(name)
        if cur:
            print(f'{name} already set in environment (using existing value).')
            to_write[name] = cur
            continue
        ...
        if val:
            ...
            print(f'{name} is required; aborting. You may re-run to try again.')
            raise SystemExit(2)
    ...
    try:
        ...
        print('Could not set permissions on', out_path)
    print('Wrote .env to', out_path)


def main(argv=None):
    ap = argparse.ArgumentParser(description='QMOI environment manager')
    ap.add_argument('--manifest', default=str(MANIFEST_DEFAULT))
    ap.add_argument('--check', action='store_true', help='Check required secrets are present and exit with non-zero if missing')
    ap.add_argument('--generate-example', action='store_true', help='Generate .env.example from manifest')
    ap.add_argument('--interactive', action='store_true', help='Interactively prompt for missing secrets and write .env')
