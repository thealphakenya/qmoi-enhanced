...
# 3. Generate secure master token if not exists
echo -e "${BLUE}3. Checking master token...${NC}"
...
routes=(
    "app/api/master/autofix/scan/route.ts"
    "app/api/master/autofix/fix-all/route.ts"
    "app/api/master/autofix/status/route.ts"
    "app/api/master/autofix/health/route.ts"
    "app/api/master/autofix/errors/route.ts"
    "app/api/master/autofix/fix/[errorId]/route.ts"
    "app/api/master/autofix/stream/route.ts"
)
...
echo "3. Navigate to: ${YELLOW}http://localhost:3000/master${NC}"
