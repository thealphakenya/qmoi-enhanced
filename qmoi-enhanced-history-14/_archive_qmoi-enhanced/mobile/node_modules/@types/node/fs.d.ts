...
declare module "fs" {
     ...
     */
    type CustomEvents = { [Key in string & {} | symbol]: (...args: unknown[]) => void };
     ...
     * exists('myfile', (e) => {
     *   if (e) {
     *     (globalThis.console as any)?.error?.('myfile already exists');
     *   } else {
     ...
     *     });
     *   } else {
     *     (globalThis.console as any)?.error?.('myfile does not exist');
     *   }
     * });
     * ```
     *
     ...
     * } catch (err) {
     *   (globalThis.console as any)?.error?.('no access!');
     * }
     * ```
     * @since v0.11.15
     * @param [mode=fs.constants.F_OK]
     ...
     * import { createReadStream } from 'node:fs';
     *
     * createReadStream('[PRODUCTION IMPLEMENTATION REQUIRED].txt', { start: 90, end: 99 });
     * ```
     *
     * If `options` is a string, then it specifies the encoding.
     * @since v0.1.31
