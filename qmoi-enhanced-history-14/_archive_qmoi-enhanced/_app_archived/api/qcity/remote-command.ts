...
const handler = requireRole(["master", "master"])(async (req: NextRequest) => {
  // Basic API key authentication for master/master users
  const apiKey = req.headers.get("x-qcity-master-key");
  if (apiKey !== ADMIN_KEY) {
    logAudit({
      action: "unauthorized",
  ...
  logAudit({ action: "run", cmd, deviceId, user: "master", status: "started" });
  if (stream) {
    ...
    logAudit({ action: "run", cmd, deviceId, user: "master", status: "done" });
