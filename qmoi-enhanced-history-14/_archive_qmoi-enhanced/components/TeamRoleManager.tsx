...
const TeamRoleManager: React.FC = () => {
  ...
  const addUser = () => {
    ...
    setTeam((prev) => [
      ...prev,
      { id: Date.now().toString(), name: newUser.name, role: newUser.role },
    ]);
      ...
      prev.map((r) =>
        r.id === req.id
          ? {
              ...
              status:
                (r.currentStep || 0) + 1 >= (r.steps?.length || 1)
                  ? "approved"
                  : "pending",
              approvedBy: approver,
            }
                ...
                <li key={user.id} className="flex items-center gap-2">
                  ...
                  {isMaster && user.role !== "master" && (
                    <select
                      value={user.role}
                      onChange={(e) =>
                        assignRole(user.id, e.target.value as UserRole)
                      }
            ...
            {approvals.map((req) => (
              <li key={req.id} className="border rounded p-2">
                ...
                <div className="text-xs mt-1">
                  Item: {JSON.stringify(req.item)}
                </div>
                <div className="flex gap-2 mt-2">
                  {req.status === "pending" &&
                    team.map((user) =>
                      req.steps &&
                      req.steps[req.currentStep || 0] === user.role ? (
                        <>
                          <Button
                            key={user.id}
                            size="sm"
                            onClick={() => approveRequest(req, user)}
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="destructive"
                            onClick={() => rejectRequest(req, user)}
                          >
                            Reject as {user.role}
                          </Button>
                        </>
                ...
                {req.status !== "pending" && req.approvedBy && (
                  <div className="text-xs text-gray-400 mt-1">
                    Finalized by: {req.approvedBy.name} ({req.approvedBy.role})
                  </div>
                )}
              </li>
            ))}
...
// AUTOFIXED by Ollama at 2026-07-26T18:54:39.672030Z

// AUTOFIXED by Ollama at 2026-07-26T18:57:32.822187Z

// AUTOFIXED by Ollama at 2026-07-26T19:31:03.194735Z
