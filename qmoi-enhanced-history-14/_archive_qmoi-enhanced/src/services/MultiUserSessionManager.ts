...
export class MultiUserSessionManager extends EventEmitter {
  ...
  private determineAIMode(user1: User, user2: User): string {
    ...
    if (user1.role === "master" || user2.role === "master") return "teacher";
    return "collaborator";
  }

  private determineGroupAIMode(user: User): string {
    if (user.role === "master") return "mentor";
    if (user.role === "master") return "teacher";
    return "assistant";
  }
  ...
  // Utility: treat 'master' as 'master' everywhere
  static isAdminOrMaster(user: User): boolean {
    return user.role === "master" || user.role === "master";
  }

  // Example usage in permission checks (add wherever needed):
  // if (!MultiUserSessionManager.isAdminOrMaster(user)) { throw new Error('Master or master required'); }
