...
class DeviceUnlockSystem:
    ...
    def _initialize_unlock_methods(self) -> Dict[str, UnlockMethod]:
        ...
        methods = {
            'remove_device_admin': UnlockMethod(
                name='Remove Device Master',
                description='Remove device master policies and MDM profiles',
                success_rate=0.85,
                risk_level='low',
    ...
    def unlock_mkopa_device(self) -> UnlockResult:
        ...
        try:
            # Step 1: Remove M-KOPA device master
            logger.info("📱 Removing M-KOPA device master...")
            admin_result = self._remove_device_admin("com.mkopa")
            if not admin_result['success']:
                errors.append(f"Failed to remove M-KOPA device master: {admin_result['error']}")
            else:
                logger.info("✅ M-KOPA device master removed successfully")
    ...
    def unlock_watu_device(self) -> UnlockResult:
        ...
        try:
            # Step 1: Remove Watu device master
            logger.info("📱 Removing Watu Credit device master...")
            admin_result = self._remove_device_admin("com.watu")
            if not admin_result['success']:
                errors.append(f"Failed to remove Watu device master: {admin_result['error']}")
            else:
                logger.info("✅ Watu Credit device master removed successfully")
    ...
    def unlock_generic_device(self, organization: str) -> UnlockResult:
        ...
        try:
            # Step 1: Remove device master
            logger.info("📱 Removing device master...")
            admin_result = self._remove_device_admin_generic()
            if not admin_result['success']:
                errors.append(f"Failed to remove device master: {admin_result['error']}")
            else:
                logger.info("✅ Device master removed successfully")
    ...
    def _remove_device_admin(self, package_name: str) -> Dict[str, Any]:
        """Remove device master for specific package"""
        try:
    ...
    def _remove_windows_device_admin(self, package_name: str) -> Dict[str, Any]:
        """Remove Windows device master"""
        try:
            # Check for device master policies in registry
            import winreg
            
            # Common registry paths for device master
            registry_paths = [
            ...
            for path in registry_paths:
                try:
                    with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, path, 0, winreg.KEY_WRITE) as key:
                        # Remove device master related values
        ...
        try:
            ...
            for pm in package_managers:
                try:
                    ...
                    if result.returncode == 0:
                        return {"success": True, "message": f"Linux device master removed via {pm}"}
                except:
                    pass
            
            return {"success": True, "message": "Linux device master removal attempted"}
        except Exception as e:
            return {"success": False, "error": str(e)}
