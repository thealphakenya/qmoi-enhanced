...
| Endpoint Category   | Master | Master | User | Sponsored | Guest |
| ------------------- | ------ | ----- | ---- | --------- | ----- |
| Auth (login)        | ✅     | ✅    | ✅   | ✅        | ❌    |
| WebAuthn            | ✅     | ✅    | ✅   | ❌        | ❌    |
| Voice               | ✅     | ✅    | ✅   | ❌        | ❌    |
...
5. [Master Endpoints](#master-endpoints)
...
**Access:** `user`, `master`, `master`, `sponsored`
...
**Access:** `user`, `master`, `master` (personal data with appropriate role checks)
...
**Access:** `user`, `master`, `master`, `sponsored` (authenticated users)
...
**Description:** Get list of all users (master/master only)
...
**Access:** `user`, `master`, `master` (with role checks)
...
**Access:** `user`, `master`, `master` (users can only update own profile)
...
**Description:** Delete user account (master/master only)
...
**Endpoint:** `POST /api/master/sponsored/create`
...
**Endpoint:** `GET /api/master/sponsored/list`
...
**Endpoint:** `DELETE /api/master/sponsored/delete`
...
- **Master endpoints:** 100 requests per minute per master
...
curl -X POST http://localhost:3000/api/webauthn/register \
  ...
  -d '{"userId":"2","username":"master","displayName":"Master User","challenge":"test","credentialData":{"id":"test","publicKey":"test","counter":0}}'
```
...
curl -X POST http://localhost:3000/api/qmoi/session \
  -H "Content-Type: application/json" \
  -d '{"userId":"2","username":"master","role":"Administrator","biometricMethods":["fingerprint"],"deviceId":"device123"}'
```
