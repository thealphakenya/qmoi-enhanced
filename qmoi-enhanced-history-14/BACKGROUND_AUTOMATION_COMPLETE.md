...
11. **`app/api/master/autofix/bootstrap/route.ts`** (New)
...
- Show you the master token (save it!)
...
```
GET /api/master/autofix/autoscan        # Auto-scan status, config, stats, logs
GET /api/master/autofix/healthmonitor   # Health monitor status, thresholds, stats
```

### Configuration

```
GET    /api/master/autofix/config       # Get current configuration
POST   /api/master/autofix/config       # Update configuration
PUT    /api/master/autofix/config       # Alternative update
...
5. **Check configuration**: `curl ... /api/master/autofix/config`
...
_Visit dashboard: `http://localhost:3000/master`_
