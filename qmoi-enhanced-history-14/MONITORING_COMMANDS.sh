...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/rate-limits?userId=USER_ID" | jq '.'
...
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?skip=0&take=50" | jq '.'

# Get logs for specific action
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?action=DELETE&skip=0&take=50" | jq '.'

# Get logs for specific resource
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?resource=user&skip=0&take=50" | jq '.'

# Get logs for specific user
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?userId=USER_ID&skip=0&take=50" | jq '.'

# Get logs in date range
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?startDate=2024-01-01&endDate=2024-01-31&skip=0&take=50" | jq '.'

# Get page 2 of results
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  "http://localhost:3000/api/master/audit-logs?skip=50&take=50" | jq '.'
...
echo "Open in browser: http://localhost:3000/master"

# ============================================
# ENVIRONMENT SETUP
# ============================================

# Set master token in environment (for scripting)
export ADMIN_TOKEN="your_token_here"

# Use in scripts
curl -H "Authorization: Bearer $ADMIN_TOKEN" \
  http://localhost:3000/api/master/monitoring | jq '.monitoring.healthScore'
...
open_dashboard() {
  if command -v xdg-open &> /dev/null; then
    xdg-open http://localhost:3000/master
  elif command -v open &> /dev/null; then
    open http://localhost:3000/master
  else
    echo "Open http://localhost:3000/master in your browser"
  fi
}
...
# NOTES
# ============================================
# 1. Replace $ADMIN_TOKEN with actual master JWT token
...
# 5. Some endpoints require master role
