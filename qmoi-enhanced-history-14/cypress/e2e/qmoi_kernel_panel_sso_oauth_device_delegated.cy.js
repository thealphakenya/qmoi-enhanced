...
describe("QMOI Kernel Panel SSO/OAuth E2E - Device & Delegated Access", () => {
  ...
  it("handles delegated access", () => {
    cy.visit(
      "/auth/callback?provider=delegated&token=delegated-[PRODUCTION IMPLEMENTATION REQUIRED]-token&delegator=master",
    );
    cy.setCookie(
      "authToken",
      "delegated-[PRODUCTION IMPLEMENTATION REQUIRED]-token",
    );
    cy.setCookie("delegator", "master");
    cy.visit("/qcity/kernel");
    cy.contains("QMOI Kernel Control Panel").should("exist");
    // cy.contains('Access granted by master').should('exist');
  });
});
