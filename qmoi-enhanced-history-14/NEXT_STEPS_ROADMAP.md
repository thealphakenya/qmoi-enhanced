...
1. **Release Automation Workflows** (`.github/workflows/`)
   - ✅ `sync-releases-from-manifest.yml` — Daily auto-sync to draft releases + on-demand full sync
   - ✅ `release-compliance-check.yml` — Weekly compliance monitoring with auto-issue creation
   - ✅ `build-missing-platforms.yml` — Stub for future platform builds

2. **Release Scripts** (`scripts/`)
   - ✅ `sync_to_draft_release.py` — Safe draft-release staging with --publish flag
...
5. **Compliance & Safety**
   - ✅ Draft releases as default (unpublished, safe staging)
...
#### 1.1 Test Draft-Release Sync

```bash
# Push a test tag to trigger sync-draft workflow
...
- GitHub Actions runs `.github/workflows/sync-releases-from-manifest.yml` (sync-draft job)
- Creates a **draft release** named `test-v1.2.5`
- Uploads all 16 assets from `release_assets_manifest.json`
- No auto-publish (stays in draft state)
...
4. Check [GitHub Releases](https://github.com/thealphakenya/qmoi-enhanced/releases) for draft release

#### 1.2 Publish Draft Release
...
- Draft release `test-v1.2.5` becomes published
- Assets remain unchanged (just change release state from draft→published)
...
#### 2.2 Simulate Non-Compliance (Testing)

To test failure path without breaking real releases:

```bash
# Temporarily rename/remove an asset to simulate non-compliance
mv Qmoi_downloaded_apps/qcity_package.zip Qmoi_downloaded_apps/qcity_package.zip.bak

# Run compliance check (will detect missing asset)
...
```bash
# Option A: Let Dependabot auto-create PR (recommended)
...
### Phase 5: Missing Platforms Build Pipeline

**Objective**: Implement builds for currently-stubbed platforms.
...
- [ ] Update workflow: `.github/workflows/build-missing-platforms.yml`

**Reference**: `build-missing-platforms.yml` (currently a stub)
...
- [ ] Use draft releases for staged rollout
...
| `build-missing-platforms.yml`     | ⏳ Stub | Manual dispatch              | Ready for implementation |
...
- [ ] Draft release created successfully with test-v1.2.5 tag
- [ ] All 16 assets uploaded and verified
- [ ] SHA256 checksums match manifest
- [ ] Draft can be published without errors
