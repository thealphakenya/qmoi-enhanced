...
- Inspect the file and its [AUTOFIXED by Ollama at 2026-07-26T18:54:42.037077Z] markers or [AUTOFIXED by Ollama at 2026-07-26T18:54:42.037077Z]s.
- If the file is safe for production, remove the [AUTOFIXED by Ollama at 2026-07-26T18:54:42.037077Z] and add tests / small PR.
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const res = await fetch('/api/qmoi-model?deviceOptimize=1', {
        headers: { 'x-master-token': localStorage.getItem('adminToken') || '' },
      });
      ...
      if (data.suggestions && data.suggestions.length) {
        for (const suggestion of data.suggestions) {
          await fetch('/api/qmoi-model?applyDeviceFeature=1', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-master-token': localStorage.getItem('adminToken') || '' },
