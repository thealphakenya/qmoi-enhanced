...
# 2. Error detection disabled
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/config | jq '.enableErrorDetection'

# 3. Scan not running
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/autoscan | jq '.status'
...
curl -X POST -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/scan
```
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/config | jq '.autoFixOnErrors'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/config | jq '.healthMonitoringEnabled'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/healthmonitor | jq '.thresholds'
```
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/config | jq '.config'
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/config | jq '.config.autoScanInterval'
```
...
**A**: Check the dashboard at `/master` or use the API:
...
4. Master is alerted (if alerting enabled)
...
# Health monitor logs
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/healthmonitor | jq '.alerts'

# Bootstrap logs
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/bootstrap | jq '.logs'
```

### Q: How do I generate a secure master token?
...
# Export via curl
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/autoscan > scan-logs.json
...
curl -H "Authorization: Bearer TOKEN" \
  http://localhost:3000/api/master/autofix/background-automation | jq '.status.services'
