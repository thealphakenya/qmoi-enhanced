...
check "[ -f .env.local ] || echo 'Create with: cp .env.example .env.local'" ".env.local exists (or create with cp .env.example .env.local)"
...
if [ -f .env.local ]; then
    ...
    echo "⚠️  .env.local not found (create with: cp .env.example .env.local)"
    ((FAIL++))
fi
...
if [ $FAIL -eq 0 ]; then
    ...
    echo "  • cp .env.example .env.local       (create .env.local)"
