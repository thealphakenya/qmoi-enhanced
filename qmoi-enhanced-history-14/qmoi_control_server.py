...
@app.route('/master/users', methods=['GET'])
...
@app.route('/master/set-pricing', methods=['POST'])
...
@app.route('/master/check-access/<username>/<feature>', methods=['GET'])
...
def deals_create():
    # Master-only endpoint to create deals
    if not _is_master_request(request):
        return jsonify({'status': 'error', 'reason': 'forbidden'}), 403
    payload = request.get_json(force=True) or {}
...
def mirror_raw(rest):
    ...
    return redirect(raw)
@app.route('/master/backup-db', methods=['POST'])
def admin_backup_db():
...
@app.route('/master/update-ngrok', methods=['POST'])
