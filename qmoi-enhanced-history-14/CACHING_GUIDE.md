...
# Production (example with AWS ElastiCache)
REDIS_URL=redis://username:password@cache.example.com:6379

# With SSL
REDIS_URL=rediss://username:password@cache.example.com:6379
```
...
const caching = withCache({
  ...
  // Conditional caching
  condition: (req) => {
    // Only cache GET requests without master parameter
    return req.method === "GET" && !req.nextUrl.searchParams.has("master");
  },
...
```typescript
// app/api/master/cache-stats/route.ts
...
```typescript
// app/api/master/cache-health/route.ts
export async function GET() {
  const stats = await cacheManager.getStats();
