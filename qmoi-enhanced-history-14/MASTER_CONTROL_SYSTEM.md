...
### 1. **Master Dashboard** (`/master/master`)
...
### 5. **Settings Management** (`/master/master/settings`)
...
### 6. **Security Center** (`/master/master/security`)
...
# Master token for API authentication
...
- **POST** `/api/master/master/auth` - Authenticate master user
- **POST** `/api/master/master/logout` - Logout and clear session
...
- **GET** `/api/master/financial/summary` - Get financial overview

### Health & Monitoring

- **GET** `/api/master/autofix/healthmonitor` - Health check status
...
- Bearer token authentication on all master endpoints
- Tokens validated on every request
- Session tokens stored in sessionStorage (not localStorage)

### Middleware Protection

- `/master/master/*` routes protected by middleware
- `/api/master/*` endpoints require valid token
...
1. Navigate to `https://your-app.com/master/master/login`
...
- Verify `/api/master/financial/summary` endpoint exists
