...
- `qmoi_control_server.py` (Flask) — central control server for PWAs, auth, WebAuthn, memory sync, attachments, master actions, and mirror proxy.
  - Port (default): 8000
  - Key endpoints: `/control`, `/ai`, `/ai/tts`, `/signup`, `/login`, `/logout`, `/webauthn/*`, `/sync-memory`, `/memories`, `/attachments`, `/mirror/*`, `/master/*`, `/ready`, `/metrics`, `/sponsored/*`
