...
- **Master-managed** - Only admins and masters can create/manage
...
```javascript
{
  ...
  restrictions: ["biometric", "file_management", "financial", "master"]
}
```

### Permissions

| Feature           | Sponsored User     | Regular User | Master   | Master  |
| ----------------- | ------------------ | ------------ | ------- | ------- |
...
```bash
curl -X POST http://localhost:3000/api/master/sponsored/create \
...
**Endpoint:** `POST /api/master/sponsored/bulk-create`

```bash
curl -X POST http://localhost:3000/api/master/sponsored/bulk-create \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>" \
  -d '{
...
**Endpoint:** `PUT /api/master/sponsored/update`

```bash
curl -X PUT http://localhost:3000/api/master/sponsored/update \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>" \
  -d '{
    "userId": "5",
...
**Endpoint:** `POST /api/master/sponsored/suspend`

```bash
curl -X POST http://localhost:3000/api/master/sponsored/suspend \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>" \
  -d '{
    "userId": "5",
...
```bash
curl -X DELETE http://localhost:3000/api/master/sponsored/delete \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>" \
  -d '{
    "userId": "5",
...
**Endpoint:** `GET /api/master/sponsored/program/:programId`
...
```
1. Master creates sponsored user via API
