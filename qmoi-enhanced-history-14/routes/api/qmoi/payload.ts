...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Simple master auth support (optional)
  const adminToken = process.env.ADMIN_TOKEN || "";
  const provided =
    (req.headers["x-master-token"] as string) || req.body?.admin_token;
  ...
  if (!q) {
    res
      .status(400)
      .json({ success: false, message: "Missing payload identifier `q`" });
    return;
  }
  ...
  const job = {
    ...
    requester: provided ? "master" : "anonymous",
  };
