...
export default function QMOIAutoFixDashboard() {
  ...
  const fetchErrors = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("/api/master/autofix/errors", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fixSpecificError = async (errorId: string) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`/api/master/autofix/fix/${errorId}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-lg shadow-2xl p-6 mb-8 border border-gray-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <button
              onClick={triggerScan}
              disabled={autoFixStatus.scanning}
            ...
            <button
              ...
              disabled={autoFixStatus.fixing || autoFixStatus.totalErrors === 0}
              className={`py-4 px-6 rounded-lg font-bold text-lg transition-all duration-300 flex items-center justify-center gap-2 ${
