...
case "$OS_ID" in
  ...
  centos|fedora|rhel)
    ...
    ;;
  *)
    echo "\nUnknown or unsupported OS. Recommended: install via nvm (non-root):"
...
echo "\nNote: This script only prints recommended commands. To auto-run an install, run with root and the flag '--install' at your own risk."
