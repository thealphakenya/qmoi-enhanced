...
```env
# Master Access Token for AutoFix System
ADMIN_TOKEN=your-secret-master-token-change-this
...
| **Configuration**  | Missing config files, invalid settings |
...
```
POST http://localhost:3000/api/master/autofix/scan
```

**Status Endpoint:**

```
GET http://localhost:3000/api/master/autofix/status
```
...
```javascript
const ADMIN_TOKEN = "your-master-token";

// Scan for errors
async function scanErrors() {
  const response = await fetch("/api/master/autofix/scan", {
    method: "POST",
    headers: {
...
async function fixAllErrors() {
  const response = await fetch("/api/master/autofix/fix-all", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${ADMIN_TOKEN}`,
      "Content-Type": "application/json",
...
async function getStatus() {
  const response = await fetch("/api/master/autofix/status", {
    headers: {
      Authorization: `Bearer ${ADMIN_TOKEN}`,
    },
  });
...
async function checkAndNotify() {
  const response = await fetch("/api/master/autofix/health", {
...
- **API Docs**: Review endpoint comments in `app/api/master/autofix/`
