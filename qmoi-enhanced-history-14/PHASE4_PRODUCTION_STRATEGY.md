...
# Force push (requires master access)
...
**Recommended**: Playwright + Appium stack (cross-platform, open-source, excellent CI integration)
...
```
created → planned → dev → testing → release → live → archived
  ↓        ↓        ↓      ↓         ↓       ↓       ↓
...
```
master       → full access (create, delete, modify, override gates)
