...
| Endpoint                   | Method | Auth | Description                |
| -------------------------- | ------ | ---- | -------------------------- |
| `/master/users`             | GET    | Yes  | List all users             |
| `/master/users/:id`         | GET    | Yes  | Get user details           |
| `/master/dashboard`         | GET    | Yes  | Master dashboard statistics |
| `/master/alerts`            | GET    | Yes  | Get system alerts          |
| `/master/audit-logs`        | GET    | Yes  | Get audit logs             |
| `/master/audit-logs/export` | GET    | Yes  | Export audit logs as CSV   |
...
| `/analytics/users`        | GET    | Yes (Master) | User analytics        |
...
```json
{
  ...
  "error": {
    ...
    "details": "This endpoint requires master role"
  }
}
```
...
| Master endpoints   | 20 requests/minute |
...
async function getAdminDashboard(token) {
  const response = await fetch(
    "https://your-domain.vercel.app/api/master/dashboard",
    {
      headers: { Authorization: `Bearer ${token}` },
    },
  );
