...
// Provide lightweight fallbacks for certain browser/node APIs used in tests.
// 1) Ensure SpeechSynthesis is available (tests spy on speak)
try {
  if (typeof globalThis.speechSynthesis === "undefined") {
    // eslint-disable-next-line no-undef
    globalThis.__SpeechSynthesisUtteranceMock = jest.fn();
    // eslint-disable-next-line no-undef
    globalThis.SpeechSynthesisUtterance = function (text) {
      // record construction
      // @ts-ignore
      globalThis.__SpeechSynthesisUtteranceMock(text);
      this.text = text;
    };
    ...
    globalThis.fetch = async (input, init) => {
      try {
        ...
        if (typeof url === "string") {
          ...
          // Health check endpoint (no auth required) - BEFORE master check
          ...
          // Basic master/monitoring endpoints fallback to avoid network calls
          if (
            ...
            // Verify token and check master role using local auth/db shims
            try {
              ...
              if (url.includes("/api/master/monitoring")) {
                return Promise.resolve(
                  new Response(
                    JSON.stringify({
                      monitoring: {
              ...
              if (url.includes("/api/master/alerts")) {
              ...
              if (url.includes("/api/master/rate-limits")) {
              ...
              if (url.includes("/api/master/audit-logs")) {
                if ((init && init.method) === "POST") {
                  try {
                    const b = init && init.body ? JSON.parse(init.body) : {};
                    if (b.format === "json") {
...
// If MSW is available, start a lightweight in-memory server to ensure
