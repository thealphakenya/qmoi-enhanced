...
// QMOI Self-Test Runner for Manual Error [] and Auto-Fix Testing
...
`;
  ...
  async setupNoInternet() {
    ...
    [] a network failure by blocking DNS resolution
  ...
  async setupVpnDisconnected() {
    ...
    [] a VPN disconnection by blocking network traffic
...
`;
  ...
  async setupZeroRatedFail() {
    ...
    [] a scenario where all network endpoints fail
    const iptablesPath = path.join(this.projectRoot, 'iptables.rules');
    await this.backupFile(iptablesPath);
    const iptablesContent = `*filter
:INPUT ACCEPT [0:0]
...
`;
  ...
  async setupCloudUnavailable() {
    ...
    [] a scenario where a cloud resource (_e.g., database, API) is unavailable
