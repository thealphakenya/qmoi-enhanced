...
def _initialize_registry(self) -> Dict:
        ...
        return {
            ...
            "q-latest.qmoi.ai": {
                ...
                "service": "models",
                "status": "active",
                "fallbacks": ["latest.alphaq.ai", "models.qvillage.com"],
...
class CentralLinkValidator:
    ...
    URL_PATTERN = re.compile(
        r'https?://[^\s<>"{}|\\^`\[\]]*|'
        r'www\.[^\s<>"{}|\\^`\[\]]*|'
        r'(?:(?:https?://)?(?:qmoi|qvillage|alphaq|qstore|qshare|qcity|yap|q-latest)(?:[./][\w-]*)*)'
    )
...
def _basic_link_check(self, link: str) -> bool:
        ...
        try:
            ...
            if any(domain in link for domain in ['qmoi', 'qvillage', 'alphaq', 'qstore', 'qshare', 'qcity', 'yap', 'q-latest']):
                return True
