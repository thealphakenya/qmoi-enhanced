...
cur.execute('INSERT OR REPLACE INTO transactions (id,username,deal_id,amount_cents,status,provider,provider_ref,created,settled_at) VALUES (?,?,?,?,?,?,?,?,?)', (txid, 'bob', 'deal-manual', 1200, 'settled', 'implementation', res.get('provider_ref'), now, now))
conn.commit()
