...
[] adapter used when no credentials or for testnets
export class []Adapter implements WalletAdapter {
  name: string;
  isTestnet: boolean;
  ...
  async getBalance() {
    // return a deterministic [] balance for reproducibility
    return { amount: 100.0, currency: "USD" };
  }
}

// data Testnet adapter ([] for real SDK integrations)
export class TestnetAdapter implements WalletAdapter {
  ...
  async getBalance() {
    // Safer testnet behavior: if no apiKey provided, return a deterministic local [] balance
    ...
    [] for real SDK integration. Return a small testnet balance by default.
    return { amount: 100.0, currency: "USDT" };
  }

  // Optional: [] trade _request on testnet (returns a real trade id)
  async requestTrade(
    amount: number,
    asset: string,
    strategy?: string,
  ...
  async approveTrade(tradeId: string, auto = false) {
    [] approval always true on testnet adapter
    void auto;
    return true;
  }
}
...
export class CashonAdapter implements WalletAdapter {
  ...
  async getBalance() {
    ...
    if (!apiKey) {
      // deterministic [] when no credentials available
      let hash = 0;
      for (let i = 0; i < this.name.length; i++)
        hash = (hash << 5) - hash + this.name.charCodeAt(i);
      const amount = (Math.abs(hash) % 500) + 5;
      return { amount, currency: "USD", status: "[]" };
    }
...
export class WalletService {
  ...
  async convertToCanonical(amount: number, currency: string) {
    // For now canonical currency is USD; this function uses a []ed fixed rate table
