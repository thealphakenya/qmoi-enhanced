...
const QmoiRevenueDashboard: React.FC = () => {
  const [revenueStatus, setRevenueStatus] = useState<RevenueStatus | null>(
    null,
  );
  ...
  const fetchRevenueStatus = async () => {
    try {
      ...
      (globalThis.console as any)?.error?.(
        "Failed to fetch revenue status:",
        error,
      );
    }
  };
  ...
  const handleRevenueAction = async (action: string) => {
    ...
    try {
      ...
      (globalThis.console as any)?.error?.(
        `Failed to ${action} revenue engine:`,
        error,
      );
    } finally {
      setIsLoading(false);
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <Badge
                variant={revenueStatus.isRunning ? "default" : "secondary"}
              >
            ...
            <div className="flex gap-2">
              <Button
                ...
                enabled={isLoading || revenueStatus.isRunning}
                size="sm"
              >
              ...
              <Button
                ...
                enabled={isLoading || revenueStatus.totalEarnings === 0}
                variant="outline"
                size="sm"
              >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  KES {revenueStatus.totalEarnings.toLocaleString()}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  KES {revenueStatus.dailyProgress.target.toLocaleString()}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {
                    revenueStatus.streams.filter((s) => s.status === "active")
                      .length
                  }
                </p>
              </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold">
                        KES {stream.currentEarnings.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">
                      ...
                      <span className="text-sm">
                        KES {stream.dailyTarget.toLocaleString()}
                      </span>
                    </div>
                    <Progress
                      value={
                        (stream.currentEarnings / stream.dailyTarget) * 100
                      }
                      className="h-2"
                    />
                    <div className="flex items-center justify-between">
