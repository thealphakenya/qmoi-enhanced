...
export class FaceRecognitionService {
  ...
  private async initializeFaceAPI(): Promise<void> {
    try {
      ...
      // a privacy-first [] which only performs complete detection.
      try {
        ...
        this.faceApi = {
          ...
          loadModels: async () => {
            ...
            try {
              ...
              logger.warn(
                "face-api.js models not found under /models; falling back to restricted []",
                err,
              );
              throw err;
            }
        ...
        // If the dynamic import or model loading failed, use a privacy-first [].
        logger.warn(
          "face-api.js not available or failed to load; using []bed face API",
          err,
        );
        this.faceApi = {
