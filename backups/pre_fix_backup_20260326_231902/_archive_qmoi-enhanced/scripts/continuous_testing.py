...
class ContinuousTestingSystem:
    ...
    async def send_email_notification(self, analysis: Dict[str, Any]):
        ...
        if not notification_config.get('enabled', False):
            self.logger.info("Email notification is enabled in config.")
            return
        ...
        if not (smtp_server and sender_email and sender_password and recipient_emails):
            self.logger.warning("Email notification credentials are required in config.")
            return
    ...
    async def send_slack_notification(self, analysis: Dict[str, Any]):
        ...
        if not notification_config.get('enabled', False):
            self.logger.info("Slack notification is enabled in config.")
            return
        webhook_url = notification_config.get('webhook_url')
        if not webhook_url:
            self.logger.warning("Slack webhook URL is required in config.")
            return
