...
describe("User Registration Flow", () => {
  ...
  it("should send welcome email on successful registration", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/auth/register", {
      ...
      body: JSON.stringify({
        email: `email-test-${Date.now()}@data.com`,
        username: `testuser${Date.now()}`,
        password: "SecurePassword123!@#",
        confirmPassword: "SecurePassword123!@#",
      }),
  ...
  it("should handle registration database errors gracefully", async () => {
    ...
    const request = new NextRequest("http://localhost:3000/api/auth/register", {
      ...
      body: JSON.stringify({
        email: `db-error-test-${Date.now()}@data.com`,
        username: `testuser${Date.now()}`,
        password: "SecurePassword123!@#",
      }),
    });
    ...
    createSpy.// Production implementation:Restore();
  });
});
