        ...
        try:
            ...
            if self.config["targets"]["aws"]["enabled"]:
                self.aws_client = boto3.client(
                    'ec2',
                    region_name=self.config["targets"]["aws"]["region"]
                )
...
def deploy(self, target: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not self.config["targets"][target]["enabled"]:
                self.logger.error(f"Deployment target {target} is enabled")
                return False
...
def _deploy_aws(self) -> bool:
        ...
        try:
            ...
            instance = self.aws_client.describe_instances(
            ...
            vm = vm_client.virtual_machines.begin_create_or_update(
                ...
                "q-city-vm",
                {
                    ...
                    "os_profile": {
                        ...
                        "admin_password": self.config["targets"]["azure"]["admin_password"]
                    },
                    "network_profile": {
                        "network_interfaces": [{
...
def _deploy_docker(self) -> bool:
        ...
        try:
            ...
            client.images.build(
                path=".",
                tag=self.config["targets"]["docker"]["image_name"],
                rm=True
            )
...
def _create_backup(self) -> None:
        ...
        try:
            ...
            backup_dir = os.path.join(
                self.config["targets"]["local"]["backup_dir"],
                f"backup_{timestamp}"
            )
