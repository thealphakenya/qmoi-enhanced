...
RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/chat" \
...
RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/projects" \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "'$MASTER_USER'",
    "type": "ai-automation",
...
RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/chat" \
  ...
  -d '{
    "userId": "'$MASTER_USER'",
    "message": "Initiate auto-evolution protocol: What new capabilities can you develop based on current project requirements?",
    "role": "master",
    "action": "auto-evolve"
  }')
...
FRIEND_RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/friendship" \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "'$MASTER_USER'",
    "action": "send-request",
...
AUDIT_RESPONSE=$(curl -s "$DEV_SERVER_URL/api/qmoi/audit-log?userId=$MASTER_USER&limit=10")
...
COMPLEX_RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/chat" \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "'$MASTER_USER'",
...
VOICE_RESPONSE=$(curl -s "$DEV_SERVER_URL/api/qmoi/voice/status?userId=$MASTER_USER")
...
SUCCESS_COUNT=0
for i in {1..5}; do
    LOAD_RESPONSE=$(curl -s -X POST "$DEV_SERVER_URL/api/qmoi/chat" \
      -H "Content-Type: application/json" \
      -d '{
        "userId": "'$MASTER_USER'",
        "message": "Performance test message '$i'. Quick response needed.",
        "role": "master"
      }')
