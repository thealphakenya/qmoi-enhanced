...
#
# Usage: ./start-production-deployment.sh
##############################################################################
...
verify_deployment() {
    ...
    if [ -f "logs/qmoi_app.log" ]; then
        ...
        tail -10 logs/qmoi_app.log
    fi

    # Check if health monitor is running
    if pm2 list | grep -q "qmoi-health-monitor"; then
        STATUS=$(pm2 list | grep "qmoi-health-monitor" | awk '{print $11}')
        if [ "$STATUS" == "online" ]; then
            log_success "Health monitor is running"
        else
            log_warning "Health monitor status: $STATUS"
        fi
    fi
}
...
display_summary() {
    ...
    echo "   - production_DEPLOYMENT_AUTO_RECOVERY.md"
