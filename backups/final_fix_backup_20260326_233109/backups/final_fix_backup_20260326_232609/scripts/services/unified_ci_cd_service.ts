...
class UnifiedCICDService {
  ...
  async deployToAWS() {
    try {
      ...
      await execAsync(
        "aws deploy start-deployment --application-name $AWS_APP_NAME --deployment-group-name $AWS_DEPLOYMENT_GROUP --s3-location bucket=$AWS_S3_BUCKET,bundleType=zip,key=$AWS_S3_KEY",
      );
  ...
  async deployWithFallback(platform = "vercel") {
    ...
    for (let i = startIdx; i < platforms.length; i++) {
      ...
      logger.warn(
        `[CI/CD] Deployment failed on ${platforms[i]}, trying next platform...`,
      );
    }
