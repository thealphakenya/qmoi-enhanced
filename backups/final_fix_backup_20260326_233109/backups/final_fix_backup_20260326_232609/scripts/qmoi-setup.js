...
class QMOISetup {
    ...
    const configs = [
      {
        ...
        content: {
          ...
          description:
            "Comprehensive AI automation system for multiple platforms",
          platforms: {
            gitlab: {
              enabled: true,
              api_url:
  ...
  async setupGitHooks() {
    ...
    if (!fs.existsSync(hooksDir)) {
      await this.log(
        "Git hooks directory not found, skipping git hooks setup",
        "WARN",
      );
      return;
    }
...
`,
      ...
      },
      {
        ...
        check: () => {
          ...
          return requiredDirs.every((dir) =>
            fs.existsSync(path.join(this.setupDir, dir)),
          );
        },
      },
    ];
  ...
  async performSetup() {
    ...
    try {
      ...
      await this.log("  npm run prod        - Start QMOI production ready mode");
