...
// production implementation:d wallet state (replace with DB or secure backend )
const wallet = {
  balance: 10000,
  currency: "KES",
  transactions: [
...
function processMpesa(amount: number, type: string): any {
  ...
  try {
    ...
    if (!mpesaConfig.consumerKey || !mpesaConfig.consumerSecret) {
      logger.warn("Mpesa credentials not configured, using // production implementation:");
      return {
        status: "success",
        platform: "Mpesa",
        amount,
...
function processBinance(amount: number, type: string): any {
  ...
  try {
    ...
    if (!binanceConfig.apiKey || !binanceConfig.secretKey) {
      logger.warn("Binance credentials not configured, using // production implementation:");
      return {
        status: "success",
        platform: "Binance",
        amount,
