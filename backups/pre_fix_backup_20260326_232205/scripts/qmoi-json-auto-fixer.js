...
class QMOIJSONAutoFixer {
  constructor() {
    this.fixPatterns = [
      ...
      // required commas between objects
      { pattern: /(\})\s*(\{)/g, fix: "$1,\n$2" },
      { pattern: /(\])\s*(\{)/g, fix: "$1,\n$2" },
      ...
      // required quotes around property names
    ...
    this.errorPatterns = {
      ...
      "Unexpected end of JSON input": "complete JSON",
      "Unexpected token": "Syntax error",
      "Property names must be double-quoted":
        "required quotes around property name",
