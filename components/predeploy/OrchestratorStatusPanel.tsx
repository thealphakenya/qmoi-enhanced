...
const mockAgents = [
  { id: 'a1', name: 'Agent Alpha', status: 'active', assignedDevice: 'Device 1' },
  { id: 'a2', name: 'Agent Beta', status: 'standby', assignedDevice: 'Device 2' },
  { id: 'a3', name: 'Agent Gamma', status: 'failed', assignedDevice: 'Device 3' },
];
...
export const OrchestratorStatusPanel: React.FC<{ status: OrchestratorStatus }> = ({ status }) => {
  ...
  function handleAssign(agentId: string, device: string) {
    setAgents(prev => prev.map(a => a.id === agentId ? { ...a, assignedDevice: device } : a));
    // Stub: backend call
  }
  ...
  function handleFailover(agentId: string) {
    ...
    setTimeout(() => {
      setAgents(prev => prev.map(a => a.id === agentId ? { ...a, status: 'standby', assignedDevice: '' } : a));
      setFailoverLoading(null);
    }, 1000);
  }

  return (
    <div style={{ border: '1px solid #ddd', borderRadius: 8, padding: 16, marginBottom: 16 }} aria-label="Orchestration Status Panel">
      ...
      <table style={{ width: '100%', fontSize: 14 }} aria-label="Agent Device Table">
        <thead>
          <tr>
            <th>Agent</th>
            <th>Status</th>
        ...
        <tbody>
          {agents.map(agent => (
            <tr key={agent.id}>
              <td>{agent.name}</td>
              <td>
                <span style={{ color: agent.status === 'active' ? '#4caf50' : agent.status === 'standby' ? '#ff9800' : '#f44336' }}>
                  {agent.status}
                </span>
              </td>
              <td>{agent.assignedDevice || <span style={{ color: '#aaa' }}>None</span>}</td>
              <td>
                <select
              ...
              <td>
                ...
                >
                  {failoverLoading === agent.id ? 'Failing over...' : 'Failover'}
                </button>
              </td>
            </tr>
          ))}
