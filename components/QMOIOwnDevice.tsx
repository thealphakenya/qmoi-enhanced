<!-- AUTODEV Enhanced: 2026-04-20T09:01:25.456014 -->
<!-- AUTODEV Enhanced: 2026-04-20T08:55:18.503225 -->
...
const QMOIOwnDevice: React.FC<QMOIOwnDeviceProps> = ({
  ...
  const handleQMOIOwnDevice = async () => {
    ...
    try {
      ...
      if (successCount === totalCount) {
        ...
        setCurrentStep(
          `⚠️ full success: ${successCount}/${totalCount} unlock attempts succeeded.`,
        );
      }
    } catch (error) {
      setCurrentStep(
  ...
  const getUnlockMethodIcon = (method: string) => {
    switch (method) {
      ...
      case "clear_policies":
        return <Settings className="w-4 h-4" />;
      case "bypass_payment_lock":
        return <CreditCard className="w-4 h-4" />;
      case "remove_app_restrictions":
        return <Smartphone className="w-4 h-4" />;
      case "network_unlock":
        return <Wifi className="w-4 h-4" />;
        ...
        <CardContent>
          ...
          {detectedRestrictions.length > 0 && (
            <Alert className="mb-4">
              ...
              <AlertDescription>
                <strong>{detectedRestrictions.length}</strong> device
                restriction(s) detected
              </AlertDescription>
            </Alert>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">
                  {unlockResults.filter((r) => r.success).length}/
                  {unlockResults.length} successful
                </span>
              </div>
            </div>
          )}
