...
interface MasterContextType {
  ...
  isMaster: boolean;
  hasPermission: (perm: 'deploy' | 'viewDashboard' | 'admin' | 'user') => boolean;
}
...
export function MasterProvider({ children }: { children: ReactNode }) {
  ...
  function hasPermission(perm: 'deploy' | 'viewDashboard' | 'admin' | 'user') {
    ...
    if (perm === 'user' && (currentRole === 'user' || currentRole === 'admin' || currentRole === 'master')) return true;
    if (perm === 'viewDashboard' && currentRole === 'master') return true;
    return false;
  }
  return (
    <MasterContext.Provider value={{ currentRole, setRole, isMaster, hasPermission }}>
      {children}
