    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            if is_valid_dmg(path):
                ...
                log_activity(
                    f"DMG too small after download", {"size": os.path.getsize(path)}
                )
        except Exception as e:
            log_activity(
                f"Error downloading {DMG_NAME}", {"error": str(e), "attempt": attempt}
            )
...
if not url:
    ...
    download_dirs = [
        ensure_download_dir("mac", "latest"),
        ensure_download_dir("mac", version_folder),
    ]
    ...
    if download_dmg(url, dmg_paths[0]):
        if dmg_paths[0] != dmg_paths[1]:
            try:
                ...
                log_activity(
                    "Copied DMG to versioned folder",
                    {"from": dmg_paths[0], "to": dmg_paths[1]},
                )
                print(f"Also saved: {dmg_paths[1]}")
            except Exception as e:
                log_activity(
                    "Failed to copy DMG to versioned folder", {"error": str(e)}
                )
    else:
