...
## Endpoints
The repository includes a lightweight control server that exposes the following endpoints (used by the Q Alpha PWA and apps). Most require JWT auth for user-level actions.

Authentication: include header `Authorization: Bearer <token>` for user endpoints. Control plane commands use `Authorization: <control-token>` or `X-API-KEY`.

### Control server endpoints (qmoi_control_server)

#### POST /control
Used by trusted controllers (QMOI) to send control commands. Requires control API key (header `Authorization: <control-token>` or `X-API-KEY`).

Request JSON examples:
```json
{ "command": "navigate", "target": "/apps/qmoi" }
{ "command": "download", "target": "q-alpha.zip" }
{ "command": "voice", "text": "Download Q Alpha" }
```

Responses vary by command; success response example:
```json
{ "status": "ok", "action": "navigate", "route": "/apps/qmoi" }
```

#### POST /ai
Run an AI prompt on behalf of the authenticated user. Requires Authorization Bearer JWT.
...
Response:
```json
{ "status": "ok", "response": { "reply": "(simulated) Received prompt from alice: Summarize ..." } }
```

#### POST /signup
Create a local user account (development only). Request body: {username, password}
...
#### POST /logout
Invalidate current token. Requires Authorization header. Tokens are revoked by `jti` when possible.

Response: {status:ok}

#### POST /sync-memory
Upload local memories for the authenticated user and merge them server-side.

Request example:
```json
{ "memories": [ { "id": null, "key": "q.ki", "value": {"note":"..."}, "created": "..." } ] }
```

Response: {status:ok, merged_count: N}

#### GET /memories
Fetch merged memories for the authenticated user.

Response: {status:ok, memories: [ ... ] }

#### WebAuthn endpoints
- POST /webauthn/register/options — begin registration (CBOR response)
- POST /webauthn/register/complete — finish registration
...
#### Mirror endpoints
- GET /mirror/app/<appname>/* — serve local app files under `pwa_apps/<appname>/` or redirect to GitHub raw fallback
- GET /mirror/raw/<path> — serve local static file or redirect to GitHub raw fallback
